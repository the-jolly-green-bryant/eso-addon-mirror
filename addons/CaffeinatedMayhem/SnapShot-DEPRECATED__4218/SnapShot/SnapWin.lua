...
function OTSnap:GetHistoryTotals(reload)                                                       -- 2021/10/10
	if SnapShot.saved.Guild < 1 then return end
	if OTSnap_M2:GetState() ~= DOWN then return end
	...
	nge,oge = #SnapShot.History[SnapShot.saved.Guild][GUILD_HISTORY_STORE],0
	if nge > 0 then
		oge = SnapShot.History[SnapShot.saved.Guild][GUILD_HISTORY_STORE][1][3]
		OTSnap_P2G1TrdX:SetText(fmt("(%s) %s",nge,os.date(dtf,oge)))
	else
		OTSnap_P2G1TrdX:SetText(fmt("(%s) %s",nge,""))
	end

	nge,oge = #SnapShot.History[SnapShot.saved.Guild][GUILD_HISTORY_ALLIANCE_WAR],0
	if nge > 0 then
		oge = SnapShot.History[SnapShot.saved.Guild][GUILD_HISTORY_ALLIANCE_WAR][1][3]
...
function OTSnap:GuildMakeList()                                                                -- 2021/10/03
	...
	for ix=1,MAX_GUILDS do
		if ix <= GetNumGuilds()
			then gName = OTX.GetGuildNameFromIndex(ix)
...
function OTSnap:WindowInitialize(wTop,wLeft)                                                   -- 2021/10/29
	...
	if SnapShot.saved.Guild > 0
		then OTSnap_GuildName:SetText(OTX.GetGuildNameFromIndex(SnapShot.saved.Guild))
