...
function EsoAR:getFontPath()
  for _, x in pairs(isNeedToChangeAdditionalFontTable) do
    if self:getLanguage() == x then return "EsoAR/fonts/" end
  end
...
function EsoAR:fontChangeWhenInit()
  ...
  for _, v in pairs(styles) do
    for k, fnt in pairs(fontFaces[v]) do
      _G[v][k]["fontFace"] = f(fnt)
    end
  end
...
local function EsoARInit()
  -- Respect menu toggle (addon features ON/OFF)
  if EsoAR_Variables and EsoAR_Variables.enabled == false then
    return
  end
  ...
  ZO_PreHook("ZO_ChatTextEntry_TextChanged", function(control, newText)
    if EsoAR_Variables and EsoAR_Variables.keyboardArabic == false then return end
    EsoAR:Convert(control.system.textEntry)
  end)

  ZO_PreHook("ZO_EditDefaultText_OnTextChanged", function(edit)
    if EsoAR_Variables and EsoAR_Variables.keyboardArabic == false then return end
    EsoAR:Convert(edit)
  end)
end
...
function EsoAR:newInit()
  -- Keep defaults if menu created them, otherwise create minimal vars
  if EsoAR_Variables == nil then
    EsoAR_Variables = ZO_SavedVars:NewAccountWide("EsoAR_Variables", 1, nil, { enabled = true, keyboardArabic = true, lang = EsoAR.langVer.stable })
  end

  -- Preserve existing settings, just ensure lang exists
  EsoAR_Variables.lang = EsoAR_Variables.lang or EsoAR.langVer.stable

  EsoAR.savedVars = EsoAR_Variables
...
local function onAddonLoaded(eventCode, addonName)
  ...
  -- Init menu (if LibAddonMenu-2.0 installed)
  if EsoAR_InitMenu then
    EsoAR_InitMenu()
  end
