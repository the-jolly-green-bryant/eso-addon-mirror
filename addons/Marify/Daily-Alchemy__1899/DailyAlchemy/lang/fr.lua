...
ZO_CreateStringId("DA_SPELL_POWER",         "Augmente la puissance")            -- Increase Power        [fr.lang.csv] "156152165","0","11","xxxxxxxx"
...
function DailyAlchemy:ConvertedItemNames(itemName)
    ...
    local function Convert2(inItemName)
        local inList = {
            {"Poison de ravage de ", "poison de Degats de "},
...
function DailyAlchemy:ConvertedJournalCondition(journalCondition)
    ...
    local list2 = {
        -- Master Writ
        {".*fabrique%w? un%w? (.*) avec.*suivant.*[:：].*%c•%s?(.*)%c•%s?(.*)%c•%s?(.*)%c•.*",  "Fabrique |c88AAFF[%1]|r|cFFFFFF...%2, %3, %4|r"},
        {".*fabrique%w? un%w? (.*) avec.*suivant.*: (.*), (.*), (.*)",                          "Fabrique |c88AAFF[%1]|r|cFFFFFF...%2, %3, %4|r"},
        {".*fabrique (.*) avec.*suivant.*%c• (.*)%c• (.*)%c• (.*)%c.*",                         "Fabrique |c88AAFF[%1]|r|cFFFFFF...%2, %3, %4|r"},
        {".*fabrique (.*) avec.*suivant.*:(.*)",                                                "Fabrique |c88AAFF[%1]|r|cFFFFFF...%2|r"},

        -- Dayly
        {".*fabrique%w? un%w? (.*) [:：].*",        "Fabrique |c88AAFF[%1]|r"},
        {".*fabrique%w? un%w? (.*)[:：].*",         "Fabrique |c88AAFF[%1]|r"},
        {".*fabrique%w? (.*):.*",                   "Fabrique |c88AAFF[%1]|r"},
        {".*preparez un%w? (.*) [:：].*",           "Fabrique |c88AAFF[%1]|r"}, 
        {".*preparez un%w? (.*)[:：].*",            "Fabrique |c88AAFF[%1]|r"},

        {".*acquer%w%w de l'eau (.*) [:：].*",      "Acquerez |c88AAFF[eau %1]|r"},
        {".*acquer%w%w de l'(absinthe) [:：].*",    "Acquerez |c88AAFF[%1]|r"},
        {".*acquer%w%w um%w? (.*) [:：].*",         "Acquerez |c88AAFF[%1]|r"},
        {".*acquer%w%w um%w? (.*)[:：].*",          "Acquerez |c88AAFF[%1]|r"},
        {".*acquer%w%w un%w? (.*) [:：].*",         "Acquerez |c88AAFF[%1]|r"}, 
        {".*acquer%w%w un%w? (.*)[:：].*",          "Acquerez |c88AAFF[%1]|r"}, 
        {".*acquer%w%w de la (.*) :.*",             "Acquerez |c88AAFF[%1]|r"},
        {".*acquer%w%w du (.*) :.*",                "Acquerez |c88AAFF[%1]|r"},
        {".*acquer%w%w les (.*) :.*",               "Acquerez |c88AAFF[%1]|r"},
        {".*acquer%w%w le (.*) :.*",                "Acquerez |c88AAFF[%1]|r"},
    }
...
function DailyAlchemy:isPoison(conditionText)
    return string.match(conditionText, "[Pp]oison")
end

function DailyAlchemy:isAlchemy(journalCondition)
    return string.match(journalCondition, "[Ff]abriquez .* avec les traits suivants")
end
