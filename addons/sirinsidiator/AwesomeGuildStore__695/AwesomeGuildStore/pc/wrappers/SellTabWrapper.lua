...
function SellTabWrapper:InitializeTextFilter()
    local textFilterTargetDescriptor =
    {
    ...
    TEXT_SEARCH_MANAGER:SetupContextTextSearch("AGSSellTabTextSearch", textFilterTargetDescriptor)

    ZO_PreHook(ZO_InventoryManager, "ActivateInventorySearch", function()
        if (self.isOpen) then
            return true
        end
    end)

    ZO_PreHook(ZO_InventoryManager, "DeactivateInventorySearch", function()
        if (self.isOpen) then
            return true
        end
    end)
...
local function GetActiveContext()
    for context, _ in pairs(TEXT_SEARCH_MANAGER.contextSearches) do
        if TEXT_SEARCH_MANAGER:IsActiveTextSearch(context) then
            return context
        end
    end
end

function SellTabWrapper:ActivateTextSearch()
    if not TEXT_SEARCH_MANAGER:IsActiveTextSearch("AGSSellTabTextSearch") then
        local currentContext = GetActiveContext()
        if TEXT_SEARCH_MANAGER:DeactivateTextSearch(currentContext) then
            PLAYER_INVENTORY:SetContextForInventories(REMOVE_CONTEXT, { INVENTORY_BACKPACK })
        end
        PLAYER_INVENTORY:SetContextForInventories("AGSSellTabTextSearch", { INVENTORY_BACKPACK, INVENTORY_CRAFT_BAG, INVENTORY_BANK })

        local searchText = TEXT_SEARCH_MANAGER:GetSearchText("AGSSellTabTextSearch")
        PLAYER_INVENTORY.inventories[INVENTORY_BACKPACK].searchBox:SetText(searchText)
        PLAYER_INVENTORY.inventories[INVENTORY_CRAFT_BAG].searchBox:SetText(searchText)
        PLAYER_INVENTORY.inventories[INVENTORY_BANK].searchBox:SetText(searchText)
    end
end

function SellTabWrapper:DeactivateTextSearch()
    if TEXT_SEARCH_MANAGER:DeactivateTextSearch("AGSSellTabTextSearch") then
        PLAYER_INVENTORY:SetContextForInventories(REMOVE_CONTEXT, { INVENTORY_BACKPACK, INVENTORY_CRAFT_BAG, INVENTORY_BANK })
    end
end
