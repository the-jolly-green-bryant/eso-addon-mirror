...
function SearchResultListWrapper:InitializeResultList(tradingHouseWrapper, searchManager)
    ...
    local function AdjustRowLayout(rowControl)
        ...
        sellPriceControl:SetResizeToFitDescendents(true)
        sellPriceControl:ClearAnchors()

        local perItemPrice = rowControl:GetNamedChild("SellPricePerUnit")
        perItemPrice:SetResizeToFitDescendents(true)
    ...
    searchResultDataType.setupCallback = function(rowControl, result)
        ...
        local perItemPriceText = perItemPrice:GetNamedChild("Text")
        local sellPriceControl = rowControl.sellPriceControl
        sellPriceControl:ClearAnchors()
        local offset = 0
        local hidden = true
        if result:GetStackCount() > 1 then
            ZO_CurrencyControl_SetSimpleCurrency(perItemPriceText, result.currencyType, result.purchasePricePerUnit, PER_UNIT_PRICE_CURRENCY_OPTIONS, nil, tradingHouse.playerMoney[result.currencyType] < result.purchasePrice)
            perItemPriceText:SetText("@" .. perItemPriceText:GetText():gsub("|t.-:.-:", "|t14:14:"))
            perItemPriceText:SetFont("ZoFontWinT2")
...
function SearchResultListWrapper:InitializeSortHeaders(tradingHouseWrapper, searchManager)
    ...
    priceHeader:SetAnchor(RIGHT, nil, RIGHT, -30, 0, ANCHOR_CONSTRAINS_XY)
    priceHeader:SetWidth(nil)
    priceHeader:SetResizeToFitDescendents(true)
    priceHeader:SetResizeToFitConstrains(ANCHOR_CONSTRAINS_X)
    priceHeader:SetHeight(20)
    local priceHeaderLabel = priceHeader:GetNamedChild("Name")
    priceHeaderLabel:ClearAnchors()
    priceHeaderLabel:SetAnchor(TOPRIGHT)
    priceHeaderLabel:SetAnchor(BOTTOMRIGHT)
    ...
    unitPriceHeader:SetAnchor(RIGHT, priceHeader, LEFT, -20, 0, ANCHOR_CONSTRAINS_XY)
    unitPriceHeader:SetWidth(nil)
    unitPriceHeader:SetResizeToFitDescendents(true)
    unitPriceHeader:SetResizeToFitConstrains(ANCHOR_CONSTRAINS_X)
    unitPriceHeader:SetHeight(20)
    local unitPriceHeaderLabel = unitPriceHeader:GetNamedChild("Name")
    unitPriceHeaderLabel:ClearAnchors()
    unitPriceHeaderLabel:SetAnchor(TOPRIGHT)
    unitPriceHeaderLabel:SetAnchor(BOTTOMRIGHT)
