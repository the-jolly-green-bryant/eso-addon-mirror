...
local function RemoveIncorrectWestWealdStoreLocation(saveData)
    local SKINGRAD_OUTLAWS_REFUGE_ID_PATTERN = ".+El$" -- ends with encoded id 2501
    local storesToRemove = {}
    for locationIndex, storeData in pairs(saveData.stores) do
        if storeData:find(SKINGRAD_OUTLAWS_REFUGE_ID_PATTERN) then
            storesToRemove[#storesToRemove + 1] = locationIndex
        end
    end

    for i = 1, #storesToRemove do
        logger:Warn("Remove incorrect store location entry '%s' for Skingrad Outlaw's Refuge", storesToRemove[i])
        saveData.stores[storesToRemove[i]] = nil
    end
end
...
local function UpdateSaveData(saveData)
    ...
    if(saveData.version < 9) then
        RemoveIncorrectWestWealdStoreLocation(saveData)
        saveData.version = 9
        requiresRescan = true
    end
