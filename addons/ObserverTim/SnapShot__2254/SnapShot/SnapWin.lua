...
local IsChecked = OTX.IsButtonPressed
...
OTSnap.P1Fields = {
    ...
    { fid = "ILK", name = "Item Link", head = "I-Link" },
}
...
function OTSnap:GetDelimiter() -- 2021/10/02
    if IsChecked(OTSnap_P9G2B1) then
        return "\t"
    end
    if IsChecked(OTSnap_P9G2B2) then
        return ";"
    end
    if IsChecked(OTSnap_P9G2B3) then
        return "/"
    end
    return ""
end

function OTSnap:ItemTypeReference() -- 2026/5/20

    OTSnap_P1G3B1T:SetText(OTX.CleanNumberList(OTSnap_P1G3B1T:GetText()))
    if not OTType then

        OTType = CreateControlFromVirtual("OTType", nil, "OT_vRef")
        OTTypeTitle:SetText("Item Type Codes")
        OTType:ClearAnchors()
        OTType:SetAnchor(TOPLEFT, OTSnap_P1G3B1T, TOPLEFT, 8, 40)
        for ix = ITEMTYPE_ITERATION_BEGIN+1, ITEMTYPE_ITERATION_END  do
            local px, py = 8 + math.floor((ix - 1) / 24) * 197, 35 + math.fmod(ix - 1, 24) * 21
            local labName = fmt("OTTypeBox%s", ix)
            local labText = OTX.GetItemTypeName(ix)
            if not _G[labName] then
                CreateControlFromVirtual(labName, OTType, "OT_vTextLine")
                _G[labName]:SetAnchor(TOPLEFT, OTType, TOPLEFT, px, py)
                _G[labName]:SetText(fmt("%s. %s", ix, labText))
            end
        end
    end
    OTX.ToggleVisible(OTType)
end
...
function OTSnap:RotateText(uiObject) -- 2021/10/27
    local roTable = {}
    if not uiObject.TextToRotate then
        uiObject:SetText("[*list error*]")
        return
    end
    for t in string.gmatch(uiObject.TextToRotate, "[^~]+") do
        table.insert(roTable, fmt("[%s]", t))
    end
    for ix = 1, #roTable do
        if uiObject:GetText() == roTable[ix] then
            if ix == #roTable then
                ...
                uiObject:SetText(roTable[ix + 1])
            end
            SnapShot.saved.Texts[uiObject:GetName()] = uiObject:GetText()
            --			if uiObject == OTSnap_P2G2B1X then OTSnap:GetHistoryTotals(false) end
            return
        end
    end
    uiObject:SetText("[not found]")
end
...
function OTSnap:ShowReport() -- 2021/10/11
    ...
    if #SnapShot.Lines > 0 then
        ...
        if SnapShot.PageLength > 600 then
    ...
    SnapShot.PageNum = 1
    if SnapShot.PageCount == 1 then
        SnapShot.PageLength = #SnapShot.Lines
    end
    d(fmt("Total |c40FF80%s|r pages of |c40FF80%s|r lines.", SnapShot.PageCount, SnapShot.PageLength))
    local tFoot = fmt("%s Pg %s of %s", SnapShot.ReportName, SnapShot.PageNum, SnapShot.PageCount)
...
function OTSnap:ShowPage()
    if SnapShot.PageNum <= 1 then
        SnapShot.PageNum = 1
    end
    if SnapShot.PageNum >= SnapShot.PageCount then
    ...
    local tFoot = fmt("%s Page %s of %s", SnapShot.ReportName, SnapShot.PageNum, SnapShot.PageCount)
    if stopAt > #SnapShot.Lines then
        stopAt = #SnapShot.Lines
    end
    OTSnap_R0Edit:SetText(table.concat(SnapShot.Lines, "\n", startAt, stopAt))
    OTSnap_R0Edit:SetCursorPosition(1)
    OTSnapPageIndicator:SetText(tFoot)
...
function OTSnap:GetHistoryTotals(reload) -- 2021/10/10
    if SnapShot.saved.Guild < 1 then
        return
    end
    if OTSnap_M2:GetState() ~= DOWN then
        return
    end
    local timeFormat = { D = "!%Y-%m-%d", H = "!%Y-%m-%d %Hh", M = "!%Y-%m-%d %H:%M", S = "!%Y-%m-%d %H:%M:%S" }
    local dtf = "!%Y-%m-%d %H:%M:%S"
    local nge, oge

    if reload then
        SnapShot:HistoryGetEvents(GUILD_HISTORY_GENERAL)
        SnapShot:HistoryGetEvents(GUILD_HISTORY_BANK)
        SnapShot:HistoryGetEvents(GUILD_HISTORY_STORE)
        SnapShot:HistoryGetEvents(GUILD_HISTORY_ALLIANCE_WAR)
    end
    nge, oge = #SnapShot.History[SnapShot.saved.Guild][GUILD_HISTORY_GENERAL], 0
    if nge > 0 then
        oge = SnapShot.History[SnapShot.saved.Guild][GUILD_HISTORY_GENERAL][1][3]
        OTSnap_P2G1GenX:SetText(fmt("(%s) %s", nge, os.date(dtf, oge)))
    else
        OTSnap_P2G1GenX:SetText(fmt("(%s) %s", nge, ""))
    end

    nge, oge = #SnapShot.History[SnapShot.saved.Guild][GUILD_HISTORY_BANK], 0
    if nge > 0 then
        oge = SnapShot.History[SnapShot.saved.Guild][GUILD_HISTORY_BANK][1][3]
        OTSnap_P2G1BnkX:SetText(fmt("(%s) %s", nge, os.date(dtf, oge)))
    else
        OTSnap_P2G1BnkX:SetText(fmt("(%s) %s", nge, ""))
    end

    nge, oge = #SnapShot.History[SnapShot.saved.Guild][GUILD_HISTORY_EVENT_CATEGORY_TRADER], 0
    if nge > 0 then
        oge = SnapShot.History[SnapShot.saved.Guild][GUILD_HISTORY_EVENT_CATEGORY_TRADER][1][3]
        OTSnap_P2G1TrdX:SetText(fmt("(%s) %s", nge, os.date(dtf, oge)))
    else
        OTSnap_P2G1TrdX:SetText(fmt("(%s) %s", nge, ""))
    end

    nge, oge = #SnapShot.History[SnapShot.saved.Guild][GUILD_HISTORY_EVENT_CATEGORY_AVA_ACTIVITY], 0
    if nge > 0 then
        oge = SnapShot.History[SnapShot.saved.Guild][GUILD_HISTORY_EVENT_CATEGORY_AVA_ACTIVITY][1][3]
        OTSnap_P2G1WarX:SetText(fmt("(%s) %s", nge, os.date(dtf, oge)))
    else
        OTSnap_P2G1WarX:SetText(fmt("(%s) %s", nge, ""))
    end
end
...
function OTSnap:GuildMakeList() -- 2021/10/03
    ...
    for ix = 1, MAX_GUILDS do
        if ix <= GetNumGuilds() then
            gName = OTX.GetGuildNameFromIndex(ix)
        else
            gName = "(empty slot)"
        end
...
function OTSnap:GuildPick(uiObject) -- 2021/10/10
    local ctrlNum = tonumber(string.sub(uiObject:GetName(), -1))
    if ctrlNum <= GetNumGuilds() then
        SnapShot.saved.Guild = ctrlNum
        OTSnap_GuildName:SetText(OTX.GetGuildNameFromIndex(SnapShot.saved.Guild))
        if not OTSnap_P2:IsHidden() then
            --			OTSnap:GetHistoryTotals(true)
        end
    end
    OTX.ToggleVisible(OTSnap_G)
end

function OTSnap:HandleCheckBox(button, buttonType) -- 2021/10/27
    local function Check(b)
        ZO_CheckButton_SetChecked(b)
        SnapShot.saved.Buttons[b:GetName()] = 1
    end
    local function Uncheck(b)
        ZO_CheckButton_SetUnchecked(b)
        SnapShot.saved.Buttons[b:GetName()] = 0
    end
    ...
    if IsChecked(button) then -- Exclusive buttons
        if bname == "OTSnap_P3G4B1" then
            Uncheck(OTSnap_P3G4B2)
        elseif bname == "OTSnap_P3G4B2" then
            Uncheck(OTSnap_P3G4B1)
        elseif bname == "OTSnap_P4G3B1" then
            Uncheck(OTSnap_P4G3B2)
        elseif bname == "OTSnap_P4G3B2" then
            Uncheck(OTSnap_P4G3B1)
        elseif bname == "OTSnap_P7G1B1" then
            Uncheck(OTSnap_P7G1B2)
        elseif bname == "OTSnap_P7G1B2" then
            Uncheck(OTSnap_P7G1B1)
        end
    end
end
...
function OTSnap:WindowInitialize(wTop, wLeft) -- 2021/10/29
    ...
    if SnapShot.saved.Guild > 0 then
        ...
        OTSnap_GuildName:SetText("(no guild)")
    end
    ...
    for k, v in pairs(SnapShot.saved.Buttons) do
        if _G[k] then
            if v == 1 then
                ZO_CheckButton_SetChecked(_G[k])
            else
                ZO_CheckButton_SetUnchecked(_G[k])
            end
        end
    end
    ...
    for i = 1, 5 do
        delimiter = delimiter or IsChecked(GetControl("OTSnap_P9G2B", i))
    end
    if not delimiter then
        OTSnap:HandleCheckBox(OTSnap_P9G2B1, "R")
    end
...
function OTSnap.LayoutRow(rowControl, data, scrollList)
    ...
    rowControl:SetHandler("OnMouseUp", function()
        OTSnap.CheckLabel(scrollList, rowControl)
    end)
end
...
function OTSnap.CheckLabel(self, control)
    ...
    if me == "OTSnap_P1FieldList" then
        ...
        if OTSnap.P1Fields[ser].checked then
            control:SetText(OTSnap.P1Fields[ser].name .. " √")
        else
            control:SetText(OTSnap.P1Fields[ser].name)
        end
    ...
    if me == "OTSnap_P2G1List" then
        ...
        if OTSnap.P2G1List[ser].checked then
            control:SetText(OTSnap.P2G1List[ser].name .. " √")
        else
            control:SetText(OTSnap.P2G1List[ser].name)
        end
