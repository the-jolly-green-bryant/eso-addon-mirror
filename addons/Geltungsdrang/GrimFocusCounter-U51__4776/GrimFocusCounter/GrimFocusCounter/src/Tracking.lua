...
function GFC:GetBuffStacks()
    -- Prefer the morph we are actually tracking so a lingering buff from a
    -- bar swap can't report the wrong morph's stacks
    local trackedStackId = nil
    if self.trackedAbilityId ~= nil then
        trackedStackId = self.skills[self.trackedAbilityId]
    end
    ...
    for i = 1, GetNumBuffs("player") do
        ...
        if trackedStackId ~= nil then
            if trackedStackId == abilityId then
                -- Fix diagnostic for GetUnitBuffInfo() returns
                return stackCount --[[@as integer]]
            end
        else
            for ability, stackId in pairs(self.skills) do
                if stackId == abilityId then
                    self:Trace(3, "Found stacks for <<1>> (<<2>>)", GetAbilityName(ability), ability)
                    -- Fix diagnostic for GetUnitBuffInfo() returns
                    return stackCount --[[@as integer]]
                end
            end
...
function GFC.OnEffectChanged(_, changeType, _, effectName, _, _, _, stackCount, _, _, _, _, _, _, _, effectAbilityId)
    ...
    if changeType == EFFECT_RESULT_FADED then
        GFC:Trace(2, "Buff faded: <<1>> (<<2>>) at <<3>> stacks", effectName, effectAbilityId, stackCount)
        GFC.currentStacks = 0
    else
        -- Gained or updated. Firing the bow now consumes only the proc
        -- threshold (4 or 5) rather than every stack, so leftovers arrive
        -- here as an update and the counter keeps running.
        GFC:Trace(1, "Stack #<<1>> (effect changed)", stackCount)
