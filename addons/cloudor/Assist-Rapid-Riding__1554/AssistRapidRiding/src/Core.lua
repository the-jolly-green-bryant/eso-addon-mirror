  ...
  for skillType = 1, GetNumSkillTypes() do
    for skillLine = 1, GetNumSkillLines(skillType) do
      for abilityIndex = 1, math.min(7, GetNumAbilities(skillType, skillLine)) do
        ...
        if not l.rmSkillInfo and string.find(texture,'ability_ava_002',1,true) then
          l.rmSkillInfo = info
...
= function(token, force)
  ...
  l.debug(2)('arr-core:l.switch SlotSkillAbilityInSlot(skillType:%d, skillLine:%d, abilityIndex:%d, slotNum:%d)',l.rmSkillInfo.skillType,
    l.rmSkillInfo.skillLine, l.rmSkillInfo.abilityIndex, slotNum)
end
