...
function MasterThief.OnInventorySlotUpdate(eventCode, bagId, slotIndex, isNew, itemSoundCategory, inventoryUpdateReason, stackCountChange)
    ...
    if isNew then
        ...
        -- Check item type and explicitly protect tools (lockpicks), crafting mats, style mats, and ingredients
        ...
        -- Handle auto-destruction of white-quality stolen junk items (while protecting quests, mats, styles, and tools/lockpicks)
        if not isQuestItem and not isProtectedMaterial and isStolen and quality <= 1 and MasterThief.savedVars.autoDestroyWhiteJunk then
            MasterThief.DebugLog("Auto-destroying white non-mat/style/lockpick stolen junk: " .. tostring(link))
