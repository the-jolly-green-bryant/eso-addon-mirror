...
--[[
Each ZoneCache stores the deserialized book nodes for one zone (LoreLibraryData[zoneId]).

Books can arrive in any order (however they happen to be encoded in
LoreLibraryData), but AddBook always inserts each node right at the end of
...
LoreLibrary.ZoneCache = ZoneCache
...
function ZoneCache:AddBook(bookId, worldX, worldY, worldZ)
	local pinTypeId = LoreLibrary.GetPinTypeForBookId(bookId)
	...
	local growing = false
	for _, otherPinTypeId in ipairs(LoreLibrary.PINTYPES) do
...
-- LoreLibrary.PINTYPES order (nil if pinTypeId is the first one) - callers
-- already iterate LoreLibrary.PINTYPES in order, so they can track it
