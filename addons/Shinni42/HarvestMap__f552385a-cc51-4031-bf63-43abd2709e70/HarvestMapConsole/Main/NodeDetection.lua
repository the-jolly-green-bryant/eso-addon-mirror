...
local CREATE_UNKNOWN_PINS = false -- false for console version
-- true for PC version
...
function Detection.VerifyPinType(event, control)
	...
	if control.mapCache and control.nodeId then
		local nodePinTypeId = control.mapCache:RetrievePinTypeId(control.nodeId)
...
function Detection.LinkControlWithNode(event, control)
	...
	if control.mapCache ~= mapCache or nodeId ~= control.nodeId then
		...
		if control.mapCache and control.nodeId then
			...
			if control.mapCache:RetrievePinTypeId(control.nodeId) == Harvest.UNKNOWN then
	...
	if mapCache == nil and nodeId == nil then
		...
		if not CREATE_UNKNOWN_PINS then return end
