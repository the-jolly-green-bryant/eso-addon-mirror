...
local LibNodeDetection = LibNodeDetection
...
function InRangePins:Initialize()
	...
	CallbackManager:RegisterForEvent(Events.SETTING_CHANGED, function(event, setting, ...)
		...
		elseif setting == "worldPinsVisible" then
			...
			self:RefreshRange()
		elseif setting == "worldBaseTexture" then
			self:RefreshWorldPins()
...
function InRangePins.pinUpdateCallback(divisionIndex, self)
	...
	local pairs = pairs
	local pinTypeIndex = 1
	local pinTypeId = nil
	local nodesOfPinTypeEnd
	local orderedPinTypes
	local UpdatePin = self.UpdatePin
	...
	for map, mapCache in pairs(self.zoneCache.mapCaches) do
		...
		--for _, pinTypeId in pairs(self.validPinTypeIds) do
			division = mapCache.divisions[divisionIndex]
			if division then
				nodesOfPinTypeEnd = mapCache.nodesOfPinTypeEnd
				pinTypeIndex = 1
				pinTypeId = mapCache.orderedPinTypes[pinTypeIndex]
				orderedPinTypes = mapCache.orderedPinTypes
				for _, nodeId in pairs(division) do
					while nodesOfPinTypeEnd[pinTypeId] < nodeId do
						pinTypeIndex = pinTypeIndex + 1
						pinTypeId = orderedPinTypes[pinTypeIndex]
					end
					UpdatePin(self, mapCache, nodeId, pinTypeId, lastUpdate, compassKeys, worldKeys)
				end
			end
		--end
	end
...
function InRangePins:UpdatePin(mapCache, nodeId, pinTypeId, lastUpdate, compassKeys, worldKeys)
	...
	local validWorldPin = self.displayWorldPins and self.worldFilterProfile[pinTypeId]
	if validWorldPin then
		if self.settings.worldSpawnFilter then
	...
	if self.settings.compassSpawnFilter then
		if Harvest.HARVEST_NODES[pinTypeId] and not mapCache.hasCompassPin[nodeId] then
			return
		end
	end
	...
	control:SetAlpha(1 - normalizedDistance*normalizedDistance*normalizedDistance*normalizedDistance)
end
...
function InRangePins:GetNewWorldControl(pinTypeId, worldX, worldY, worldZ)
	...
	beam:SetTexture(self.settings.worldBaseTexture) --"HarvestMapConsole/Textures/worldMarker.dds")
