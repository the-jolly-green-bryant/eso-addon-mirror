...
-- Last Updated April 18, 2020 by @tinydog
...
function tcc_Initialize()
	...
	-- Restore item type selections from SavedVars
	--tcc_SetItemLevel(tcc.SavedVars.Local.ItemLevelSelected)
	--tcc.ItemBuilder.Level = tcc.SavedVars.Local.ItemLevelSelected
	tccLevelSlider:SetValue(tcc_GetLevelIndex(tcc_GetPlayerLevel()))
	--tcc_SetItemType(tcc.SavedVars.Local.ArmorCategorySelected)
	--tcc_SetItemType(tcc.SavedVars.Local.BodyPartSelected)
	--tcc_SetItemType(tcc.SavedVars.Local.OtherItemTypeSelected)
	...
	--tcc.OrderItemCategories = { [1] = [[OrderItem]] }
	...
	--if tcc.IntegrateMM then tcc.CheapestEssenceRune = tcc_GetCheapestEssenceRune() else tcc.CheapestEssenceRune = tcc.EssenceRunes["Dekeipa"] end
...
function tcc_Sort(tableToSort, indexOrFieldName) --, descending)
	...
	if indexOrFieldName ~= nil and (type(indexOrFieldName) == "number" or type(indexOrFieldName) == "string") then
		...
		table.sort(tableToSort, function(a, b)
				...
				--return (descending and (b[indexOrFieldName] < a[indexOrFieldName]) or (a[indexOrFieldName] < b[indexOrFieldName]))
			end)
	else
		table.sort(tableToSort, function(a, b)
				return (a < b)
				--return (descending and (b < a) or (a < b))
			end)
	end
end
...
--function tcc_ShowOverlay(control, r, g, b, a, blockControl)
--	if control == nil then return end
--	if r == nil then r = 0 end
--	if g == nil then g = 0 end
--	if b == nil then b = 0 end
--	if a == nil then a = 0.8 end
--	if blockControl == nil then blockControl = true end
--	control.Overlay = tcc_GetOrCreateControl(GuiRoot, control:GetName() .. "Overlay", CT_TOPLEVELCONTROL)
--	control.Overlay:SetInheritScale(true)
--	control.Overlay:ClearAnchors()
--	control.Overlay:SetAnchor(TOPLEFT, control, TOPLEFT, 0, 0)
--	control.Overlay:SetAnchor(BOTTOMRIGHT, control, BOTTOMRIGHT, 0, 0)
--	control.Overlay:SetHidden(false)
--	control.Overlay:SetAllowBringToTop(true)
--	control.Overlay:BringWindowToTop()
--	control.Overlay:SetAllowBringToTop(false)
--	if blockControl then control.Overlay:SetMouseEnabled(true) end
--	local bg = tcc_GetOrCreateControl(control.Overlay, control.Overlay:GetName() .. "BG", CT_TEXTURE)
--	bg:SetInheritAlpha(false)
--	bg:SetColor(r, g, b, a)
--	bg:ClearAnchors()
--	bg:SetAnchor(TOPLEFT)
--	bg:SetAnchor(BOTTOMRIGHT)
