...
-- ESO Constants
local TCC_CHAMPION_ABILITY_INDEX_INSPIRATION_BOOST = 223
...
function tcc_InitXPCheckboxes()
	-- tccInspirationBoostCheckbox (TODO: Disabled until I figure out the new champion skill syntax)
	--local isInspirationBoostUnlocked = GetNumPointsSpentInChampionDiscipline(TCC_CHAMPION_DISCIPLINE_INDEX_TOWER) >= GetChampionSkillUnlockLevel(TCC_CHAMPION_DISCIPLINE_INDEX_TOWER, TCC_CHAMPION_ABILITY_INDEX_INSPIRATION_BOOST)
	--tcc_InitCheckbox(tccInspirationBoostCheckbox, tcc_PopulateXpTable, "Inspiration Boost?", "The Inspiration Boost champion point passive ability (under The Tower) adds 20% to Inspiration", isInspirationBoostUnlocked)
...
function tcc_SelectCraft(craftName)
	...
	tccXpItemQualityGreen:SetHidden(craftName == "Jewelry")
	tccXpItemQualityBlue:SetHidden(craftName == "Jewelry")
	tccXpItemQualityPurple:SetHidden(craftName == "Jewelry")
...
function tcc_PopulateEnchantingXpCell_CostOfMaterials(xpIndex, craftingXp, numItems)
	...
	--local essenceRune, essencePrice = tcc_GetCheapestEssenceRune()
...
--[[
function tcc_GetCheapestEssenceRune()
	local cheapestRune, cheapestPrice, avgPrice
	for rune in pairs(tcc.EssenceRunes) do
		if cheapestRune == nil then cheapestRune = rune end
		avgPrice = MasterMerchant:itemStats(rune.ItemLink)["avgPrice"]
		if avgPrice ~= nil then
			if cheapestPrice == nil or avgPrice < cheapestPrice then
				cheapestRune = rune
				cheapestPrice = avgPrice
			end
		end
	end
	return cheapestRune, cheapestPrice
end
]]
