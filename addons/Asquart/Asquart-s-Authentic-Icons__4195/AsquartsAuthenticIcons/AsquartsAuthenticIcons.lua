...
AAI.name     = "AsquartsAuthenticIcons"
AAI.version  = "1.4.1"
AAI.author   = "|c24abfe@Asquart|r"
AAI.active   = false

local ADDON_ICONS_PATH = "/AsquartsAuthenticIcons/icons/"
...
local function InitializeIcons()
    if AbilityIconsFramework and AbilityIconsFramework.AddCustomIconPack then
        AAI.settings.use128Version = AAI.savedVariables.use128Version
        
        local iconsPath
        if AAI.settings.use128Version then
            iconsPath = ADDON_ICONS_PATH .. "128/"
        else
            iconsPath = ADDON_ICONS_PATH .. "64/"
        end
        -- Add the custom icon pack
        AbilityIconsFramework.AddCustomIconPack(iconsPath, ADDON_ICONS)
    else
        d("AsquartDpsIconPack: AbilityIconsFramework not found or incompatible!")
    end
end

function AAI.OnAddonLoaded(_, addonName)
	if addonName ~= AAI.name then
		return
	end

    AAI.savedVariables = ZO_SavedVars:NewAccountWide("AsquartsAuthenticIconsSavedVariables", 1, nil, AAI.settings)
    AAI.AddonMenu()

    InitializeIcons()

	EVENT_MANAGER:UnregisterForEvent(AAI.name, EVENT_ADD_ON_LOADED)
end

EVENT_MANAGER:RegisterForEvent(AAI.name, EVENT_ADD_ON_LOADED, AAI.OnAddonLoaded)