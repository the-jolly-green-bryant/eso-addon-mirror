...
function LibAlchemy:InitializePrices(source)
  ...
  if source == LibAlchemy.SOURCE_MT and LibAlchemy.isMTInitialized then return end
  if source == LibAlchemy.SOURCE_TSC and LibAlchemy.isTSCInitialized then return end
  ...
  if source == LibAlchemy.SOURCE_MT then LibAlchemy.isMTInitialized = true end
  if source == LibAlchemy.SOURCE_TSC then LibAlchemy.isTSCInitialized = true end
...
---- Most of functions below this point will have no explanations as they probably
---- won't be of any use on their own and they're there to be used in functions above

function LibAlchemy:getCraftingCostWithoutItemLink(effect1, effect2, effect3, level, priceFunc)
  if not (effect1 and effect2 and effect3 and level and priceFunc) then return nil end

  local function reagentHasEffects(reagentId)
    local reagent = LibAlchemy.reagents[reagentId]
    return reagent and reagent[3]
  end

  -- Collect valid reagents for each effect
  local valid1 = LibAlchemy.effects[effect1] or {}
  local valid2 = LibAlchemy.effects[effect2] or {}
  local valid3 = LibAlchemy.effects[effect3] or {}

  -- Use set-based lookup for performance
  local valid2Set = {}
  for _, id in ipairs(valid2) do valid2Set[id] = true end
  local valid3Set = {}
  for _, id in ipairs(valid3) do valid3Set[id] = true end

  -- Search combinations
  for _, id1 in ipairs(valid1) do
    if reagentHasEffects(id1) then
      for _, id2 in ipairs(valid2) do
        if id2 ~= id1 and reagentHasEffects(id2) then
          for _, id3 in ipairs(valid3) do
            if id3 ~= id1 and id3 ~= id2 and reagentHasEffects(id3) then
              local found = false
              local reagents = { id1, id2, id3 }
              -- Flatten all effects
              local allEffects = {}
              for _, rId in ipairs(reagents) do
                for _, effect in ipairs(LibAlchemy.reagents[rId][3]) do
                  allEffects[effect] = (allEffects[effect] or 0) + 1
                end
              end
              -- Check if all required effects appear at least twice
              if (allEffects[effect1] or 0) >= 2 and
                (allEffects[effect2] or 0) >= 2 and
                (allEffects[effect3] or 0) >= 2 then
                found = true
              end

              if found then
                -- Look up a matching solvent
                local solvent = nil
                for tier, data in pairs(LibAlchemy.solvents) do
                  if type(data) == "table" and data[level] then
                    solvent = data[level][1]
                    break
                  end
                end
                if not solvent then
                  LibAlchemy:dm("Warn", "No solvent found for level", level)
                  return nil
                end

                local cost = priceFunc(solvent)
                for _, rId in ipairs(reagents) do
                  cost = cost + priceFunc(rId)
                end
                return cost
              end
            end
  ...
  LibAlchemy:dm("Info", "No valid reagent combination found for effects", effect1, effect2, effect3)
  return nil
end
