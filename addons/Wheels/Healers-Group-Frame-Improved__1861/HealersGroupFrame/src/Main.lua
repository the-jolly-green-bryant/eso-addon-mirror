...
------------------------------------------------------------
	...
	* Author: @ShaiT
	*
	* Modified January 2, 2020 by @Amasuriel, removed unused regen options, added options for warhorn, berserk and both warhorn and berserk
]]----------------------------------------------------------

HGF = {}
HGF.name = "HealersGroupFrame"
HGF.version = "1.5.1"
HGF.savedVarsVersion = 1.2
...
HGF.horns = {
	...
	--[114841] = "Sun shield, for easy testing",
}

HGF.berserk = {
	[62636] = "Minor Berserk" --from Combat Prayer IV
}
...
function HGF.UpdateVolatileUnitInfo(unitTag)
	...
	if IsUnitOnline(unitTag) then
		if IsUnitDead(unitTag) then
			...
			local currentHp, maxHp, effectiveMaxHp, value, maxValue, shield, warhorn, berserk, warhornAndBerserk
			...
			warhornAndBerserk = 0
			for i = 1, numbuffs do
				local _,_,_,_,_,_,_,_,_,_,ID = GetUnitBuffInfo(unitTag,i)
				if HGF.horns[ID] then
					warhorn = 1
				end
				if HGF.berserk[ID] then
					berserk = 1
				end
				if warhorn >= 1 and berserk >= 1 then
					warhornAndBerserk = 1
				end
			end
			...
			HGF.UI.SetOptionalBuffs(unitTag, warhorn, berserk, warhornAndBerserk)
			HGF.UI.SetUnitStealth(unitTag, GetUnitStealthState(unitTag))
			HGF.UI.UpdateUnitName(unitTag, GetUnitName(unitTag), GetUnitDisplayName(unitTag))
		end
	...
	local defaultVars = {
		...
		["presets"] = {
			[1] = {
				...
				["showWarhorn"] = true,
				["showBerserk"] = true,
				["showWarhornAndBerserk"] = true,
				["healthBarColor"] = {1, 0, 0},
				["hornBarColor"] = {0, 1, 0},
				["berserkBarColor"] = {0, 0, 1},
				["hornAndBerserkBarColor"] = {.5, .5, .5},
