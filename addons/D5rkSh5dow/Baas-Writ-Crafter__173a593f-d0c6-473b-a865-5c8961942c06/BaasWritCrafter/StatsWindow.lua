...
function RewardsScroll:BuildMasterList()
	...
	for craft, craftRewards in pairs(self.data) do
		...
		local header = GetControl(BaasLazyWritStatsWindowBackdropCraftHeader,"Craft"..craft)
		...
		local questGoldControl= GetControl(BaasLazyWritStatsWindowBackdrop, "QuestGold")
		...
		local itemValue = GetControl(BaasLazyWritStatsWindowBackdrop, "ItemGold")
		itemValue:SetText("Total Item Value: "..formatNum(totalValue).."g")
		local itemValue = GetControl(BaasLazyWritStatsWindowBackdrop, "TotalGold")
...
function WritCreater.setupScrollLists()
	WritCreater.rewardsScrollManager = RewardsScroll:New(BaasLazyWritStatsWindowRewardScroll) -- check
end
...
local function updateNonScrollElements()
	...
	BaasLazyWritStatsWindowBackdropWritCounter:SetText(zo_strformat(WritCreater.strings['statsWitsDone'] ,WritCreater.savedVarsAccountWide.total , daysSinceReset))
	
	local itemGold= GetControl(BaasLazyWritStatsWindowBackdrop, "ItemGold")
	local totalGold= GetControl(BaasLazyWritStatsWindowBackdrop, "TotalGold")
	local estimateWarning = GetControl(BaasLazyWritStatsWindowBackdrop, "EstimateWarning")
end
...
function WritCreater.initializeStatsScene()
	if not WritCreater.writStatsScene  then
		...
		WritCreater.writStatsScene:AddFragment(ZO_SimpleSceneFragment:New(BaasLazyWritStatsWindow))
		-- local myButtonGroup = {
		-- {
		-- 	name = "Craft Writ",
	...
	if GetDate()%10000 == 1031 then
		BaasLazyWritStatsWindowBackdropTitle:SetText("Baas Writ Crafter")
	end
end
