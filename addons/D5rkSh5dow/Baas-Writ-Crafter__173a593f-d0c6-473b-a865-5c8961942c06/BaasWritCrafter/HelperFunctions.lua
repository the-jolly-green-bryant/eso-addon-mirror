...
function WritCreater.showQRCode(url)
    if not BaasLazyWritQRCode or not LibQRCode then
    ...
    if not WritCreater.qrCodeScene then
        ...
        WritCreater.qrCodeScene:AddFragment(ZO_SimpleSceneFragment:New(BaasLazyWritQRCode))
    end
    LibQRCode.DrawQRCode(BaasLazyWritQRCodeBackdropOutput, url)
