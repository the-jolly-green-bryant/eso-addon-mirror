	...
	for i = 1, GROUP_SIZE_MAX do
		...
		if DoesUnitExist(unitTag) and IsUnitGrouped("player") then
			...
			if unit.index ~= GetGroupIndexByUnitTag(unitTag) then
				...
				Jo.SetShield(nil, unitTag)
				Jo.SetTrauma(nil, unitTag)
			...
			unit.classId = GetUnitClassId(unitTag)
			...
			-- Class
			-- The keyboard icon (index 7) has too much padding and looks blurrier than the gamepad icon (index 8),
			-- though the lack of padding means we need to compensate with an extra space after the icon
			if (unit.classId > 0) then
				local path = select(8, GetClassInfo(GetClassIndexById(unit.classId)))
				unit.classIcon = zo_iconTextFormat(path, Jo.classIconSize, Jo.classIconSize, "")
			end
			...
			if i==1 then
				if Jo.savedVars.colours.customColour then
					frame.bar:SetColor(Jo.GetColorRGBA(Jo.savedVars.colours.customTankColour))
				else
					ZO_StatusBar_SetGradientColor(frame.bar, Jo.barColour[Jo.savedVars.colours.tankColour])
				end
			elseif i==2 then
				if Jo.savedVars.colours.customColour then
					frame.bar:SetColor(Jo.GetColorRGBA(Jo.savedVars.colours.customHealerColour))
				else
					ZO_StatusBar_SetGradientColor(frame.bar, Jo.barColour[Jo.savedVars.colours.healerColour])
				end
			else
				if Jo.savedVars.colours.customColour then
					frame.bar:SetColor(Jo.GetColorRGBA(Jo.savedVars.colours.customDpsColour))
				else
...
function Jo.VisualAdded(event, unitTag, unitAttributeVisual, statType, attributeType, powerType, value, maxValue)
	...
	if unitAttributeVisual == ATTRIBUTE_VISUAL_POWER_SHIELDING then
		Jo.SetShield(event, unitTag, value)
	elseif unitAttributeVisual == ATTRIBUTE_VISUAL_TRAUMA then
		Jo.SetTrauma(event, unitTag, value)
	elseif unitAttributeVisual == ATTRIBUTE_VISUAL_INCREASED_REGEN_POWER then
...
function Jo.VisualUpdated(event, unitTag, unitAttributeVisual, statType, attributeType, powerType, oldValue, newValue, oldMaxValue, newMaxValue)
	...
	if unitAttributeVisual == ATTRIBUTE_VISUAL_POWER_SHIELDING then
		Jo.SetShield(event, unitTag, newValue)
	elseif unitAttributeVisual == ATTRIBUTE_VISUAL_TRAUMA then
		Jo.SetTrauma(event, unitTag, newValue)
	end
end
...
-- Shield
function Jo.SetShield(event, unitTag, value)
	if not DoesUnitExist(unitTag) then return end
	if not ZO_Group_IsGroupUnitTag(unitTag) then return end
	if Jo.unit[unitTag] == nil then return end
	local unit = Jo.unit[unitTag]
	...
	value = value or GetUnitAttributeVisualizerEffectInfo(unitTag, ATTRIBUTE_VISUAL_POWER_SHIELDING, STAT_MITIGATION, ATTRIBUTE_HEALTH, COMBAT_MECHANIC_FLAGS_HEALTH)
...
-- Trauma
function Jo.SetTrauma(event, unitTag, value)
	if not DoesUnitExist(unitTag) then return end
	if not ZO_Group_IsGroupUnitTag(unitTag) then return end
	if Jo.unit[unitTag] == nil then return end
	local unit = Jo.unit[unitTag]
	...
	value = value or GetUnitAttributeVisualizerEffectInfo(unitTag, ATTRIBUTE_VISUAL_TRAUMA, STAT_MITIGATION, ATTRIBUTE_HEALTH, COMBAT_MECHANIC_FLAGS_HEALTH)

	if value and value > 0 then
		unit.trauma = value
		frame.traumaBar:SetWidth(zo_min(value / powerEffectiveMax, 1) * 0.5 * frame.bar:GetWidth())
...
function Jo.UpdatePower(event, unitTag, powerIndex, powerType, powerValue, powerMax, powerEffectiveMax)
	...
	local trauma = unit.trauma and unit.trauma or 0

	local RoundK = function( num )
		return string.format("%dk", zo_round(num / 1000))
	end
	...
	if powerValue and powerValue > 0 then
		local health = RoundK(powerValue)
		...
		local maxHealth = RoundK(powerEffectiveMax)
		if Jo.savedVars.show.fullNumberHealth then
		...
		if shield > 0 or trauma > 0 then
			local shieldK = (shield > 0) and RoundK(shield) or ""
			local shieldRaw = (shield > 0) and shield or ""
			local traumaK = (trauma > 0) and string.format("|c%06X-%s|r", Jo.GetColorInt24(Jo.savedVars.colours.trauma), RoundK(trauma)) or ""
			local traumaRaw = (trauma > 0) and string.format("|c%06X-%s|r", Jo.GetColorInt24(Jo.savedVars.colours.trauma), trauma) or ""
			values = string.format("%s [%s%s]", values, shieldK, traumaK)
			tipValues = string.format("%s [%s%s]", tipValues, shieldRaw, traumaRaw)
		end
		...
		for i = 1, numBuffs do
			...
			if Jo.savedVars.show.warHorn then
				if Jo.isHorny[abilityId] then
					frame.bgColor:SetCenterColor(Jo.GetColorRGBA(Jo.savedVars.colours.warHorn))
					frame.bgColor:SetAlpha(0.5)
				end
			end

			if Jo.savedVars.show.crShade then
				if abilityId == Jo.crShade then
					frame.bgColor:SetCenterColor(Jo.GetColorRGBA(Jo.savedVars.colours.crShade))
					frame.bgColor:SetAlpha(0.5)
				end
			end
		end
...
function Jo.CustomizeUnits()
	...
	for i = 1, GROUP_SIZE_MAX do
		...
		Jo.classIconSize = fontSize-(1*sf)
		...
		if compactMode then
			...
			frame.class:SetAnchor(TOPLEFT, frame.statusBar, TOPLEFT, 0, -(4*sf))
		else
			-- Normal size
			...
			frame.class:ClearAnchors()
			frame.class:SetAnchor(BOTTOMLEFT, frame.statusBar, TOPLEFT, 0, -(4*sf))
		end
		...
		frame.shieldGloss:SetDimensions(frame.shieldBar:GetDimensions())
		frame.shieldBar:SetColor(Jo.GetColorRGBA(Jo.savedVars.colours.shield))
		frame.traumaBar:SetCenterColor(Jo.GetColorRGBA(Jo.savedVars.colours.trauma))
...
function Jo.RegisterEvents()
	...
	evm:AddFilterForEvent(Jo.name, EVENT_UNIT_CREATED, REGISTER_FILTER_UNIT_TAG_PREFIX, "group")
	evm:AddFilterForEvent(Jo.name, EVENT_UNIT_DESTROYED, REGISTER_FILTER_UNIT_TAG_PREFIX, "group")
	evm:AddFilterForEvent(Jo.name, EVENT_LEVEL_UPDATE, REGISTER_FILTER_UNIT_TAG_PREFIX, "group")
	evm:AddFilterForEvent(Jo.name, EVENT_CHAMPION_POINT_UPDATE, REGISTER_FILTER_UNIT_TAG_PREFIX, "group")
	evm:AddFilterForEvent(Jo.name, EVENT_ZONE_UPDATE, REGISTER_FILTER_UNIT_TAG_PREFIX, "group")

	evm:AddFilterForEvent(Jo.name, EVENT_POWER_UPDATE, REGISTER_FILTER_UNIT_TAG_PREFIX, "group")
	evm:AddFilterForEvent(Jo.name, EVENT_UNIT_DEATH_STATE_CHANGED, REGISTER_FILTER_UNIT_TAG_PREFIX, "group")
	evm:AddFilterForEvent(Jo.name, EVENT_UNIT_ATTRIBUTE_VISUAL_ADDED, REGISTER_FILTER_UNIT_TAG_PREFIX, "group")
	evm:AddFilterForEvent(Jo.name, EVENT_UNIT_ATTRIBUTE_VISUAL_UPDATED, REGISTER_FILTER_UNIT_TAG_PREFIX, "group")
	evm:AddFilterForEvent(Jo.name, EVENT_UNIT_ATTRIBUTE_VISUAL_REMOVED, REGISTER_FILTER_UNIT_TAG_PREFIX, "group")
	evm:AddFilterForEvent(Jo.name, EVENT_STEALTH_STATE_CHANGED, REGISTER_FILTER_UNIT_TAG_PREFIX, "group")
	evm:AddFilterForEvent(Jo.name, EVENT_DISGUISE_STATE_CHANGED, REGISTER_FILTER_UNIT_TAG_PREFIX, "group")

	evm:RegisterForEvent(Jo.name, EVENT_EFFECT_CHANGED, function(_, _, _, _, unitTag) Jo.UpdateBuffs(unitTag) end)
	evm:AddFilterForEvent(Jo.name, EVENT_EFFECT_CHANGED, REGISTER_FILTER_UNIT_TAG_PREFIX, "group")
	evm:AddFilterForEvent(Jo.name, EVENT_EFFECT_CHANGED, REGISTER_FILTER_ABILITY_ID, Jo.crShade)
