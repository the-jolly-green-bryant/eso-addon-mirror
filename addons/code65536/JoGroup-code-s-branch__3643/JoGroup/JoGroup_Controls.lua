...
--]]
	...
	for i = 1, GROUP_SIZE_MAX do
		...
		frame.statusBar:SetAnchor(BOTTOM, frame, BOTTOM, 0, -(3*sf))
			...
			frame.traumaBar = wim:CreateControl(nil, frame.bar, CT_BACKDROP)
			frame.traumaBar:SetAnchor(TOPLEFT, frame.bar, TOPLEFT)
			frame.traumaBar:SetAnchor(BOTTOMRIGHT, frame.bar, BOTTOMRIGHT, nil, nil, ANCHOR_CONSTRAINS_Y)
			frame.traumaBar:SetHidden(true)
			frame.traumaBar:SetDrawLayer(DL_CONTROLS)
			frame.traumaBar:SetDrawLevel(5)
			frame.traumaBar:SetEdgeColor(0, 0, 0, 0)
			frame.traumaBar:SetEdgeTexture(nil, 1, 1, 1)
