...
local function escape_char(cha)
  return "\\" .. (escape_char_map[cha] or string.format("u%04x", cha:byte()))
end
...
encode = function(val, stack)
  local typ = type(val)
  local func = type_func_map[typ]
  if func then
    return func(val, stack)
  end
...
local function parse_unicode_escape(str)
  local n1 = tonumber( str:sub(1, 4),  16 )
  local n2 = tonumber( str:sub(7, 10), 16 )
  if n2 then
...
local function parse_number(str, i)
  ...
  local str1 = str:sub(i, x - 1)
  local n = tonumber(str1)
  if not n then
    decode_error(str, i, "invalid number '" .. str1 .. "'")
  end
  return n, x
end
