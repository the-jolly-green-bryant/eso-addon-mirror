...
local function UCT_GetLastKATime(charId)	return UCT.sVar.timeData[charId].timeKA  end
local function UCT_GetLastRGTime(charId)	return UCT.sVar.timeData[charId].timeRG  end
...
local function UCT_SetKATime(charId)	UCT.sVar.timeData[charId].timeKA = GetTimeStamp()  end
local function UCT_SetRGTime(charId)	UCT.sVar.timeData[charId].timeRG = GetTimeStamp()  end
...
local function UCT_UpdateTime(eventCode, isCompleted, journalIndex, questName, zoneIndex, poiIndex, questID)
	...
	if(isCompleted) then
		...
		if(questID == UCT.data.QuestId.KA and UCT_GetTimeRemaining(UCT.sVar.timeData[charId].timeKA) <= 0) then UCT_SetKATime(charId) return end
		if(questID == UCT.data.QuestId.RG and UCT_GetTimeRemaining(UCT.sVar.timeData[charId].timeRG) <= 0) then UCT_SetRGTime(charId) return end
	end
end
...
local function UCT_FillLine(currLine, currItem)
	...
	currLine.KAtime:SetText(	currItem == nil and "" or currItem.KAtime)
	currLine.RGtime:SetText(	currItem == nil and "" or currItem.RGtime)
end
	...
	if(GetNumCharacters() > 0 and UCT.sVar.numChars ~= nil and UCT.sVar.numChars > 0)then
		for k, v in pairs(UCT.sVar.charInfo) do
			table.insert(dataLines, {
				...
				KAtime = UCT_SecondsToClock(UCT_GetTimeRemaining(UCT.sVar.timeData[v.charId].timeKA), "d:hh:mm:ss"),
				RGtime = UCT_SecondsToClock(UCT_GetTimeRemaining(UCT.sVar.timeData[v.charId].timeRG), "d:hh:mm:ss"),
			})
		end
	end
...
local function UCT_CreateLine(i, predecessor, parent)
	...
	record.KAtime = record:GetNamedChild("_TimeKA")
	record.RGtime = record:GetNamedChild("_TimeRG")
...
local function UCT_OnStart()
	...
	UCT_GUI_Header_HeaderKA:SetText(zf("<<t:1>>", GetZoneNameById(UCT.data.ZoneId.KA)))
	UCT_GUI_Header_HeaderRG:SetText(zf("<<t:1>>", GetZoneNameById(UCT.data.ZoneId.RG)))
end
...
local function UCT_HelpSlash()
	...
	d(strF(GetString(UCT_MSG_CMD_OPTION_10), zf("<<t:1>>", GetZoneNameById(UCT.data.ZoneId.KA))))
	d(strF(GetString(UCT_MSG_CMD_OPTION_11), zf("<<t:1>>", GetZoneNameById(UCT.data.ZoneId.RG))))
	d(GetString(UCT_MSG_CMD_OPTION_12))
end
...
local function UCT_TestHarness()
	...
	UCT_UpdateTime(0, true, 0, "", GetZoneIndex(UCT.data.ZoneId.KA), GetZoneIndex(UCT.data.ZoneId.KA), UCT.data.QuestId.KA)
	UCT_UpdateTime(0, true, 0, "", GetZoneIndex(UCT.data.ZoneId.RG), GetZoneIndex(UCT.data.ZoneId.RG), UCT.data.QuestId.RG)
	]]--
end
...
local function UCT_Initialized(eventCode, addonName)
	...
	SLASH_COMMANDS["/uct"] = function(keyWord, argument)
		...
		elseif(string.lower(keyWord) == "ka") then
			--Show KA time elapsed
			d(strF("%s%s", GetString(UCT_MSG_LAST_KA), UCT_SecondsToClock(UCT_GetTimeElapsed(UCT_GetLastKATime(GCCId())))))
			d(strF("%s%s", GetString(UCT_MSG_NEXT_KA), UCT_SecondsToClock(UCT_GetTimeRemaining(UCT_GetLastKATime(GCCId())))))
		elseif(string.lower(keyWord) == "rg") then
			--Show RG time elapsed
			d(strF("%s%s", GetString(UCT_MSG_LAST_RG), UCT_SecondsToClock(UCT_GetTimeElapsed(UCT_GetLastRGTime(GCCId())))))
			d(strF("%s%s", GetString(UCT_MSG_NEXT_RG), UCT_SecondsToClock(UCT_GetTimeRemaining(UCT_GetLastRGTime(GCCId())))))
