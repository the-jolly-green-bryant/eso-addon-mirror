-- LPC03.lua - Consumables Database
LibPriceCache = LibPriceCache or {}
LibPriceCache.LPC03 = LibPriceCache.LPC03 or {}
local M = LibPriceCache.LPC03
M.name = "LPC03"

local serverKey = GetDisplayName() .. "_" .. GetWorldName()

function M:Init()
    M.db = LibSavedVars:NewAccountWide("LibPriceCache_LPC03_DB", "Data", {data = {}}, nil, serverKey)
    if LibPriceCache.Report then
        LibPriceCache.Report:Log("|c00FF00LPC03 initialized|r")
    end
end
...
function M:CleanOldData(maxAgeDays)
    ...
    if cleaned > 0 and LibPriceCache.Report then
        LibPriceCache.Report:Log(string.format("LPC03 cleaned %d expired items", cleaned))
    end
end

EVENT_MANAGER:RegisterForEvent("LPC03_Init", EVENT_ADD_ON_LOADED, function(_, addon)
