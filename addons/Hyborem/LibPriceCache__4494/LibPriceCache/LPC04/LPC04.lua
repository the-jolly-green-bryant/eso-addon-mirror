-- LPC04.lua - Crafting & Collectibles Database
LibPriceCache = LibPriceCache or {}
LibPriceCache.LPC04 = LibPriceCache.LPC04 or {}
local M = LibPriceCache.LPC04
M.name = "LPC04"

local serverKey = GetDisplayName() .. "_" .. GetWorldName()

function M:Init()
    M.db = LibSavedVars:NewAccountWide("LibPriceCache_LPC04_DB", "Data", {data = {}}, nil, serverKey)
    if LibPriceCache.Report then
        LibPriceCache.Report:Log("|c00FF00LPC04 initialized|r")
    end
end
...
function M:CleanOldData(maxAgeDays)
    ...
    if cleaned > 0 and LibPriceCache.Report then
        LibPriceCache.Report:Log(string.format("LPC04 cleaned %d expired items", cleaned))
    end
end

EVENT_MANAGER:RegisterForEvent("LPC04_Init", EVENT_ADD_ON_LOADED, function(_, addon)
