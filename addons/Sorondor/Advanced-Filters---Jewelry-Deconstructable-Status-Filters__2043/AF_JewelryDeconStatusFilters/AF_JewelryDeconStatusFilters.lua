...
--[[----------------------------------------------------------------------------
    ...
    A return value of true means the item in question will be shown while the
        ...
        is active.
--]]----------------------------------------------------------------------------
local function GetFilterCallback_ItemDeconstructable()
    return function(slot, slotIndex)
        ...
        return not IsItemLinkForcedNotDeconstructable(itemLink)
        
    end
end

local function GetFilterCallback_ItemNotDeconstructable()
    return function(slot, slotIndex)
        if util.prepareSlot ~= nil then
            if slotIndex ~= nil and type(slot) ~= "table" then
                slot = util.prepareSlot(slot, slotIndex)
...
local dropdownCallbacks = {
    [1] = {name = "DeconstructableJewelry",     filterCallback = GetFilterCallback_ItemDeconstructable()},
    [2] = {name = "NotDeconstructableJewelry",  filterCallback = GetFilterCallback_ItemNotDeconstructable()},
}
...
--]]----------------------------------------------------------------------------
local stringsEN = {
    ["DeconstructableJewelry"]      = "Deconstructable",
    ["NotDeconstructableJewelry"]   = "Not deconstructable",
}
local stringsDE = {
    ["DeconstructableJewelry"]      = "Kann verwertet werden",
    ["NotDeconstructableJewelry"]   = "Kann nicht verwertet werden",
}
local stringsFR = {
    ["DeconstructableJewelry"]      = "Peut être déconstruit",
    ["NotDeconstructableJewelry"]   = "Ne peut être déconstruit",
}
