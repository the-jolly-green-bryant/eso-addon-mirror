...
function EWPFinder:DisplayBuff(buffName, buffOn)
    ...
    if buffName == "Major Brutality" and buffOn == "removed" then
        ...
        return string.format("|c%sSTATUS: Buff Inactive|r", inactivecolor)
...
function EWPFinder:DisplayDamage()
    ...
    if weapon_damage > spell_damage then
        if weapon_damage == EWPFinder.savedVars.weapondamage then
            return string.format("|c%sPOWER: WD %d|r", hexcolor, EWPFinder.savedVars.weapondamage)
        else
    ...
    elseif spell_damage > weapon_damage then
        if spell_damage == EWPFinder.savedVars.spelldamage then
            return string.format("|c%sPOWER: SD %d|r", hexcolor, EWPFinder.savedVars.spelldamage)
        else
...
function EWPFinder:Initialize()
    ...
    if not self.db.settings.enablebuff and not self.db.settings.enableUp then
        self.db.settings.enablebuff = true
        self.db.settings.enableUp = true
    end
...
function EWPFinder:SetBuffAppearance(value)
    ...
    local showBars = showBuff or showUp
