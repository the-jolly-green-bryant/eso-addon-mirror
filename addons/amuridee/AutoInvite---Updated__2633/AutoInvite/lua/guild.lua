...
function AutoInvite.executeNameLookup(hasChar, charName, zone)
    ...
    if AutoInvite.cfg.cyrCheck then
        -- isCyrodiil() was being called twice here (each call also internally hit
        -- GetUnitZone twice, see AutoInvite.isCyrodiil), for up to 4 API calls per
        -- lookup. Compute it once and reuse the result.
        local inCyrodiil = AutoInvite.isCyrodiil()
        if AutoInvite.debug then
            dbg("In Cyrodiil? " .. b(inCyrodiil) .. " / Zone: " .. zone)
        end

        if inCyrodiil and zone ~= "Cyrodiil" then
...
function AutoInvite.accountNameLookup(channel, acctName, fromDisplayName)

    if ZO_ShouldPreferUserId() then return fromDisplayName end
