...
function AutoInvite:checkSentInvites()
    -- This runs every 1s tick while listening. The group-membership scan below
    -- (a GetGroupSize() loop plus one GetUnitName() call per member) exists only
    -- to check names against self.sentInvite, so skip it entirely when there's
    -- nothing outstanding -- the common case, since invites resolve or expire
    -- within 15-30s.
    if next(self.sentInvite) == nil then return end
...
function AutoInvite:IsInQueue(name)
    -- queue.back is always one past the last real item (that's how push/pop
    -- maintain it), so queue.vals[queue.back] is always nil -- looping through
    -- it was a harmless but wasted extra comparison every call.
