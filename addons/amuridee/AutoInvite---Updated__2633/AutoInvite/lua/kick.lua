...
function AutoInvite.kickCheck()
    ...
    for p, t in pairs(AutoInvite.kickTable) do
        ...
        if offTime > AutoInvite.cfg.kickDelay then
            ...
            -- Runs every 1s tick for as long as this player is offline (up to
            -- cfg.kickDelay, which defaults to 300s+). Gate the concatenation
            -- so it isn't built and discarded every tick when debug is off.
