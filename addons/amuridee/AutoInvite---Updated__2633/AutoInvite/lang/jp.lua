...
ZO_CreateStringId("SI_AUTO_INVITE_ERROR_NOT_GROUP_LEADER", "あなたはグループリーダーではありません！")
...
ZO_CreateStringId("SI_AUTO_INVITE_OPT_ENROLMENT", "自動入力される募集メッセージ")
ZO_CreateStringId("SI_AUTO_INVITE_TT_ENROLMENT", "上部にあるボタンをクリックした際、募集メッセージを投稿するためにチャット欄へ自動入力される文章です。以下のタグを使用すると、AutoInviteによって自動的に置換されます：\n++cn 現在のPvE/PvPキャンペーン名\n++gs 現在のグループ人数\n++rs グループの残り空き枠数\n++ro 募集する役割（下記で設定）")
ZO_CreateStringId("SI_AUTO_INVITE_OPT_NEEDED", "募集メッセージ用の必要なグループの役割（自身の役割を含む）")

ZO_CreateStringId("SI_AUTO_INVITE_OTHER_SETTINGS", "主な設定")
ZO_CreateStringId("SI_AUTO_INVITE_CHATS_LISTEN", "聞くべきチャット")
ZO_CreateStringId("SI_AUTO_INVITE_GROUP_SETTINGS", "グループ設定")
