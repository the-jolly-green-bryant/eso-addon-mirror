...
function HGF.UI.UpdateTextWidth(unitTag)
	...
	if HGF.activeVars["textType"][1] == HGF.textName then
...
function HGF.UI.UpdateUnitName(unitTag, name)
	if HGF.UI.unitFrames[unitTag] == nil then
		return end
...
function HGF.UI.SetHealthRegen(unitTag, regen)
	if not HGF.activeVars["showHealthRegen"] or
			HGF.UI.unitFrames[unitTag] == nil then
		return end

	-- if regen > 0 then
	-- 	HGF.UI.unitFrames[unitTag].hpBar:SetColor(HGF.activeVars["incHealthRegenColor"][1], HGF.activeVars["incHealthRegenColor"][2], HGF.activeVars["incHealthRegenColor"][3])
	-- else
		HGF.UI.unitFrames[unitTag].hpBar:SetColor(HGF.activeVars["healthBarColor"][1], HGF.activeVars["healthBarColor"][2], HGF.activeVars["healthBarColor"][3])
	-- end
end

function HGF.UI.SetPurge(unitTag)
	-- local rC, gC, bC = HGF.UI.unitFrames[unitTag].hpBar:GetColor()
	for i = 1, GetNumBuffs(unitTag) do
		local buffName, timeStarted, timeEnding, buffSlot, stackCount, iconFilename, buffType, effectType, abilityType, statusEffectType, abilityId, xyz, castByPlayer = GetUnitBuffInfo(unitTag, i)
		...
		if effectType == BUFF_EFFECT_TYPE_DEBUFF then
			...
			if (permanent) then
				HGF.UI.unitFrames[unitTag].hpBar:SetColor(0, 0, 1) 
			elseif (timeEnding - timeStarted >= 3) then
				HGF.UI.unitFrames[unitTag].hpBar:SetColor(0, 1, 0) 
			end
		
		return end
			
	end
	-- HGF.UI.unitFrames[unitTag].hpBar:SetColor(0.3, 0.3, 0.3)
