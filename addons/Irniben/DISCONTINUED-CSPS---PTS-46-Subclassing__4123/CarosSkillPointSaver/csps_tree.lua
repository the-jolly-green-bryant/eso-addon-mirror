...
local function NodeSetup(node, control, data, open, userRequested, enabled)
	...
	if i == 1 then		
		j = CSPS.currentClassSkillLines[j] 
...
function CSPS.NodeSectionSetup(node, control, data, open, userRequested, enabled)
	...
	if data.variant == TREE_SECTION_SKILLTYPES or data.variant == TREE_SECTION_SKILLLINES then -- Skill type/line
		...
		if data.variant == TREE_SECTION_SKILLLINES and data.skillType == 1 then
			local skillLineIndex = CSPS.currentClassSkillLines[data.skillLineIndex]
			...
			myCtrBtnApply:SetNormalTexture("esoui/art/progression/train_up.dds")
			myCtrBtnApply:SetMouseOverTexture("esoui/art/progression/train_over.dds")
			myCtrBtnApply:SetPressedTexture("esoui/art/progression/train_down.dds")
