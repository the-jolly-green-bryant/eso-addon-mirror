...
function CSPS.showSkillProfileTT(control, myType, myId)
	...
	local activeSkills, passiveSkills, changedRace, crafted, subclasses = false, false, false, false, false
	
	if myProfile.hbComp then activeSkills, passiveSkills, changedRace, crafted, subclasses = CSPS.skTableExtract(myProfile.hbComp.prog, myProfile.hbComp.pass, true, true, myProfile.hbComp.crafted, myProfile.hbComp.subclasses) end
...
local function loadSkillProfile(myType, myId, _)
	...
	if myProfile.hbComp then
		local morphs, upgrades, _, crafted, styles, subclasses = CSPS.skTableExtract(myProfile.hbComp.prog, myProfile.hbComp.pass, false, false, myProfile.hbComp.crafted, myProfile.hbComp.styles, myProfile.hbComp.subclasses)
		CSPS.populateSkills(morphs, upgrades, true, crafted, styles, subclasses) -- true = don't reset the lists beforehand
	end
