...
local ROUND_SOLID = "ESOAdventurerSuite/Art/Modern/Rounded/solid.dds"
local ROUND_CORNER = "ESOAdventurerSuite/Art/Modern/Rounded/corner_fill.dds"
local ROUND_BORDER = "ESOAdventurerSuite/Art/Modern/Rounded/corner_border.dds"
local ROUND_PANEL_ROOT = "ESOAdventurerSuite/Art/eas_round_panel.dds"
local ROUND_SHELL_ROOT = "ESOAdventurerSuite/Art/eas_round_shell.dds"
local ROUND_PILL_ROOT = "ESOAdventurerSuite/Art/eas_round_pill.dds"
...
local CLASS_CARDS = {
    {key="dragonknight", label="Dragonknight", path="ESOAdventurerSuite/Art/eas_class_dragonknight.dds"},
    {key="sorcerer", label="Sorcerer", path="ESOAdventurerSuite/Art/eas_class_sorcerer.dds"},
    {key="nightblade", label="Nightblade", path="ESOAdventurerSuite/Art/eas_class_nightblade.dds"},
    {key="warden", label="Warden", path="ESOAdventurerSuite/Art/eas_class_warden.dds"},
    {key="necromancer", label="Necromancer", path="ESOAdventurerSuite/Art/eas_class_necromancer.dds"},
    {key="templar", label="Templar", path="ESOAdventurerSuite/Art/eas_class_templar.dds"},
    {key="arcanist", label="Arcanist", path="ESOAdventurerSuite/Art/eas_class_arcanist.dds"},
}
local COMPANION_CARDS = {
    ...
    {key="azandar", label="Azandar", collectibleId=11114, path="ESOAdventurerSuite/Art/eas_companion_azandar.dds"},
    {key="bastian", label="Bastian Hallix", collectibleId=9245, path="ESOAdventurerSuite/Art/eas_companion_bastian.dds"},
    {key="ember", label="Ember", collectibleId=9911, path="ESOAdventurerSuite/Art/eas_companion_ember.dds"},
    {key="isobel", label="Isobel Veloise", collectibleId=9912, path="ESOAdventurerSuite/Art/eas_companion_isobel.dds"},
    {key="mirri", label="Mirri Elendis", collectibleId=9353, path="ESOAdventurerSuite/Art/eas_companion_mirri.dds"},
    {key="sharp", label="Sharp-as-Night", collectibleId=11113, path="ESOAdventurerSuite/Art/eas_companion_sharp.dds"},
    {key="tanlorin", label="Tanlorin", collectibleId=12172, path="ESOAdventurerSuite/Art/eas_companion_tanlorin.dds"},
    {key="zerithvar", label="Zerith-var", collectibleId=12173, path="ESOAdventurerSuite/Art/eas_companion_zerithvar.dds"},
}
...
local function appIconPath(key)
    ...
    return "ESOAdventurerSuite/Art/eas_appicon_"..tostring(icon)..".dds"
end
...
local function panel(name, parent, x, y, w, h, color, radius)
    ...
    -- v0.28.95: exactly one visible edge on internal controls. The outer shell
    -- has no edge at all, which removes the black outside line.
    if isShell then c:SetEdgeTexture(nil,1,1,0) else c:SetEdgeTexture("EsoUI/Art/Miscellaneous/centerscreen_floating_center.dds",1,1,1) end
    if c.SetInsets then c:SetInsets(isShell and 0 or 1,isShell and 0 or 1,isShell and 0 or -1,isShell and 0 or -1) end
    c:ClearAnchors()
...
function M:CreateShell()
    ...
    -- v0.28.99: one much darker borderless translucent glass sheet sits behind the entire
    -- application. This keeps the game world visible, but visually connects the
    -- top tabs, rail controls, cards, and detail panels so they no longer look
    -- like unrelated floating boxes. It intentionally has NO edge texture.
    local shellFallback=wm:CreateControl("EAS_ModernShellFallback02890",root,CT_BACKDROP)
    shellFallback:SetAnchor(TOPLEFT,root,TOPLEFT,8,8); shellFallback:SetAnchor(BOTTOMRIGHT,root,BOTTOMRIGHT,-8,-8)
    shellFallback:SetCenterTexture("EsoUI/Art/Miscellaneous/centerscreen_floating_center.dds"); shellFallback:SetEdgeTexture(nil,1,1,0)
    shellFallback:SetCenterColor(GLASS_BG[1],GLASS_BG[2],GLASS_BG[3],GLASS_BG[4]); shellFallback:SetDrawLevel(0)
    shellFallback:SetHidden(false)
    self.shellFallback=shellFallback
    self.glassBackdrop02899=shellFallback
    ...
    self.profileName=label("EAS_ModernProfileName02890",top,"Adventurer",0,15,220,22,"ZoFontGameBold")
    self.profileName:ClearAnchors(); self.profileName:SetAnchor(TOPRIGHT,top,TOPRIGHT,-64,15); self.profileName:SetHorizontalAlignment(TEXT_ALIGN_RIGHT)
    self.profileSub=label("EAS_ModernProfileSub02890",top,"",0,39,220,16,"ZoFontGameSmall",ACCENT)
    self.profileSub:ClearAnchors(); self.profileSub:SetAnchor(TOPRIGHT,top,TOPRIGHT,-64,39); self.profileSub:SetHorizontalAlignment(TEXT_ALIGN_RIGHT)
...
function M:Reflow()
    ...
    if tw>0 then
        ...
        local showProfileName=rootW>=1320
        local showProfileSub=rootW>=1420
        local startX=compact and 225 or 310
        local rightReserve=showProfileSub and 300 or (showProfileName and 225 or 58)
        if self.profileName then self.profileName:SetHidden(not showProfileName) end
        if self.profileSub then self.profileSub:SetHidden(not showProfileSub) end
        if self.brand then
...
function M:RefreshProfile()
    ...
    if self.profileName then self.profileName:SetText(name ~= "" and name or "Adventurer") end
    if self.profileSub then
        self.profileSub:SetColor(ACCENT[1],ACCENT[2],ACCENT[3],1)
        self.profileSub:SetText((class ~= "" and class or "Character") .. "  •  " .. (cp > 0 and ("CP "..cp) or ("Level "..level)))
    end
...
local function companionNativeArtForEntry(entry)
    ...
    -- v0.29.48: Companion collectible *background* images can legitimately be
    -- ESO's red question-mark placeholder.  Do not use those backgrounds for
    -- the gallery.  ESO's own Companion Manager uses GetCollectibleIcon() for
    -- companion imagery, so prefer that exact icon path first.
    ...
    -- Legacy name lookup supports future entries, but the cache also prefers
    -- collectible icons after the v0.29.48 change below.
...
local function currentClassPath()
    ...
    return "ESOAdventurerSuite/Art/eas_class_sorcerer.dds", clean(safe(GetUnitClass,"Character","player"))
end
...
function M:CreateCardGallery(tab,cards,titleText,subText)
    ...
    for i,entry in ipairs(cards) do
        ...
        if tab=="COMPANIONS" then
            -- v0.29.50: custom portrait paths can fail when the addon is installed
            -- inside a versioned outer folder.  Use ESO's own collectible icon
            -- path again because that is install-path independent and was proven
            -- visible in-game.  Keep it closer to native size to reduce blur.
            tex:ClearAnchors()
            tex:SetAnchor(CENTER,c,CENTER,0,-18)
            tex:SetDimensions(128,128)
        ...
        elseif tab=="CHARACTER" then
            -- v0.29.51: render the Suite's sharper class artwork directly,
            -- centered at a smaller native-like size so the class cards stay
            -- crisp like the updated companion cards.
            tex:ClearAnchors()
            tex:SetAnchor(CENTER,c,CENTER,0,-18)
            tex:SetDimensions(128,128)
        ...
        if tab=="CHARACTER" and fallback then
            fallback:SetAlpha(0.14); fallback:SetDimensions(120,120); fallback:SetColor(unpack(ESO_GOLD)); fallback:SetDrawLevel(6)
        end
...
function M:CreateInteractive(tab)
    ...
    p.commandShelf=panel("EAS_ModernCommandShelf02884_"..tab,p,8,88,1150,52,{0.030,0.045,0.064,0.995},18)
    setPanelVisual(p.commandShelf,{0.030,0.045,0.064,0.995},{0.27,0.39,0.53,0.72})
    ...
    p.listPanel=panel("EAS_ModernListPanel02884_"..tab,p,8,154,680,490,{0.020,0.034,0.049,0.998},22)
    p.detailPanel=panel("EAS_ModernDetailPanel02884_"..tab,p,704,154,454,490,{0.024,0.038,0.055,0.998},22)
    setPanelVisual(p.listPanel,{0.020,0.034,0.049,0.998},{0.28,0.41,0.56,0.70})
    setPanelVisual(p.detailPanel,{0.024,0.038,0.055,0.998},{0.28,0.41,0.56,0.74})
    ...
    for i=1,10 do
        ...
        local bg=panel("EAS_ModernRowBG02884_"..tab.."_"..i,row,0,0,nil,nil,SURFACE_2,16)
...
function M:CreateTextPage(tab)
    ...
    p.left=panel("EAS_ModernTextLeft02895_"..tab,p,8,90,566,526,SURFACE)
    p.right=panel("EAS_ModernTextRight02895_"..tab,p,590,90,568,526,SURFACE)
...
function M:CreatePursuits()
    ...
    p.list=panel("EAS_ModernPursuitList02895",p,8,90,744,526,SURFACE); p.detail=panel("EAS_ModernPursuitDetail02895",p,768,90,390,526,SURFACE)
    ...
    for i=1,9 do
        ...
        local bg=panel("EAS_ModernPursuitRowBG02895_"..i,b,0,0,nil,nil,SURFACE_2,11); bg:ClearAnchors(); bg:SetAnchorFill(b); bg:SetDrawLevel(0); setPanelVisual(bg,SURFACE_2,{EDGE_SOFT[1],EDGE_SOFT[2],EDGE_SOFT[3],0.68})
...
function M:CreateNotes()
    ...
    p.list=panel("EAS_ModernNotesList02895",p,8,90,350,526,SURFACE); p.editor=panel("EAS_ModernNotesEditor02895",p,374,90,784,526,SURFACE)
...
function M:CreatePins()
    ...
    p.list=panel("EAS_ModernPinsList02880",p,8,90,458,526,SURFACE); p.detail=panel("EAS_ModernPinsDetail02880",p,482,90,676,526,SURFACE)
...
function M:CreateCodex()
    ...
    p.card=panel("EAS_ModernCodexCard02895",p,8,142,1150,474,SURFACE)
...
function M:CreateSettings()
    ...
    p.card=panel("EAS_ModernSettingsCard02883",p,8,96,1150,490,SURFACE)
    ...
    p.supportCard=panel("EAS_ModernSettingsSupportCard02960",p.card,26,282,1098,200,SURFACE_2)
    ...
    p.supportCreatorLabel=label("EAS_ModernSettingsSupportCreatorLabel02960",p.supportCard,"CREATOR",18,42,220,18,"ZoFontGameBold",ACCENT)
...
function M:CreateDice()
    ...
    p.launch=panel("EAS_ModernDiceLaunch02895",p,8,90,684,526,SURFACE); p.result=panel("EAS_ModernDiceResult02895",p,708,90,450,526,SURFACE)
...
if type(SLASH_COMMANDS)=="table" then
    SLASH_COMMANDS["/easui"] = function(arg)
        arg=string.lower(tostring(arg or ""))
        if arg=="assets" then
            local page=M.pages and (M.pages.COMPANIONS or M.pages.CHARACTER)
            local row=page and page.cardEntries and page.cardEntries[1]
            local art=row and row.art
            local fallback=row and row.fallback
            local loaded=art and type(art.IsTextureLoaded)=="function" and art:IsTextureLoaded() or false
            local fallbackLoaded=fallback and type(fallback.IsTextureLoaded)=="function" and fallback:IsTextureLoaded() or false
            local tw,th=0,0
            if art and type(art.GetTextureFileDimensions)=="function" then tw,th=art:GetTextureFileDimensions() end
            local msg=string.format("UI assets: custom=%s %dx%d native=%s customPath=%s nativePath=%s",tostring(loaded),tonumber(tw) or 0,tonumber(th) or 0,tostring(fallbackLoaded),tostring(row and row.path or "not-created"),tostring(row and row.nativePath or ""))
            if EPC and EPC.Print then EPC:Print(msg) elseif type(d)=="function" then d("[ESO Adventurer Suite] "..msg) end
            return
        end
        local msg="UI: Modern Application 0.28.61 active."
        if EPC and EPC.Print then EPC:Print(msg) elseif type(d)=="function" then d("[ESO Adventurer Suite] "..msg) end
    end
end
