...
local Serialization = EASLoreLibrary.serialization

local Data = {}
zo_mixin(Data, ZO_CallbackObject)
EASLoreLibrary:RegisterModule("data", Data)

--[[
Keeps one ZoneCache per zoneId, so re-entering a zone does not require
re-parsing EASLoreLibraryData every time.
...
function Data:Initialize()
	...
	EVENT_MANAGER:RegisterForEvent("EASLoreLibrary-Data", EVENT_LORE_BOOK_LEARNED, function(eventCode, categoryIndex, collectionIndex, bookIndex)
	...
	EVENT_MANAGER:RegisterForEvent("EASLoreLibrary-Data", EVENT_PLAYER_ACTIVATED, function()
...
function Data:CreateNewCache(zoneId)
	local zoneCache = EASLoreLibrary.ZoneCache:New(zoneId)
