...
EASLoreLibrary.ProximityPinSet = ProximityPinSet
...
function ProximityPinSet:InitializeBase(config)
	...
	self.range = EASLoreLibrary.settings:Get(config.distanceSettingKey)

	EASLoreLibrary.data:RegisterCallback("BookRemoved", function(zoneId, nodeId, pinTypeId) 
		self:RemovePin(zoneId, nodeId, pinTypeId) 
	end)
	EASLoreLibrary.settings:RegisterCallback("FilterChanged", function()
		if self.running then
			self:RefreshPins()
		end
	end)
	EASLoreLibrary.settings:RegisterCallback("SettingChanged", function(key, value)
		if key == "loreBooksEnabled" or key == config.enabledSettingKey then
			self:UpdateEnabled()
...
function ProximityPinSet:UpdateEnabled()
	...
	if EASLoreLibrary.settings:Get("loreBooksEnabled") ~= false and EASLoreLibrary.settings:Get(config.enabledSettingKey) then
...
function ProximityPinSet:RefreshZoneCache()
	...
	local zoneId = EASLoreLibrary.GetPlayerZoneId()
...
function ProximityPinSet:RefreshPins()
	...
	for _, pinTypeId in ipairs(EASLoreLibrary.PINTYPES) do
		local enabled = EASLoreLibrary.settings:IsPinTypeEnabled(pinTypeId)
		...
		for nodeId = firstNodeId, lastNodeId do
			if zoneCache.bookId[nodeId] then
				...
				if enabled then
					local allowed = true
					if type(self.ShouldAcquireNode) == "function" then
						allowed = self:ShouldAcquireNode(pinTypeId, nodeId) ~= false
					end
					if allowed then
