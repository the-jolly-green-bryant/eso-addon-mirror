...
function EASLoreLibrary.GetPlayer3DPosition()
...
-- Returns the native icon assigned by ESO to this lore entry. Unlike the old
-- single lore-book glyph, these textures preserve the actual book/scroll
-- artwork (including the cover colors) used by the Lore Library.
function EASLoreLibrary.GetBookIcon(bookId, fallbackTexture)
	local categoryIndex, collectionIndex, bookIndex = GetLoreBookIndicesFromBookId(bookId)
	if not categoryIndex then return fallbackTexture end
	local _, icon = GetLoreBookInfo(categoryIndex, collectionIndex, bookIndex)
...
local SHOW_ON_MAP_POEASLL_INTERVAL_MS = 16 -- roughly one frame
local SHOW_ON_MAP_POEASLL_TIMEOUT_S = 2 -- give up waiting and pan anyway past this point
...
local function PanWhenMapIsReady(normalizedX, normalizedY, deadlineS)
	...
	end, SHOW_ON_MAP_POEASLL_INTERVAL_MS)
end
...
-- parenting)
function EASLoreLibrary.ShowAurbisMap()
	WORLD_MAP_MANAGER:SetMapById(AURBIS_MAP_ID)
	ZO_WorldMap_ShowWorldMap()
end

function EASLoreLibrary.ShowLocationOnMap(location)
	if not location then return end
	if location.mapId <= 0 then
		EASLoreLibrary.ShowAurbisMap()
		return
	end
