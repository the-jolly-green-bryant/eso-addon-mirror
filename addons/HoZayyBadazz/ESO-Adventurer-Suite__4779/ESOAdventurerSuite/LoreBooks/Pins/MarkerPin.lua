...
EASLoreLibrary:RegisterModule("markerPin", MarkerPin)
...
function MarkerPin:Initialize()
	...
	EASLoreLibrary.settings:RegisterCallback("FilterChanged", function(pinTypeId)
		-- nil means the Suite master Lore Books & Scrolls switch changed.
		if pinTypeId == nil or pinTypeId == EASLoreLibrary.MARKER then
	...
	EASLoreLibrary.data:RegisterCallback("BookRemoved", function(zoneId, nodeId, pinTypeId, bookId)
...
function MarkerPin:AddPins(pinManager)
	if not EASLoreLibrary.settings:IsPinTypeEnabled(EASLoreLibrary.MARKER) then return end
