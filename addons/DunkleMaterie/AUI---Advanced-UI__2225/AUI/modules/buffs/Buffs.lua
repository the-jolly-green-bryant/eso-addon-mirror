...
local function Sort(controlLeft, controlRight, _sortMode)
	--DebugMessage--
	if AUI_DEBUG then
		AUI.DebugMessage("AUI.Buffs.Sort")
	end
	--/DebugMessage--
...
local function GetBuffData(_unitTag, _effectType)
	...
	return size, alignment, distance, fontArt, fontSize, showBuffTime
end
...
local function UpdateBuffsLayout(_unitTag, _effectType)
	--DebugMessage--
	if AUI_DEBUG then
		AUI.DebugMessage("AUI.Buffs.UpdateBuffsLayout")
	end
	--/DebugMessage--
...
local function SetBuffLayout(_control)
	--DebugMessage--
	if AUI_DEBUG then
		AUI.DebugMessage("AUI.Buffs.SetBuffLayout")
	end
	--/DebugMessage--
	...
	textControl:SetFont(LMP:Fetch('font', _control.buffData.fontArt) .. "|" .. (_control.buffData.size + _control.buffData.fontSize) / 4.5 .. "|" .. "soft-shadow-thick")
...
local function CreateNewBuffControl(_virtualControl, _buffName, _unitTag, _effectType)
	--DebugMessage--
	if AUI_DEBUG then
		AUI.DebugMessage("AUI.Buffs.CreateNewBuffControl | " .. _buffName .. " | " .. _unitTag .. " | " .. _effectType)
	end
	--/DebugMessage--
	...
	if not control then
		...
		if parent then
			...
			local size, alignment, distance, fontArt, fontSize, showBuffTime = GetBuffData(_unitTag, _effectType)
...
local function GetBuffControl(_buffName, _unitTag, _effectType)
	--DebugMessage--
	if AUI_DEBUG then
		AUI.DebugMessage("AUI.Buffs.GetBuffControl | " .. _buffName .. " | " .. _unitTag .. " | " .. _effectType)
	end
	--/DebugMessage--
	...
	for id, control in pairs(controls) do			
		if control.buffData.BuffName == _buffName then	
			return control
		end
	end
	...
	if _unitTag == AUI_PLAYER_UNIT_TAG then
		if _effectType == BUFF_EFFECT_TYPE_BUFF then
			return CreateNewBuffControl(currentTemplate.templateData[AUI_BUFF_TYPE_PLAYER].name, _buffName, _unitTag, _effectType)
		elseif _effectType == BUFF_EFFECT_TYPE_DEBUFF then
			return CreateNewBuffControl(currentTemplate.templateData[AUI_DEBUFF_TYPE_PLAYER].name, _buffName, _unitTag, _effectType)
		end		
	elseif _unitTag == AUI_TARGET_UNIT_TAG then
		if _effectType == BUFF_EFFECT_TYPE_BUFF then
			return CreateNewBuffControl(currentTemplate.templateData[AUI_BUFF_TYPE_TARGET].name, _buffName, _unitTag, _effectType)
		elseif _effectType == BUFF_EFFECT_TYPE_DEBUFF then
			return CreateNewBuffControl(currentTemplate.templateData[AUI_DEBUFF_TYPE_TARGET].name, _buffName, _unitTag, _effectType)
		end		
	end
...
local function GetBuff(_buffName, _unitTag, _effectType)
	--DebugMessage--
	if AUI_DEBUG then
		AUI.DebugMessage("AUI.Buffs.GetBuff | " .. _buffName .. " | " .. _unitTag .. " | " .. _effectType)
	end
	--/DebugMessage--
...
local function RemoveBuff(_buffName, _unitTag, _effectType, _fadeMode)
	--DebugMessage--
	if AUI_DEBUG then
		AUI.DebugMessage("AUI.Buffs.RemoveBuff | " .. _buffName .. " | " .. _unitTag .. " | " .. _effectType)
	end
	--/DebugMessage--

	EVENT_MANAGER:UnregisterForUpdate("AUI_Update_BuffControl" .. tostring(_unitTag) .. _buffName)

	local activeBuffs = GetActiveBuffControls(_unitTag, _effectType)
	if activeBuffs and activeBuffs[_buffName] then	
		if _fadeMode then
			AUI.Fade.Out(activeBuffs[_buffName], 500)
		else
			AUI.Fade.Out(activeBuffs[_buffName])
		end

		activeBuffs[_buffName] = nil
				
		UpdateBuffsLayout(_unitTag, _effectType)
	end
end

local function UpdateBuffTime(_buffName, _unitTag, _effectType, _control)
	...
	if textControl then
		...
		if _control.buffData.StartDuration > 0 then
			if timeLeft >= 1 then
				formatedTimeString = AUI.Time.GetFormatedString(timeLeft, 0, AUI_TIME_FORMAT_SHORT)
			else
				formatedTimeString = AUI.Time.GetFormatedString(timeLeft, 1, AUI_TIME_FORMAT_SHORT)
			end
		else
			EVENT_MANAGER:UnregisterForUpdate("AUI_Update_BuffControl" .. tostring(_unitTag) .. _buffName)
		end	
		local buffTimeText = formatedTimeString
		if AUI.Settings.Buffs.show_buff_names then
		  buffTimeText = AUI.String.Wrap(_buffName) .. "\n" .. buffTimeText
		end

		textControl:SetText(buffTimeText)
	end
...
local function UpdateBuff(_buffName, _unitTag, _effectType, _iconTexture, _startTime, _endTime)
	--DebugMessage--
	if AUI_DEBUG then
		AUI.DebugMessage("AUI.Buffs.UpdateBuff | " .. _buffName .. " | " .. _unitTag .. " | " .. _effectType)
	end
	--/DebugMessage--
	...
	if activeBuffs and activeBuffs[_buffName] then
		...
		local control = activeBuffs[_buffName]
		...
		control.buffData.BuffName = _buffName
		control.buffData.StartDuration = startDuration
			
		UpdateBuffsLayout(_unitTag, _effectType)
		
		EVENT_MANAGER:RegisterForUpdate("AUI_Update_BuffControl" .. tostring(_unitTag) .. _buffName, 0, function() UpdateBuffTime(_buffName, _unitTag, _effectType, control) end)
	end
end

local function AddBuff(_buffName, _unitTag, _effectType, _iconTexture, _startTime, _endTime, _fadeMode)	
	--DebugMessage--
	if AUI_DEBUG then
		AUI.DebugMessage("AUI.Buffs.AddBuff | " .. _buffName .. " | " .. _unitTag .. " | " .. _effectType)
	end
	--/DebugMessage--

	local activeBuffs = GetActiveBuffControls(_unitTag, _effectType)	
	if activeBuffs and not activeBuffs[_buffName] then
		local control = GetBuffControl(_buffName, _unitTag, _effectType)
...
local function RemoveBuffs(_unitTag, _fadeMode)
	--DebugMessage--
	if AUI_DEBUG then
		AUI.DebugMessage("AUI.Buffs.RemoveBuffs |" .. _unitTag)
	end
	--/DebugMessage--
	...
	if activeBuffs and activeBuffs[_unitTag] then
		for effectType, buffControls in pairs(activeBuffs[_unitTag]) do
			for _, control in pairs(buffControls) do
				local buffName = control.buffData.BuffName
				RemoveBuff(buffName, _unitTag, effectType, _fadeMode)
			end
		end
	end
end
...
local function RemoveAllBuffs()
	--DebugMessage--
	if AUI_DEBUG then
		AUI.DebugMessage("AUI.Buffs.RemoveAllBuffs")
	end
	--/DebugMessage--
	...
	if activeBuffs then
		for unitTag, unitData in pairs(activeBuffs) do
			for effectType, buffControls in pairs(unitData) do	
				for _, control in pairs(buffControls) do
					local buffName = control.buffData.BuffName	
					RemoveBuff(buffName, unitTag, effectType, false)
				end
			end
		end
	end
...
local function IsBuffAllowed(_unitTag, _startTime, _endTime, _effectType, _abilityType)
...
local function RefreshBuffs(_unitTag)
	--DebugMessage--
	if AUI_DEBUG then
		AUI.DebugMessage("AUI.Buffs.RefreshBuffs | " .. _unitTag)
	end
	--/DebugMessage--
	...
	for buffIndex = 1, numBuffs do
		local buffName, startTime, endTime, buffSlot, stackCount, iconFile, unitTag, effectType, abilityType, statusEffectType = GetUnitBuffInfo(_unitTag, buffIndex) 	
		if buffSlot > 0 and buffName ~= "" then
			if IsBuffAllowed(_unitTag, startTime, endTime, effectType, abilityType) then
				AddBuff(buffName, _unitTag, effectType, iconFile, startTime, endTime, false)				
			end	
		end
	end	
end

function AUI.Buffs.OnEffectChanged(_changeType, _effectSlot, _effectName, _unitTag, _beginTime, _endTime, _stackCount, _iconName, _buffType, _effectType, _abilityType, _statusEffectType, _unitName, _unitId, _abilityId)
	if not _unitTag or isPreviewShow or not isLoaded then
		return
	end

	local isBuffAlowed = IsBuffAllowed(_unitTag, _beginTime, _endTime, _effectType, _abilityType)
	
	if _changeType == 1 then
		if isBuffAlowed then
			AddBuff(_effectName, _unitTag, _effectType, _iconName, _beginTime, _endTime, false)
		end
	elseif _changeType == 2 then
		 RemoveBuff(_effectName, _unitTag, _effectType, false)			 
	elseif _changeType == 3 then	
		if isBuffAlowed then
			UpdateBuff(_effectName, _unitTag, _effectType, _iconName, _beginTime, _endTime)
		end
	end	
end

local function AddPreviewBuff(_buffName, _unitTag, _effectType, _textureFile, _time)
	local seconds = _time / 1000

	AddBuff(_buffName, _unitTag, _effectType, _textureFile, GetFrameTimeSeconds(), GetFrameTimeSeconds() + seconds, false)
	
	if _time > 0 then
		EVENT_MANAGER:UnregisterForUpdate("AUI_Buffs_Preview" .. _buffName)
		EVENT_MANAGER:RegisterForUpdate("AUI_Buffs_Preview" .. _buffName, _time,
		function() 
			UpdateBuff(_buffName, _unitTag, _effectType, _textureFile, GetFrameTimeSeconds(), GetFrameTimeSeconds() + seconds)
		end)
	end
end

local function UpdatePreview()
	if not isLoaded  or not isPreviewShow then
		return
	end
...
function AUI.Buffs.SetToDefaultPosition(_defaultSettings)
	...
	_, AUI.Settings.Buffs.player_buff_position = _defaultSettings.player_buff_position
	...
	_, AUI.Settings.Buffs.player_debuff_position = _defaultSettings.player_debuff_position	
	...
	_, AUI.Settings.Buffs.target_buff_position = _defaultSettings.target_buff_position	
	...
	_, AUI.Settings.Buffs.target_debuff_position = _defaultSettings.target_debuff_position	
end

function AUI.Buffs.RefreshAll()
	--DebugMessage--
	if AUI_DEBUG then
		AUI.DebugMessage("AUI.Buffs.RefreshAll")
	end
	--/DebugMessage--

	for _, control in pairs(mainBuffControls) do	
		local size, alignment, distance, fontArt, fontSize, showBuffTime = GetBuffData(control.unitTag, control.effectType)		
		...
		for unitTag, unitTagList in pairs(buffDataList) do	
			for effectType, effectTypeList in pairs(unitTagList) do
				for id, control in pairs(effectTypeList) do
					local size, alignment, distance, fontArt, fontSize, showBuffTime = GetBuffData(unitTag, effectType)
...
function AUI.Buffs.OnTargetChanged()
	if not isLoaded or isPreviewShow then
		return
	end
...
function AUI.Buffs.OnPlayerActivated()
	if not isLoaded then
		if not currentTemplate then	
			local templateName = AUI.Settings.Template.Buffs
