...
local function SetScrollingControlLayout(_control, _data)
	...
	textControl:SetFont(LMP:Fetch('font', fontArt) .. "|" .. size .. "|" .. "soft-shadow-thick")
	textControl:SetText(text)
	
	if suffixControl then
		suffixControl:SetFont(LMP:Fetch('font', fontArt) .. "|" .. size - (size / 2.7) .. "|" .. "soft-shadow-thick")
...
function AUI.Combat.Text.InsertMessage(_value, _scrollingTextType, _panelName, _iconTexture, _suffix)	
	if not isLoaded or not _panelName then
		return
	end	
