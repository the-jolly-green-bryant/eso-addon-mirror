...
function ListBox:Initialize()
	...
	scrollBarUp:SetHandler("OnMouseUp", 
	...
	self.scrollControl:SetHandler("OnMouseWheel", function(_control, _delta) self:Scroll(_delta) end)			
...
function ListBox:CreateCell(rowControl, cellData)
	...
	if cellControl and cellControl.inner then	
		...
		if self.useMouseEvents then
			...
			cellControl:SetHandler("OnMouseUp", 
			function(_eventCode, _button, _ctrl, _alt, _shift)
				if self.showRowSelectedHoverEffect and cellData.ControlType ~= "button" then
					self:SelectRow(_eventCode, _button, rowControl)
				end
...
function ListBox:CreateRow(itemData)
	...
	if self.useMouseEvents then
		...
		rowControl:SetHandler("OnMouseUp", 
		function(_eventCode, _button, _ctrl, _alt, _shift)
...
function ListBox:SelectRow(_eventCode, _button, _rowControl)
	if self.selectedRowControl then
		self.selectedRowControl.selected = false
	end
	
	if self.selectedRowControl then
		if self.selectedRowControl.NormalTexture then
			self.selectedRowControl:SetNormalTexture(_rowControl.NormalTexture)
		else
			self.selectedRowControl:SetNormalTexture(AUI_ROW_BG_COLOR_DEFAULT)
		end

		if _rowControl.rowKey  == self.selectedRowControl.rowKey then
			self.selectedRowControl.selected = false
			self.selectedRowControl = nil
			return
		end
	...
	if _rowControl.SelectedTexture then
		_rowControl:SetNormalTexture(_rowControl.SelectedTexture)
	else
		_rowControl:SetNormalTexture(AUI_ROW_BG_COLOR_SELECTED_DEFAULT)
	end
	
	_rowControl.selected = true
...
function ListBox:SetRowMouseUpCallback(_self, _caellback)
...
function ListBox:RefreshVisible()
	...
	--DebugMessage--
	if AUI_DEBUG then
		AUI.DebugMessage("ListBox:RefreshVisible")
	end
	--/DebugMessage--	
end
...
function ListBox:Refresh()
	...
	--DebugMessage--
	if AUI_DEBUG then
		AUI.DebugMessage("ListBox:Refresh")
	end
	--/DebugMessage--	
end
