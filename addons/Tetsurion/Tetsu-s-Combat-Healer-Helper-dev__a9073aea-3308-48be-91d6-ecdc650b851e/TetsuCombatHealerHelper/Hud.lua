...
function H.OnCombatEvent(_, result, isError, abilityName, _g, _slot, _srcName, _srcType, targetName, _tgtType, _hit, _pwr, _dmg, _log, _sid, _tid, abilityId)
    ...
    local key = T.LookupKeyForAbilityId(abilityId, abilityName)
    if key ~= "illustrious" and key ~= "orbLockout" then return end
    if key == "illustrious" and result and not HEAL_RESULTS[result] then return end
...
function H.RefreshAll()
    ...
    if not vars or vars.enabled == false or vars.hudList == false then
                    ...
                    if key == "off" then
                        ...
                        if has then
                            if d.fill.SetCenterColor then
                                d.fill:SetCenterColor(COL_HEAL[1], COL_HEAL[2], COL_HEAL[3], 1)
                            else
                                d.fill:SetColor(COL_HEAL[1], COL_HEAL[2], COL_HEAL[3], 1)
                            end
                        else
                            if d.fill.SetCenterColor then
...
local combatHooked = false
local function HookCombatIds()
    if combatHooked or not EVENT_MANAGER then return end
    combatHooked = true
    local function onCombat(...)
        H.OnCombatEvent(...)
    end
    if not REGISTER_FILTER_ABILITY_ID then
        EVENT_MANAGER:RegisterForEvent("TetsuCHH_CE", EVENT_COMBAT_EVENT, onCombat)
        return
    end
    local seen = {}
    local function hook(id)
        ...
        seen[id] = true
        local n = "TetsuCHH_CE" .. id
        EVENT_MANAGER:RegisterForEvent(n, EVENT_COMBAT_EVENT, onCombat)
        EVENT_MANAGER:AddFilterForEvent(n, EVENT_COMBAT_EVENT, REGISTER_FILTER_ABILITY_ID, id)
    end
    if T.Ability then
        local list = T.Ability.illustrious or {}
        for i = 1, #list do hook(list[i]) end
        list = T.Ability.orbLockout or {}
        for i = 1, #list do hook(list[i]) end
    end
end
...
function H.Start()
    ...
    EnsureHud()
    H.ScanGroupBuffs()
    EVENT_MANAGER:RegisterForUpdate(UPDATE_NAME, 1000, function()
        pcall(H.RefreshAll)
    end)
    EVENT_MANAGER:RegisterForUpdate(SCAN_NAME, 2000, function()
        pcall(H.ScanGroupBuffs)
    end)
    EVENT_MANAGER:RegisterForUpdate(PANEL_NAME, 1000, function()
