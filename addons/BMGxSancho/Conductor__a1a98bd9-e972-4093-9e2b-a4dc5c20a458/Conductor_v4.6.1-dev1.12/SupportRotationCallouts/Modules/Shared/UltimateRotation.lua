...
function Factory:Create(config)
    ...
    function M:RefreshDisplay()
        -- Legacy modules remain observers for readiness/cast confirmation only.
        -- Timeline/Sequence Runtime are the sole live instruction owners.
        if SRC.Display then SRC.Display:ClearModuleState(self.config.key) end
        return
        --[[
    ...
    function M:StartUpdateLoop()
        -- Observer-only stabilization: no legacy 200 ms sequencing loop.
        return
        --[[
