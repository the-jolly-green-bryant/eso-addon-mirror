...
local function LocalRole()
    if C.RunContext then
        if C.RunContext:IsTrialLeadView() then return "lead" end
        local role = string.upper(tostring(C.RunContext:GetCombatRole() or "DAMAGE"))
        if role == "TANK" or role == "HEALER" then return "support" end
        return "dd"
    end
    local role = string.lower(tostring((SRC.saved and SRC.saved.displayRole) or "lead"))
    if role == "tank" or role == "healer" then return "support" end
...
function Timeline:Stop(reason)
    self.running = false
    if EVENT_MANAGER and self.updateName then EVENT_MANAGER:UnregisterForUpdate(self.updateName) end
