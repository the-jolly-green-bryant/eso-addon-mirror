...
function Scanner:ScanGroup(forceLocalScan)
    if not C.Database then return end
    local seen = {}
    -- Roster refreshes do not require a full gear/skill/Scribing capability scan.
    -- Reuse the last normalized local profile unless the caller explicitly asks
    -- for a capability refresh or no snapshot exists yet.
    local localSnapshot = (forceLocalScan == true or not self.lastLocalSnapshot)
        and self:ScanLocalPlayer(false) or self.lastLocalSnapshot
...
function Scanner:ScheduleGroupScan(delayMs, forceLocalScan)
    delayMs = tonumber(delayMs) or 250
    self.groupScanGeneration = (self.groupScanGeneration or 0) + 1
    local generation = self.groupScanGeneration
    local function runIfCurrent()
        if generation ~= self.groupScanGeneration then return end
        if not SRC.saved or SRC.saved.enabled ~= true then return end
        self:ScanGroup(forceLocalScan == true)
    end
    if zo_callLater then zo_callLater(runIfCurrent, delayMs) else runIfCurrent() end
end
...
function Scanner:RegisterEvents()
    ...
    if EVENT_GROUP_UPDATE then
        EVENT_MANAGER:RegisterForEvent(self.eventNamespace .. "GroupUpdate", EVENT_GROUP_UPDATE, function()
            Scanner:ScheduleGroupScan(300, false)
        end)
    end
    if EVENT_GROUP_MEMBER_JOINED then
        EVENT_MANAGER:RegisterForEvent(self.eventNamespace .. "GroupJoined", EVENT_GROUP_MEMBER_JOINED, function()
            Scanner:ScheduleGroupScan(300, false)
        end)
    end
    if EVENT_GROUP_MEMBER_LEFT then
        EVENT_MANAGER:RegisterForEvent(self.eventNamespace .. "GroupLeft", EVENT_GROUP_MEMBER_LEFT, function()
            Scanner:ScheduleGroupScan(300, false)
        end)
    end
    ...
    if EVENT_PLAYER_ACTIVATED then
        EVENT_MANAGER:RegisterForEvent(self.eventNamespace .. "Activated", EVENT_PLAYER_ACTIVATED, function()
            -- Zone activation can emit inventory, skills, hotbar, and group events
            -- together. One settled refresh replaces several overlapping scans.
            self:ScheduleLocalRefresh(1200)
            self:ScheduleGroupScan(1350, false)
        end)
    end
...
function Scanner:Initialize()
    ...
    EVENT_MANAGER:RegisterForUpdate(self.updateName, self.scanIntervalMs, function()
        if not SRC.saved or SRC.saved.enabled ~= true then return end
        if IsUnitGrouped and IsUnitGrouped("player") then self:ScanGroup(false) end
    end)
    self:RegisterEvents()
    self:ScanGroup(true)
