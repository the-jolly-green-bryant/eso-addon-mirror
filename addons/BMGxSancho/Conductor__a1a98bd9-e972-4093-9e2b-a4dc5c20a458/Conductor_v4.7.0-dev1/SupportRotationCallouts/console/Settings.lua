...
function Settings:RegisterPanel()
    ...
    panel:AddSettings({
        ...
        { type = LHAS.ST_DROPDOWN, label = "Combat role", tooltip = "Your combat role is separate from Run Host authority and Trial Lead view.", items=function() return {
            {name="Tank",data="TANK"},{name="Healer",data="HEALER"},{name="Damage Dealer",data="DAMAGE"}
        } end, getFunction=function() return {data=SRC.saved.combatRole or "DAMAGE"} end, setFunction=function(_,_,data)
            SRC.saved.combatRole=data.data
            if Conductor.RunContext then Conductor.RunContext:SetPersonalIdentity(data.data, SRC.saved.trialLeadView == true) end
            Settings:ApplyDisplayMode(); Settings:ApplyDisplaySettings()
        end },
        { type = LHAS.ST_CHECKBOX, label = "Trial Lead view", tooltip = "Show the complete command Timeline. This does not make you the Run Host and does not replace your combat role.", getFunction=function() return SRC.saved.trialLeadView == true end, setFunction=function(v)
            SRC.saved.trialLeadView=v == true
            if Conductor.RunContext then Conductor.RunContext:SetPersonalIdentity(SRC.saved.combatRole, v) end
            Settings:ApplyDisplayMode(); Settings:ApplyDisplaySettings()
        end },
        { type = LHAS.ST_LABEL, label = "|cFFD447START CONDUCTOR|r" },
        { type = LHAS.ST_BUTTON, buttonText = "START GROUP RUN", tooltip = "Create the authoritative run beacon for this group. Only one Run Host exists even when several players use Trial Lead view.", clickHandler=function()
            local ok, err = Conductor.RunSync and Conductor.RunSync:StartGroupRun()
            Notify(ok and "Conductor group run started." or ("Unable to start run: " .. tostring(err or "unknown error")))
        end },
        { type = LHAS.ST_BUTTON, buttonText = "JOIN GROUP RUN", tooltip = "Join the active Conductor run without importing a full Raid Plan.", clickHandler=function()
            local ok, err = Conductor.RunSync and Conductor.RunSync:JoinAvailableRun()
            Notify(ok and "Joined Conductor run." or ("Unable to join: " .. tostring(err or "no active run found")))
        end },
        { type = LHAS.ST_BUTTON, buttonText = "START LOCAL CONDUCTOR", tooltip = "Run encounter guidance locally when no group Run Host is available.", clickHandler=function()
            if Conductor.RunSync then Conductor.RunSync:StartLocalRun(); Notify("Local Conductor started.") end
        end },
        { type = LHAS.ST_BUTTON, buttonText = "STOP CONDUCTOR RUN", clickHandler=function()
            if Conductor.RunSync then Conductor.RunSync:StopRun("stopped by user"); Notify("Conductor run stopped.") end
        end },
        { type = LHAS.ST_CHECKBOX, label = "Unlock Conductor windows", tooltip = "Unlock the Timeline and Buffs & Debuffs Dashboard so they can be moved. Positions save automatically when movement stops.", getFunction=function() return not (SRC.saved.windowsLocked == true) end, setFunction=function(v)
            if Conductor.WindowController then Conductor.WindowController:SetLocked(not v) else SRC.saved.windowsLocked = not v end
        end },
