...
function Scanner:RegisterEvents()
    ...
    if EVENT_GROUP_UPDATE then
        EVENT_MANAGER:RegisterForEvent(self.eventNamespace .. "GroupUpdate", EVENT_GROUP_UPDATE, function()
            zo_callLater(function() Scanner:ScanGroup() end, 150)
        end)
    end
...
function Scanner:Initialize()
    ...
    EVENT_MANAGER:RegisterForUpdate(self.updateName, self.scanIntervalMs, function() if SRC.saved and SRC.saved.enabled == true then self:ScanGroup() end end)
    self:RegisterEvents()
    self:ScanGroup()
