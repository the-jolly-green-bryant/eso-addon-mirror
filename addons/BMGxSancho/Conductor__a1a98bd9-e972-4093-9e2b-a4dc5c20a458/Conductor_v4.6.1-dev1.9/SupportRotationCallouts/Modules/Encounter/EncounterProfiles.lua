...
AddEncounter(asylum, FoundationEncounter("ENCOUNTER_AS_OLMS", "Saint Olms the Just", {
    mechanics = {
        "STEAM_BREATH", "STORM_THE_HEAVENS", "ORDINATED_PROTECTOR",
        "OPPRESSIVE_BOLTS", "TELEPORT_STRIKE", "JUMPS", "EXECUTE"
    },
    validationCallouts = true,
    researchConfidence = "researched_pending_live_validation",
    intelligenceDriven = true,
    strategyProfiles = {
        automatic = { label="Automatic vAS+2", conservative=true },
        hardmode_trifecta = { label="Immortal Redeemer", conservative=true, objective="IMMORTAL_REDEEMER" },
        score_push = { label="Voice of Reason", conservative=false, objective="VOICE_OF_REASON" },
    },
    burnWindowGuide = {
        { id="AS_OLMS_OPENING", type="FULL_BURN", label="OPENING BURN", trigger={type="ENCOUNTER_STATE",state="OPENING"}, confidence="COMMUNITY_STRATEGY" },
        { id="AS_OLMS_HOLD_90", type="HOLD", label="HOLD - LLOTHIS ENTERING", trigger={type="BOSS_HEALTH",percent=90}, priority="CRITICAL", confidence="VERIFIED_MECHANIC" },
        { id="AS_OLMS_LLOTHIS_STACK", type="FULL_BURN", label="BURN WHEN LLOTHIS STACKED", trigger={type="SIGNAL",key="LLOTHIS_ACTIVE"}, confidence="LIVE_VALIDATION" },
        { id="AS_OLMS_HOLD_75", type="HOLD", label="HOLD - FELMS ENTERING", trigger={type="BOSS_HEALTH",percent=75}, priority="CRITICAL", confidence="VERIFIED_MECHANIC" },
        { id="AS_OLMS_FULL_STACK", type="FULL_BURN", label="MAX CLEAVE WHEN FULLY STACKED", trigger={type="SIGNAL",key="FELMS_ACTIVE"}, confidence="LIVE_VALIDATION" },
        { id="AS_OLMS_PROTECTOR_HOLD", type="HOLD", label="ORDINATED PROTECTOR - NO ULTIMATES", trigger={type="SIGNAL",key="PROTECTOR_ACTIVE"}, priority="CRITICAL", confidence="VERIFIED_MECHANIC" },
        { id="AS_OLMS_POST_PROTECTOR", type="FULL_BURN", label="OLMS VULNERABLE - SHORT BURN", trigger={type="SIGNAL",key="PROTECTOR_DEAD"}, confidence="COMMUNITY_STRATEGY" },
        { id="AS_OLMS_HOLD_50", type="HOLD", label="HOLD FOR 50% JUMP", trigger={type="BOSS_HEALTH",percent=50}, priority="CRITICAL", confidence="VERIFIED_MECHANIC" },
        { id="AS_OLMS_EXECUTE", type="EXECUTE", label="FINAL BURN - FOCUS OLMS", trigger={type="BOSS_HEALTH",percent=25}, priority="ENCOUNTER", confidence="VERIFIED_MECHANIC" },
    },
    observationSignals = {
        health = {
            { id="AS_OLMS_90", key="OLMS_90", label="90% jump", percent=90, confidence="verified_mechanic" },
            { id="AS_OLMS_75", key="OLMS_75", label="75% jump", percent=75, confidence="verified_mechanic" },
            { id="AS_OLMS_50", key="OLMS_50", label="50% jump", percent=50, confidence="verified_mechanic" },
            { id="AS_OLMS_25", key="EXECUTE", label="Execute", percent=25, state="FINAL_BURN", confidence="verified_mechanic" },
        },
        combat = {
            { id="AS_OPPRESSIVE_BOLTS", key="INTERRUPT_NOW", label="INTERRUPT NOW", abilityNames={"Oppressive Bolts","Soul-Stained Corruption","Soul Stained Corruption"}, repeatable=true, blocksBurn=true, durationSeconds=6, state="MECHANIC", confidence="reference_addon" },
            { id="AS_STORM", key="STORM_MOVE", label="STORM - MOVE TOGETHER", abilityName="Storm the Heavens", repeatable=true, blocksBurn=true, durationSeconds=8, state="MECHANIC", confidence="reference_addon" },
            { id="AS_GUSTS", key="OLMS_JUMP", label="OLMS JUMP - HOLD", abilityNames={"Gusts of Steam","Gust of Steam"}, repeatable=true, blocksBurn=true, durationSeconds=8, state="TRANSITION", confidence="reference_addon" },
            { id="AS_TRIAL_BY_FIRE", key="SPREAD", label="SPREAD - TRIAL BY FIRE", abilityName="Trial by Fire", repeatable=true, blocksBurn=true, durationSeconds=8, state="MECHANIC", confidence="reference_addon" },
            { id="AS_FELMS_JUMP", key="FELMS_JUMP", label="FELMS JUMP - BACK KITE", abilityName="Teleport Strike", repeatable=true, blocksBurn=false, durationSeconds=3, confidence="reference_addon" },
            { id="AS_LLOTHIS_CONE", key="LLOTHIS_CONE", label="LLOTHIS CONE", abilityNames={"Defiling Blast","Defiled Blast"}, repeatable=true, blocksBurn=false, durationSeconds=4, confidence="reference_addon" },
            { id="AS_LLOTHIS_TELEPORT", key="LLOTHIS_TELEPORT", label="LLOTHIS TELEPORT", abilityName="Pernicious Transmission", repeatable=true, blocksBurn=false, durationSeconds=3, confidence="reference_addon" },
        },
    },
    researchSources = {
        "Asylum Sanctorium Status Panel 1.2.0",
        "RaidNotifier Asylum implementation",
        "ESO Mechanics Asylum Sanctorium guide",
        "Conductor community vAS+2 strategy"
    },
}), "Saint Olms the Just", "Saint Olms", "Olms")
