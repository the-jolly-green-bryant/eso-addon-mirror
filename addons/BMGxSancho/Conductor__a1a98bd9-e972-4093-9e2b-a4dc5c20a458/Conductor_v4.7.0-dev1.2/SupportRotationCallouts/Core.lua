...
SRC.defaults = {
    ...
    trashUltimateTeam4 = { "", "", "", "", "", "" },
    displayRole = "lead", -- compatibility mirror only
    combatRole = "DAMAGE",
    trialLeadView = true,
    windowsLocked = true,
    windowPositions = {},
    runtimeTestingOutsideTrials = true,
    runFlowMigrationVersion = 0,
    ...
    personalAssignmentsEnabled = false, -- retired into Timeline
...
function SRC:ResetRaidAssignments(reason)
    local rotationKeys = {
        "rotation", "warhornRotation", "barrierRotation", "nazarayRotation",
        "pillagerRotation", "warMachineRotation", "masterArchitectRotation",
        "roaringOpportunistRotation",
        "trashUltimateTeam1", "trashUltimateTeam2", "trashUltimateTeam3", "trashUltimateTeam4",
    }
    for _, key in ipairs(rotationKeys) do
        local size = string.find(key, "trashUltimateTeam", 1, true) and 6 or 4
        self.saved[key] = {}
        for index = 1, size do self.saved[key][index] = "" end
    end
    self.saved.rotationCount = 1
    self.saved.warhornRotationCount = 1
    self.saved.barrierRotationCount = 1
    self.saved.nazarayRotationCount = 1
    self.saved.pillagerRotationCount = 1
    self.saved.warMachineRotationCount = 1
    self.saved.masterArchitectRotationCount = 1
    self.saved.roaringOpportunistRotationCount = 1
    for team = 1, 4 do
        self.saved["trashUltimateTeam" .. team .. "Count"] = 1
    end
    self.saved.raidPlanManualResponsibilities = {}
    self.saved.activeRaidPlan = nil
    self.saved.activeEncounterAssignmentKey = ""
    self.saved.encounterAssignmentName = ""
    self.saved.trashCurrentTeam = 1
    if self.LiveSession then self.LiveSession:CancelRuntime(reason or "raid assignments reset") end
    if self.EventBus then self.EventBus:Publish("RAID_ASSIGNMENTS_RESET", { reason=reason or "raid assignments reset" }) end
end

function SRC:RemoveAssignmentsNotInRoster(roster, reason)
    roster = roster or {}
    local changed = 0
    local keys = {
        "rotation", "warhornRotation", "barrierRotation", "nazarayRotation",
    ...
    for _, key in ipairs(keys) do
        local list = self.saved[key] or {}
        local size = string.find(key, "trashUltimateTeam", 1, true) and 6 or 4
        for index = 1, size do
            local account = self:NormalizeAccountName(list[index] or "")
            if account ~= "" and account ~= "@" and not roster[account] then
                list[index] = ""
                changed = changed + 1
            end
        end
    ...
    if changed > 0 then
        self.saved.activeRaidPlan = nil
        if self.EventBus then self.EventBus:Publish("STALE_ASSIGNMENTS_REMOVED", { count=changed, reason=reason }) end
    end
    return changed
end
...
local function OnPlayerActivated()
    ...
    SRC.activationGeneration = (SRC.activationGeneration or 0) + 1
    local generation = SRC.activationGeneration
    zo_callLater(function()
        if generation ~= SRC.activationGeneration then return end
        if not SRC.saved or SRC.saved.enabled ~= true then return end
        SRC.Roster:Refresh()
        -- PlayerScanner owns the settled post-zone capability and roster scan.
        -- Do not start a second full scan from Core.
...
function SRC:SetEnabled(enabled)
    ...
    if not enabled then
        ...
        if self.TimelineEngine then self.TimelineEngine:Clear("addon disabled") end
        if self.SequenceRuntime then self.SequenceRuntime:Reset("addon disabled") end
        if self.EncounterSequenceEngine and self.EncounterSequenceEngine.Reset then
            self.EncounterSequenceEngine:Reset("addon disabled")
        end
        ...
        if self.Display and self.Display.HideAll then self.Display:HideAll() end
        if self.TimelineDisplay and self.TimelineDisplay.window then
            self.TimelineDisplay.window:SetHidden(true)
            if self.TimelineDisplay.ReleaseAll then self.TimelineDisplay:ReleaseAll() end
        end
        return
    end

    self.Roster:Refresh()
    if self.PlayerScanner then self.PlayerScanner:ScanGroup() end
    self:MonitorBossState()
    if self.Display and self.Display.ApplyMode then self.Display:ApplyMode() end
    if self.TimelineDisplay and self.TimelineDisplay.Update then self.TimelineDisplay:Update() end
end
...
function SRC:Initialize()
    ...
    if (tonumber(self.saved.runtimeStabilizationVersion) or 0) < 1 then
        self.saved.diagnosticMode = "off"
        self.saved.diagnostics = false
        self.saved.developerDiagnostics = false
        self.saved.diagnosticOverlay = false
        self.saved.researchCaptureEnabled = false
        self.saved.activeDiagnosticSessionId = nil
        self:ResetRaidAssignments("v4.6.1 stabilization migration")
        self.saved.runtimeStabilizationVersion = 1
    end
    ...
    if (tonumber(self.saved.runFlowMigrationVersion) or 0) < 1 then
        local legacyRole = tostring(self.saved.displayRole or "lead")
        self.saved.trialLeadView = legacyRole == "lead"
        self.saved.combatRole = legacyRole == "dd" and "DAMAGE" or (legacyRole == "support" and "HEALER" or "DAMAGE")
        self.saved.personalAssignmentsEnabled = false
        self.saved.rotationDashboardEnabled = false
        self.saved.damageDealerDashboardEnabled = false
        self.saved.teamSharingEnabled = false
        self.saved.runFlowMigrationVersion = 1
    end
    ...
    if self.WindowController then self.WindowController:Initialize() end
    if self.RunContext then self.RunContext:Initialize() end
    if self.RunSync then self.RunSync:Initialize() end
    ...
    -- Full Raid Plan transfer is no longer part of normal run startup.
    -- SessionSharing remains loadable for future optional import/export compatibility.
    ...
    if self.AsylumSanctorium then self.AsylumSanctorium:Initialize() end
    ...
    EVENT_MANAGER:RegisterForUpdate(self.bossMonitorName, 500, function() if self.saved and self.saved.enabled == true then self:MonitorBossState() end end)
