...
function Transport:AcquireExclusive(owner)
    owner = tostring(owner or "")
    if owner == "" then return false, "exclusive owner is required" end
    if self.exclusiveOwner and self.exclusiveOwner ~= owner then
        return false, "transport is reserved by " .. tostring(self.exclusiveOwner)
    end
    self.exclusiveOwner = owner
    self.exclusiveSince = Now()
    return true
end

function Transport:ReleaseExclusive(owner)
    owner = tostring(owner or "")
    if self.exclusiveOwner and owner ~= "" and self.exclusiveOwner ~= owner then return false end
    self.exclusiveOwner = nil
    self.exclusiveSince = nil
    return true
end

function Transport:IsExclusiveFor(owner)
    return self.exclusiveOwner ~= nil and self.exclusiveOwner == tostring(owner or "")
end

function Transport:IsBlockedFor(owner)
    return self.exclusiveOwner ~= nil and self.exclusiveOwner ~= tostring(owner or "")
end

function Transport:Send(payload, owner)
    if self:IsBlockedFor(owner) then
        self.lastSendError = "transport reserved by " .. tostring(self.exclusiveOwner)
...
function Transport:GetStatus()
    return {
        ...
        exclusiveOwner = self.exclusiveOwner,
        exclusiveSince = self.exclusiveSince or 0,
    }
end
