...
function Context:InitializeProtocol()
    if self.protocol and self.ready then return true end
    ...
    if not ok or not handler then
        self.ready = false
        self.lastError = tostring(handler or "handler registration failed")
    ...
    if not okProtocol or not protocol then
        self.ready = false
        self.lastError = tostring(protocol or "protocol declaration failed")
        return false, self.lastError
    end
