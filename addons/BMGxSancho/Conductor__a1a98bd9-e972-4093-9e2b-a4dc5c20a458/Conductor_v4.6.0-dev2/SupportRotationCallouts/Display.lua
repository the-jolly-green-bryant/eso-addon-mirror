...
local function ShowsPersonalDashboard()
    ...
    return SRC.saved.calloutsEnabled ~= false and (role == ROLE_LEAD or role == ROLE_SUPPORT)
end
...
function Display:Initialize()
    ...
    dashboard:SetHandler("OnMoveStop", function(control)
        local rootCenterX = (GuiRoot:GetWidth() or 1920) / 2
        local rootCenterY = (GuiRoot:GetHeight() or 1080) / 2
        SRC.saved.offsetX = control:GetLeft() + (control:GetWidth() * (control:GetScale() or 1) / 2) - rootCenterX
        SRC.saved.offsetY = control:GetTop() + (control:GetHeight() * (control:GetScale() or 1) / 2) - rootCenterY
    end)
    ...
    effectsDashboard:SetMouseEnabled(true)
    effectsDashboard:SetMovable(true)
    effectsDashboard:SetHandler("OnMoveStop", function(control)
        local rootCenterX = (GuiRoot:GetWidth() or 1920) / 2
        local rootCenterY = (GuiRoot:GetHeight() or 1080) / 2
        SRC.saved.buffsDebuffsOffsetX = control:GetLeft() + (control:GetWidth() * (control:GetScale() or 1) / 2) - rootCenterX
        SRC.saved.buffsDebuffsOffsetY = control:GetTop() + (control:GetHeight() * (control:GetScale() or 1) / 2) - rootCenterY
    end)
...
local function ClampCenteredOffset(control, offsetX, offsetY, scale)
    ...
    -- Allow the visible edge of the control to sit flush with the actual
    -- GuiRoot edge. ESO's built-in SetClampedToScreen uses a conservative
    -- safe area, which prevented the dashboards from reaching the screen edge.
    local maxX = math.max(0, (rootWidth - (controlWidth * scale)) / 2)
    local maxY = math.max(0, (rootHeight - (controlHeight * scale)) / 2)
    return zo_clamp(tonumber(offsetX) or 0, -maxX, maxX),
           zo_clamp(tonumber(offsetY) or 0, -maxY, maxY)
end

function Display:ClampSavedDisplayPositions()
    local x, y = ClampCenteredOffset(self.dashboard, SRC.saved.offsetX, SRC.saved.offsetY, SRC.saved.scale)
    SRC.saved.offsetX, SRC.saved.offsetY = x, y

    x, y = ClampCenteredOffset(self.effectsDashboard, SRC.saved.buffsDebuffsOffsetX or 430, SRC.saved.buffsDebuffsOffsetY or -120, SRC.saved.buffsDebuffsScale or 0.9)
    SRC.saved.buffsDebuffsOffsetX, SRC.saved.buffsDebuffsOffsetY = x, y

    x, y = ClampCenteredOffset(self.personal, SRC.saved.personalOffsetX, SRC.saved.personalOffsetY, SRC.saved.personalScale)
    SRC.saved.personalOffsetX, SRC.saved.personalOffsetY = x, y

    x, y = ClampCenteredOffset(self.ddCallout, SRC.saved.damageDealerOffsetX or 0, SRC.saved.damageDealerOffsetY or -80, SRC.saved.damageDealerScale or 1.0)
...
function Display:ShowLeadCallout(accountName, action, suffix, holdMs)
    ...
    self.calloutName1:SetText(names[1] or "UNKNOWN")
