...
local function PrepareCurrentRaid()
    SyncRaidRosterFromNetwork()
...
local function ShareCurrentRaid()
    local session = PrepareCurrentRaid()
    if not session then return false end
    SRC.saved.teamSharingEnabled = true
    local ok = ShareRaidSession()
    if ok then Notify("Raid Plan invitation sent.") end
    return ok
end
...
function Settings:RegisterPanel()
    ...
    panel:AddSettings({
        ...
        { type = LHAS.ST_DROPDOWN, label = "Trial or Dungeon", items=function() return SRC.Profiles:GetInstanceItems(SRC.saved.profileDraftCategory or "trial") end, getFunction=function() return {data=SRC.saved.profileDraftInstance or ""} end, setFunction=function(_,_,data) SRC.saved.profileDraftInstance=data.data end },
        { type = LHAS.ST_DROPDOWN, label = "Difficulty", items=function() return SRC.Profiles:GetDifficultyItems() end, getFunction=function() return {data=SRC.saved.profileDraftDifficulty or "veteran"} end, setFunction=function(_,_,data) SRC.saved.profileDraftDifficulty=data.data end },
        ...
        { type = LHAS.ST_BUTTON, buttonText = "PREPARE RAID PLAN", tooltip = "Scan the current group, resolve available responsibilities, build trash ultimate teams, and prepare the selected encounter strategy.", clickHandler=function() PrepareCurrentRaid() end },
