...
function Transfer:Start(snapshot, serialized)
    ...
    local sent = self.protocols.header:Send({
    ...
    if not sent then self.outgoing=nil return false, "LibGroupBroadcast could not queue the transfer header." end
    self:SendNextChunk()
    return true, token
end
...
function Transfer:SendNextChunk()
    ...
    if position > outgoing.count then
        outgoing.state = "COMPLETING"
        return self.protocols.header:Send({
    ...
    local sent = self.protocols.data:Send({ token=outgoing.token, position=position, data=outgoing.chunks[position] }, { replaceQueuedMessages=false })
    if sent then outgoing.nextPosition = position + 1 end
    return sent
end
...
function Transfer:OnData(unitTag, values)
    ...
    if IsSelf(unitTag) then
        ...
        if position > 0 then
            ...
            if not self:SendNextChunk() then
                if C.SessionShareDiagnostics then C.SessionShareDiagnostics:Fail("LibGroupBroadcast could not queue the next transfer packet.") end
                Alert("|cFF5555Conductor could not continue the Raid Plan transfer.|r")
                self.outgoing = nil
            end
        end
        return
...
function Transfer:SendResponse(token, host, status)
    ...
    return self.protocols.response:Send({
...
function Transfer:SendControl(command)
    ...
    return self.protocols.control:Send({ token=token, command=command, host=Normalize(GetDisplayName and GetDisplayName() or "") }, { replaceQueuedMessages=false })
end
...
function Transfer:MaintenanceTick()
    ...
    if self.outgoing and self.outgoing.state ~= "SENT" and now - self.outgoing.startedAt > self.OUTGOING_TIMEOUT_MS then
