...
function Display:BuildCollisionSafeLayout(events, now, leftBound, rightBound)
    ...
    for _, source in ipairs(events or {}) do
        ...
        if event._x >= leftBound and event._x <= rightBound then
            local duplicateKey = DuplicateKey(event)
            local prior = duplicateIndex[duplicateKey]
            if prior and math.abs((prior.targetMs or now) - (event.targetMs or now)) <= 1200 then
                if EventPriority(event, now) > EventPriority(prior, now) then
                    for key, value in pairs(event) do prior[key] = value end
                end
                prior._duplicateCount = (prior._duplicateCount or 1) + 1
            else
                duplicateIndex[duplicateKey] = event
    ...
    table.sort(candidates, function(a, b)
        if a._x == b._x then return EventPriority(a, now) > EventPriority(b, now) end
        return a._x < b._x
    end)
    ...
    for _, event in ipairs(candidates) do
        ...
        if not cluster or event._x - cluster.lastX >= COLLISION_DISTANCE then
            cluster = { items={}, firstX=event._x, lastX=event._x }
            clusters[#clusters + 1] = cluster
        else
            cluster.lastX = event._x
        end
        cluster.items[#cluster.items + 1] = event
        cluster.lastX = event._x
    end

    local rows = self:GetAvailableRows()
    local layout = {}
    for clusterIndex, cluster in ipairs(clusters) do
        table.sort(cluster.items, function(a, b)
            local pa, pb = EventPriority(a, now), EventPriority(b, now)
            if pa == pb then return math.abs(a._x - centerX) < math.abs(b._x - centerX) end
            return pa > pb
        end)

        local clusterX = 0
        for _, event in ipairs(cluster.items) do clusterX = clusterX + event._x end
        clusterX = Clamp(clusterX / math.max(1, #cluster.items), leftBound, rightBound)

        local shown = math.min(#cluster.items, rows)
        local overflow = #cluster.items - shown
        if overflow > 0 then shown = math.max(1, rows - 1) end

        for index = 1, shown do
            local event = cluster.items[index]
            event._row = index
            event._x = clusterX
        ...
        if overflow > 0 then
            local hiddenCount = #cluster.items - shown
            local summary = {
                id = string.format("TL-COLLISION-%d", clusterIndex),
                key = "COLLISION_SUMMARY",
                label = string.format("%d MORE", hiddenCount),
                displayAssignedText = "SIMULTANEOUS",
                targetMs = now,
                status = "PENDING",
                accuracy = "YELLOW",
                lane = "RAID",
                _x = clusterX,
                _row = math.min(rows, shown + 1),
                _collisionSummary = true,
            }
...
function Display:Initialize()
    ...
    EVENT_MANAGER:RegisterForUpdate("ConductorTimelineDisplayUpdate", 100, function() self:Update() end)
    if C.EventBus then
