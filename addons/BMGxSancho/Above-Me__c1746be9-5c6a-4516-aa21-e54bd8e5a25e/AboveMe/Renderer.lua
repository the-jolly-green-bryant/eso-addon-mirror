...
function AM:GetSelectionForUnit(unitTag)
    ...
    if not displayName or displayName == "" then return 1 end

    -- Local overrides affect only this client and always take precedence over
    -- a selection received from the player through future synchronization.
    local override = self.saved.playerOverrides and self.saved.playerOverrides[displayName]
    if override then
        if override.mode == "hide" then return 1 end
        if override.mode == "local" then return override.iconId or 1 end
    end

    return self.remoteSelections[displayName] or 1
end
...
function AM:UpdateRenderer()
    ...
    local function RenderUnit(unitTag)
        ...
        local icon = self:GetIcon(self:GetSelectionForUnit(unitTag))
