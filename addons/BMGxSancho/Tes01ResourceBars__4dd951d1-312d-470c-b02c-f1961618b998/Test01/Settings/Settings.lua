...
local RESOURCE_VALUE_ITEMS = {{name="Numbers",data="NUMBERS"},{name="Percentage",data="PERCENT"},{name="None",data="NONE"}}
...
function Settings:Initialize()
    ...
    panel:AddSettings({
        {type=LHAS.ST_LABEL,label="|cFFD447TEST01 RESOURCE BARS|r\n|cFFD447Standalone Better Buffs Resource Bars validation harness|r\nVersion "..T.version.."\nNo Better Buffs runtime, analytics, effect cache, or polling loop is included."},
        {type=LHAS.ST_SECTION,label="Resource Bars"},
        {type=LHAS.ST_LABEL,label="Master replacement switch. OFF leaves ESO's native player Health/Magicka/Stamina bars alone. ON suppresses those native player bars and displays the enabled Test01 replacements. Turning OFF releases the native bars immediately."},
        {type=LHAS.ST_LABEL,label="|cFFD447Auto-size standard:|r 30,000 maximum resource = the configured Length value. Default Length 350 therefore gives 350 px at 30,000 before the bar's Scale setting is applied. All resources use this same standard so their physical lengths are directly comparable."},
        {type=LHAS.ST_CHECKBOX,label="Use Test01 Resource Bars",tooltip="For clean isolation, keep Better Buffs Resource Bars disabled while testing Test01. This package has its own namespace and SavedVariables.",getFunction=function() return T.saved.resourceBars.enabled==true end,setFunction=function(v) T.ResourceBars:SetEnabled(v); if v then T.ResourceBars:ShowPreview() else T.ResourceBars:HidePreview() end end},
        ...
        {type=LHAS.ST_CHECKBOX,label="Show Resource Names",tooltip="Shows HEALTH / MAGICKA / STAMINA / SHIELD beneath the bars. Off keeps the premium crescent presentation minimal and leaves only the selected number or percentage.",getFunction=function() return T.saved.resourceBars.showNames==true end,setFunction=function(v) T.ResourceBars:SetShowNames(v); T.ResourceBars:ShowPreview() end},
