...
function Timer:GetCurrentScore()
    -- The live HUD must display ESO's authoritative current raid score.
    -- Do not prefer the recorder's cached score-event value here because it can
    -- lag behind the score ESO is currently presenting on screen.
...
function Timer:CreateControl()
    ...
    -- Width is fitted to the rendered text below. A large fixed-width control
    -- leaves invisible padding at both sides and prevents true edge placement.
    -- Start tiny; UpdateControlBounds() fits the top-level control to the
    -- actual rendered text. Oversized invisible height would prevent the
    -- visible line from reaching the top and bottom screen edges.
...
function Timer:UpdateControlBounds()
    if not self.control or not self.label then return false end

    local textWidth = nil
    local textHeight = nil

    if type(self.label.GetTextWidth) == "function" then
        textWidth = tonumber(self.label:GetTextWidth())
    end
    if type(self.label.GetTextHeight) == "function" then
        textHeight = tonumber(self.label:GetTextHeight())
    end
    ...
    -- Keep only a tiny safety pad around the actual rendered glyphs. The
    -- top-level window used to be 64px tall even though the label is only one
    -- line high, so ESO clamped that invisible box before the visible text
    -- could reach the top or bottom edge.
    local desiredWidth = math.ceil(textWidth + 4)
    local desiredHeight = math.ceil((textHeight and textHeight > 0 and textHeight or 30) + 4)
    local currentWidth = tonumber(self.control:GetWidth()) or 0
    local currentHeight = tonumber(self.control:GetHeight()) or 0

    if math.abs(currentWidth - desiredWidth) < 1 and math.abs(currentHeight - desiredHeight) < 1 then
        return false
    end

    self.control:SetDimensions(desiredWidth, desiredHeight)
    return true
end
...
function Timer:UpdateText()
    ...
    if self:UpdateControlBounds() then
        self:UpdateAnchor()
    end
end
...
function Timer:Initialize()
    ...
    EVENT_MANAGER:RegisterForEvent("TrialRecorder_ActiveTimerScore", EVENT_RAID_TRIAL_SCORE_UPDATE, function()
        if Timer:IsRaidTimerActive() and TR.sv.settings.activeTimerEnabled then
            Timer:UpdateText()
        end
    end)
