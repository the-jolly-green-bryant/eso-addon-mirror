...
local defaults = {
    schemaVersion = 1,
    addonVersion = "1.0.012",
    trackingStartedAt = 0,
    settings = {
        showClearNotifications = true,
        debugEnabled = false,
        activeTimerEnabled = true,
        activeTimerX = 50,
        activeTimerY = 12,
        activeTimerScale = 1.0,
    },
...
function TR:InitializeStorage()
    ...
    if self.sv.settings.showClearNotifications == nil then self.sv.settings.showClearNotifications = true end
    if self.sv.settings.debugEnabled == nil then self.sv.settings.debugEnabled = false end
    if self.sv.settings.activeTimerEnabled == nil then self.sv.settings.activeTimerEnabled = true end
    if self.sv.settings.activeTimerX == nil then self.sv.settings.activeTimerX = 50 end
    if self.sv.settings.activeTimerY == nil then self.sv.settings.activeTimerY = 12 end
    if self.sv.settings.activeTimerScale == nil then self.sv.settings.activeTimerScale = 1.0 end
    ...
    self.sv.runReports = self.sv.runReports or { order = {}, byId = {} }
    self.sv.runReports.order = self.sv.runReports.order or {}
    self.sv.runReports.byId = self.sv.runReports.byId or {}
