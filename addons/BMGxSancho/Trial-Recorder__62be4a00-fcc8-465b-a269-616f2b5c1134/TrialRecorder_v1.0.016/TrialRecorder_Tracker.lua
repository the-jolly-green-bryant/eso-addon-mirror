...
function TR:OnRaidComplete(trialName, score, totalTime)
    ...
    if saved then
        if self.RunReport then
            local ok, reportOrError = pcall(function()
                return self.RunReport:OnRunSaved(result)
            end)
            if not ok then
                self:AddDiagnostic({ event = "RUN_REPORT_BUILD_ERROR", error = tostring(reportOrError), runId = result.id })
            end
        end
