...
function Timer:UpdateControlWidth()
    ...
    -- Keep only a tiny safety pad around the actual rendered text. This makes
    -- 0% and 100% correspond to the visible text line, not an oversized box.
    local desiredWidth = math.ceil(textWidth + 4)
    local currentWidth = tonumber(self.control:GetWidth()) or 0
    if math.abs(currentWidth - desiredWidth) < 1 then
        return false
    end

    self.control:SetWidth(desiredWidth)
    return true
end
...
function Timer:UpdateText()
    ...
    if self:UpdateControlWidth() then
        self:UpdateAnchor()
    end
end
