...
function BT:CreateSettingsMenu()
    ...
    panel:AddSettings({
        ...
        {type=LHAS.ST_CHECKBOX,label="Split Phase — Dome & Bash",tooltip="Captures the Twin when ESO confirms a preferred hard target while both bosses are present. Readiness begins 24 seconds after Lylanar or 26 seconds after Turlassil, remains visible through mechanic queue delays, and BASH NOW is confirmed by the real channel event.",getFunction=function() return BT.sv.splitEnabled==true end,setFunction=function(v) BT.sv.splitEnabled=v==true;if not v then BT.state.lockedBoss=nil;BT.state.preferredTargetValid=false;BT.hud.root:SetHidden(true) end end},
