...
function GuildEventsUI:GetEvents()
	...
	-- Added Debug
			--CHAT_SYSTEM.primaryContainer.currentBuffer:AddMessage("|CFFFF00".."EventId: "..eventId.." eventTitle: "..eventTitle.." eventDate: "..eventDate.." eventTime: "..eventTime)
	...
	-- Added Debug
			--CHAT_SYSTEM.primaryContainer.currentBuffer:AddMessage("|CFFFF00"..GuildEventsUI:getAttendingText(eventId))
...
--  *** Note this conversion to numbere will cause all notes to be blank if an event is created without just a number **							
                ...
                if note ~= newNote then
					...
					d("Updating Notes for User: "..name)
					d("oldNote: "..note)
					d("newNote: "..newNote)
                end
            end
...
function GuildEventsUI:saveEvents()
	--**ADDED DUMP OF EVENTS BEFORE UPDATAING MOTD**
	dumpGuildEvents()
...
function dumpGuildEvents()
	CHAT_SYSTEM.primaryContainer.currentBuffer:AddMessage("|CFFFF00".."Dumping Guild Event List...\n")
	
	local tmpMotd = GetGuildMotD(GuildEventsUI.selectedGuildId)
	CHAT_SYSTEM.primaryContainer.currentBuffer:AddMessage("\nCurrent MOTD: \n\n"..tmpMotd.."\n\n")
	
    local previousEventList = GuildEventsUI:loadEvents()
    if table.getn(previousEventList) > 0 then
        local memberIndex = GetPlayerGuildMemberIndex(GuildEventsUI.selectedGuildId)
        for i=1, table.getn(previousEventList) do
            local eventId, eventTitle, eventDate, eventTime  = GuildEventsUI:getEvent(previousEventList[i])
	--Added Debug
			CHAT_SYSTEM.primaryContainer.currentBuffer:AddMessage("|CFFFF00EventId: |C00FFFF"..eventId.."|CFFFF00 eventTitle: |C00FFFF"..eventTitle.."|CFFFF00 eventDate: |C00FFFF"..eventDate.."|CFFFF00 eventTime: |C00FFFF"..eventTime)
	-- Added Debug
			CHAT_SYSTEM.primaryContainer.currentBuffer:AddMessage("     |CFFFF00"..GuildEventsUI:getAttendingText(eventId))
		end
	end
...
--/script tmpTestVal1,tmpTestVal2 = GetGuildMemberInfo(GuildEventsUI.selectedGuildId, 1) tmpTestVal3 = trimString(tmpTestVal2) d("untrimmed"..tmpTestVal2) d("trimmed: "..tmpTestVal3) tmpTestVal4=tmpTestVal3:split(";") d(tmpTestVal4) d(#tmpTestVal4)