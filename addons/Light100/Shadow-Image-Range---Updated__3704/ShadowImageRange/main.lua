...
local function Initialize()
	...
	local eventCastK = NAME .. 'CastK'
	local eventCastA = NAME .. 'CastA'
	...
	local function CastK(_, change, _, _, unitTag, _, endTime, _, _, _, _, _, _, _, unitId)
		if change == EFFECT_RESULT_GAINED then
			imageActive = false -- image is casted, but it's not active yet
			icon:SetTexture(TEXTURE_ACTIVE) -- although we change texture to active to avoid confusion
			imageEnd, imageTag, imageId = endTime, unitTag, unitId
	...
	local function CastA(_, change, _, _, unitTag, _, endTime, _, _, _, _, _, _, _, unitId)
		if change == EFFECT_RESULT_GAINED then
			imageActive = false -- image is casted, but it's not active yet
			icon:SetTexture(TEXTURE_ACTIVE) -- although we change texture to active to avoid confusion
			imageEnd, imageTag, imageId = endTime, unitTag, unitId
	...
	-- Shadow Image cast. For humanoids
	...
	-- Shadow Image cast. For Khajiits
	EM:RegisterForEvent(eventCastK, EVENT_EFFECT_CHANGED, CastK)
	EM:AddFilterForEvent(eventCastK, EVENT_EFFECT_CHANGED, REGISTER_FILTER_ABILITY_ID, 88696)
	EM:AddFilterForEvent(eventCastK, EVENT_EFFECT_CHANGED, REGISTER_FILTER_SOURCE_COMBAT_UNIT_TYPE, COMBAT_UNIT_TYPE_PLAYER)
	
	-- Shadow Image cast. For Argonians
	EM:RegisterForEvent(eventCastA, EVENT_EFFECT_CHANGED, CastK)
	EM:AddFilterForEvent(eventCastA, EVENT_EFFECT_CHANGED, REGISTER_FILTER_ABILITY_ID, 88697)
	EM:AddFilterForEvent(eventCastA, EVENT_EFFECT_CHANGED, REGISTER_FILTER_SOURCE_COMBAT_UNIT_TYPE, COMBAT_UNIT_TYPE_PLAYER)
