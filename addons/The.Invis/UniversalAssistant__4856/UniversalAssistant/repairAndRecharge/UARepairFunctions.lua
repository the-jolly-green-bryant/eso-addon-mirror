local ua = UAssistant
ua.Repair = ua.Repair or {}
local repair = ua.Repair

local EVENT_NAMESPACE = ua.addonName .. "RepairAndRecharge"
...
local function ClampThreshold(value)
    return zo_clamp(zo_round(tonumber(value) or 10), 0, 20)
end

local function ClampResourceThreshold(value)
    return zo_clamp(zo_round(tonumber(value) or 10), 1, 50)
end
...
function repair.GetSettings()
    ...
    if settings.useCrownRepairKitsFirst == nil then
        settings.useCrownRepairKitsFirst = settings.repairKitMode == "crown_first"
            or settings.repairKitMode == "crown_only"
    end

    if settings.useCrownSoulGemsFirst == nil then
        settings.useCrownSoulGemsFirst = settings.soulGemMode == "crown_first"
    ...
    settings.repairThreshold = ClampThreshold(settings.repairThreshold)
    settings.rechargeThreshold = ClampThreshold(settings.rechargeThreshold)
    settings.repairResourceThreshold = ClampResourceThreshold(settings.repairResourceThreshold)
    settings.rechargeResourceThreshold = ClampResourceThreshold(settings.rechargeResourceThreshold)
...
function repair.Chat(key, ...)
    ...
    local isRechargeMessage = key == "RECHARGE_THRESHOLD_SET"
        or key == "ITEM_RECHARGED"
        or key == "SOUL_GEMS_REMAINING"

    if
        (key == "REPAIR_THRESHOLD_SET" or key == "ITEM_REPAIRED")
    ...
    if
        (key == "RECHARGE_THRESHOLD_SET" or key == "ITEM_RECHARGED")
        and not settings.rechargeChatMessages
    then
        return
    end

    local message = ua.GetString("REPAIR_" .. key)
    if not message then
        return
    end
    ...
    local moduleName = isRechargeMessage and "Recharge" or "Repair"
    local prefix = "|cFFFFFF[|r|c3A92FFU|r|cFFFF00A|r" .. "|cFFFFFF" .. moduleName .. "]|r "
...
local function GetEquippedItemLink(slotIndex)
    return GetItemLink(BAG_WORN, slotIndex, LINK_STYLE_BRACKETS)
end
...
local function CollectEquippedRepairTargets(threshold)
    ...
    for slotIndex = 0, bagSize - 1 do
        ...
        if itemLink ~= "" and DoesItemHaveDurability(BAG_WORN, slotIndex) then
...
local function CollectEquippedRechargeTargets(threshold)
    ...
    for slotIndex = 0, bagSize - 1 do
        ...
        if itemLink ~= "" and IsItemChargeable(BAG_WORN, slotIndex) then
            local charges, maxCharges = GetChargeInfoForItem(BAG_WORN, slotIndex)

            if maxCharges and maxCharges > 0 then
                local percentage = zo_round((charges / maxCharges) * 100)
...
local function CollectRepairKits()
    ...
    for slotIndex = 0, bagSize - 1 do
        local isRepairKit = IsItemRepairKit(BAG_BACKPACK, slotIndex)
        local itemType = GetItemType(BAG_BACKPACK, slotIndex)
        local isCrownRepairKit = (ITEMTYPE_CROWN_REPAIR and itemType == ITEMTYPE_CROWN_REPAIR)
            or (isRepairKit and not IsItemNonCrownRepairKit(BAG_BACKPACK, slotIndex))

        if isRepairKit or isCrownRepairKit then
            local resource = {
                slotIndex = slotIndex,
                remaining = GetStackCount(BAG_BACKPACK, slotIndex),
                tier = GetRepairKitTier(BAG_BACKPACK, slotIndex) or 0,
            }

            if isRepairKit and IsItemNonCrownRepairKit(BAG_BACKPACK, slotIndex) then
...
local function CollectSoulGems()
    ...
    for slotIndex = 0, bagSize - 1 do
        if IsItemSoulGem(SOUL_GEM_TYPE_FILLED, BAG_BACKPACK, slotIndex) then
            local tier = select(1, GetSoulGemItemInfo(BAG_BACKPACK, slotIndex)) or 0
            ...
            local isCrown = IsItemFromCrownStore and IsItemFromCrownStore(BAG_BACKPACK, slotIndex)
...
local function NotifyRepairKitCount(settings, standardKits)
    if not settings.repairResourceTracking then
        return
    end

    local remaining = GetRemainingResourceCount(standardKits)
...
local function NotifySoulGemCount(settings, standardGems)
    if not settings.rechargeResourceTracking then
        return
    end

    local remaining = GetRemainingResourceCount(standardGems)
    ...
    for _, resource in ipairs(resources) do
        if resource.remaining > 0 then
            ...
            if amount > 0 then
                ...
                local repairedPercentage =
                    zo_clamp(zo_round(math.min(amount, 100 - target.condition)), 0, 100)

                repair.Chat("ITEM_REPAIRED", target.itemLink, repairedPercentage)
...
local function TryUseCrownRepairKit(crownKits, damagedItems)
    ...
    if CanUseItem and not CanUseItem(BAG_BACKPACK, resource.slotIndex) then
        return false
    end
    ...
    if IsProtectedFunction("UseItem") then
        success = CallSecureProtected("UseItem", BAG_BACKPACK, resource.slotIndex)
    else
    ...
    for _, target in ipairs(damagedItems) do
        repair.Chat("ITEM_REPAIRED", target.itemLink, 100 - target.condition)
    end
...
local function RepairEquippedItems(settings)
    if not settings.autoRepair or (IsUnitInCombat("player") and not settings.repairInCombat) then
        return 0
    end

    local targets, damagedItems = CollectEquippedRepairTargets(settings.repairThreshold)
    ...
    if settings.useCrownRepairKitsFirst and GetAvailableResource(crownKits) then
        if TryUseCrownRepairKit(crownKits, damagedItems) then
            return 1
        end
    end

    for _, target in ipairs(targets) do
        if TryRepairWithStandardKit(target, standardKits) then
            actionCount = actionCount + 1
        end
    end

    if actionCount > 0 then
        NotifyRepairKitCount(settings, standardKits)
    end
...
local function TryRechargeWithSoulGem(target, resources)
    for _, resource in ipairs(resources) do
        if resource.remaining > 0 then
            ...
            if amount > 0 then
                ChargeItemWithSoulGem(BAG_WORN, target.slotIndex, BAG_BACKPACK, resource.slotIndex)

                resource.remaining = resource.remaining - 1

                local chargeAdded = math.min(amount, target.maxCharges - target.charges)

                local chargedPercentage =
                    zo_clamp(zo_round((chargeAdded / target.maxCharges) * 100), 0, 100)

                repair.Chat("ITEM_RECHARGED", target.itemLink, chargedPercentage)
...
local function RechargeEquippedWeapons(settings)
    if
        not settings.autoRecharge
        or (IsUnitInCombat("player") and not settings.rechargeInCombat)
    then
        return 0
    end

    local targets = CollectEquippedRechargeTargets(settings.rechargeThreshold)
    ...
    for _, target in ipairs(targets) do
        ...
        if settings.useCrownSoulGemsFirst then
            charged = TryRechargeWithSoulGem(target, crownGems)
        end

        if not charged then
            charged = TryRechargeWithSoulGem(target, standardGems)
    ...
    if usedStandardGem then
        NotifySoulGemCount(settings, standardGems)
    end
...
function repair.ScanEquipped()
    ...
    if repair.nextActionTime and now < repair.nextActionTime then
        repair.ScheduleScan(repair.nextActionTime - now)
        return
    end

    local settings = repair.GetSettings()
    local actionCount = RepairEquippedItems(settings) + RechargeEquippedWeapons(settings)
...
function repair.ScheduleScan(delayMilliseconds)
    ...
    if repair.nextActionTime and now < repair.nextActionTime then
        delay = math.max(delay, repair.nextActionTime - now)
    end
...
function repair.Initialize()
    ...
    local weaponChargeEventName = EVENT_NAMESPACE .. "WeaponCharge"
    ...
    EVENT_MANAGER:RegisterForEvent(EVENT_NAMESPACE .. "PlayerAlive", EVENT_PLAYER_ALIVE, function()
    ...
    EVENT_MANAGER:RegisterForUpdate(
        ...
        function()
            ...
            if
                ua.savedVariables.repairEnabled
