...
merchant.RegisterProfile("consumables", CreateDefaults)
...
local function PassesQuality(profile, qualityField, bagId, slotIndex)
    local quality = merchant.GetItemQuality(bagId, slotIndex)

    return quality ~= nil and quality <= profile[qualityField]
end

local function IsProtectedCP150(profile, excludeField, itemLink)
    ...
    return championPoints ~= nil and championPoints >= 150
end

local function PassesCraftedFilter(profile, nonCraftedField, itemLink)
    if not profile[nonCraftedField] then
        return true
    end

    return not IsItemLinkCrafted(itemLink)
end
...
merchant.RegisterCategory("consumables", function(profile, bagId, slotIndex, itemLink)
    ...
    if IsFoodOrDrink(itemType) then
        return profile.foodDrinkEnabled
            and PassesQuality(profile, "foodDrinkMaxQuality", bagId, slotIndex)
            and not IsProtectedCP150(profile, "excludeFoodDrinkCP150", itemLink)
            and PassesCraftedFilter(profile, "nonCraftedFoodDrink", itemLink)
    end

    if IsPotionOrPoison(itemType) then
        return profile.potionPoisonEnabled
            and not IsProtectedCP150(profile, "excludePotionPoisonCP150", itemLink)
            and PassesCraftedFilter(profile, "nonCraftedPotionPoison", itemLink)
    end

    if itemType ~= ITEMTYPE_RECIPE or not IsItemLinkRecipeKnown(itemLink) then
        return false
    end

    if STANDARD_RECIPE_TYPES[specializedItemType] then
        return profile.knownRecipesEnabled
            and PassesQuality(profile, "recipeMaxQuality", bagId, slotIndex)
    end

    if HOUSING_RECIPE_TYPES[specializedItemType] then
        return profile.knownHousingPatternsEnabled
            and PassesQuality(profile, "housingPatternMaxQuality", bagId, slotIndex)
    end
