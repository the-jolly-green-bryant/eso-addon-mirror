...
local function executeBankingItemTransfers()
    ...
    if IsFurnitureVault(PAB.currentBankBag) and IsESOPlusSubscriber() and PAB.SavedVars.Advanced.FurnitureVaultDump then --Furnishing Vault
      ...
      PAEM.addFunctionToQueue(PAB.FurnitureVaultDump, PAB.AddonName, 100)
      PAEM.addFunctionToQueue(_stackBags, PAB.AddonName)
      
      -- Execute the function queue
      PAEM.executeNextFunctionInQueue(PAB.AddonName)
       
    elseif IsHouseBankBag(PAB.currentBankBag) then -- house chest
      SHARED_INVENTORY:RefreshInventory(BAG_BACKPACK)
      SHARED_INVENTORY:RefreshInventory(PAB.currentBankBag)
      if IsESOPlusSubscriber() then
    ...
    elseif not PAB.isBankItemTransferBlocked then
