...
--------------------------------------------------------------------------
-- PABanking   Advanced.LearnableSpecializedItemTypes      moveMode
---------------------------------
local function getPABankingAdvancedLearnableSpecializedItemTypeMoveSetting(specializedItemType, isKnown)
    if isNoProfileSelected() then return end
    if isKnown then
        return PAB.SavedVars.Advanced.LearnableSpecializedItemTypes[specializedItemType].Known
    else
        return PAB.SavedVars.Advanced.LearnableSpecializedItemTypes[specializedItemType].Unknown
    end
end

local function setPABankingAdvancedLearnableSpecializedItemTypeMoveSetting(specializedItemType, value, isKnown)
    if isNoProfileSelected() then return end
    if isKnown then
        PAB.SavedVars.Advanced.LearnableSpecializedItemTypes[specializedItemType].Known = value
    else
        PAB.SavedVars.Advanced.LearnableSpecializedItemTypes[specializedItemType].Unknown = value
    end
end
...
--------------------------------------------------------------------------
-- PABanking   Advanced.UnidentifiedSurveys      moveMode
---------------------------------
local function getPABankingAdvancedUnidentifiedSurveysMoveSetting(itemId)
    if isNoProfileSelected() then return end
    return PAB.SavedVars.Advanced.UnidentifiedSurveys[itemId]
end

local function setPABankingAdvancedUnidentifiedSurveysMoveSetting(itemId, value)
    if isNoProfileSelected() then return end
    PAB.SavedVars.Advanced.UnidentifiedSurveys[itemId] = value
end
...
--------------------------------------------------------------------------
-- PABanking   Advanced.specializedItemType      advancedItemTypeStylePageMoveSetting 
---------------------------------
...
-- PABanking   Advanced.LearnableSpecializedItemTypes         moveMode
---------------------------------
local function isAdvancedLearnableSpecializedItemsDisabledOrAllLearnableItemTypesMoveModeIgnore(itemTypeList)
    if isDisabled({"Advanced", "advancedItemsEnabled"}) then return true end

    for _, SpecializedItemType in ipairs(itemTypeList) do
        if PAB.SavedVars.Advanced.LearnableSpecializedItemTypes[SpecializedItemType].Known ~= PAC.OPERATOR.NONE then return false end
        if PAB.SavedVars.Advanced.LearnableSpecializedItemTypes[SpecializedItemType].Unknown ~= PAC.OPERATOR.NONE then return false end
    end
    -- if there was no 'false' returned until here; then return true
    return true
end
...
local function isAdvancedItemsDisabledOrAllSpecializedTropySurveyReportsMoveModeIgnore()
    if isDisabled({"Advanced", "advancedItemsEnabled"}) then return true end
    for _, itemId in ipairs(PAC.BANKING_ADVANCED.UNIDENTIFIED_SURVEYS) do
        if PAB.SavedVars.Advanced.UnidentifiedSurveys[itemId] ~= PAC.OPERATOR.NONE then return false end
    end
...
local function setPABankingAdvancedItemTypeMoveAllSettings(value)
    ...
    for itemId, _ in pairs(PAB.SavedVars.Advanced.UnidentifiedSurveys) do
        PAB.SavedVars.Advanced.UnidentifiedSurveys[itemId] = value
    end
...
local PABankingMenuFunctions = {
    ...
    getAdvancedLearnableSpecializedItemTypeMoveSetting = getPABankingAdvancedLearnableSpecializedItemTypeMoveSetting,
    setAdvancedLearnableSpecializedItemTypeMoveSetting = setPABankingAdvancedLearnableSpecializedItemTypeMoveSetting,
    ...
    getAdvancedUnidentifiedSurveysMoveSetting = getPABankingAdvancedUnidentifiedSurveysMoveSetting,
    setAdvancedUnidentifiedSurveysMoveSetting = setPABankingAdvancedUnidentifiedSurveysMoveSetting,
    ...
    getUnopenedTreasureMapMoveSetting = function() return getValue({"Advanced", "UnopenedTreasureMap"}) end, 
    setUnopenedTreasureMapMoveSetting = function(value) setValueAndRefreshEvents(value, {"Advanced", "UnopenedTreasureMap"}) end,

    getFurnitureVaultDumpSetting = function() return getValue({"Advanced", "FurnitureVaultDump"}) end, 
    setFurnitureVaultDumpSetting = function(value) setValueAndRefreshEvents(value, {"Advanced", "FurnitureVaultDump"}) end,
    ...
    isRecipeTransactionMenuDisabled = function() return isAdvancedLearnableSpecializedItemsDisabledOrAllLearnableItemTypesMoveModeIgnore(PAC.BANKING_ADVANCED.LEARNABLE.RECIPE) end,
    ...
    isFurnishingItemsTransactionMenuDisabled = function() return isAdvancedItemsDisabledOrAllItemTypesMoveModeIgnore(PAC.BANKING_ADVANCED.REGULAR.FURNISHINGS) and isDisabled({"Advanced", "FurnitureVaultDump"}) end, -- hereherehere
    isScribingItemsTransactionMenuDisabled = function() return isAdvancedLearnableSpecializedItemsDisabledOrAllLearnableItemTypesMoveModeIgnore(PAC.BANKING_ADVANCED.LEARNABLE.SCRIBING) end,
