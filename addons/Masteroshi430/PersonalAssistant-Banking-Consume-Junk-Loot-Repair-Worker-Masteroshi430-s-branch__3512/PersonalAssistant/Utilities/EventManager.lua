...
local function _getCallbackEsoIdentifier(addonName, callbackName, paIdentifier)
    return "CB_" .. _getEsoIdentifier(addonName, callbackName, paIdentifier)
end
...
local function RefreshPALootEventRegistration()
    ...
    if PAL and PAPM.PALoot and PAPM.PALoot.hasActiveProfile() then
		...
		if PALMenuFunctions.getAutoLootEnabledSetting() then
		     -- Register PALoot to check loot for autoloot
			 RegisterForEvent(PAL.AddonName, EVENT_LOOT_UPDATED, PAL.autoLoot, "lootWindow")
		else
		     -- Unregister PALoot to check loot for autoloot
			 UnregisterForEvent(PAL.AddonName, EVENT_LOOT_UPDATED, "lootWindow")
		end
...
local function RefreshPAConsumeEventRegistration()
    ...
    if PACO and PAPM.PAConsume and PAPM.PAConsume.hasActiveProfile() then
        ...
        local AutoConsumePotion = PACOMenuFunctions.getAutoConsumePotionSetting()

        if AutoConsumePoison then
            -- Register for auto poison
             RegisterForEvent(PACO.AddonName.."poison", EVENT_PLAYER_COMBAT_STATE, PACO.CheckPoison)
             -- Register for auto potion
             if AutoConsumePotion then
                RegisterForEvent(PACO.AddonName.."potion", EVENT_PLAYER_COMBAT_STATE, PACO.CheckPotion)
             else
                 UnregisterForEvent(PACO.AddonName.."potion", EVENT_PLAYER_COMBAT_STATE) 
             end
        elseif AutoConsumePotion then
             -- Register for auto potion
             RegisterForEvent(PACO.AddonName.."potion", EVENT_PLAYER_COMBAT_STATE, PACO.CheckPotion)
             if not AutoConsumePoison then
                 UnregisterForEvent(PACO.AddonName.."poison", EVENT_PLAYER_COMBAT_STATE)
             end
        else
            UnregisterForEvent(PACO.AddonName.."poison", EVENT_PLAYER_COMBAT_STATE)
            UnregisterForEvent(PACO.AddonName.."potion", EVENT_PLAYER_COMBAT_STATE)
