...
local TREASURE_ITEM_TAGS_SET = {}
local TRASH_ITEMIDS_SET = {}

local function _initTreasureItemTagsSet()
    if next(TREASURE_ITEM_TAGS_SET) then return end
    for name, list in pairs(TREASURE_ITEM_TAGS) do
        local set = {}
        for _, val in ipairs(list) do
            set[val] = true
        end
        TREASURE_ITEM_TAGS_SET[name] = set
    end
end

local function _initTrashItemIdsSet()
    if next(TRASH_ITEMIDS_SET) then return end
    TRASH_ITEMIDS_SET.NIBBLES_AND_BITS = {}
    for _, id in ipairs(PAC.JUNK.TRASH_ITEMIDS.NIBBLES_AND_BITS) do
        TRASH_ITEMIDS_SET.NIBBLES_AND_BITS[id] = true
    end
    TRASH_ITEMIDS_SET.MORSELS_AND_PECKS = {}
    for _, id in ipairs(PAC.JUNK.TRASH_ITEMIDS.MORSELS_AND_PECKS) do
        TRASH_ITEMIDS_SET.MORSELS_AND_PECKS[id] = true
    end
end
...
local function _getUniqueSellFenceUpdateIdentifier(bagId, slotIndex)
    return SELL_FENCE_CALL_LATER_FUNCTION_NAME .. bagId .. "_" .. slotIndex
end

local function _getUniqueSellMerchantUpdateIdentifier(bagId, slotIndex)
    return SELL_MERCHANT_CALL_LATER_FUNCTION_NAME .. bagId .. "_" .. slotIndex
end
...
local function _markItemAsJunkOrAutoDestroyIfPossible(bagId, slotIndex, itemLink, markAsJunkSuccessMessageKey)
    local itemId = GetItemId(bagId, slotIndex)
    if itemId == 33235 or itemId == 223701 or itemId == 224302 then -- exclude Wabbajack & nowhere keys
        return
    end
    local PAJunkSavedVars = PAJ.SavedVars
    ...
    for itemTagIndex = 1, numItemTags do
        ...
        if itemTagCategory == TAG_CATEGORY_TREASURE_TYPE then
            ...
            if PAJunkSavedVars.QuestProtection.ClockworkCity.excludeAMatterOfLeisure then
                if TREASURE_ITEM_TAGS_SET.A_MATTER_OF_LEISURE[itemTagDescriptionFmt] then return false end
            end
            -- check Quest: A Matter of Respect
            if PAJunkSavedVars.QuestProtection.ClockworkCity.excludeAMatterOfRespect then
                if TREASURE_ITEM_TAGS_SET.A_MATTER_OF_RESPECT[itemTagDescriptionFmt] then return false end
            end
            -- check Quest: A Matter of Tributes
            if PAJunkSavedVars.QuestProtection.ClockworkCity.excludeAMatterOfTributes then
                if TREASURE_ITEM_TAGS_SET.A_MATTER_OF_TRIBUTES[itemTagDescriptionFmt] then return false end
            end
            -- check Quest: The Covetous Countess
            if PAJunkSavedVars.QuestProtection.ThievesGuild.excludeTheCovetousCountess then
                if TREASURE_ITEM_TAGS_SET.THE_COVETOUS_COUNTESS[itemTagDescriptionFmt] then return false end
            end
        end
    end
...
local function AutoLaunder()
     ...
     local isPirharri = string.find(unitName, "Pirharri", 1, true) ~= nil
	 if isPirharri then
	    return
	 end
	 local totalLaunders, laundersUsed = GetFenceLaunderTransactionInfo()
         ...
         for _, slotData in pairs(bagpack) do
             if IsItemStolen(slotData.bagId, slotData.slotIndex) then
                 ...
                 elseif (ItemType == ITEMTYPE_CONTAINER or ItemType == ITEMTYPE_CONTAINER_CURRENCY) and PAJunkSavedVars.AutoLaunder.autoLaunderContainers then
                      CheckAndLaunderItem(slotData.bagId, slotData.slotIndex, quantity)
                 elseif IsItemRepairKit(slotData.bagId, slotData.slotIndex) and PAJunkSavedVars.AutoLaunder.autoLaunderRepairKits then
                      CheckAndLaunderItem(slotData.bagId, slotData.slotIndex, quantity)
                 end
             end
         end   
...
local function OnInventorySingleSlotUpdate(eventCode, bagId, slotIndex, isNewItem, itemSoundCategory, inventoryUpdateReason, stackCountChange)
    ...
    if PAJProfileManager.hasActiveProfile() then
		...
		local isReconstructed = IsItemLinkReconstructed(itemLink)
        ...
        if (not isCrafted or not PAJunkSavedVars.ignoreCraftedItems) and
                ...
                (PA.WindowStates.isTransmuteStationClosed) and not (IsItemLinkSetCollectionPiece(itemLink) and not IsItemSetCollectionPieceUnlocked(GetItemLinkItemId(itemLink))) and not isReconstructed then
