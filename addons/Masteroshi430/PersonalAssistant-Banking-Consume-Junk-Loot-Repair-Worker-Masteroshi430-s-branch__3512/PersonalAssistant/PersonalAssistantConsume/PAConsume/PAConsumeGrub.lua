...
local function GetBackpackInventory(itemLink) -- Returns: number inventoryCount
	if itemLink == "" then return 0 end
	return GetItemLinkInventoryCount(itemLink, INVENTORY_COUNT_BAG_OPTION_BACKPACK)
end
...
local function IsEXPBuffActiveAndGetTimeLeft(unitTag)
  ...
  if hasActiveEffects then
    ...
    or abilityID == 262221 -- Increased favor, '100% Faction favor bonus for all enemy and district event sources for 30 minutes.'
    
	  then  
        -- checkBuffTextureName:find("ability_alchemy_001")
		    indexXPBuff = i
	  elseif checkBuffTextureName and checkBuffTextureName:find("crownstore_consumable_entremet_cake") then -- Colovian War Torte
	      indexXPBuff = i
	  elseif checkBuffTextureName and checkBuffTextureName:find("ava_skill_boost_food") then  -- Molten War Torte & White-Gold War Torte
	      indexXPBuff = i
    end
  end
...
local function IsAPDrink(bagId, slotIndex) -- Returns: boolean isAPDrink
  ...
  elseif itemTexture:find("alliancewarscroll") then return true -- Alliance war Scrolls / AP boost
	else return false end
end

-- --------------------------------------------------------------------------------------------------------------------

local function IsFFDrink(bagId, slotIndex) -- Returns: boolean isFFDrink
	-- get the bag item's itemId
	local itemId = GetItemId(bagId, slotIndex)
	local itemTexture = GetItemInfo(bagId, slotIndex)

	-- check to see if the itemId matches any of the ff drinks, and return true if it does
  if	itemId == 224839	then return	true -- Tonic of Portent Favor/ Faction favor boost
	else return false end
end
...
local function IsValidFoodOrDrink(bagId, slotIndex) -- Returns: boolean isValidFood
	...
	if IsExperienceDrink(bagId, slotIndex) or IsAPDrink(bagId, slotIndex) or IsFFDrink(bagId, slotIndex) then return false end
...
local function AddContextMenuItem(rowControl)
	...
	elseif IsExperienceDrink(rowControl.bagId, rowControl.slotIndex) or IsAPDrink(rowControl.bagId, rowControl.slotIndex) or IsFFDrink(rowControl.bagId, rowControl.slotIndex) then
		local itemLink = GetItemLink(rowControl.bagId, rowControl.slotIndex, LINK_STYLE_BRACKETS)
...
local function TryUseFoodItem(bagId, slotIndex) -- Returns: boolean success
  ...
  if IsCurrentCampaignVengeanceRuleset() then return false end
...
local function TryUseEXPItem(bagId, slotIndex) -- Returns: boolean success
	...
	if IsExperienceDrink(bagId, slotIndex) or (IsAPDrink(bagId, slotIndex) and (IsPlayerInAvAWorld() or IsActiveWorldBattleground())) or (IsFFDrink(bagId, slotIndex) and IsInAdventureZone()) then
		if CanUseItem(bagId, slotIndex) then
			-- use the exp item
			-- UseItem is protected, so CallSecureProtected is used to make the call
			local success = CallSecureProtected("UseItem", bagId, slotIndex)
