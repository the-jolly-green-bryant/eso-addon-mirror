...
-- Labels come from the client so they match whatever the game calls them
-- (and localise for free). Fallbacks are English.
...
-- One pass over the stored tables: which groups exist, and the totals used
-- for the parent submenu's header.
...
-- The game's own heading for an item (Axe, Blacksmith, Potion, Amulet...),
-- falling back to the broad filter category when it can't be resolved.
...
-- Matches ZO_GamepadInventory_DefaultItemSortComparator: category name first,
-- then item name.
...
-- filter is nil (everything), a location string, or a predicate function.
...
local function HookCenteredTooltip()
    ...
    ZO_PreHook(ZO_Tooltip, "LayoutSettingTooltip", function(tooltip, tooltipText, warningText)
        ...
        if item and item.itemLink and item.itemLink ~= "" and tooltip.LayoutItem then
            if shownTooltip == tooltip and shownItemLink == item.itemLink
               and tooltip.HasControls and tooltip:HasControls() then
                return true
            end
...
-- Marked Items
--------------------------------------------------
-- Marks are keyed by the stored item record, so they survive page rebuilds
-- and follow you across characters. They are dropped once the items have been
-- put in chat, or when the menu closes.
local MARK_PREFIX = "|cFFCC00*|r "

local markedItems = {}
local markedSet   = {}

-- Assigned further down, once the chat length maths exists. Declared here so
-- the page header can show the live count.
...
local function ApplyPageHeader(state)
    ...
    -- Marks follow you between pages, so the count follows too.
...
local function BuildPageRows(pageId, state)
    ...
    -- A grouped page (Weapons, Materials, ...) holds a single category, so the
    -- section header would just repeat the submenu's own name.
    ...
    for i = first, last do
        ...
        -- Same index the compiled row will land on, so the keybind can read
        -- the item straight off the selected list data.
...
-- Control:Setup only copies the keys it knows, so the item is attached to the
-- live controls after insertion. GetSelectedData returns these objects, which
-- is how the Link to Chat keybind knows what is highlighted.
...
-- Grouped Pages
--------------------------------------------------
-- Builds Pack / Weapons / Apparel / Accessories for one source, the way the
-- console inventory lays it out. Empty groups are left out, as in the game.
-- Every grouped source gets an All page except the Craft Bag, which is all
-- materials anyway, and the Furniture Vault, which is a single flat list.
...
local function BuildGroupedOptions(idPrefix, getTables, includeAll)
    ...
    -- Everything in one list, grouped by the game's own item categories.
...
-- A source (character, bank, guild bank, chest) becomes a submenu of groups
-- rather than one flat page. Header carries the totals the flat page used to.
        ...
        options       = {
            {
                ...
                default = { "currencies", "characters", "companions", "craftBag", "bank", "guildBanks", "storage" },
                ...
                tooltip = "Choose what Where Is It? keeps track of.\n\n"
                       ...
                       .. "Tick it again and it comes back empty. It fills back in the next "
                       .. "time you log in on a character, summon a companion, open the bank, "
                       .. "open a guild bank or visit a house.\n\n"
                       .. "The list updates the next time you open Where Is It?",
            },
            {
                type    = "button",
                name    = "Info",
                tooltip = "Everything saves itself as you play. This is when each one "
                       .. "happens.\n\n"
                       .. "Characters - logging in, logging out, and any time your "
                       .. "inventory changes.\n\n"
                       .. "Currencies - whenever an amount changes.\n\n"
                       .. "Companions - when you summon one.\n\n"
                       .. "Craft Bag - logging in, and any time it changes.\n\n"
                       .. "Bank - opening it.\n\n"
                       .. "Guild Banks - opening one.\n\n"
                       .. "Storage Chests - traveling to a house you own, and any time it changes.\n\n"
                       .. "Furniture Vault - the same as storage chests.",
                func    = function() end,
            },
        },
    }
...
local RebuildContents = function()
...
-- Link to Chat
--------------------------------------------------
-- The keybind strip is shared by every LibConsoleMenu addon, and other addons
-- bind Y to Reset. So this descriptor resolves at runtime like the shoulder
-- binds do: it is only visible inside this menu, on a row that actually holds
-- an item. Everywhere else it reports invisible and leaves Y alone.
...
-- Reports the outcome in the page's own header band, which LibConsoleMenu
-- draws and lays out via messageText.
...
-- nil when the setting can't be read, so an unknown state keeps the old path.
...
-- Opens chat with the text already in it. Which route depends on the player's
-- chat HUD setting: the normal one rewrites that setting to "on" and leaves it
-- that way, so when it is off we maximize by hand and pass dontShowHUDWindow
-- to leave the setting alone. Either way the entry opens and they press enter.
...
-- Items saved before the scanner asked for LINK_STYLE_BRACKETS carry a bare
-- style and show as a plain name in chat. Restyle those on the way out, so
-- old records read the same as new ones without waiting for a rescan. Anything
-- that does not match the expected shape is left exactly as it was.
local function WithBracketStyle(itemLink)
    local style, data, text = itemLink:match("^|H(%d+):(.-)|h(.-)|h$")
    if not (style and data and text) then return itemLink end
    if text == "" or text:sub(1, 1) == "[" then return itemLink end

    local bracketStyle = rawget(_G, "LINK_STYLE_BRACKETS")
    if type(bracketStyle) ~= "number" then bracketStyle = style end

    return string.format("|H%s:%s|h[%s]|h", tostring(bracketStyle), data, text)
end
...
-- The chat entry is capped by the client's own constant, and item links are
-- long, so only a few fit per message. Fill up to the cap and leave the rest
-- marked so the next press sends them.
-- Ask the chat box itself rather than assuming. GetMaxInputChars is the real
-- cap for this client; the constant and the literal are only fallbacks.
...
-- Live count for the page header: how many are marked, and how many of those
-- the next press will actually fit into one chat message.
...
local function BuildMarkKeybind()
    return {
        ...
        name         = function()
            local item = SelectedItem()
            if item and IsMarked(item) then return "Unmark" end
            return "Mark"
        end,
        keybind      = "UI_SHORTCUT_SECONDARY",
        visible      = function()
            if not CanLinkToChat() then return false end
...
local function BuildLinkKeybind()
    return {
        ...
        callback     = function()
            if not CanLinkToChat() then return end

            local formatted, sent = BuildLinkText()
            if formatted == "" then return end

            -- Push the links into the chat entry and let the player press
            -- enter. Nothing here may submit: SubmitTextEntry ends at
            -- SendChatMessage, which the API marks *private*, so an addon
            -- calling it errors out (sharedchatsystem.lua, SendChatMessage
            -- line). Opening the box with the text pre-set is the supported
            -- route, and the player's own enter goes through the game's path.
            if not PushToChat(formatted) then return end

            -- Only the items that fit lose their mark; anything left over
            -- stays marked for the next press.
            for i = 1, #sent do
                RemoveMark(sent[i])
            end
...
local function HookPreselect()
    ...
    lcm.scene:RegisterCallback("StateChange", function(_, newState)
        -- Marks last as long as the menu is open, no longer.
