...
-- In-Game Groups
--------------------------------------------------
-- Mirrors the console inventory category list: a "Pack" group holding the
-- non-equipment categories, then the three equipment visual categories.
-- Slot -> visual category mapping copied from
-- esoui/publicallingames/character/character_utils.lua.
local GROUP_PACK        = 1
...
-- EQUIP_TYPE_POISON is deliberately absent. The game files poison slots under
-- the Weapons visual category, but a poison is a slottable item, so it falls
-- through to the filter-type checks below and lands in Slottable Items.
...
local function ClassifyItem(itemLink, location)
    ...
    -- Companion gear a player is carrying belongs to the Pack. Gear the
    -- companion is actually wearing keeps its equipment group instead.
    ...
    -- These three filter types, and only these three, are what
    -- ZO_InventoryUtils_DoesNewItemMatchSupplies carves out of Supplies.
    -- ITEMFILTERTYPE_CONSUMABLE is deliberately NOT one of them: motifs,
    -- recipes, treasure maps and most trophies carry it without being
    -- slottable, and the game leaves them in Supplies.
...
-- Item Category Names
--------------------------------------------------
-- The per-item heading the game shows inside a category (Axe, Blacksmith,
-- Potion, Amulet...). ZO_InventoryUtils_Gamepad_GetBestItemCategoryDescription
-- reads only itemType / equipType / actorCategory / itemLink, and falls back to
-- GetItemLinkWeaponType when there is no bagId, so a link is enough.
...
local function ItemCategoryName(itemLink)
    ...
    -- Furnishings carry a real category tree; the generic path would call
    -- every one of them "Furnishing".
    ...
    -- ZOS internal. Guarded so a rename in a future patch degrades instead of
    -- breaking the menu.
    ...
    if type(Describe) == "function" then
        ...
        -- GetCategoryFromWeapon / GetCategoryFromArmor internalassert on an
        -- unmapped type, so this stays wrapped.
...
-- Records saved before this version have no group. Classify them on first
-- read and write it back, so the cost is paid once per item.
...
--------------------------------------------------
-- Bump whenever ClassifyItem's rules change. Stored group/packSub values are
-- cleared once so they get recomputed, rather than keeping a stale answer.
-- Saved items themselves are untouched.
...
-- false means "already looked, no name available" - don't retry every draw.
...
local function ScanSlotIntoTable(bagId, slotIndex, tbl, location, index)
    ...
    -- LINK_STYLE_BRACKETS renders as [Item Name] in chat. Without it the link
    -- is styled bare and shows only the name.
