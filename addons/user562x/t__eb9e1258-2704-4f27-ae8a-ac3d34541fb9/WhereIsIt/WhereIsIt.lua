...
local function VerifyBags(label, bagIds)
    if not ZO_IterateBagSlots then return end

    local slots, filled, zeroCount = 0, 0, 0
    local groups = {}
    local available = false

    for b = 1, #bagIds do
        local bagId = bagIds[b]
        local ok, iterator, state, initialIndex = pcall(ZO_IterateBagSlots, bagId)
            ...
            while true do
                ...
                if rawName and rawName ~= "" then
                    ...
                    if count <= 0 then
                        ...
                        local hasLink    = itemLink and itemLink ~= ""
                        local trait      = hasLink and GetItemLinkTraitInfo(itemLink) or 0
                        local searchName = hasLink and GetItemLinkName(itemLink) or rawName
                        local key = zo_strlower(searchName) .. "  [trait " .. tostring(trait) .. "]"
    ...
    if not available then
        d(string.format("|cFFCC00%s|r  not reachable right now", label))
        return
    end

    local unique, mergedAway = 0, 0
    local mergedList = {}
    for key, n in pairs(groups) do
        unique = unique + 1
        if n > 1 then
            mergedAway = mergedAway + (n - 1)
            mergedList[#mergedList + 1] = { key = key, n = n }
        end
    end
    table.sort(mergedList, function(a, b) return a.n > b.n end)

    d(string.format("|cFFCC00%s|r  slots:%d  filled:%d  zero-count:%d  entries:%d  merged away:%d",
        label, slots, filled, zeroCount, unique, mergedAway))

    for i = 1, zo_min(10, #mergedList) do
        d(string.format("    %d slots share: %s", mergedList[i].n, mergedList[i].key))
    end
end

local function VerifyScan()
    d("|cFFCC00[WhereIsIt]|r filled - merged away = entries")
    VerifyBags("Backpack",        { BAG_BACKPACK })
    VerifyBags("Bank",            { BAG_BANK, BAG_SUBSCRIBER_BANK })
    VerifyBags("Craft Bag",       { BAG_VIRTUAL })
    VerifyBags("Furniture Vault", { BAG_FURNITURE_VAULT })
end
...
local function OnAddonLoaded(event, addonName)
    ...
    SLASH_COMMANDS["/wiiverify"] = VerifyScan
