...
local ADDON_AUTHOR = "|c66ccffErian Kalil|r & TaxTalis, |ccc0000t|r.vicson, |cff704dSimon_111|"
local ADDON_WEBSITE = "https://www.esoui.com/downloads/info3889-WorldMapWayshinresControllerUpdated.html"
...
local function RemovePinsFromMaps()
	...
	-- These execptions are the main wayshrines for each zone that is shown on the Tamriel map or have 3 or more guild traders nearby
	local isWayshrineException = {
		...
		[275] = true, -- Vvardenfell: Balmora
		[281] = true, -- Vvardenfell: Sadrith Mora
		[284] = true, -- Vvardenfell: Vivec City
		[350] = true, -- Summerset: Shimmerene
		...
		[529] = true, -- Galen and Y'ffelon: Vastyr
		[536] = true, -- Necrom: Necrom
			...
			if(SV.NodeOverwrite ~= nil and SV.NodeOverwrite[nodeIndex] ~= nil) then
				...
				if SV.hideTamriel then
					...
					if(poiType == POI_TYPE_WAYSHRINE) then
						...
						if ((SV.hideTamrielWayhsrines == 1 and not isWayshrineException[nodeIndex]) -- hide execpt capitals
...
local function BuildSettingsMenu()
	--Set this to true when trying to identify the NodeIndex for wayshrines
	local DEBUG = false
	...
	for nodeIndex = 0, GetNumFastTravelNodes() do
		...
		if(poiType == POI_TYPE_WAYSHRINE) then
			...
			if(DEBUG) then
				table.insert(Wayshrines.Names, name .. ' ' .. nodeIndex)
			else
