...
_addon.LoadSavedVariables = function( self )
	...
	setDefault("worn", true)
	setDefault("verbose", true)
	setDefault("threshold", 100)
	setDefault("enable_max_gold", false)
	setDefault("max_gold", 100)
end
...
_addon.LoadSettingsMenu = function( self )
	...
	local optionsTable = {
		...
		[6] = self:MakeStandardOption( self.Settings, "Enable Max Gold Limit", "enable_max_gold", false, "checkbox", { default=false, } ),
		[7] = self:MakeStandardOption( self.Settings, "Maxiumum Auto_Repair Amount", "max_gold", 100, "slider", { min=1, max=2000, step=10, default=100, } ),
	}
...
_addon.HandleRepairs = function()
	...
	if _addon:GetOrDefault( false, _addon.Settings["enable_max_gold"] ) then
		tempCost = 0
		for bag = 0, maxBag, 1 do
			for slot = 0, GetBagSize(bag) do
				...
				if itemName ~= "" and itemCondition < _addon:GetOrDefault( 100, _addon.Settings["threshold"] ) then
					...
					tempCost = tempCost + repairCost
				end
			end
		end
		if tempCost > _addon.Settings["max_gold"] then
			d( "Total repair cost exceeds maximum configured repair amount." )
			return
		end
	end
