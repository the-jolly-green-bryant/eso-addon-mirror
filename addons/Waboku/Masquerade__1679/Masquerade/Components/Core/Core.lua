...
function masqueradeAddon.onLoad(_, addon)
  ...
  if db.currentSet and IA.SVC.IdleProfile then
...
function masqueradeAddon.list()
  ...
  for setName, _ in pairs(db) do
    if setName ~= "setBindings" then
      ...
      msg("<<LOM-YELLOW>>*<<LOM-CLEAR>> <<LOM-GREEN>><<LOM-NAME>><<LOM-CLEAR>>", {GREEN=lom.greenColor, YELLOW=lom.yellowColor, CLEAR=lom.clearColor, NAME=string.gsub(" "..(setName or "?"), "%W%l", string.upper):sub(2)})
    end
  end
