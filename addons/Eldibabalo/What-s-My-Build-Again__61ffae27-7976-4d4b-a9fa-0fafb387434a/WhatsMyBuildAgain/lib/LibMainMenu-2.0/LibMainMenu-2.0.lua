...
-- PS5-safe fallback: some console clients do not expose MAIN_MENU_KEYBOARD.
-- Provide the minimal API SuperStar needs (category + scene-group toggle)
-- without touching keyboard main-menu internals.
if type(MAIN_MENU_KEYBOARD) ~= "table" then
	lib._categories = {}
	lib._sceneGroups = {}
	lib._nextCategory = 0
	...
	function lib:AddCategory(data)
		self._nextCategory = self._nextCategory + 1
		local descriptor = self._nextCategory
		self._categories[descriptor] = data or {}
		return descriptor
	end

	function lib:AddSceneGroup(category, sceneGroupName, menuBarIconData)
		self._sceneGroups[category] = {
			sceneGroupName = sceneGroupName,
			menuBarIconData = menuBarIconData or {},
		}
	end

	local function IsAnySceneInGroupShowing(sceneGroupName)
		if type(SCENE_MANAGER) ~= "table" then return false end
		local sceneGroup = SCENE_MANAGER:GetSceneGroup(sceneGroupName)
		if sceneGroup and type(sceneGroup.GetNumScenes) == "function" and type(sceneGroup.GetSceneName) == "function" then
			for i = 1, sceneGroup:GetNumScenes() do
				local sceneName = sceneGroup:GetSceneName(i)
				if sceneName and SCENE_MANAGER:IsShowing(sceneName) then
					return true
				end
			end
		end
	...
	local function ResolveTargetScene(sceneGroupName, menuBarIconData)
		if type(SCENE_MANAGER) ~= "table" then return nil end
		local sceneGroup = SCENE_MANAGER:GetSceneGroup(sceneGroupName)
		if sceneGroup and type(sceneGroup.GetActiveScene) == "function" then
			local activeScene = sceneGroup:GetActiveScene()
			if activeScene then return activeScene end
		end
		if type(menuBarIconData) == "table" and menuBarIconData[1] and menuBarIconData[1].descriptor then
			return menuBarIconData[1].descriptor
		end
		return nil
	end

	function lib:ToggleCategory(category)
		if WINDOW_MANAGER and WINDOW_MANAGER:IsSecureRenderModeEnabled() then
			return
		end
		local groupData = self._sceneGroups[category]
		if not groupData then return end

		if IsAnySceneInGroupShowing(groupData.sceneGroupName) then
		...
		local sceneName = ResolveTargetScene(groupData.sceneGroupName, groupData.menuBarIconData)
		if sceneName and SCENE_MANAGER and type(SCENE_MANAGER.Show) == "function" then
	...
	function lib:ShowSceneGroup(sceneGroupName)
		local sceneName = ResolveTargetScene(sceneGroupName, nil)
		if sceneName and SCENE_MANAGER and type(SCENE_MANAGER.Show) == "function" then
			SCENE_MANAGER:Show(sceneName)
		end
	end
	...
	function lib:ToggleScene(sceneName)
		if not sceneName or not SCENE_MANAGER then return end
		if SCENE_MANAGER:IsShowing(sceneName) and type(SCENE_MANAGER.ShowBaseScene) == "function" then
			SCENE_MANAGER:ShowBaseScene()
	...
	function lib:AddMenuItem(...)
		return
	end

	function lib:SelectMenuItem(...)
		return
	end
