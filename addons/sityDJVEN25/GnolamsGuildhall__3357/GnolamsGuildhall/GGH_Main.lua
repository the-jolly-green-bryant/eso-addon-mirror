local AddonName="GnolamsGuildhall"
local lang=GetCVar("language.2")
local Settings={
	Guilds={
		[453784]=true,	--Gnolams Orion Guild
		
		},
	Logo=true,
	Label={en="",ru=""},
	LabelFont="ZoFontWinH4",
	Position={TOP,nil,TOP,40,5},	--/script local control=ZO_GuildHome_GnolamsGuildhall control:ClearAnchors() control:SetAnchor(TOP,nil,TOP,40,5)
	Vertical=false,
...
local ButtonData={
[1]={
	tooltip={en="Gnolams Orion GuildHall", ru="Gnolams Orion GuildHall"},
	house={"@falc_on81"},
...
local function MakeButton(control,num)
	...
	local name="ZO_GuildHome_GnolamsGuildhall_Button"..num
...
local function UI_Init()
	local control=ZO_GuildHome_GnolamsGuildhall or WINDOW_MANAGER:CreateControl("ZO_GuildHome_GnolamsGuildhall", ZO_GuildHome, CT_CONTROL)
	...
	if Settings.Logo then
		...
		texture:SetTexture("/GnolamsGuildhall/Gnolams_logo.dds")
		texture:SetAlpha(.4)
...
--	PGH_Vars=ZO_SavedVars:NewAccountWide("PGH_Settings", 3, nil, Defaults)
	ZO_PreHookHandler(ZO_GuildHome,"OnEffectivelyShown",function()
		ZO_GuildHome_GnolamsGuildhall:SetHidden(not Settings.Guilds[GUILD_SELECTOR.guildId])
	end)
...
EVENT_MANAGER:RegisterForEvent("GGH_Event", EVENT_ADD_ON_LOADED, OnAddOnLoaded)