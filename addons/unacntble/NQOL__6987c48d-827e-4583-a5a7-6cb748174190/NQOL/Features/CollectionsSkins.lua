...
local CollectionsSkins = {}

local C = {
    SEARCH_DIALOG_NAME = "NQOL_COLLECTIONS_SKINS_SEARCH",
    DRAW_LEVEL = 240,
    SCALE_MIN = 50,
    SCALE_MAX = 150,
...
local FILTERS = {
    { label = NQOL.L("features.collections_skins.all_skins_b081a60") },
    { label = NQOL.L("features.collections_skins.acquired_skins_f371e15") },
    { label = NQOL.L("features.collections_skins.missing_skins_ad684d5") },
}
NQOL.Lexicon.RegisterTableField(FILTERS, "label", {
    "features.collections_skins.all_skins_b081a60", "features.collections_skins.acquired_skins_f371e15",
    "features.collections_skins.missing_skins_ad684d5",
})

local ALPHABET_GROUPS = {
    { first = "A", last = "D", label = NQOL.L("features.collections_skins.a_d_cc8033d") },
    { first = "E", last = "H", label = NQOL.L("features.collections_skins.e_h_26f3d03") },
    { first = "I", last = "L", label = NQOL.L("features.collections_skins.i_l_00eb52e") },
    { first = "M", last = "P", label = NQOL.L("features.collections_skins.m_p_94a2986") },
    { first = "Q", last = "T", label = NQOL.L("features.collections_skins.q_t_bbb7a1f") },
    { first = "U", last = "Z", label = NQOL.L("features.collections_skins.u_z_d38f1c6") },
}
NQOL.Lexicon.RegisterTableField(ALPHABET_GROUPS, "label", {
    "features.collections_skins.a_d_cc8033d", "features.collections_skins.e_h_26f3d03",
    "features.collections_skins.i_l_00eb52e", "features.collections_skins.m_p_94a2986",
    "features.collections_skins.q_t_bbb7a1f", "features.collections_skins.u_z_d38f1c6",
})
...
local function GetSettings()
    ...
    if type(collections.skins) ~= "table" then
        collections.skins = {}
    end

    local settings = collections.skins
    for key, value in pairs(defaults.collections.skins) do
        if settings[key] == nil then
            settings[key] = value
        end
    end

    settings.horizontalPosition = Clamp(tonumber(settings.horizontalPosition) or defaults.collections.skins.horizontalPosition, 0, 100)
    settings.verticalPosition = Clamp(tonumber(settings.verticalPosition) or defaults.collections.skins.verticalPosition, 0, 100)
    settings.scale = Clamp(Round(tonumber(settings.scale) or defaults.collections.skins.scale), C.SCALE_MIN, C.SCALE_MAX)
    settings.backgroundOpacity = Clamp(Round(tonumber(settings.backgroundOpacity) or defaults.collections.skins.backgroundOpacity), C.BACKGROUND_OPACITY_MIN, C.BACKGROUND_OPACITY_MAX)
...
local function FormatName(name)
    ...
    return tostring(name or NQOL.L("collections.unknown", NQOL.L("collections.skin")))
end
...
local function IsBrowsableSkinCollectible(collectibleData)
    if not collectibleData or not collectibleData.GetCategoryType then return false end
    if collectibleData:GetCategoryType() ~= COLLECTIBLE_CATEGORY_TYPE_SKIN then return false end
...
local function CompareSkins(left, right)
...
local function BuildSkinRecords()
    ClearTable(skinRecords)
    dataReady = false

    if not ZO_COLLECTIBLE_DATA_MANAGER or not COLLECTIBLE_CATEGORY_TYPE_SKIN then
        return
    end

    local collectibles = ZO_COLLECTIBLE_DATA_MANAGER:GetAllCollectibleDataObjects(nil, { IsBrowsableSkinCollectible })
    for _, collectibleData in ipairs(collectibles or {}) do
        local collectibleId = collectibleData:GetId()
        local isAcquired = collectibleData:IsUnlocked()
        local categoryName = collectibleData.GetCategoryFormattedName and collectibleData:GetCategoryFormattedName() or NQOL.L("collections.skins")
        ...
        local record = {
            ...
            categoryName = categoryName ~= "" and categoryName or NQOL.L("collections.skins"),
            groupKey = groupKey,
            groupName = groupName,
            name = name,
            ...
            actionLabel = interactionStringId and GetString and GetString(interactionStringId) or (isActive and NQOL.L("collections.remove_skin") or NQOL.L("common.set_active")),
        }
        record.detailText = BuildSkinDetailText(record)
        local searchableText = table.concat({ record.name, record.categoryName, record.tags }, " ")
        ...
        skinRecords[#skinRecords + 1] = record
    end

    table.sort(skinRecords, CompareSkins)
    dataReady = true
end
...
local function RebuildFilteredRecords(preferredCollectibleId)
    ...
    for _, record in ipairs(skinRecords) do
...
local function EnsureHud()
    ...
    hud.control = WINDOW_MANAGER:CreateTopLevelWindow("NQOLCollectionsSkins")
    hud.control:SetHidden(true)
    MoveAbove(hud.control, C.DRAW_LEVEL)
    hud.UpdateDirectionalInput = function()
        CollectionsSkins.UpdateDirectionalInput()
    end
    ...
    hud.photoPlaceholder:SetText(NQOL.L("features.collections_skins.no_thumbnail_is_available_for_this_skin_8c1b696"))
    NQOL.Util.ConfigureContentTexture(hud.photo, hud.photoPlaceholder, hud.photoFrame)
    hud.skinName = CreateLabel(hud.rightViewport, 2, COLORS.text)
    hud.skinName:SetVerticalAlignment(TEXT_ALIGN_TOP)
...
local function ApplyFonts()
    ...
    hud.skinName:SetFont(GetFont(2))
...
local function LayoutHud(layout)
    ...
    hud.skinName:ClearAnchors()
    hud.skinName:SetDimensions(layout.detailWidth, 58)
    hud.skinName:SetAnchor(TOPLEFT, hud.rightViewport, TOPLEFT, 0, 0)
    ...
    hud.meta:SetAnchor(TOPLEFT, hud.skinName, BOTTOMLEFT, 0, 0)
...
local function RenderSkinList(layout)
    ...
    for poolIndex = 1, math.max(layout.visibleRows, #(hud.listRows or {})) do
        ...
        if visible then
            ...
            if entry.isHeader then
                ...
                row.header:SetText(NQOL.Util.Upper(entry.label or NQOL.L("collections.skins.title")))
            else
                ...
                row.status:SetText(entry.isActive and NQOL.L("common.active") or entry.isAcquired and NQOL.L("common.acquired") or NQOL.L("common.missing"))
    ...
    if #filteredRecords == 0 then
        if searchText ~= "" then
            hud.empty:SetText(NQOL.L("collections.no_search_results", NQOL.Util.Lower(NQOL.L("collections.skins.title")), searchText))
        else
            hud.empty:SetText(NQOL.L("features.collections_skins.no_skins_match_the_selected_filter_63363e4"))
        end
    end
end

local function RenderSkinListSelection(layout)
    if KeepSelectionVisible(layout) then
        RenderSkinList(layout)
        return
    end
    local selectedRecord = GetSelectedRecord()
    local selectedRow
    for _, row in ipairs(hud.listRows or {}) do
        if row.entry == selectedRecord then selectedRow = row break end
    end
...
BuildSkinDetailText = function(record)
...
local function RenderSkinDetails(record)
    if not record then
        ReleasePhotoTexture()
        hud.photo:SetHidden(true)
        hud.photoFrame:SetHidden(true)
        hud.photoPlaceholder:SetHidden(false)
        hud.skinName:SetText(NQOL.L("features.collections_skins.no_skin_selected_93f96ae"))
        hud.status:SetText("")
        hud.meta:SetText("")
        hud.details:SetText("")
        return
    ...
    hud.skinName:SetText(record.name)
    local skinNameHeight = hud.skinName.GetTextHeight and math.ceil(hud.skinName:GetTextHeight()) or (DEFAULT_FONT_SIZE + 4)
    hud.skinName:SetHeight(Clamp(skinNameHeight, 1, 58))
    hud.status:SetText(record.isActive and NQOL.L("common.active") or record.isAcquired and NQOL.L("common.acquired") or NQOL.L("common.not_acquired"))
...
local function GetSkinActionLabel(record)
...
local function GetInputHint()
    ...
    if record and record.isAcquired then
        activateHint = NQOL.L("collections.action_hint", GetBindingIcon("UI_SHORTCUT_PRIMARY", "X"), GetSkinActionLabel(record))
    end
...
local function RenderHud()
    ...
    hud.title:SetText(searchText ~= "" and NQOL.L("collections.title_search", NQOL.L("collections.skins.title"), searchText) or NQOL.L("collections.skins.title"))
    hud.summary:SetText(NQOL.L("collections.summary", acquiredCount, #skinRecords, #filteredRecords))
    hud.leftHeader:SetText(NQOL.Util.Upper(FILTERS[filterIndex].label))
    hud.rightHeader:SetText(NQOL.L("features.collections_skins.skin_details_73023f2"))
    hud.hint:SetText(GetInputHint())
    RenderSkinList(layout)
    RenderSkinDetails(GetSelectedRecord())
...
local function RenderSelection()
    ...
    RenderSkinListSelection(hud.layout)
    RenderSkinDetails(GetSelectedRecord())
    hud.hint:SetText(GetInputHint())
    RefreshHudKeybinds()
end
...
local function RegisterSearchDialog()
    ...
    ZO_Dialogs_RegisterCustomDialog(C.SEARCH_DIALOG_NAME, {
        ...
        title = {
            text = function() return NQOL.L("collections.search_title", NQOL.L("collections.skins.title")) end,
        },
            ...
            dialog.info.parametricList = {
                {
                    ...
                    templateData = {
                        ...
                        setup = function(control, data, selected)
                            ...
                            if editBox.SetDefaultText then editBox:SetDefaultText(NQOL.L("collections.search_name_placeholder", NQOL.Util.Lower(NQOL.L("collections.skins.title")))) end
                ...
                },
                {
                    ...
                    templateData = {
                        text = NQOL.L("features.collections_skins.apply_search_96b8b79"),
                ...
                },
                {
                    template = "ZO_GamepadTextFieldSubmitItem",
                    templateData = {
                        text = NQOL.L("features.collections_skins.clear_search_87e328d"),
                        setup = SetupSearchAction,
                        callback = function()
                            ApplySearch("")
                            ReleaseSearchDialog()
...
local function CanUseSelectedSkin()
    local record = GetSelectedRecord()
    if not record then return false end
    if not record.isAcquired then return false, NQOL.L("collections.not_acquired", NQOL.L("collections.skin")) end
    if record.isBlocked then return false, record.blockReason ~= "" and record.blockReason or NQOL.L("collections.cannot_use_now", NQOL.L("collections.skin")) end
    if not record.isUsable then return false, NQOL.L("collections.skin_cannot_change_now") end
    return true
end

local function UseSelectedSkin()
    if not CanUseSelectedSkin() then return end
    local record = GetSelectedRecord()
    ...
    if zo_callLater then
        zo_callLater(RefreshSkinsData, 100)
    else
        RefreshSkinsData()
    end
end
...
hudKeybindGroup = {
    {
        name = function()
            return GetSkinActionLabel(GetSelectedRecord())
        end,
        keybind = "UI_SHORTCUT_PRIMARY",
        callback = UseSelectedSkin,
        visible = function()
            local record = GetSelectedRecord()
            return record and record.isAcquired or false
        end,
        enabled = CanUseSelectedSkin,
...
function CollectionsSkins.UpdateDirectionalInput()
...
function CollectionsSkins.InitializeSavedVariables()
    savedVariables = NQOL.Settings.NewAccountWide(defaults)
    GetSettings()
end

function CollectionsSkins.Initialize()
    if initialized then return end
    initialized = true
end

function CollectionsSkins.SetSettingsPanelVisible(value)
...
function CollectionsSkins.GetHorizontalPosition() return GetSettings().horizontalPosition end
function CollectionsSkins.SetHorizontalPosition(value) GetSettings().horizontalPosition = Clamp(value, 0, 100); ApplyPosition() end
function CollectionsSkins.GetVerticalPosition() return GetSettings().verticalPosition end
function CollectionsSkins.SetVerticalPosition(value) GetSettings().verticalPosition = Clamp(value, 0, 100); ApplyPosition() end
function CollectionsSkins.GetFontChoices() return NQOL.Util.GetFontChoices() end
function CollectionsSkins.GetFontChoiceNames() return NQOL.Util.GetFontChoiceNames() end
function CollectionsSkins.GetFont() return GetSettings().font end
function CollectionsSkins.SetFont(value) if not NQOL.Util.IsFontChoice(value) then value = NQOL.Util.GetDefaultFont() end; GetSettings().font = value; fontStringCache = {}; ApplyFonts(); Refresh() end
function CollectionsSkins.GetScale() return GetSettings().scale end
function CollectionsSkins.SetScale(value) GetSettings().scale = Clamp(Round(value), C.SCALE_MIN, C.SCALE_MAX); Refresh() end
function CollectionsSkins.GetScaleMin() return C.SCALE_MIN end
function CollectionsSkins.GetScaleMax() return C.SCALE_MAX end
function CollectionsSkins.GetBackgroundOpacity() return GetSettings().backgroundOpacity end
function CollectionsSkins.SetBackgroundOpacity(value) GetSettings().backgroundOpacity = Clamp(Round(value), C.BACKGROUND_OPACITY_MIN, C.BACKGROUND_OPACITY_MAX); ApplyBackground() end
function CollectionsSkins.GetBackgroundOpacityMin() return C.BACKGROUND_OPACITY_MIN end
function CollectionsSkins.GetBackgroundOpacityMax() return C.BACKGROUND_OPACITY_MAX end

function CollectionsSkins.GetHorizontalPositionLabel() return NQOL.L("features.collections_skins.horizontal_position_label") end
function CollectionsSkins.GetHorizontalPositionTooltip() return NQOL.L("features.collections_skins.horizontal_position_tooltip") end
function CollectionsSkins.GetVerticalPositionLabel() return NQOL.L("features.collections_skins.vertical_position_label") end
function CollectionsSkins.GetVerticalPositionTooltip() return NQOL.L("features.collections_skins.vertical_position_tooltip") end
function CollectionsSkins.GetFontLabel() return NQOL.L("features.collections_skins.font_label") end
function CollectionsSkins.GetFontTooltip() return NQOL.L("features.collections_skins.font_tooltip") end
function CollectionsSkins.GetScaleLabel() return NQOL.L("features.collections_skins.scale_label") end
function CollectionsSkins.GetScaleTooltip() return NQOL.L("features.collections_skins.scale_tooltip") end
function CollectionsSkins.GetBackgroundOpacityLabel() return NQOL.L("features.collections_skins.background_opacity_label") end
function CollectionsSkins.GetBackgroundOpacityTooltip() return NQOL.L("features.collections_skins.background_opacity_tooltip") end

NQOL.Features.CollectionsSkins = CollectionsSkins
