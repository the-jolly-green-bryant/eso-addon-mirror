...
local C = {
    SEARCH_DIALOG_NAME = "NQOL_COLLECTIONS_GEAR_SEARCH",
    DRAW_LEVEL = 240,
    SCALE_MIN = 50,
    SCALE_MAX = 150,
...
local FILTERS = {
    { label = NQOL.L("features.collections_gear.all_sets_6ece2fa"), summaryLabel = NQOL.L("features.collections_gear.summary_sets") },
    { label = NQOL.L("features.collections_gear.completed_sets_5094bc4"), summaryLabel = NQOL.L("features.collections_gear.summary_completed_sets") },
    { label = NQOL.L("features.collections_gear.incomplete_sets_a81fe8d"), summaryLabel = NQOL.L("features.collections_gear.summary_incomplete_sets") },
    { label = NQOL.L("features.collections_gear.mythics_965733d"), summaryLabel = NQOL.L("features.collections_gear.summary_mythics") },
    { label = NQOL.L("features.collections_gear.monster_sets_6bed3c4"), summaryLabel = NQOL.L("features.collections_gear.summary_monster_sets") },
    { label = NQOL.L("features.collections_gear.class_sets_d8f88fa"), summaryLabel = NQOL.L("features.collections_gear.summary_class_sets") },
}
...
NQOL.Lexicon.RegisterTableField(FILTERS, "summaryLabel", {
...
local antiquitySetIndexReady = false
...
local function GetSettings()
    ...
    if type(collections.gear) ~= "table" then
        collections.gear = {}
    end

    local settings = collections.gear
    for key, value in pairs(defaults.collections.gear) do
    ...
    settings.horizontalPosition = Clamp(tonumber(settings.horizontalPosition) or defaults.collections.gear.horizontalPosition, 0, 100)
    settings.verticalPosition = Clamp(tonumber(settings.verticalPosition) or defaults.collections.gear.verticalPosition, 0, 100)
    settings.scale = Clamp(Round(tonumber(settings.scale) or DEFAULT_SCALE), C.SCALE_MIN, C.SCALE_MAX)
    settings.backgroundOpacity = Clamp(Round(tonumber(settings.backgroundOpacity) or defaults.collections.gear.backgroundOpacity), C.BACKGROUND_OPACITY_MIN, C.BACKGROUND_OPACITY_MAX)
...
local function FormatSetName(rawName)
    ...
    return rawName or NQOL.L("features.collections_gear.unknown_set")
end
...
local function FormatAntiquityName(rawName)
    ...
    return rawName or NQOL.L("features.collections_gear.unknown_lead")
end
...
local function CreatePieceMetadata(pieceId)
    ...
    if not name then
        name = inventoryCategoryName or (equipType and GetString and GetString("SI_EQUIPTYPE", equipType)) or NQOL.L("features.collections_gear.unknown_piece")
    end
...
local function GetCategoryName(setId)
    if not GetItemSetCollectionCategoryId or not GetItemSetCollectionCategoryName then
        return NQOL.L("features.collections_gear.set_collection")
    end
...
local function CompareSetRecords(left, right)
    local leftName = NQOL.Util.Lower(left.baseName or left.name)
    local rightName = NQOL.Util.Lower(right.baseName or right.name)
...
local function IsMythicSet(itemSetId)
    return antiquitySetIdByItemSetId[itemSetId] ~= nil
end
...
local function BuildAntiquitySetIndex()
    if antiquitySetIndexReady then return true end
    if not GetNextAntiquityId or not GetAntiquitySetId or not GetAntiquitySetRewardId or not GetItemRewardItemLink or not GetItemLinkSetInfo then
        return false
    end

    local antiquityId = GetNextAntiquityId(nil)
    ...
    while antiquityId do
        ...
        if antiquitySetId and antiquitySetId ~= 0 and not seenAntiquitySetIdsScratch[antiquitySetId] then
            seenAntiquitySetIdsScratch[antiquitySetId] = true
            local rewardId = GetAntiquitySetRewardId(antiquitySetId)
            local isItemReward = rewardId and rewardId ~= 0 and (not GetRewardType or not REWARD_ENTRY_TYPE_ITEM or GetRewardType(rewardId) == REWARD_ENTRY_TYPE_ITEM)
            if isItemReward then
    ...
    ClearTable(seenAntiquitySetIdsScratch)
    antiquitySetIndexReady = mappedCount > 0
    return antiquitySetIndexReady
end

local function AttachAntiquitySetIds()
    BuildAntiquitySetIndex()
    for _, record in ipairs(setRecords) do
        record.antiquitySetId = record.isMythic and antiquitySetIdByItemSetId[record.id] or nil
    end
end
...
local function RecordMatchesFilter(record)
    ...
    if searchNeedle ~= "" then
        local setName = NQOL.Util.Lower(record.name or "")
        if not string.find(setName, searchNeedle, 1, true) then return false end
    end
    return true
end

local function RebuildFilteredList(preferredSetId)
    ...
    for index, record in ipairs(setRecords) do
        if RecordMatchesFilter(record) then
            ...
            local groupKey, groupLabel = NQOL.Util.GetCollectionLetterGroup(record.baseName)
...
local function BuildSetRecords()
    setRecords = {}
    setIndexById = {}
    letterGroups = {}
    listEntries = {}
    setListEntryIndices = {}
    filteredSetIndices = {}
    filteredSummaryText = NQOL.L("features.collections_gear.summary", 0, NQOL.L("features.collections_gear.sets_cba9e41"), 0, 0)
    dataReady = false
    ...
    while itemSetId do
        ...
        if total > 0 then
            ...
            setRecords[#setRecords + 1] = {
                id = itemSetId,
                name = isPerfected and NQOL.L("features.collections_gear.perfected_name", baseName) or baseName,
                baseName = baseName,
                baseSetId = baseSetId,
                isPerfected = isPerfected,
                isMythic = IsMythicSet(itemSetId),
...
local function EnsurePieceTile(index)
    ...
    NQOL.Util.ConfigureContentTexture(tile.icon, tile.iconLoading)
...
local function EnsureHud()
    ...
    hud.control = WINDOW_MANAGER:CreateTopLevelWindow("NQOLCollectionsGear")
    ...
    hud.empty:SetText(NQOL.L("features.collections_gear.no_pieces_are_available_for_this_set_e297c61"))
...
local function SetPieceTileIcon(tile, icon, desaturated)
    if icon and icon ~= "" then
        ...
        if tile.icon.nqolRequestedTexturePath ~= icon then
            NQOL.Util.LoadContentTexture(tile.icon, icon)
        end
    else
        NQOL.Util.ReleaseContentTexture(tile.icon)
    end
end
...
local function RenderPieces(layout, record, emptyText)
    ...
    FinalizePieceTiles(layout, pieceCount, emptyText or NQOL.L("features.collections_gear.no_pieces_are_available_for_this_set_e297c61"))
end
...
local function RenderMythicLeads(layout, record)
    ...
    local leadCount = canReadLeads and (GetNumAntiquitySetAntiquities(record.antiquitySetId) or 0) or 0
    local columns = layout.rightWidth >= 450 and 3 or 2
    ...
    FinalizePieceTiles(layout, displayCount, NQOL.L("features.collections_gear.no_mythic_lead_data"))
end
...
local function GetInputHint()
    ...
    if USE_LETTER_GROUPS then
        inputHintTextCache[filterIndex] = NQOL.L("features.collections_gear.input_hint", leftShoulder, rightShoulder, leftStickClick, rightStickClick, FILTERS[filterIndex].label, stickIcon)
    else
        inputHintTextCache[filterIndex] = NQOL.L("features.collections_gear.input_hint_flat", leftStickClick, rightStickClick, FILTERS[filterIndex].label, stickIcon)
    end
...
local function RenderSetCard(layout, record)
    ...
    local categoryName = record and GetCategoryName(record.id) or NQOL.L("common.item_set")
    hud.cardTitle:SetText(Uppercase(record and record.name or NQOL.L("features.collections_gear.no_set_selected")))
    ...
    if record then
        local setType
        if record.isMythic then
            setType = NQOL.L("features.collections_gear.type_mythic")
        elseif record.isMonster then
            setType = NQOL.L("features.collections_gear.type_monster")
        elseif record.isClass then
            setType = NQOL.L("features.collections_gear.type_class")
        elseif record.isPerfected then
            setType = NQOL.L("features.collections_gear.type_perfected")
        else
            setType = NQOL.L("features.collections_gear.type_item")
        end
        if record.isMythic then
    ...
    hud.cardBonusesHeader:SetText(numBonuses == 1 and NQOL.L("features.collections_gear.set_bonus") or NQOL.L("features.collections_gear.set_bonuses"))
    hud.cardBonuses:SetFont(GetFont(numBonuses > 7 and -8 or (numBonuses > 5 and -6 or -4)))
    hud.cardBonuses:SetText(#setBonusTextScratch > 0 and table.concat(setBonusTextScratch, "\n") or NQOL.L("features.collections_gear.no_bonus_details"))
...
local function RenderSelectedSetDetails(layout, record)
    if record and record.isMythic and not record.antiquitySetId and not antiquitySetIndexReady then
        AttachAntiquitySetIds()
    end
    ...
    elseif record then
        ...
        RenderPieces(layout, nil, NQOL.L("features.collections_gear.no_sets_match_filter"))
    end
...
local function RenderHud()
    local control = EnsureHud()
    if not control then return end
    if not dataReady then BuildSetRecords() end
    local layout = GetLayout()
    LayoutHud(layout)
    ApplyPosition(layout)

    hud.title:SetText(searchText ~= "" and NQOL.L("features.collections_gear.search_title", searchText) or NQOL.L("features.collections_gear.title"))
    hud.summary:SetText(filteredSummaryText)
    hud.hint:SetText(GetInputHint())
    hud.footerMeta:SetText(string.format("%s · NQOL v%s", GetPlayerServerText(), tostring(NQOL.version or 0)))
    RenderWatermark(layout)

    local record = setRecords[selectedIndex]
    if record then
        GetSettings().selectedSetId = record.id
        hud.leftHeader:SetText(NQOL.L("features.collections_gear.sets_cba9e41"))
        RenderSetList(layout)
        ...
        RenderSetList(layout)
        if searchText ~= "" then
            RenderPieces(layout, nil, NQOL.L("features.collections_gear.no_search_results", searchText))
        else
            RenderPieces(layout, nil, NQOL.L("features.collections_gear.no_sets_match_filter"))
        end
...
local function RegisterSearchDialog()
    ...
    ZO_Dialogs_RegisterCustomDialog(C.SEARCH_DIALOG_NAME, {
        ...
        title = {
            text = function() return NQOL.L("collections.search_title", NQOL.L("features.collections_gear.sets_cba9e41")) end,
        },
            ...
            dialog.info.parametricList = {
                {
                    ...
                    templateData = {
                        ...
                        setup = function(control, data, selected)
                            ...
                            if editBox.SetDefaultText then editBox:SetDefaultText(NQOL.L("collections.search_name_placeholder", NQOL.Util.Lower(NQOL.L("features.collections_gear.sets_cba9e41")))) end
                ...
                },
                {
                    ...
                    templateData = {
                        text = NQOL.L("features.collections_gear.apply_search_96b8b79"),
                ...
                },
                {
                    template = "ZO_GamepadTextFieldSubmitItem",
                    templateData = {
                        text = NQOL.L("features.collections_gear.clear_search_87e328d"),
                        setup = SetupSearchAction,
                        callback = function()
                            ApplySearch("")
                            ReleaseSearchDialog()
...
function CollectionsGear.InitializeSavedVariables()
    ...
    if type(savedVariables.progress) == "table" then
        savedVariables.progress.gear = nil
    end
    if type(savedVariables.collections) == "table" and type(savedVariables.collections.gear) == "table" then
        local gearSettings = savedVariables.collections.gear
        gearSettings.enabled = nil
        gearSettings.showInSettings = nil
        gearSettings.fontSize = nil
    end
    GetSettings()
end
...
function CollectionsGear.GetShowWatermarkDefault() return defaults.collections.gear.showWatermark end
...
function CollectionsGear.GetFontChoices() return NQOL.Util.GetFontChoices() end
function CollectionsGear.GetFontChoiceNames() return NQOL.Util.GetFontChoiceNames() end
function CollectionsGear.GetFont() return GetSettings().font end
function CollectionsGear.SetFont(value) if not NQOL.Util.IsFontChoice(value) then value = NQOL.Util.GetDefaultFont() end; GetSettings().font = value; fontStringCache = {}; ApplyFonts(); Refresh() end
...
function CollectionsGear.GetSetCardLabel() return NQOL.L("features.collections_gear.set_card_label") end
function CollectionsGear.GetSetCardTooltip() return NQOL.L("features.collections_gear.set_card_tooltip") end
function CollectionsGear.GetShowWatermarkLabel() return NQOL.L("features.collections_gear.show_watermark_label") end
function CollectionsGear.GetShowWatermarkTooltip() return NQOL.L("features.collections_gear.show_watermark_tooltip") end
function CollectionsGear.GetHorizontalPositionLabel() return NQOL.L("features.collections_gear.horizontal_position_label") end
function CollectionsGear.GetHorizontalPositionTooltip() return NQOL.L("features.collections_gear.horizontal_position_tooltip") end
function CollectionsGear.GetVerticalPositionLabel() return NQOL.L("features.collections_gear.vertical_position_label") end
function CollectionsGear.GetVerticalPositionTooltip() return NQOL.L("features.collections_gear.vertical_position_tooltip") end
function CollectionsGear.GetFontLabel() return NQOL.L("features.collections_gear.font_label") end
function CollectionsGear.GetFontTooltip() return NQOL.L("features.collections_gear.font_tooltip") end
function CollectionsGear.GetScaleLabel() return NQOL.L("features.collections_gear.scale_label") end
function CollectionsGear.GetScaleTooltip() return NQOL.L("features.collections_gear.scale_tooltip") end
function CollectionsGear.GetBackgroundOpacityLabel() return NQOL.L("features.collections_gear.background_opacity_label") end
function CollectionsGear.GetBackgroundOpacityTooltip() return NQOL.L("features.collections_gear.background_opacity_tooltip") end

NQOL.Features.CollectionsGear = CollectionsGear
