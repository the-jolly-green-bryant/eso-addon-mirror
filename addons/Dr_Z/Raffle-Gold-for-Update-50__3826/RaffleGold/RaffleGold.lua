...
function RaffleGold:Menu()
	...
	local panelData = {
		...
		displayName = self.ADDON_DISPLAY_NAME,
	...
	local optionsTable = {
		...
		{
			type = "description",
			text = self.ADDON_DISPLAY_NAME .. " will display any error messages and the results of the raffle drawing in the chat window."
		},
		{
			type = "editbox",
			name = "Entry Price     $",
...
--[[ Things to add - Get Guild Rank Names for Guild Raffle select -- exclude guild rank ]]--
--[[
function RaffleGold:GetGuildsRankNames(gRank)
	guildRanks = {}
	guildRanks[1] = "-"
	if GetNumGuilds() > 0 then
		for rank = 1, GetNumGuilds() do
			local guildId = GetGuildId(guild)
			local guildName = GetGuildName(guildId)
			if(not guildName or (guildName):len() < 1) then
				guildName = "Guild " .. guildId
			end
			if gRank ~= nil and gRank == guildName then
				return guildId
			end
			guildRanks[rank + 1] = guildName
		end
	end
	return guildRanks
end
...
function RaffleGold:DrawRaffle(out, p)
	...
	if m ~= nil then
		CHAT_SYSTEM:AddMessage(self.ADDON_DISPLAY_NAME .. " -- \n|cFF0000Required: " .. m .. "\r")
		return false
	end
	if out == false then
		return true
	...
	if out == true then
		CHAT_SYSTEM:AddMessage(self.ADDON_DISPLAY_NAME .. sysMes)
	end
	return true
end
...
function RaffleGold:GuildRaffle(p)
	...
	if self.db.restriction == "One" then
		...
		if usercount > 0 then CHAT_SYSTEM:AddMessage(self.ADDON_DISPLAY_NAME .. " -- Unique # of usernames: " .. usercount) end
	end
	n = n-1
	if eT > cT then
		if stAmt ~= nil then
...
function RaffleGold:BuildHistory(gID, sT, cT, tot, eT)
	...
	nE = GetNumGuildHistoryEvents(gID, GUILD_HISTORY_EVENT_CATEGORY_BANKED_CURRENCY)
	...
	_, secondsSinceDeposit, _, eventType, depositerName, _, amount, _ = GetGuildHistoryBankedCurrencyEventInfo(gID, GUILD_HISTORY_EVENT_CATEGORY_BANKED_CURRENCY, nE)
	...
	if tot ~= nil then
		CHAT_SYSTEM:AddMessage(self.ADDON_DISPLAY_NAME .. " is now ready.")
	end
	return nE
end
...
function RaffleGold:GetRecentHistory(gID, cT)
	...
	d("ct")
	d(ct)
	_, secondsSinceDeposit, _, _, _, _, _, _ = GetGuildHistoryBankedCurrencyEventInfo(gID, GUILD_HISTORY_EVENT_CATEGORY_BANKED_CURRENCY, nE)
...
function RaffleGold.Cmd(txt)
	if txt == "" then
		CHAT_SYSTEM:AddMessage(self.ADDON_DISPLAY_NAME .. ': type "/rg help" for a list of commands.')
		return
	end
	arr = {}
	i = 1
	...
	if txt == "help" or arr[1] == "help" or arr[2] == "help" then
		...
		CHAT_SYSTEM:AddMessage(self.ADDON_DISPLAY_NAME .. " commands:")
		if arr[2] == "set" then
	...
	if arr[1] == "set" then
		...
		elseif string.find(arr[2], "ent") and arr[3] == "price" then
			...
			CHAT_SYSTEM:AddMessage(self.ADDON_DISPLAY_NAME .. ' set the "Entry Price" to $' .. self.db.entryPrice .. "\nIf you do not see the change(s) in the menu, type: /reloadui")
			return
		end
		if string.find(arr[2], "tick") and arr[3] == "price" then
			...
			CHAT_SYSTEM:AddMessage(self.ADDON_DISPLAY_NAME .. ' set the "Ticket Price" to ' .. arr[4] .. "\nIf you do not see the change(s) in the menu, type: /reloadui")
			return
		end
		if (arr[2] == "first" or arr[2] == "1st") and arr[3] == "place" then
			...
			CHAT_SYSTEM:AddMessage(self.ADDON_DISPLAY_NAME .. ' set "1st Place" to ' .. self.db.placeFirst .. "%\nIf you do not see the change(s) in the menu, type: /reloadui")
			return
		end
		if (arr[2] == "second" or arr[2] == "2nd") and arr[3] == "place" then
			...
			CHAT_SYSTEM:AddMessage(self.ADDON_DISPLAY_NAME .. ' set "2nd Place" to ' .. self.db.placeSecond .. "%\nIf you do not see the change(s) in the menu, type: /reloadui")
			return
		end
		if (arr[2] == "third" or arr[2] == "3rd") and arr[3] == "place" then
			...
			CHAT_SYSTEM:AddMessage(self.ADDON_DISPLAY_NAME .. ' set "3rd Place" to ' .. self.db.placeThird .. "%\nIf you do not see the change(s) in the menu, type: /reloadui")
			return
		end
		if string.find(arr[2], "tot") and string.find(arr[2], "ent") then
			...
			CHAT_SYSTEM:AddMessage(self.ADDON_DISPLAY_NAME .. ' set the "Total Entries" for the Basic Raffle to ' .. arr[4] .. "\nIf you do not see the change(s) in the menu, type: /reloadui")
			return
		end
		if string.find(arr[2], "tot") and (arr[3] == "amount" or arr[3] == "amt") then
			...
			CHAT_SYSTEM:AddMessage(self.ADDON_DISPLAY_NAME .. ' set the "Total Amount" for the Basic Raffle to ' .. arr[4] .. "\nIf you do not see the change(s) in the menu, type: /reloadui")
			return
		end
		if arr[2] == "guild" then
			...
			CHAT_SYSTEM:AddMessage(self.ADDON_DISPLAY_NAME .. ' set the "Guild" for the Guild Bank Raffle to "' .. self.db.guild .. "\"\nIf you do not see the change(s) in the menu, type: /reloadui")
			return
		end
		if string.find(arr[2], "rank") then
			...
			CHAT_SYSTEM:AddMessage(self.ADDON_DISPLAY_NAME .. ' set the "Exclude Guild Rank(s)" for the Guild Bank Raffle to "' .. arr[3] .. "\"\nIf you do not see the change(s) in the menu, type: /reloadui")
			return
		end
		if string.find(arr[2], "start") and (arr[3] == "amount" or arr[3] == "amt") then
			...
			CHAT_SYSTEM:AddMessage(self.ADDON_DISPLAY_NAME .. ' set the "Starting Amount" for the Guild Bank Raffle to ' .. arr[4] .. "\nIf you do not see the change(s) in the menu, type: /reloadui")
			return
		end
		if string.find(arr[2], "start") and arr[3] == "date" then
			...
			CHAT_SYSTEM:AddMessage(self.ADDON_DISPLAY_NAME .. ' set the "Starting Date" for the Guild Bank Raffle to ' .. self.db.dateStart .. "\nIf you do not see the change(s) in the menu, type: /reloadui")
			return
		end
		if string.find(arr[2], "start") and arr[3] == "time" then
			...
			CHAT_SYSTEM:AddMessage(self.ADDON_DISPLAY_NAME .. ' set the "Starting Time" for the Guild Bank Raffle to ' .. self.db.timeStart .. "\nIf you do not see the change(s) in the menu, type: /reloadui")
			return
		end
		if string.find(arr[2], "end") and arr[3] == "date" then
			...
			CHAT_SYSTEM:AddMessage(self.ADDON_DISPLAY_NAME .. ' set the "Ending Date" for the Guild Bank Raffle to ' .. self.db.dateEnd .. "\nIf you do not see the change(s) in the menu, type: /reloadui")
			return
		end
		if string.find(arr[2], "end") and arr[3] == "time" then
			...
			CHAT_SYSTEM:AddMessage(self.ADDON_DISPLAY_NAME .. ' set the "Ending Time" for the Guild Bank Raffle to ' .. self.db.timeEnd .. "\nIf you do not see the change(s) in the menu, type: /reloadui")
			return
		end
		if string.find(arr[2], "prize") then
			...
			CHAT_SYSTEM:AddMessage(self.ADDON_DISPLAY_NAME .. ' set the prizes per winner to ' .. self.db.restriction .. "\nIf you do not see the change(s) in the menu, type: /reloadui")
			return
		end
		if (string.find(arr[2], "bon") and string.find(arr[3], "m")) or (string.find(arr[3], "bon") and string.find(arr[2], "m")) then
			...
			CHAT_SYSTEM:AddMessage(self.ADDON_DISPLAY_NAME .. ' set the bonus multiplier to "' .. arr[4] .. '"')
			return
		end		
		...
		if string.find(arr[2], "bon") and tonumber(arr[3]) ~= nil and tonumber(arr[4]) ~= nil then
			...
			CHAT_SYSTEM:AddMessage(self.ADDON_DISPLAY_NAME .. ' set ' .. self.db.bonusTickets["amount"][k][2] .. ' bonus ticket' .. arr[4] .. ' when ' .. RaffleGold:ConvertNumber(arr[3]) .. ' is deposited')
			table.sort(self.db.bonusTickets["amount"], function(a, b) return a[1] > b[1] end)
			return
		end
