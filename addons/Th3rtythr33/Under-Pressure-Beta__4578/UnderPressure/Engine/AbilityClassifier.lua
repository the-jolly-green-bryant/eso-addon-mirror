...
function UP.Classifier.classify(effect)
    -- effect = { abilityId = number, statusEffectType = number_or_nil }
    if UP.features and UP.features.statusEffectType and effect.statusEffectType then
        local cat = UP.Classifier.classifyByStatusType(effect.statusEffectType)
        if cat ~= UP.RISK.NONE then return cat end
    end
    return UP.Classifier.classifyById(effect.abilityId)
end
