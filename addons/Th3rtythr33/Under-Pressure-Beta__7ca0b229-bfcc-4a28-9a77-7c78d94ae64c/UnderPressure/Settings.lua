...
-- not engine tunables -- so they are named once here and referenced from
-- getFunc's fallback, the LAM "default" field, AND the tooltip's "Default: N"
-- text below, rather than three separate hardcoded copies free to drift apart
-- the way the old, tooltip-less TTD sliders offered no such guarantee at all.
-- Keep in sync with DEFAULT_SAVED in UnderPressure.lua.
...
local optionsTable = {
    ...
    },
    {
        ...
        getFunc = function() return sv().scale or UI_DEFAULT_SCALE end,
        setFunc = function(v)
            sv().scale = v
    ...
    },
    {
        ...
        getFunc = function() return sv().offset_x or UI_DEFAULT_OFFSET_X end,
        setFunc = function(v)
            sv().offset_x = v
            if UP.UI and UP.UI.ApplyAnchor then
    ...
    },
    {
        ...
        getFunc = function() return sv().offset_y or UI_DEFAULT_OFFSET_Y end,
        setFunc = function(v)
            sv().offset_y = v
            if UP.UI and UP.UI.ApplyAnchor then
    ...
    },
    {
        ...
        min = 1, max = 5, step = 1,
        tooltip = ("Rolling window used to count distinct recent attackers. Default: %.0fs."):format(UP.Defaults.attacker_window_s),
    ...
    },
    {
        ...
        getFunc = function() return sv().counter_font_size or UI_DEFAULT_COUNTER_FONT_SIZE end,
        setFunc = function(v)
    ...
    },
    {
        ...
        tooltip = ("Below this estimated time-to-die, the indicator shows three stacked red triangles -- immediately lethal. Default: %.1fs."):format(UP.Defaults.red_three_ttd),
    ...
    },
    {
        ...
        tooltip = ("Below this estimated time-to-die, the indicator shows two stacked red triangles -- extreme. Default: %.1fs."):format(UP.Defaults.red_two_ttd),
    ...
    },
    {
        ...
        tooltip = ("Below this estimated time-to-die, the indicator shows one red triangle -- high. Default: %.1fs."):format(UP.Defaults.red_one_ttd),
    ...
    },
    {
        ...
        tooltip = ("Below this estimated time-to-die, the indicator fills solid yellow -- moderate. Default: %.1fs."):format(UP.Defaults.yellow_filled_ttd),
    ...
    },
    {
        ...
        min = 3, max = 30, step = 1,
        tooltip = ("After the last hit or debuff, how long the indicator keeps showing the yellow ring (recent pressure, not currently lethal) before dropping to the green square. Default: %.0fs."):format(UP.Defaults.recent_pressure_window_s),
