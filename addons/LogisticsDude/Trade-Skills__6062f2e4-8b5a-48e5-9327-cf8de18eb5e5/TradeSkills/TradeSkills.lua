...
-- Version 2.7 - Console compatibility
...
-- Safe debug output: d() may not be available on console
TradeSkills._isConsole = IsConsoleUI and IsConsoleUI() or false

if not d then
    d = function() end
end

-- Safe SLASH_COMMANDS: may be restricted on console
...
function TradeSkills.Carpentry:CreateFurnitureStoreWindow()
    ...
    TradeSkills.AddGamepadClose(window)
...
function TradeSkills.Carpentry:CountItemInInventory(searchLink)
    ...
    local total = 0
    local bagsToSearch = { BAG_BACKPACK, BAG_BANK, BAG_SUBSCRIBER_BANK, BAG_VIRTUAL }
                ...
                if resultLink and resultLink ~= "" then
                    ...
                    if resultType == ITEMTYPE_FURNISHING then
                        ...
                        for i = 1, numIngredients do
                            ...
                            -- Check if this ingredient is a style material (skip 200+ requirement)
                            ...
                            if isStyleMat then
                                -- Style materials: just need enough to craft (no 200+ buffer)
                                ...
                                -- Normal ingredients: need 200+ margin
...
function TradeSkills.Cooking:GetFoodStatType(itemLink)
    ...
    if (string.find(combined, TradeSkills.L.Stats.health) and string.find(combined, TradeSkills.L.Stats.magicka) and string.find(combined, TradeSkills.L.Stats.stamina)) then
        return "tri"
    end
    
    local hasHealth = string.find(combined, TradeSkills.L.Stats.health) and true or false
    local hasMagicka = string.find(combined, TradeSkills.L.Stats.magicka) and true or false
    local hasStamina = string.find(combined, TradeSkills.L.Stats.stamina) and true or false
...
function TradeSkills.Cooking:SelectFoodToQuickslot()
    ...
    if chosen then
        -- Find a quickslot to put food in without changing the player's active slot
        -- Priority: 1) slot that already has food, 2) empty slot, 3) slot 8
        local targetSlot = nil
        local emptySlot = nil
        for i = 1, 8 do
            local link = GetSlotItemLink(i, HOTBAR_CATEGORY_QUICKSLOT_WHEEL)
            if link and link ~= "" then
                if GetItemLinkItemType(link) == ITEMTYPE_FOOD then
                    targetSlot = i
                    break
                end
        ...
        targetSlot = targetSlot or emptySlot or 8
        CallSecureProtected("SelectSlotItem", BAG_BACKPACK, chosen.slot, targetSlot, HOTBAR_CATEGORY_QUICKSLOT_WHEEL)
    end
end
...
-- Check if any quickslot has food in it
function TradeSkills.Cooking:QuickslotHasFood()
    for i = 1, 8 do
        local itemLink = GetSlotItemLink(i, HOTBAR_CATEGORY_QUICKSLOT_WHEEL)
        if itemLink and itemLink ~= "" then
            if GetItemLinkItemType(itemLink) == ITEMTYPE_FOOD then return true end
        end
    end
    return false
end

-- Start monitoring quickslots to ensure food is always slotted somewhere
function TradeSkills.Cooking:StartQuickslotMonitor()
    ...
    EVENT_MANAGER:RegisterForUpdate(TradeSkills.name .. "_FOODSLOT", 3000, function()
        ...
        if not TradeSkills.Cooking:QuickslotHasFood() then
...
function TradeSkills.Cooking:CountItemInInventory(searchLink)
    ...
    for _, bagId in ipairs(bagsToSearch) do
        if bagId == BAG_VIRTUAL then
            -- Craft bag uses GetItemLinkItemId to look up virtual stack
            local itemId = GetItemLinkItemId(searchLink)
            if itemId and itemId > 0 then
                local _, stackCount = GetItemInfo(BAG_VIRTUAL, itemId)
                if stackCount and stackCount > 0 then
        ...
        for recipeIndex = 1, numRecipes do
            ...
            if known and numIngredients and numIngredients > 0 then
                ...
                if resultLink and resultLink ~= "" then
                    ...
                    if itemType == ITEMTYPE_FOOD then
                        -- Check if we have 200+ of EACH ingredient
                        local allAboveMargin = true
                        local maxCrafts = math.huge
                        ...
                        for i = 1, numIngredients do
                            ...
                            -- How many can we craft while staying above margin
                            local available = haveCount - MARGIN
...
function TradeSkills.Brewing:SelectDrinkToQuickslot()
    ...
    if chosen then
        -- Find a quickslot to put drink in without changing the player's active slot
        -- Priority: 1) slot that already has a drink, 2) empty slot, 3) slot 8
        local targetSlot = nil
        local emptySlot = nil
        for i = 1, 8 do
            local link = GetSlotItemLink(i, HOTBAR_CATEGORY_QUICKSLOT_WHEEL)
            if link and link ~= "" then
                if GetItemLinkItemType(link) == ITEMTYPE_DRINK then
                    targetSlot = i
                    break
                end
            elseif not emptySlot then
...
-- Check if any quickslot has a drink in it
function TradeSkills.Brewing:QuickslotHasDrink()
    for i = 1, 8 do
        local itemLink = GetSlotItemLink(i, HOTBAR_CATEGORY_QUICKSLOT_WHEEL)
        if itemLink and itemLink ~= "" then
            if GetItemLinkItemType(itemLink) == ITEMTYPE_DRINK then return true end
        end
    end
    return false
end

-- Start monitoring quickslots to ensure a drink is always slotted somewhere
function TradeSkills.Brewing:StartQuickslotMonitor()
    ...
    EVENT_MANAGER:RegisterForUpdate(TradeSkills.name .. "_DRINKSLOT", 3000, function()
        ...
        if not TradeSkills.Brewing:QuickslotHasDrink() then
...
function TradeSkills.Brewing:StartEfficiencyPolling()
    ...
    EVENT_MANAGER:RegisterForUpdate(pollName, 500, function()
        ...
        -- pcall wrapped: other addons (LazyWritCrafter) modify PROVISIONER state
        local ok, err = pcall(function()
            ...
            if provObj then
                ...
                if not (recipeListIndex and recipeIndex) then
                    ...
                    if tree then
                        local node = tree.selectedNode or (tree.GetSelectedNode and tree:GetSelectedNode())
                        if node then
...
function TradeSkills.StyleMastery:CreateCompletionistPopup()
    ...
    TradeSkills.AddGamepadClose(popup)
...
function TradeSkills.TraitMastery:InitSalvageForecastTooltip()
    ...
    -- Hook ItemTooltip (keyboard mode) - not available on console
    if ItemTooltip then
    local origSetBagItem = ItemTooltip.SetBagItem
    ItemTooltip.SetBagItem = function(control, bagId, slotIndex, ...)
        origSetBagItem(control, bagId, slotIndex, ...)
        pcall(TradeSkills.TraitMastery.AddSalvageForecastToTooltip, TradeSkills.TraitMastery, control, GetItemLink(bagId, slotIndex))
    end
    ...
    if origSetLink then
        ItemTooltip.SetLink = function(control, itemLink, ...)
            ...
            pcall(TradeSkills.TraitMastery.AddSalvageForecastToTooltip, TradeSkills.TraitMastery, control, itemLink)
        end
    end
    end -- if ItemTooltip
    ...
    if not self._achievements then
        ...
        for i = 1, 6000 do
            ...
            if name and name ~= "" and not seenIds[i] then
                local lowerName = string.lower(name)
                if (string.find(lowerName, TradeSkills.L.AnglerKeywords.angler) or string.find(lowerName, TradeSkills.L.AnglerKeywords.fish_boon)) and not string.find(lowerName, TradeSkills.L.AnglerKeywords.strangler) then
                    seenIds[i] = true
...
TradeSkills.Fishing.WATER_TYPE_MAP = TradeSkills.L.WaterTypes

-- Bait names for each water type, in priority order (special bait first, then common)
TradeSkills.Fishing.BAIT_PRIORITY = TradeSkills.L.BaitPriority
...
function TradeSkills.Fishing:GetZoneFishingAchievement()
    ...
    for i = 1, 6000 do
        ...
        if name and name ~= "" then
            local lowerName = string.lower(name)
            if (string.find(lowerName, TradeSkills.L.AnglerKeywords.angler, 1, true) or string.find(lowerName, TradeSkills.L.AnglerKeywords.fish_boon, 1, true)) 
               and not string.find(lowerName, TradeSkills.L.AnglerKeywords.strangler, 1, true) then
...
function TradeSkills.Mining:InitHeatMapPins()
    ...
    local function pinTypeAddCallback(pinManager)
        if not TradeSkills.Mining:HasPerk(30) then return end
    ...
    passiveAbilities = {
        {
            ...
            level = 10,
            description = "When viewing an alchemy reagent tooltip, shows which potions and poisons that plant is a primary ingredient for.",
        },
        {
...
function TradeSkills.Herbalism:InitFloraIDTooltip()
    ...
    -- Keyboard mode hooks - not available on console
    if ItemTooltip then
        local origSetBagItem = ItemTooltip.SetBagItem
        ItemTooltip.SetBagItem = function(control, bagId, slotIndex, ...)
            origSetBagItem(control, bagId, slotIndex, ...)
            pcall(TradeSkills.Herbalism.AddFloraIDToTooltip, TradeSkills.Herbalism, control, GetItemLink(bagId, slotIndex))
        end
        ...
        if origSetLink then
            ItemTooltip.SetLink = function(control, itemLink, ...)
                origSetLink(control, itemLink, ...)
                pcall(TradeSkills.Herbalism.AddFloraIDToTooltip, TradeSkills.Herbalism, control, itemLink)
            end
        end
    end -- if ItemTooltip
...
function TradeSkills.Herbalism:AddFloraIDToTooltip(tooltip, itemLink)
    ...
    -- Only for alchemy reagents
    local itemType = GetItemLinkItemType(itemLink)
    if itemType ~= ITEMTYPE_REAGENT then return end
    
    local name = string.lower(zo_strformat("<<1>>", GetItemLinkName(itemLink)))
    -- Map localized reagent name to English key for FLORA_POTION_DATA lookup
    local lookupName = TradeSkills.L.ReagentToEnglish[name] or name
    local data = TradeSkills.FLORA_POTION_DATA[lookupName]
    if not data then return end
    
    local labels = TradeSkills.L.FloraLabels
    tooltip:AddLine(" ", "ZoFontGameSmall", 1, 1, 1, 1, MODIFY_TEXT_TYPE_NONE, TEXT_ALIGN_CENTER, true)
    tooltip:AddLine("|c4DCC66[" .. labels.flora_id .. "]|r", "ZoFontGameSmall", 1, 1, 1, 1, MODIFY_TEXT_TYPE_NONE, TEXT_ALIGN_CENTER, true)
    
    if data.potions and #data.potions > 0 then
        tooltip:AddLine("|c88CC88" .. labels.potions .. ":|r " .. table.concat(data.potions, ", "), "ZoFontGameSmall", 1, 1, 1, 1, MODIFY_TEXT_TYPE_NONE, TEXT_ALIGN_LEFT, true)
    end
    if data.poisons and #data.poisons > 0 then
        tooltip:AddLine("|cCC6666" .. labels.poisons .. ":|r " .. table.concat(data.poisons, ", "), "ZoFontGameSmall", 1, 1, 1, 1, MODIFY_TEXT_TYPE_NONE, TEXT_ALIGN_LEFT, true)
    end
end
...
function TradeSkills.Herbalism:CreateGreenhouseWindow()
    ...
    window:SetDimensions(260, 70)
    window:SetAnchor(TOP, GuiRoot, TOP, 0, 150)
    window:SetHidden(true)
    window:SetDrawLayer(DL_OVERLAY)
    window:SetDrawTier(DT_HIGH)
...
function TradeSkills.Herbalism:ShowGreenhouseTracker(itemName)
    ...
    if HasCraftBagAccess() then
        local craftBagSize = GetBagSize(BAG_VIRTUAL)
        for slot = 0, craftBagSize - 1 do
            local slotLink = GetItemLink(BAG_VIRTUAL, slot)
            if slotLink and slotLink ~= "" then
                local slotName = string.lower(zo_strformat("<<1>>", GetItemLinkName(slotLink)))
                if slotName == lowerName then
                    local _, count = GetItemInfo(BAG_VIRTUAL, slot)
    ...
    for i = 1, numCentered do
        ...
        if pinType == MAP_PIN_TYPE_HARVEST_NODE then
            ...
            if description and type(description) == "string" then
                if string.find(string.lower(description), TradeSkills.L.Columbine) then
...
function TradeSkills.Herbalism:CheckForColumbine(interactionName)
    ...
    if string.find(string.lower(interactionName), TradeSkills.L.Columbine) then
        self:TriggerColumbineAlert()
    end
end
        ...
        if DoesUnitExist(unitTag) and not IsUnitDead(unitTag) and not IsUnitPlayer(unitTag) then
            ...
            if rawName and rawName ~= "" then
                ...
                if isScrapCreature then
                    ...
                    return  -- Don't hide loot window either
                end
            end
        end
        -- Not looking at a scrap creature - hide both
...
function TradeSkills.Skinning:CreateBeastIndicator()
    ...
    indicator:SetDimensions(400, 24)
    ...
    label:SetAnchor(CENTER, indicator, CENTER, 0, 0)
    ...
    TradeSkills.AddGamepadClose(indicator)
end
...
function TradeSkills.Skinning:ShouldRecordLootName(name, quality)
    ...
    -- Exclude items matching localized filter keywords (potions, gear, etc.)
    for _, keyword in pairs(TradeSkills.L.LootFilter) do
        if string.find(lowerName, keyword, 1, true) then return false end
    end
...
function TradeSkills.ScanTraits()
    ...
    -- Crafting types that have researchable traits (same as CraftStore)
    local craftingTypes = {
    ...
    for _, craftingType in ipairs(craftingTypes) do
        -- Get the number of research lines for this crafting type
        local numLines = GetNumSmithingResearchLines(craftingType)
        
        if numLines and numLines > 0 then
            for researchLineIndex = 1, numLines do
                -- CraftStore checks up to 9 traits per line
                for traitIndex = 1, 9 do
                    -- This is the key API call from CraftStore - it returns traitType, description, and known status
                    local traitType, _, known = GetSmithingResearchLineTraitInfo(craftingType, researchLineIndex, traitIndex)
                    
                    -- Check if trait exists and is known
                    if traitType and traitType ~= 0 then
                        if known == true then
                            -- Trait is fully researched
                            traitCount = traitCount + 1
                        elseif known == false then
                            -- Check if currently being researched
                            local _, remaining = GetSmithingResearchLineTraitTimes(craftingType, researchLineIndex, traitIndex)
                            -- If remaining > 0, it's being researched but we don't count it as known
                        end
                    end
                end
            end
...
-- Font mapping: keyboard -> gamepad equivalents (larger for TV distance)
TradeSkills.GAMEPAD_FONTS = {
    ["ZoFontGame"]       = "ZoFontGamepad27",
    ["ZoFontGameSmall"]  = "ZoFontGamepad22",
    ["ZoFontGameBold"]   = "ZoFontGamepadBold27",
    ["ZoFontWinH4"]      = "ZoFontGamepad34",
    ["ZoFontWinH1"]      = "ZoFontGamepadBold48",
    ["ZoFontCallout"]    = "ZoFontGamepad34",
}

-- Return the appropriate font for the current input mode (console always uses gamepad fonts)
function TradeSkills.GetFont(keyboardFont)
    if TradeSkills._isConsole or (IsInGamepadPreferredMode and IsInGamepadPreferredMode()) then
        return TradeSkills.GAMEPAD_FONTS[keyboardFont] or keyboardFont
    end
    return keyboardFont
end

-- Check if gamepad mode is active (always true on console)
function TradeSkills.IsGamepad()
    if TradeSkills._isConsole then return true end
    return IsInGamepadPreferredMode and IsInGamepadPreferredMode()
end

-- Scale factor for gamepad mode (UI elements need to be larger for TV distance)
function TradeSkills.GetGamepadScale()
    if TradeSkills.IsGamepad() then
        return 1.25
    end
    return 1.0
end

-- Add keybind strip to a window for gamepad navigation
function TradeSkills.AddGamepadKeybinds(window, keybinds)
    if not window._gamepadKeybindStripDescriptor then
        window._gamepadKeybindStripDescriptor = keybinds
    end
    
    window:SetHandler("OnShow", function()
        if TradeSkills.IsGamepad() and window._gamepadKeybindStripDescriptor then
            KEYBIND_STRIP:AddKeybindButtonGroup(window._gamepadKeybindStripDescriptor)
        end
    end)
    
    local origHide = window:GetHandler("OnHide")
    window:SetHandler("OnHide", function(...)
        if window._gamepadKeybindStripDescriptor then
            KEYBIND_STRIP:RemoveKeybindButtonGroup(window._gamepadKeybindStripDescriptor)
        end
        if origHide then origHide(...) end
    end)
end

-- Add simple close-on-back keybind to any popup window
function TradeSkills.AddGamepadClose(window)
    TradeSkills.AddGamepadKeybinds(window, {
        {
            name = "Close",
            keybind = "UI_SHORTCUT_NEGATIVE",
            callback = function() window:SetHidden(true) end,
        },
    })
end
...
-- Apply gamepad-appropriate fonts to main window labels
function TradeSkills.ApplyGamepadFonts()
    if not TradeSkills.window then return end
    local gp = TradeSkills.IsGamepad()
    
    -- Main panel labels
    if TradeSkills.skillNameLabel then
        TradeSkills.skillNameLabel:SetFont(gp and "ZoFontGamepadBold27" or "ZoFontWinH4")
    end
    if TradeSkills.skillLevelLabel then
        TradeSkills.skillLevelLabel:SetFont(gp and "ZoFontGamepadBold48" or "ZoFontWinH1")
    end
    if TradeSkills.xpText then
        TradeSkills.xpText:SetFont(gp and "ZoFontGamepad22" or "ZoFontGameSmall")
    end
    if TradeSkills.totalLabel then
        TradeSkills.totalLabel:SetFont(gp and "ZoFontGamepad27" or "ZoFontGame")
    end
    if TradeSkills.rankLabel then
        TradeSkills.rankLabel:SetFont(gp and "ZoFontGamepad27" or "ZoFontGame")
    end
    if TradeSkills.descText then
        TradeSkills.descText:SetFont(gp and "ZoFontGamepad22" or "ZoFontGameSmall")
    end
    
    -- Tab buttons
    if TradeSkills.skillButtons then
        for _, button in ipairs(TradeSkills.skillButtons) do
            if button.text then
                button.text:SetFont(gp and "ZoFontGamepad22" or "ZoFontGameSmall")
            end
            if button.levelText then
                button.levelText:SetFont(gp and "ZoFontGamepad18" or "ZoFontGameSmall")
            end
        end
    end
end
function TradeSkills.CreateWindow()
    ...
    -- ESO uses various scene names across versions; try all known ones
    local gamepadSkillScenes = {
        "gamepad_skills_root",
        "gamepad_skills",
        "gamepadSkills",
    }
    ...
    -- Fallback: if no gamepad scene was found, hook the scene manager directly
    -- to catch ANY scene that contains "skills" in gamepad mode
    if not TradeSkills._gamepadSkillsHooked then
        SCENE_MANAGER:RegisterCallback("SceneStateChanged", function(scene, oldState, newState)
            if not TradeSkills.IsGamepad() then return end
            if not TradeSkills.window then return end
            local sceneName = scene:GetName()
            if sceneName and string.find(sceneName, "skill") then
                if newState == SCENE_SHOWING then
                    TradeSkills.ScanRecipes()
                    TradeSkills.ScanTraits()
                    TradeSkills.UpdateWindow()
        ...
        TradeSkills._gamepadSkillsHooked = true
    end
    
    -- Gamepad keybind strip for main window (LB/RB tabs, A toggle perk, B close)
    TradeSkills.AddGamepadKeybinds(window, {
        {
            name = "Previous Skill",
            keybind = "UI_SHORTCUT_LEFT_SHOULDER",
            callback = function()
                local newTab = (TradeSkills.activeTab or 1) - 1
                if newTab < 1 then newTab = #TradeSkills.skills end
                TradeSkills.SelectTab(newTab)
            end,
        },
        {
            name = "Next Skill",
            ...
            callback = function()
                local newTab = (TradeSkills.activeTab or 1) + 1
                if newTab > #TradeSkills.skills then newTab = 1 end
                TradeSkills.SelectTab(newTab)
            end,
        },
        {
...
function TradeSkills.UpdateWindow()
    ...
    -- Apply gamepad font scaling if mode changed
    if TradeSkills.IsGamepad() ~= TradeSkills._lastGamepadMode then
        TradeSkills._lastGamepadMode = TradeSkills.IsGamepad()
        TradeSkills.ApplyGamepadFonts()
    end
...
local function OnInventoryUpdate(eventCode, bagId, slotId, isNewItem)
    ...
    -- Check if this item is in the mining whitelist (localized)
    ...
    -- Check if this item is in the herbalism whitelist (localized)
    if TradeSkills.L.HerbalismWhitelist[lowerName] then
        isHerbalismMaterial = true
        if TradeSkills.debug then
            d(string.format("[Herbalism Debug] '%s' matched whitelist!", itemName))
        end
    -- Special check: Type 39 items that are in the fiber list (ESO miscategorizes these)
    elseif itemType == 39 and TradeSkills.L.HerbalismFibers[lowerName] then
        isHerbalismMaterial = true
        if TradeSkills.debug then
            d(string.format("[Herbalism Debug] '%s' (Type 39) matched fiber whitelist!", itemName))
        end
    elseif TradeSkills.debug then
    ...
    -- Check itemType + localized keyword partial matching
    local isSkinningMaterial = false
    
    if itemType == 39 then
        for _, keyword in ipairs(TradeSkills.L.SkinningKeywords) do
            if string.find(lowerName, string.lower(keyword), 1, true) then
                isSkinningMaterial = true
                if TradeSkills.debug then
                    d(string.format("[Skinning Debug] '%s' (Type 39) matched keyword '%s'!", itemName, keyword))
                end
                break
            end
        end
...
function TradeSkills.OnAddOnLoaded(event, addonName)
    ...
    TradeSkills.ScanRecipes()
    TradeSkills.ScanTraits()
    -- Delay motif scan slightly - IsSmithingStyleKnown may not be ready at initial load
    zo_callLater(function()
    ...
    -- Update UI when player switches between keyboard and gamepad mode
    EVENT_MANAGER:RegisterForEvent(TradeSkills.name .. "_GAMEPAD", EVENT_GAMEPAD_PREFERRED_MODE_CHANGED, function()
        TradeSkills.ApplyGamepadFonts()
        TradeSkills.UpdateWindow()
    end)
    ...
    EVENT_MANAGER:RegisterForEvent(TradeSkills.name .. "_RECIPE", EVENT_RECIPE_LEARNED, function(_, recipeListIndex, recipeIndex)
        ...
        if known then
            ...
            if recipeName then
                -- Rescan motif pages whenever any recipe is learned (catches motif pages)
    ...
    EVENT_MANAGER:RegisterForEvent(TradeSkills.name .. "_LOREBOOK", EVENT_LORE_BOOK_LEARNED, function(_, categoryIndex, collectionIndex, bookIndex)
        -- Rescan motif pages when any lore book is learned (catches motif books)
        do
    ...
    EVENT_MANAGER:RegisterForUpdate("TradeSkills_HerbReticle", 500, function()
        if not TradeSkills.Herbalism:IsPassiveActive(25) and not TradeSkills.Herbalism:IsPassiveActive(40) then return end
        ...
        -- Check if this is an alchemy plant node (localized reagent names)
        local isHerb = false
        if TradeSkills.L.HerbalismReagents[lowerName] then
            isHerb = true
        else
            -- Partial match for interactable names that include extra text
            for reagent, _ in pairs(TradeSkills.L.HerbalismReagents) do
                if string.find(lowerName, reagent, 1, true) then
                    isHerb = true
                    break
                end
            end
...
-- =======================
-- SLASH COMMANDS (not available on all platforms)
-- =======================
