...
function itemCache:Initialize()
    ...
    -- remove this when the issue with the EVENT_INVENTORY_SINGLE_SLOT_UPDATE is fixed. this is only a temporary fix to make sure that the masterList is updated after using a treasure map and the pin gets removed.
    -- fix added: March 14th 2026
    EVENT_MANAGER:RegisterForEvent("LostTreasure_TemporaryFix", EVENT_INVENTORY_SINGLE_SLOT_UPDATE, function(_, bagId, slotIndex)
        if bagId ~= BAG_BACKPACK then
            return
        end

        itemCache:BuildMasterLists()
        LostTreasure:ProzessQueue(LOST_TREASURE_PIN_TYPE_TREASURE, function() LostTreasure:RefreshPinTypePins(LOST_TREASURE_PIN_TYPE_TREASURE) end, "EVENT_INVENTORY_SINGLE_SLOT_UPDATE")
    end)
