...
function RSM.onOpenStore(event)
	...
	local defGold = RSM.savedVariables.defGold
	...
	for i = 1, 9 do
		...
		if smActuals < smTargetCount then
			...
			RSM.purchaseItem(i, smDelta, defGold)
		end	
	end
	...
	if smImpActual < smTargetCount then
		...
		RSM.purchaseItem(34, smDelta, defGold)
	end
...
function RSM.purchaseItem(int, amount, defGold)
