...
local dot_off,dot_on="esoui/art/buttons/featuredot_inactive.dds","esoui/art/buttons/featuredot_active.dds"	--"/SatuveXboxUI/textures/point_off.dds","/SatuveXboxUI/textures/point_on.dds"
...
local function Frame_Player_UI()	--UI init
	...
	local fs=math.min(BUI.Vars.FrameFontSize,h*.75)
	...
	local player	=BUI.UI.Control("BUI_PlayerFrame",				SatuveUI,	{w1,h1},		anchor,				not BUI.Vars.PlayerFrame)
	...
	health.dot		=BUI.UI.Texture("BUI_PlayerFrame_HealthDoT",		health,	{h*3/2,h*3/4},	{RIGHT,CENTER,-6-b1,0},		'/SatuveXboxUI/textures/regen_sm.dds', true)
	...
	--Ornament
	local orn1=Border==4 and '/SatuveXboxUI/textures/gear_large.dds' or Border==7 and '/SatuveXboxUI/textures/dragon_large.dds' or nil
	local orn2=Border==4 and '/SatuveXboxUI/textures/gear_small.dds' or nil
	if orn1 then
...
local function Frame_Target_UI()	--UI init
	...
	local b,b1,b2=BUI.border[Border][2],BUI.border[Border][3],BUI.border[Border][4]
	local color=BUI.border[Border][5] and {1,1,1,1} or theme_color
	local w,h=BUI.Vars.TargetWidth,math.max(18, math.min(BUI.Vars.TargetHeight or 18, 20))
	...
	local target	=BUI.UI.Control("BUI_TargetFrame",SatuveUI,{w+b1*2,h+b2*2+fs*1.5*2},BUI.Vars.BUI_TargetFrame,true)
	...
	local plate		=BUI.UI.Control("BUI_TargetFrame_Plate",target.base,{w,fs+10},{TOP,TOP,0,-2},false)
	plate.name		=BUI.UI.Label("BUI_TargetFrame_PlateName",plate,{w-42,fs+12},{CENTER,CENTER,6,0},BUI.UI.Font(BUI.Vars.FrameFont1,math.max(fs+4,22),true,true),nil,{1,1},"TargetName(Level)",false)
	...
	target.plate=plate
	local health	=BUI.UI.Backdrop("BUI_TargetFrame_Health",target.base,{w+b1*2,h+b2*2},	{TOP,TOP,0,fs+10},{0,0,0,0}, color, nil, false)
...
--	health.bar:SetGradientColors(ch[1],ch[2],ch[3],ch[4],ch1[1],ch1[2],ch1[3],ch1[4])
	local targetValueFontSize=math.max(20, math.min(h+2,22))
	health.textbg	=BUI.UI.Backdrop("BUI_TargetFrame_HealthTextBg",health.bg,{math.floor(w*0.52),h+8},{CENTER,CENTER,0,0},{0,0,0,0.45},{0,0,0,0},nil,false)
	health.current	=BUI.UI.Label("BUI_TargetFrame_HealthCurrent",health.bg,{w-8,h+10},{CENTER,CENTER,0,0},BUI.UI.Font(BUI.Vars.FrameFont1,targetValueFontSize,true,true),{1,1,1,1},{1,1},'Health',false)
	health.pct		=BUI.UI.Label("BUI_TargetFrame_HealthPct",health.bg,{w*1/3,h+4},{textpos.percent,textpos.percent,-textpos.spacing,0},BUI.UI.Font(BUI.Vars.FrameFont2,targetValueFontSize-1,true,true),nil,{2,1},'Pct%',true)
	-- Keep target values above health, shield and trauma bars on the Xbox-PC client.
	if health.textbg.SetDrawLayer then health.textbg:SetDrawLayer(2) end
	if health.textbg.SetDrawLevel then health.textbg:SetDrawLevel(99) end
	for _,label in ipairs({health.current,health.pct}) do
		if label.SetDrawTier then label:SetDrawTier(2) end
		if label.SetDrawLayer then label:SetDrawLayer(3) end
	...
	health.current:ClearAnchors() health.current:SetAnchor(CENTER,health.bg,CENTER,0,0) health.current:SetHorizontalAlignment(TEXT_ALIGN_CENTER)
	health.pct:SetHidden(true)
	--Default target bar
...
--	member.bg		=BUI.UI.Backdrop("BUI_RaidFrame"..i.."_Bg",	member,	{w-6*s+fs*1.2,h-2*s},		{TOPLEFT,TOPLEFT,-fs*1.2+2*s,0},{.3,.3,.2,.7}, {0,0,0,0}, nil, not comp) member.bg:SetDrawLayer(0)
	...
	health.pct:SetDrawLayer(3)
	health.hot		=BUI.UI.Texture(	"BUI_RaidFrame"..i.."_HoT",	health,	{hs*3/2,hs*3/4},	{LEFT,CENTER,-w/6,0},	'/SatuveXboxUI/textures/regen_sm.dds', true)
	health.hot:SetDrawLayer(3)
	health.dot		=BUI.UI.Texture(	"BUI_RaidFrame"..i.."_DoT",	health,	{hs*3/2,hs*3/4},	{RIGHT,CENTER,w/6,0},	'/SatuveXboxUI/textures/regen_sm.dds', true) health.dot:SetTextureRotation(math.pi)
...
-- Xbox-PC compatibility: SXUI_MAX_BOSSES is absent on some clients.
local SXUI_MAX_BOSSES = tonumber(MAX_BOSSES) or 3
...
function BUI.Frames.Bosses_UI()	--UI init
	...
	local bosses	=BUI.UI.Control(	"BUI_BossFrame",				SatuveUI,	{w,h*3+8},		BUI.Vars.BUI_BossFrame or {TOPRIGHT,TOP,-710/2,5,ZO_CompassFrame},		not BUI.Vars.BossFrame)
...
--		if BUI.Vars.DeveloperMode and BUI.BossFight then d(BUI.TimeStamp().."|cEE2222Boss fight end|r") end
		...
		for i=1, SXUI_MAX_BOSSES do BUI.Frame["boss"..i]:SetHidden(true) end
...
function BUI.Frames.StabilizePlayerPosition()
	if not BUI_PlayerFrame or not BUI or not BUI.Vars then return end
	local anchor=BUI.Vars.FrameHorisontal and BUI.Vars.BUI_HPlayerFrame or BUI.Vars.BUI_PlayerFrame
	if type(anchor)~="table" then return end
	local parent=SatuveUI or GuiRoot
	local gap=(SatuveXboxUI and SatuveXboxUI.actionBarGap) or 35
	local point=anchor[1] or BOTTOM
	local relativePoint=anchor[2] or point
	local x=tonumber(anchor[3]) or 0
	local y=(tonumber(anchor[4]) or 0)-gap
	BUI_PlayerFrame:ClearAnchors()
	BUI_PlayerFrame:SetAnchor(point,parent,relativePoint,x,y)
end
...
function BUI.Frames:Initialize()
	...
	if BUI.Vars.RaidFrames then
		...
		-- Xbox-PC builds may not expose the keyboard guild-roster control or its handlers.
		-- Only attach hooks when the target control and handler actually exist.
		local guildRoster = ZO_GuildRoster
		local canHookGuildRoster = guildRoster
		    and BUI_RaidFrame
		    and type(ZO_PreHookHandler) == "function"
		    and type(guildRoster.GetHandler) == "function"

		if canHookGuildRoster and guildRoster:GetHandler("OnShow") then
		    ZO_PreHookHandler(guildRoster, "OnShow", function()
		        if IsUnitGroupLeader("player") then
		            BUI_RaidFrame:SetParent(guildRoster)
		            BUI_RaidFrame:ClearAnchors()
		            BUI_RaidFrame:SetAnchor(RIGHT, guildRoster, LEFT, 0, 0)
		        end
		    end)
		end

		if canHookGuildRoster and guildRoster:GetHandler("OnHide") then
		    ZO_PreHookHandler(guildRoster, "OnHide", function()
		        local parent = SatuveUI or GuiRoot
		        local anchorData = BUI.Vars and BUI.Vars.BUI_RaidFrame
		        BUI_RaidFrame:SetParent(parent)
		        BUI_RaidFrame:ClearAnchors()
		        if anchorData then
		            BUI_RaidFrame:SetAnchor(anchorData[1], parent, anchorData[2], anchorData[3], anchorData[4])
		        else
		            BUI_RaidFrame:SetAnchor(CENTER, parent, CENTER, 0, 0)
		        end
		    end)
		end
	end
...
function BUI.Frames.Attribute(unitTag, attribute, powerValue, powerMax, pct, shieldValue, traumaValue)
	...
	if enabled then
		...
		if unitTag=='reticleover' and attribute=='health' then
			local hpText = BUI.DisplayNumber(math.max(powerValue,0))
			if powerValue > 999999 then hpText = BUI.DisplayNumber(powerValue/1000,1).."k" end
			control.current:SetText(hpText.." ("..pctLabel..")")
			control.pct:SetText("")
			control.pct:SetHidden(true)
		else
...
function BUI.Frames:SetupAltBar()
	...
	if IsMounted() then
		...
		pct=maxValue>0 and math.min((tonumber(current) or 0)/maxValue,1) or 0
