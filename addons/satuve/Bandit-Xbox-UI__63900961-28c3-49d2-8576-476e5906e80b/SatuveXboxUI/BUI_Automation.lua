...
--				local w=parent:GetWidth()
	...
	-- On the managed Xbox-PC client the keyboard dungeon finder can be created after add-on startup.
	-- Install the handlers only once the control exists.
	local function InstallUndauntedHooks()
		local section=ZO_DungeonFinder_KeyboardListSection
		if not section then return false end
		if section.SatuveUndauntedHooksInstalled then return true end

		ZO_PreHookHandler(section,'OnEffectivelyShown',function()
			Pledges,haveQuest=GetGoalPledges()
			BUI.CallLater("MarkPledges",200,MarkPledges)
		end)
		ZO_PreHookHandler(section,'OnEffectivelyHidden',function() Pledges={} end)
		section.SatuveUndauntedHooksInstalled=true
		return true
	end

	if not InstallUndauntedHooks() then
		local attempts=0
		EVENT_MANAGER:UnregisterForUpdate("SatuveUI_UndauntedHooks")
		EVENT_MANAGER:RegisterForUpdate("SatuveUI_UndauntedHooks",1000,function()
			attempts=attempts+1
			if InstallUndauntedHooks() or attempts>=60 then
				EVENT_MANAGER:UnregisterForUpdate("SatuveUI_UndauntedHooks")
			end
		end)
	end
...
local function SynergyHandler()
	if not ZO_Synergy then return end
	local hooked=ZO_Synergy.OnSynergyAbilityChanged
	if type(hooked)~="function" then return end

	local function GetSafeSynergyName()
		if type(GetSynergyInfo)=="function" then
			local synergyName,iconFilename=GetSynergyInfo()
			return synergyName,iconFilename
		end
		if type(GetCurrentSynergyInfo)=="function" then
			local synergyName,iconFilename=GetCurrentSynergyInfo()
			return synergyName,iconFilename
		end
		if SYNERGY then
			return SYNERGY.lastSynergyName, nil
		end
		return nil,nil
	end

	ZO_Synergy.OnSynergyAbilityChanged=function(...)
		local synergyName,iconFilename=GetSafeSynergyName()
		if synergyName then
--			if synergyName=="Feed" and BUI.Vars.FeedSynergy and not IsUnitPlayer('reticleover') then return
			if BUI.Vars.AdvancedSynergy then
				local lastName = SYNERGY and SYNERGY.lastSynergyName
				local hoarfrost = (BUI.Cloudrest and BUI.Cloudrest.Hoarfrost) or 0
				if synergyName~="Welkynar's Light" and (lastName=="Shed Hoarfrost" or GetGameTimeMilliseconds()-hoarfrost<2000) then return end
				if lastName=="Gateway" or lastName=="Wind of the Welkynar" then return end
				if synergyName=="Gateway" and BUI.Player.role=="Healer" then return end
				if synergyName=="Charged Lightning" and (BUI.Player.role=="Tank" or BUI.Player.role=="Healer") then return end
			end
		end
		return hooked(...)
	end
...
--	EVENT_MANAGER:UnregisterForEvent("KeyboardNotifications",EVENT_GUILD_DESCRIPTION_CHANGED)
	...
	if BUI.Vars.HousePins~=4 then
		RedirectTexture("/esoui/art/icons/poi/poi_group_house_glow.dds","/SatuveXboxUI/textures/theme/blank.dds")
		if BUI.Vars.HousePins==3 or BUI.Vars.HousePins==1 then
			RedirectTexture("/esoui/art/icons/poi/poi_group_house_unowned.dds","/SatuveXboxUI/textures/theme/blank.dds")
		end
		if BUI.Vars.HousePins<=2 then
			RedirectTexture("/esoui/art/icons/poi/poi_group_house_owned.dds","/SatuveXboxUI/textures/theme/blank.dds")
		end
	end
	...
	if BUI.Vars.PlayerToPlayer and PLAYER_TO_PLAYER and type(PLAYER_TO_PLAYER.AddMenuEntry)=="function" and not PLAYER_TO_PLAYER.SatuveMenuHookInstalled then	--Provided by @zelenin
		...
		PLAYER_TO_PLAYER.AddMenuEntry=function(self, text, ...)
			...
			return hooked(self, text, ...)
		end
		PLAYER_TO_PLAYER.SatuveMenuHookInstalled=true
	end
	...
	-- The managed Xbox-PC client may create MAIL_INBOX only after the mailbox is opened.
	-- Install the override lazily instead of indexing a nil object during add-on startup.
	local function InstallDeleteMailOverride()
		local inbox=MAIL_INBOX
		if not inbox then return false end
		if inbox.SatuveDeleteMailInstalled then return true end

		inbox.Delete=function(self)
			if not self or not self.mailId then return end

			local numAttachments,attachedMoney=0,0
			if GetMailAttachmentInfo then
			...
			if numAttachments>0 or attachedMoney>0 then
				ZO_Dialogs_ShowDialog("BUI_DELETE_CONFIRMATION", {
					confirmationCallback=function()
						if DeleteMail then DeleteMail(self.mailId) end
						if SOUNDS and SOUNDS.MAIL_ITEM_DELETED then PlaySound(SOUNDS.MAIL_ITEM_DELETED) end
					end,
					title=SI_PROMPT_TITLE_DELETE_MAIL_ATTACHMENTS,
					body=BUI.Loc("DeleteMailConfirm"),
				})
			else
				if DeleteMail then DeleteMail(self.mailId) end
				if SOUNDS and SOUNDS.MAIL_ITEM_DELETED then PlaySound(SOUNDS.MAIL_ITEM_DELETED) end
		...
		inbox.SatuveDeleteMailInstalled=true
		return true
	end

	EVENT_MANAGER:UnregisterForUpdate("SatuveUI_DeleteMailInstall")
	if EVENT_OPEN_MAILBOX then EVENT_MANAGER:UnregisterForEvent("SatuveUI_DeleteMailInstall",EVENT_OPEN_MAILBOX) end

	if BUI.Vars.DeleteMail and not InstallDeleteMailOverride() then
		-- Retry briefly because the mail controls can be initialized after the add-on.
		local attempts=0
		EVENT_MANAGER:RegisterForUpdate("SatuveUI_DeleteMailInstall",1000,function()
			attempts=attempts+1
			if InstallDeleteMailOverride() or attempts>=30 then
				EVENT_MANAGER:UnregisterForUpdate("SatuveUI_DeleteMailInstall")
			end
		end)

		if EVENT_OPEN_MAILBOX then
			EVENT_MANAGER:RegisterForEvent("SatuveUI_DeleteMailInstall",EVENT_OPEN_MAILBOX,function()
				if InstallDeleteMailOverride() then
					EVENT_MANAGER:UnregisterForEvent("SatuveUI_DeleteMailInstall",EVENT_OPEN_MAILBOX)
					EVENT_MANAGER:UnregisterForUpdate("SatuveUI_DeleteMailInstall")
				end
			end)
		end
	...
	if BUI.Vars.JumpToLeader and PLAYER_TO_PLAYER and PLAYER_TO_PLAYER.control then
	...
	if BUI.Vars.GroupLeave then
		local function InstallGroupLeaveOverride()
			local descriptor=GROUP_LIST and GROUP_LIST.keybindStripDescriptor
			local entry=descriptor and descriptor[4]
			if not entry then return false end
			entry.callback=function()
				if GroupLeave then GroupLeave() end
			end
			return true
		end

		if not InstallGroupLeaveOverride() then
			EVENT_MANAGER:RegisterForUpdate("BUI_GroupLeaveOverride",1000,function()
				if InstallGroupLeaveOverride() then
					EVENT_MANAGER:UnregisterForUpdate("BUI_GroupLeaveOverride")
				end
			end)
		end
	end
...
--	/script StartChatInput(string.sub(GetOfferedQuestInfo(),5,12))
	if BUI.Vars.CovetousCountess or BUI.Vars.DarkBrotherhoodSpree then
		local lastInteractableName
		local interactionHookInstalled=false
		local function InstallStartInteractionHook()
			if interactionHookInstalled then return true end
			local manager=FISHING_MANAGER or INTERACTIVE_WHEEL_MANAGER
			if not manager or type(manager.StartInteraction)~="function" then return false end
			SecurePostHook(manager, "StartInteraction", function()
				local _, name=GetGameCameraInteractableActionInfo()
		...
		local chatterHooked=setmetatable({}, {__mode="k"})
		local function OverwritePopulateChatterOption(interaction)
			if not interaction or chatterHooked[interaction] or type(interaction.PopulateChatterOption)~="function" then return false end
			local PopulateChatterOption=interaction.PopulateChatterOption
			interaction.PopulateChatterOption=function(self, index, fun, txt, optionType, ...)
				local ZoneId=GetZoneId(GetUnitZoneIndex("player"))
				local offeredQuest=GetOfferedQuestInfo and GetOfferedQuestInfo() or ""
				if BUI.Vars.DarkBrotherhoodSpree and contractBook[lastInteractableName] and ZoneId==826 and nonSpreeDialog[string.sub(offeredQuest or "",5,12)] then
					if EndInteraction then EndInteraction(INTERACTION_QUEST) end
					if ZO_Alert then ZO_Alert(UI_ALERT_CATEGORY_ALERT, nil, BUI.Loc("AUTOMATION_Quest_Low_Reward")) end
					return
				end
				return PopulateChatterOption(self, index, fun, txt, optionType, ...)
			end
			chatterHooked[interaction]=true
			return true
		end

		local function InstallChatterHooks()
			InstallStartInteractionHook()
			local gamepadReady=OverwritePopulateChatterOption(GAMEPAD_INTERACTION)
			local keyboardReady=OverwritePopulateChatterOption(INTERACTION)
			if gamepadReady and keyboardReady then EVENT_MANAGER:UnregisterForUpdate("BUI_InstallChatterHooks") end
		end
		InstallChatterHooks()
		EVENT_MANAGER:RegisterForUpdate("BUI_InstallChatterHooks",1000,InstallChatterHooks)
	end
	...
	-- Xbox-PC may initialize the add-on before CHAT_SYSTEM exists.
	local function SXUI_UpdateChatDimensions()
		if CHAT_SYSTEM and GuiRoot and type(GuiRoot.GetDimensions)=="function" then
