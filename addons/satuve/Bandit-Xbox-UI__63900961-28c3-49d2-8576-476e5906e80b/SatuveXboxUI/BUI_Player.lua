...
function BUI.Player:Initialize()
	...
	for i=1, #stats, 1 do
		local current, maximum, effectiveMaximum=GetUnitPower("player", stats[i].id)
		local displayMaximum=(effectiveMaximum and effectiveMaximum > 0) and effectiveMaximum or maximum or 0
		local pct = displayMaximum > 0 and zo_roundToNearest((current or 0) / displayMaximum, 0.01) or 0
		BUI.Player[stats[i].name]={["current"]=current or 0, ["max"]=displayMaximum, ["pct"]=pct}
	end
	...
	local shieldPct = (maxValue and maxValue > 0) and zo_roundToNearest((value or 0) / maxValue, 0.01) or 0
	BUI.Player.shield		={["current"]=value or 0, ["max"]=maxValue or 0, ["pct"]=shieldPct}
	...
	local traumaPct = (maxValue and maxValue > 0) and zo_roundToNearest((value or 0) / maxValue, 0.01) or 0
	BUI.Player.trauma		={["current"]=value or 0, ["max"]=maxValue or 0, ["pct"]=traumaPct}
...
function BUI.Player:UpdateAttribute(unitTag, powerType, powerValue, powerMax, powerEffectiveMax)
	...
	-- Xbox-PC may report the value shown by ESO as effectiveMax rather than max.
