...
local function UI_Init()
	...
	-- Keep the first button safely below the top HUD. The previous bottom-to-top
	-- anchor could push the Settings button above the visible screen.
	local top=math.max(90, math.floor(rootHeight*0.09))
	...
	for _,data in pairs(PanelContent) do
		...
		elseif BUI.Vars.SidePanel[data.var] and BUI.Vars.SidePanel[data.var]~=0 and not disabled then
			...
			button:SetHandler("OnMouseExit", function(self)
				...
				if data.tooltip then ClearTooltip(InformationTooltip) end
			end)
