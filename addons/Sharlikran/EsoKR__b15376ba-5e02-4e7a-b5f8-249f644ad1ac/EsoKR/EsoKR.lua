...
local flags = { EsoKR.langVer.stable, "en", }
local korean = { EsoKR.langVer.stable, }
local isNeedToChangeAdditionalFontTable = { EsoKR.langVer.stable, "en", }
...
function EsoKR:fontChangeWhenInit()
  ...
  local antiqueFont     = "EsoKR/fonts/proseantiquepsmt.slug"
  local handwrittenFont = "EsoKR/fonts/univers55.slug"
  local tabletFont      = "EsoKR/fonts/ftn57.slug"

  _G["GetBookMediumFontInfo"] = function(mediumId, isGamepad, ...)
    local titleFontName, titleFontSize, titleFontStyle, bodyFontName, bodyFontSize, bodyFontStyle, fontColorR, fontColorG, fontColorB, fontColorA, fontStyleColorR, fontStyleColorG, fontStyleColorB, fontStyleColorA = orgGetBookMediumFontInfo(mediumId, isGamepad, ...)
    d("mediumId: " .. tostring(mediumId))
