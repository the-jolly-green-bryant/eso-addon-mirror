...
local function addFilteredBackpackCategoryIfPopulated(filterType, iconFile)
	...
	if not isListEmpty then
		...
		addCategory(name, iconFile, ZO_InventoryUtils_DoesNewItemMatchFilterType, nil, filterType, BAG_BACKPACK, nil)
	end
end
...
function IJA_GPInventory:DynamicMapHeaders_SetTextAndCount(index, header, count)
    if header and count then
        ...
        self.customHeaders.mapsHeaderData["data" .. index .. "HeaderText"] = ""
        self.customHeaders.mapsHeaderData["data" .. index .. "Text"] = ""
    end
end
...
function IJA_GPInventory:PerformDeferredInitialize()
	...
	function GAMEPAD_INVENTORY:RefreshActiveCategoryList(selectDefaultEntry)
...
local function sortJunkToTOP(left, right)
	...
	elseif leftOrder == VAR_ENTRY_ORDER_OTHER then
		...
		if left.isJunk or right.isJunk then
			return ZO_TableOrderingFunction(left, right, "isJunk", DEFAULT_SORT_KEYS, IJA_GPINVENTORY.savedVars.withdraw)
		end
...
local function sortJunkToBottom(data1, data2)
	if data1.isJunk or data2.isJunk then
		return ZO_TableOrderingFunction(data1, data2, "isJunk", DEFAULT_SORT_KEYS, IJA_GPINVENTORY.savedVars.deposit)
	end
...
function IJA_GPInventory:InitJunkSort()
	IJA_GPINVENTORY.savedVars = self.savedVars
	-------------------------------------
