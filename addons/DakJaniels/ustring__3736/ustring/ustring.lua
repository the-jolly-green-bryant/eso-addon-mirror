...
local function checkPattern(name, pattern)
    ...
    if S.len(pattern) > ustring.maxPatternLength then
        local msg = S.format("bad argument #2 to '%s' (pattern is longer than %d bytes)", name, ustring.maxPatternLength)
        error(msg, 3)
    end
end
...
local function find(s, cps, rawpat, pattern, init, noAnchor)
    local charsets = ustring.charsets
    local anchor = false
...
local function patternIsSimple(pattern)
    ...
    return not (S.find(pattern, "[\128-\255]") or S.find(pattern, "%[%^") or S.find(pattern, "%%[acdlpsuwxACDLPSUWXZ]") or S.find(pattern, "%.[^*+-]") or S.find(pattern, "%.$") or S.find(pattern, "()", 1, true) or pattern == "" or findWithPcall("", pattern))
end
...
local function internalDecompose(cps, decomp)
    local cp = {}
    local normal = ustring.normalization_data
