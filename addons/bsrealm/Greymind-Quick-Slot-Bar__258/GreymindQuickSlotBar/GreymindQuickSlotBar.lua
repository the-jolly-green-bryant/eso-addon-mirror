...
v2.7.2.24 (260614) {{{
- [color="yellow"]Checked with v9.0.0 "Necrom"[/color]
}}}
v2.7.2.23 (260315) {{{
- [color="Wheat"]Checked with Update 49 "The Night Market"[/color]
}}}
v2.7.2.22 (251101) {{{
- [color="gray"]Checked with Update 48 "Seasons of the Worm Cult"[/color]
}}}
v2.7.2.21 (250823) {{{
- [color="magenta"]Checked with Update 47 "Feast of Shadows"[/color]
}}}
v2.7.2.20 (250608) {{{
- [color="lightblue"]Checked with Update 46 "Seasons of the Worm Cult Part 1"[/color]
- [color="lightgray"]Dependency updated from [color="orange"]LibAddonMenu-2.0>=32[/color] [color="yellow"]LibAddonMenu-2.0>=40[/color]
}}}
v2.7.2.19 (250608) {{{
- [color="lightblue"]Checked with Update 46 "Seasons of the Worm Cult Part 1"[/color]
}}}
v2.7.2.18 (250311) {{{
- [color="yellow"]Checked with Update 45 "Fallen Banners"[/color]
- [color="brown" ]Buttons font outlined for better visibility[/color]
}}}
v2.7.2.17 (241228) {{{
- [color="yellow"]Checked with Update 44 (API 101044)[/color]
- [color="brown" ]Buttons font outlined for better visibility[/color]
}}}
v2.7.2.16 (241102) {{{
- [color="yellow"]Checked with Update 44 (API 101044)[/color]
...
v2.7.2.15 (240831) {{{
- [color="orange"]Checked with Update 43 "Home Tours" (10.1.5) (API 101043)[/color]
}}}
v2.7.2.14 (240608) {{{
- [color="darkRed"]Checked with Update 42 "Gold Road" (10.0.5) (API 101042)[/color]
}}}
v2.7.2.13 (240428) {{{
- [color="lightBlue"]Checked with "Scions of Ithelia" DLC (9.3.8) (API 101041)[/color]
}}}
v2.7.2.12 (240316) {{{
- [color="lightBlue"]Checked with "Scions of Ithelia" DLC (9.3.5) (API 101041)[/color]
}}}
v2.7.2.11 (240218) {{{
- [color="Brown"]Checked with Update 41 Infinite Archive (9.2.10) (API 101041)[/color]
}}}
v2.7.2.10 (240124) {{{
- [color="red"]Addons Initialisation chat-messages removed[/color]
- [color="Wheat"]Checked with Update 40 Infinite Archive (9.2.8) (API 101040)[/color]
}}}
v2.7.2.9 (231223) {{{
- [color="Wheat"]Checked with Update 40 Infinite Archive (9.2.8) (API 101040)[/color]
}}}
...
[color="#FF0"]4 - nailed the wrong 'NOT SAVED' then 'RELOAD' on single-slot-updated[/color]
...
v2.6.6   211101 {{{
- [color="blue"]Checked with Update 32 Deadlands (7.2.5): (API 101032)[/color]
1 - Keeping depleted items equipped in slot (will show on refill)
2 - Default Warning and Alert sounds set to none
3 - Warning and and Alert sounds controled by [color="magenta"]ChatMute Settings[/color] option
...
local COLOR_NORMAL                  = { R =1  , G =1  , B =1  , A = 1   } -- grayish
...
local MyGetSlotItemCount
local MyGetSlotItemLink
local MyGetSlotName
local MyGetSlotTexture
local MyIsValidCollectibleForSlot
local MyIsValidItemForSlot
local MyIsValidQuestItemForSlot
...
--}}}
-- D_QSB {{{
local function D_QSB()

    for bNum = 1, QSB.ButtonCountMax do
        if(QSB.Settings.SlotItemTable[bNum].slotId) then
            c(       COLOR_X[bNum].." bNum_"     ..tostring(                            bNum            )
            ..                      " slotIndex_"..tostring(          bNum_to_slotIndex(bNum)           )..":"
            ...
            .."\n…".. COLOR_5      .." slotId_"  ..tostring( QSB.Settings.SlotItemTable[bNum].slotId    )
            .."\n…".. COLOR_6      .." Level_"   ..tostring( QSB.Settings.SlotItemTable[bNum].itemLevel )
            )
        else
            c("["..bNum.."] "..COLOR_8.." EMPTY SLOT")
        end
    end
...
if(log_this) then c(QSB.Settings.PresetName..COLOR_M.." → RELOADING "..selectedPreset.." → NOT SAVED") end
        ...
        return --// (211105 .. that was the presumably LockThisPreset bug)
    ...
    elseif QSB.Settings.PresetName then
        ...
        or               COLOR_M.."LOCKED|r but "..COLOR_4.."Settings MENU is OPENED"
...
D(      "SelectPreset_cloneTo_empty_content:")
    ...
    if Loaded_Preset ~= nil then
        from =            Loaded_Preset
        from_PresetName = Loaded_Preset.PresetName
    else
        from = QSB.Settings.Presets[currentPreset]
        from_PresetName =           currentPreset
    end
    to       = QSB.Settings
...
local log_this = DEBUG_EQUIP or DEBUG_TASK or DEBUG_STATUS or DEBUG_ITEM
    ...
    for bNum = 1, QSB.ButtonCountMax do
        ...
        local slottedName     =     MyGetSlotName( slotIndex)
...
if(log_this) then D_EQUIP(ITEM_5_EQUIP_CHANGED, bNum, itemId, itemType, itemLevel, itemLink) end
    ...
    if( itemType == "item" and itemLink) then
        ...
        local is_valid   = MyIsValidItemForSlot(BAG_BACKPACK, bagIndex, slotIndex)
        if(QSB.Settings.LockThisPreset) then
            MySelectSlotItem(BAG_BACKPACK, bagIndex, slotIndex)

            if(    count <= 0 or not is_valid) then
        ...
        elseif(    count >  0 and    is_valid) then
            MySelectSlotItem(BAG_BACKPACK, bagIndex, slotIndex)
...
D_EQUIP(ITEM_2_EQUIP_ERROR_SLOT, bNum, itemId, itemType, itemLevel, itemLink)
    ...
    --}}}
    -- [collectible] {{{
    elseif(itemType == "collectible" and MyIsValidCollectibleForSlot(itemId, slotIndex)) then
        MySelectSlotSimpleAction(ACTION_TYPE_COLLECTIBLE, itemId, slotIndex)

    --}}}
    -- [quest_item] {{{
    elseif(itemType == "quest_item" and MyIsValidQuestItemForSlot(itemId, slotIndex)) then
        MySelectSlotSimpleAction(ACTION_TYPE_QUEST_ITEM , itemId, slotIndex)
...
function is_same_slotted_item( bNum )
        ...
        local  slot_itemLink  = MyGetSlotItemLink(bNum_to_slotIndex( bNum ))
...
if(log_this) then c("save_QSB_to_SlotItemTable("..bNum.."):") end
    ...
    local itemName  = MyGetSlotName    ( slotIndex)
    local texture   = MyGetSlotTexture ( slotIndex)
    local itemLink  = MyGetSlotItemLink( slotIndex)
...
function getItem_tooltip(bNum)
    ...
    --}}}
    -- count {{{
    local       count = MyGetSlotItemCount(slotIndex) or 0
...
--  local  tt = color  .. label .."|r"
    ...
    if(DEBUG_TOOLTIP and itemName) then
        ...
        local count_from_slotIndex   = MyGetSlotItemCount        ( slotIndex)
        ...
        --}}}
        -- ITEM ......... f(bagIndex) {{{
        local bagIndex  = getItem_slot_from_name( MyGetSlotName(slotIndex) )
        if(   bagIndex ~= -1) then
            ...
            .."\n- IsValidItemForSlot("..      slotIndex..")"       ..COLOR_3.. " ["..tostring(  is_valid                           ).."]|r"
...
--c("l1=["..l1.."] == l2=["..l2.."] .. return "..tostring(result)) end
...
if(DEBUG_ITEM) then c(COLOR_8.."Refresh "..tostring(_caller)) end
    ...
    -- following calls will be queued .. until no more incoming .. (but not forever)
...
if(log_this) then c("...Refresh_handler() .. Refresh_count_down=[".. tostring(Refresh_count_down) .."]") end
...
if(log_this) then c("background_color=[ R="..r.." G="..g.." B="..b.." ]") end
    ...
    -- IDLE - VERSION .. ACCOUNT/PLAYER
        ...
        GreymindQuickSlotBarUI.data = {
            tooltipText
            =  QSB.NAME.." "..QSB.VERSION.." "..QSB.UPDATE
            ..(QSB.AccountWideSettings.SaveAccountWide and COLOR_3.."AccountWide Settings"
    ...
    local button_font = "EsoUI/Common/Fonts/esocartographer-bold.otf".."|"..tostring(QSB.Settings.ButtonFontSize).."|".."outline"
    ----- button_font = "EsoUI/Common/Fonts/univers57.otf"           .."|"..tostring(QSB.Settings.ButtonFontSize).."|".."soft-shadow-thin"
...
--          quantityLabel               :SetDrawLayer(7)
        ...
        local keyLabel          = QSB.KeyLabels[bNum]
        local quantityLabel     = QSB.QuantityLabels[bNum]
        local slotItemCount     = MyGetSlotItemCount(slotIndex)
        local buttonTexture     = MyGetSlotTexture  (slotIndex)
...
--d("...bNum=["..bNum.."] .....itemName=["..COLOR_3..QSB.Settings.SlotItemTable[bNum].itemName.."|r]")
        end
--d("...bNum=["..bNum.."] slotIndex=["..slotIndex.."] buttonTexture=["..COLOR_4..buttonTexture.."|r]")
...
if(buttonTexture) then
            ...
            if not emptySlot then
                quantityLabel:SetText( MyGetSlotItemCount(slotIndex) )
            else
...
D_ITEM("IsEmptySlot(slotIndex " ..tostring( slotIndex )..")")
    ...
    local slotName =       MyGetSlotName( slotIndex)
    local     bNum = slotIndex_to_bNum( slotIndex  )
...
D_ITEM(   color.."...return "..tostring(is_empty))
    return is_empty
end
--}}}
-- string_split {{{
...
c_log("[removeServerDataOnce]WorldName: " ..tostring(WorldName))

    local copyOfOldSV = ZO_ShallowTableCopy(svTableOld)

    if copyOfOldSV[WorldName] ~= nil then
c_log(">>found existing servername data -> stripped!")
...
--  LAM:AddHeader(panel, "QSB_UserInterface", "")
    ...
    local panelData = {
        ...
        displayName         = SETTINGSPANELNAME..COLOR_8.." - "..QSB.VERSION.." - "..QSB.UPDATE,
        author              = "ivanwfr",
        version             = QSB.version,
    ...
    control = {
        ...
        .." key combinations.\n"
        .."\n"
        .."3. ...and you may have to insist, going back and forth\n"
        .."from CONTROLS-Keybindings to clicking this button!",
        func        = function()
            ApplyKeyBindingsModifier()
            ApplyKeyBindingsModifier_SWAPS()
        end,
...
if(DEBUG_EQUIP) then c(COLOR_5.."ITEM UPDATED: [ID "..bagIndex.."] [Level "..tostring(itemLevel).."] ["..tostring(itemName).."]") end
    ...
    --}}}
    -- .. HIDE___ .. RETICLE_TARGET_CHANGED --{{{
    ...
    --}}}
    -- .7 Refresh .. PLAYER_COMBAT_STATE .. GameActionButtonHideHandler  --{{{
...
--c("GetActiveWeaponPairInfo() returns ["..tostring(GetActiveWeaponPairInfo()).."]")
    ...
    --}}}
    -- 17 Refresh ========================= loaded {{{
    D(string.format("VERSION %s %s loaded", QSB.VERSION, QSB.UPDATE))

    Refresh(COLOR_7.."RegisterForEvent")             -- 191125 .. apply Show Policy onload
    --}}}
end
--}}}
-- MouseDown {{{
...
function OnSlashCommand(arg)
    ...
    if (arg == "") then
        ...
        ..(QSB.AccountWideSettings.SaveAccountWide and COLOR_6.." AccountWide"
...
function d_signature()
    d("\r\n!! GQSB"..COLOR_C.." "..QSB.VERSION.." "..COLOR_9.." "..QSB.UPDATE.." (API "..QSB.API..") ("..QSB.TRACE_TAG..")|r\n"
...
:vnew|n $TEMP/APIPatchNotes*.txt $TEMP/ESOUIDocumentation*.txt
