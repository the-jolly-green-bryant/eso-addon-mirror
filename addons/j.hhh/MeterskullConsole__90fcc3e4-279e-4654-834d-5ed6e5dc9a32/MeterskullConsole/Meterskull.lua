...
function MS.OnAddOnLoaded(event, addOnName)
    -- Console package uses MeterskullConsole.addon, while the original addon
    -- internally identifies itself as "Meterskull". Accept both identities.
    if addOnName ~= MS.name and addOnName ~= "MeterskullConsole" then return end
