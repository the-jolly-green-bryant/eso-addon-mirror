...
local function InitializeMenu(MS)
    ...
    local positionModules = {
        { key="armorskull", name="Armor" },
        { key="hybridarmorskull", name="Hybrid Armor" },
        { key="powerskull", name="Power" },
        { key="critskull", name="Criticals" },
        { key="critresiskull", name="Critical Resistance" },
        { key="penskull", name="Penetration" },
        { key="healthskull", name="Health Recovery" },
        { key="magskull", name="Magicka Recovery" },
        { key="stamskull", name="Stamina Recovery" },
    }

    local function ApplyConsolePosition(key)
        local mod = MS.modules[key]
        local db = MS.db[key]
        if not mod or not mod.uiRefs or not mod.uiRefs.main or not db then return end
        mod.uiRefs.main:ClearAnchors()
        mod.uiRefs.main:SetAnchor(TOPLEFT, GuiRoot, TOPLEFT, db.location.x, db.location.y)
    end

    local positionControls = {
        {
            type = "description",
            text = "|cd9d9d9Console positioning: use X/Y sliders with the controller. Mouse dragging remains available on PC while UI Locked is disabled.|r"
        }
    }

    for _, info in ipairs(positionModules) do
        local key = info.key
        local displayName = info.name
        positionControls[#positionControls + 1] = {
            type = "submenu",
            name = displayName .. " Position",
            controls = {
                {
                    type = "slider", name = "X Position", min = 0, max = 3840, step = 5,
                    getFunc = function() return MS.db[key].location.x end,
                    setFunc = function(v)
                        MS.db[key].location.x = v
                        ApplyConsolePosition(key)
                    end,
                    default = MS.defaults[key].location.x,
                },
                {
                    type = "slider", name = "Y Position", min = 0, max = 2160, step = 5,
                    getFunc = function() return MS.db[key].location.y end,
                    setFunc = function(v)
                        MS.db[key].location.y = v
                        ApplyConsolePosition(key)
                    end,
                    default = MS.defaults[key].location.y,
                },
            },
        }
    end
