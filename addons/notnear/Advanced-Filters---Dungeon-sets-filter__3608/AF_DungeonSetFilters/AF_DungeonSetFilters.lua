...
local transl = afDS.translation
if not transl then return end

--Constant
local addonPluginName 		= "AF_DungeonSetFilters"
local subMenuPrefix			= addonPluginName .. "SubmenuDungeonSet_"
local subMenuEntryAllSets   = "DungeonSetsAll"

--Constants for ALL SETS
local allSetsConstantName 	= "--ALL_DUNGEON_SETS--"
...
local function toggleonlyall()
	...
	if sv.onlyall == true then
		sv.onlyall = false
		d(GetString(AFDS_COMMAND_MESSAGE_1))
	elseif sv.onlyall == false then
		sv.onlyall = true
		d(GetString(AFDS_COMMAND_MESSAGE_2))
	end
end

local function registerSlashCommands()
	SLASH_COMMANDS["/af_dungeonset"] = function () toggleonlyall() end
end
...
local function addOnPluginLoaded(event, name)
	...
	afDS.ASV = ZO_SavedVars:NewAccountWide(addonPluginName .. "_Data", 1, nil, addonDefaults)

	registerSlashCommands()

	--Check if library LibSets is loaded and load the sets data if missing yet
	if (not areSetsLoaded() or libSets.dungeonSets == nil) then
		libSets.LoadSets(true, addonPluginName)
		return
	end
	if (not areSetsLoaded() or libSets.dungeonSets == nil) then
		d("[AdvancedFilters Plugin]: \'DungeonSets\' is missing LibSets dungeon sets data!")
		return
	end
	...
	local dungeonSetIds = {}
	stc(libSets.dungeonSets, dungeonSetIds)
	local setNamesWithId = {}
	local setNamesSorted = {}
	--Get the name of the sets
	for setId, _  in pairs(dungeonSetIds) do
		local setName = ""
...
--afDS._setNamesWithId = setNamesWithId
--afDS._setNamesSorted = setNamesSorted
	...
	local function GetFilterCallbackForSets( setId )
		return function( slot , slotIndex)
			...
			if setId == allSetsConstantId then
				for setIdToCompareToLoop, _ in pairs(dungeonSetIds) do
	...
	local allDungeonSetsDropdownCallback = { { name = subMenuEntryAllSets, filterCallback = GetFilterCallbackForSets( allSetsConstantId ) } }
...
--afDS._entriesForDropdownFiveSetEachCallbacks = entriesForDropdownFiveSetEachCallbacks
--afDS._fullDropdownSetsCallbacks 		= fullDropdownSetsCallbacks
--afDS._dropdownFiveSetEachCallbacks 	= dropdownFiveSetEachCallbacks
--afDS._dropdownFiveSetEachCharactersKey = dropdownFiveSetEachCharactersKey
		end
	end
	...
	local function buildFilterInformation()
		...
		for _, fiveCharacters in ipairs(dropdownFiveSetEachKeys) do
			...
			local stringsEN = {
				[allSetsConstantName]      = "-All dungeon sets-",
			}
			local stringsDE = {
				[allSetsConstantName]      = "-Alle Dungeon Sets-",
			}
			local stringsFR = {
				[allSetsConstantName]      = "-Tous les set des dungeon-",
			}
			stringsEN[fiveCharactersSubMenuName] = transl.DUNGEON_SETS_MENU_ENTRY ..fiveCharacters
			stringsDE[fiveCharactersSubMenuName] = transl.DUNGEON_SETS_MENU_ENTRY ..fiveCharacters
			stringsFR[fiveCharactersSubMenuName] = transl.DUNGEON_SETS_MENU_ENTRY ..fiveCharacters
	...
	--First add 1 entry for "All dungeon sets"
	local stringsAllEN = { [subMenuEntryAllSets] = transl.ALL_DUNGEON_SETS_SUBMENU_ENTRY }
	local stringsAllDE = { [subMenuEntryAllSets] = transl.ALL_DUNGEON_SETS_SUBMENU_ENTRY }
	local stringsAllFR = { [subMenuEntryAllSets] = transl.ALL_DUNGEON_SETS_SUBMENU_ENTRY }

	local filterInformationAllDungeonSets = {
		callbackTable = allDungeonSetsDropdownCallback,
		filterType = {
			ITEMFILTERTYPE_ARMOR,
			ITEMFILTERTYPE_WEAPONS,
			ITEMFILTERTYPE_JEWELRY,
	...
	AdvancedFilters_RegisterFilter(filterInformationAllDungeonSets)

	filterInformationAllDungeonSets = {
		callbackTable = allDungeonSetsDropdownCallback,
		filterType = ITEMFILTERTYPE_ALL,
		onlyGroups = {"Armor", "ArmorRetrait", "Weapons"},
		subfilters = {"All",},
	...
	if afDS.ASV.onlyall == false then
		buildFilterInformation()
	end
end
