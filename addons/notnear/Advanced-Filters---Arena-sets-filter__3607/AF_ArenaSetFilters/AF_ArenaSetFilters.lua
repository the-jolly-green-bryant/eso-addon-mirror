...
local transl = afAS.translation
if not transl then return end

--Constant
local addonPluginName 		= "AF_ArenaSetFilters"
local subMenuPrefix			= addonPluginName .. "SubmenuArenaSet_"
local subMenuEntryAllSets   = "ArenaSetsAll"

--Constants for ALL SETS
local allSetsConstantName 	= "--ALL_ARENA_SETS--"
...
local function toggleonlyall()
	...
	if sv.onlyall == true then
		sv.onlyall = false
		d(GetString(AFAS_COMMAND_MESSAGE_1))
	elseif sv.onlyall == false then
		sv.onlyall = true
		d(GetString(AFAS_COMMAND_MESSAGE_2))
	end
end

local function registerSlashCommands()
	SLASH_COMMANDS["/af_arenaset"] = function () toggleonlyall() end
end
...
local function addOnPluginLoaded(event, name)
	...
	afAS.ASV = ZO_SavedVars:NewAccountWide(addonPluginName .. "_Data", 1, nil, addonDefaults)

	registerSlashCommands()

	--Check if library LibSets is loaded and load the sets data if missing yet
	if (not areSetsLoaded() or libSets.arenaSets == nil) then
		libSets.LoadSets(true, addonPluginName)
		return
	end
	if (not areSetsLoaded() or libSets.arenaSets == nil) then
		d("[AdvancedFilters Plugin]: \'ArenaSets\' is missing LibSets arena sets data!")
		return
	end
	...
	local arenaSetIds = {}
	stc(libSets.arenaSets, arenaSetIds)
	local setNamesWithId = {}
	local setNamesSorted = {}
	--Get the name of the sets
	for setId, _  in pairs(arenaSetIds) do
		local setName = ""
...
--afAS._setNamesWithId = setNamesWithId
--afAS._setNamesSorted = setNamesSorted
	...
	local function GetFilterCallbackForSets( setId )
		return function( slot , slotIndex)
			...
			if setId == allSetsConstantId then
				for setIdToCompareToLoop, _ in pairs(arenaSetIds) do
	...
	local allArenaSetsDropdownCallback = { { name = subMenuEntryAllSets, filterCallback = GetFilterCallbackForSets( allSetsConstantId ) } }
...
--afAS._entriesForDropdownFiveSetEachCallbacks = entriesForDropdownFiveSetEachCallbacks
--afAS._fullDropdownSetsCallbacks 		= fullDropdownSetsCallbacks
--afAS._dropdownFiveSetEachCallbacks 	= dropdownFiveSetEachCallbacks
--afAS._dropdownFiveSetEachCharactersKey = dropdownFiveSetEachCharactersKey
		end
	end
	...
	local function buildFilterInformation()
		...
		for _, fiveCharacters in ipairs(dropdownFiveSetEachKeys) do
			...
			local stringsEN = {
				[allSetsConstantName]      = "-All arena sets-",
			}
			local stringsDE = {
				[allSetsConstantName]      = "-Alle Arena Sets-",
			}
			local stringsFR = {
				[allSetsConstantName]      = "-Tous les set des arena-",
			}
			stringsEN[fiveCharactersSubMenuName] = transl.ARENA_SETS_MENU_ENTRY ..fiveCharacters
			stringsDE[fiveCharactersSubMenuName] = transl.ARENA_SETS_MENU_ENTRY ..fiveCharacters
			stringsFR[fiveCharactersSubMenuName] = transl.ARENA_SETS_MENU_ENTRY ..fiveCharacters
	...
	--First add 1 entry for "All arena sets"
	local stringsAllEN = { [subMenuEntryAllSets] = transl.ALL_ARENA_SETS_SUBMENU_ENTRY }
	local stringsAllDE = { [subMenuEntryAllSets] = transl.ALL_ARENA_SETS_SUBMENU_ENTRY }
	local stringsAllFR = { [subMenuEntryAllSets] = transl.ALL_ARENA_SETS_SUBMENU_ENTRY }

	local filterInformationAllArenaSets = {
		callbackTable = allArenaSetsDropdownCallback,
		filterType = {
			ITEMFILTERTYPE_ARMOR,
			ITEMFILTERTYPE_WEAPONS,
			ITEMFILTERTYPE_JEWELRY,
	...
	AdvancedFilters_RegisterFilter(filterInformationAllArenaSets)

	filterInformationAllArenaSets = {
		callbackTable = allArenaSetsDropdownCallback,
		filterType = ITEMFILTERTYPE_ALL,
		onlyGroups = {"Armor", "ArmorRetrait", "Weapons"},
		subfilters = {"All",},
	...
	if afAS.ASV.onlyall == false then
		buildFilterInformation()
	end
end
