...
local Templates = {
	[LHASFork.ST_CHECKBOX] = "ZO_GamepadOptionsCheckboxRow",
	[LHASFork.ST_SLIDER] = "LHASForkGamepadSlider",
	[LHASFork.ST_EDIT] = "LHASForkGamepadEdit",
	[LHASFork.ST_DROPDOWN] = "ZO_GamepadHorizontalListRow",
	[LHASFork.ST_COLOR] = "ZO_GamepadOptionsColorRow",
	[LHASFork.ST_BUTTON] = "ZO_GamepadOptionsLabelRow",
	[LHASFork.ST_LABEL] = "ZO_GamepadOptionsLabelRow",
	[LHASFork.ST_SECTION] = "ZO_Options_SectionTitle_WithDivider",
	[LHASFork.ST_ICONPICKER] = "LHASForkGamepadIconPicker"
}
...
local function SetNameControlState(control, enabled)
	...
	if nameControl then
		local selected = LHASFork.list:GetSelectedControl() == control
...
local updateControlFunctions = {
	[LHASFork.ST_CHECKBOX] = function(self, control)
	...
	[LHASFork.ST_SLIDER] = function(self, control)
		control:GetNamedChild("Name"):SetText(self:GetString(self:GetValueOrCallback(self.labelText)))
	...
	[LHASFork.ST_BUTTON] = function(self, control)
	...
	[LHASFork.ST_EDIT] = function(self, control)
		control:SetHidden(false)
		control:GetNamedChild("Name"):SetText(self:GetString(self:GetValueOrCallback(self.labelText)))
	...
	[LHASFork.ST_DROPDOWN] = function(self, control)
		control:GetNamedChild("Name"):SetText(self:GetString(self:GetValueOrCallback(self.labelText)))
	...
	[LHASFork.ST_LABEL] = function(self, control)
	...
	[LHASFork.ST_SECTION] = function(self, control)
		local label = control.label
		label:SetText(self:GetString(self:GetValueOrCallback(self.labelText)))
		control:SetHeight(label:GetTextHeight() + 4)
	end,
	[LHASFork.ST_COLOR] = function(self, control)
	...
	[LHASFork.ST_ICONPICKER] = function(self, control)
		control:GetNamedChild("Name"):SetText(self:GetString(self:GetValueOrCallback(self.labelText)))
		local combobox = control:GetDropDown()
		combobox:SetOnSelectedDataChangedCallback(nil)
		combobox:Clear()
...
local createControlFunctions = {
	[LHASFork.ST_CHECKBOX] = function(self, lastControl)
		LHASFork.list:AddEntry(Templates[self.type], self)
	end,
...
function LHASFork.AddonSettingsControl:UpdateControl()
...
function LHASFork.AddonSettings:UpdateControls()
	...
	LHASFork.list:RefreshVisible()
	needUpdate = false
end

function LHASFork.AddonSettings:RefreshSelection()
	local list = LHASFork.list
...
function Settings_ParametricList:Initialize(control)
	ZO_Gamepad_ParametricList_Screen.Initialize(self, control, false, true, LHASFork.scene)
end
...
function Settings_ParametricList:InitializeKeybindStripDescriptors()
	local CONTROL_TYPES_WITH_PRIMARY_ACTION = {
		[LHASFork.ST_CHECKBOX] = true,
		[LHASFork.ST_BUTTON] = true,
		[LHASFork.ST_COLOR] = true,
		[LHASFork.ST_EDIT] = true
	}
	local CONTROL_TYPES_WITH_INPUT = {
		[LHASFork.ST_SLIDER] = true,
		[LHASFork.ST_DROPDOWN] = true,
		[LHASFork.ST_ICONPICKER] = true
	}
	...
	self.keybindStripDescriptor = {
		{
			...
			name = function()
				local data = LHASFork.list:GetSelectedData()
				if data and data.type == LHASFork.ST_CHECKBOX then
					return GetString(SI_GAMEPAD_TOGGLE_OPTION)
				elseif data and data.type == LHASFork.ST_BUTTON then
			...
			order = -500,
			callback = function()
				local control = LHASFork.list:GetSelectedControl()
				local data = LHASFork.list:GetSelectedData()
				if not data then
					return
				end
				...
				if controlType == LHASFork.ST_CHECKBOX then
					ZO_CheckButton_OnClicked(control:GetNamedChild("Checkbox"))
				elseif controlType == LHASFork.ST_BUTTON then
					PlaySound(SOUNDS.DEFAULT_CLICK)
					LHASFork.list:GetSelectedData():ValueChanged(control)
				elseif controlType == LHASFork.ST_COLOR then
					control:ShowDialog()
				elseif controlType == LHASFork.ST_EDIT then
					control:Activate()
				end
			end,
			enabled = function()
		...
		},
		{
			...
			callback = function()
				ZO_Dialogs_ShowGamepadDialog("LHASFork_Defaults")
			end
		}
	}
	...
	LHASFork.scene:RegisterCallback(
		"StateChange",
		function(newState)
...
local function OptionsWindowFragmentStateChangeRefresh(oldState, newState)
	if newState == SCENE_FRAGMENT_HIDING then
		needUpdate = true
		if currentSettings then
			currentSettings.lastSelectedRow = LHASFork.list:GetSelectedData()
		end
	elseif newState == SCENE_FRAGMENT_SHOWING then
		if needUpdate and currentSettings ~= nil then
			LHASFork:RefreshAddonSettings()
		elseif currentSettings == nil and #LHASFork.addons == 1 then
...
function LHASFork:CreateAddonSettingsPanel()
	...
	for i = 1, #self.addons do
		...
		subItems[#subItems + 1] = {
			...
			activatedCallback = function()
				...
				SCENE_MANAGER:Push("LHASForkScene")
			end,
			enabled = function()
	...
	if #self.addons > 0 then
		local title = "SP " .. GetString(SI_GAME_MENU_ADDONS)
	...
	local control = WINDOW_MANAGER:CreateControlFromVirtual("LHASForkList", GuiRoot, "LHASForkGamepadTopLevel")

	local fragment = ZO_FadeSceneFragment:New(control)

	self.container = control:GetNamedChild("MaskContainer")

	local scene = ZO_Scene:New("LHASForkScene", SCENE_MANAGER)
