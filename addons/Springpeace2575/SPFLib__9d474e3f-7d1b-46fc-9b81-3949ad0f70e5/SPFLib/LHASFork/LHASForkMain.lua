if LHASFork then
	error("Library loaded already. Please remove all LHASFork in sub folders.")
end

LHASFork = {}
LHASFork.version = 20002
local LHASFork = LHASFork

-----
-- Control Types
-----
LHASFork.ST_CHECKBOX = 1
LHASFork.ST_SLIDER = 2
LHASFork.ST_EDIT = 3
LHASFork.ST_DROPDOWN = 4
LHASFork.ST_COLOR = 5
LHASFork.ST_BUTTON = 6
LHASFork.ST_LABEL = 7
LHASFork.ST_SECTION = 8
LHASFork.ST_ICONPICKER = 9
-----
...
LHASFork.AddonSettings = AddonSettings
LHASFork.AddonSettingsControl = AddonSettingsControl
...
function AddonSettingsControl:SettingValueChangedCallback(changedSetting)
	...
	if self.type == LHASFork.ST_LABEL or self.type == LHASFork.ST_SECTION then
		return
	end
...
function AddonSettingsControl:SetAnchor(lastControl)
	...
	if lastControl == LHASFork.container then
...
function AddonSettingsControl:ResetToDefaults()
	...
	if self.type == LHASFork.ST_DROPDOWN then
	...
	elseif self.type == LHASFork.ST_COLOR then
		self:SetValue(unpack(self.default))
		self.setFunction(unpack(self.default))
	elseif self.type == LHASFork.ST_ICONPICKER then
...
function AddonSettings:AddSetting(params, index, playAnimation)
	...
	if not IsConsoleUI() and self.selected then
		LHASFork.container.endHeight = self:GetOverallHeight() + 8
		--Conditionally show the animation. Useful for simulating submenus
		if not playAnimation then
			LHASFork.openTimeline:PlayInstantlyToEnd()
		else
			LHASFork.openTimeline:PlayFromStart()
		end
	end
...
function AddonSettings:Select()
	...
	if not isConsoleUI then
		LHASFork:DetachContainer()
	end
	CALLBACK_MANAGER:FireCallbacks("LHASFork_AddonSelected", self.name, self)

	if not isConsoleUI then
		LHASFork:AttachContainerToControl(self.control)
		if self.prev then
			self.control:ClearAnchors()
			self.control:SetAnchor(TOPLEFT, self.prev.control, BOTTOMLEFT, 0, 8)
		end
		if self.next then
			LHASFork:AttachControlToContainer(self.next.control)
		end
	end
...
function LHASFork:AddAddon(name, options)
...
function LHASFork:DetachContainer()
	if IsConsoleUI() then
		return
	end
...
function LHASFork:AttachControlToContainer(control)
	if IsConsoleUI() then
		return
	end
...
function LHASFork:AttachContainerToControl(control)
	if IsConsoleUI() then
		return
	end
	self.container:ClearAnchors()
