...
local changeControlStateFunctions = {
	[LHASFork.ST_CHECKBOX] = function(control, state)
	...
	[LHASFork.ST_DROPDOWN] = function(control, state)
	...
	[LHASFork.ST_SLIDER] = function(control, state)
		SetNameControlState(control, state)
	...
	[LHASFork.ST_BUTTON] = function(control, state)
		SetNameControlState(control, state)
		control:GetNamedChild("Button"):SetEnabled(state)
	end,
	[LHASFork.ST_EDIT] = function(control, state)
		SetNameControlState(control, state)
...
local updateControlFunctions = {
	[LHASFork.ST_CHECKBOX] = function(self, lastControl)
	...
	[LHASFork.ST_SLIDER] = function(self, lastControl)
		self:SetAnchor(lastControl)
		self.control:GetNamedChild("Name"):SetText(self:GetString(self:GetValueOrCallback(self.labelText)))
	...
	[LHASFork.ST_BUTTON] = function(self, lastControl)
	...
	[LHASFork.ST_EDIT] = function(self, lastControl)
		self.control:SetHidden(false)
		self:SetAnchor(lastControl)
		self.control:GetNamedChild("Name"):SetText(self:GetString(self:GetValueOrCallback(self.labelText)))
	...
	[LHASFork.ST_DROPDOWN] = function(self, lastControl)
		self:SetAnchor(lastControl)
		self.control:GetNamedChild("Name"):SetText(self:GetString(self:GetValueOrCallback(self.labelText)))
	...
	[LHASFork.ST_LABEL] = function(self, lastControl)
		self.control:SetHidden(false)
		self:SetAnchor(lastControl)
	...
	[LHASFork.ST_SECTION] = function(self, lastControl)
		self:SetAnchor(lastControl)
		local label = self.control:GetNamedChild("Label")
		label:SetText(self:GetString(self:GetValueOrCallback(self.labelText)))
		self.control:SetHeight(label:GetTextHeight() + 4)
	end,
	[LHASFork.ST_COLOR] = function(self, lastControl)
		self:SetAnchor(lastControl)
		local label = self.control:GetNamedChild("Name")
		label:SetText(self:GetString(self:GetValueOrCallback(self.labelText)))
	...
	[LHASFork.ST_ICONPICKER] = function(self, lastControl)
		self:SetAnchor(lastControl)
		self.control:GetNamedChild("Name"):SetText(self:GetString(self:GetValueOrCallback(self.labelText)))
		...
		local function OnIconPickerClicked()
			...
			ZO_Dialogs_ShowDialog("LHASForkIconPicker")
		end
...
local createControlFunctions = {
	[LHASFork.ST_CHECKBOX] = function(self, lastControl)
		self.control, self.controlKey = LHASFork.checkboxPool:AcquireObject()
		self.control.data = self.control.data or ZO_Object:New(self)
		updateControlFunctions[LHASFork.ST_CHECKBOX](self, lastControl)
	...
	[LHASFork.ST_SLIDER] = function(self, lastControl)
		self.control, self.controlKey = LHASFork.sliderPool:AcquireObject()
		self.control.data = self.control.data or ZO_Object:New(self)
		...
		updateControlFunctions[LHASFork.ST_SLIDER](self, lastControl)
	...
	[LHASFork.ST_BUTTON] = function(self, lastControl)
		self.control, self.controlKey = LHASFork.buttonPool:AcquireObject()
		self.control.data = self.control.data or ZO_Object:New(self)
		updateControlFunctions[LHASFork.ST_BUTTON](self, lastControl)
		local button = self.control:GetNamedChild("Button")
		button:SetHandler("OnClicked", function(...) self:ValueChanged(...) end)
		self.button = button
	end,
	[LHASFork.ST_EDIT] = function(self, lastControl)
		self.control, self.controlKey = LHASFork.editPool:AcquireObject()
		self.control.data = self.control.data or ZO_Object:New(self)
		updateControlFunctions[LHASFork.ST_EDIT](self, lastControl)
		local editControl = self.control:GetNamedChild("EditBackdrop"):GetNamedChild("Edit")
	...
	[LHASFork.ST_DROPDOWN] = function(self, lastControl)
		self.control, self.controlKey = LHASFork.dropdownPool:AcquireObject()
		self.control.data = self.control.data or ZO_Object:New(self)
		updateControlFunctions[LHASFork.ST_DROPDOWN](self, lastControl)
	end,
	[LHASFork.ST_LABEL] = function(self, lastControl)
		self.control, self.controlKey = LHASFork.labelPool:AcquireObject()
		self.control.data = self.control.data or ZO_Object:New(self)
		updateControlFunctions[LHASFork.ST_LABEL](self, lastControl)
	end,
	[LHASFork.ST_SECTION] = function(self, lastControl)
		self.control, self.controlKey = LHASFork.sectionPool:AcquireObject()
		self.control.data = self.control.data or ZO_Object:New(self)
		updateControlFunctions[LHASFork.ST_SECTION](self, lastControl)
	end,
	[LHASFork.ST_COLOR] = function(self, lastControl)
		self.control, self.controlKey = LHASFork.colorPool:AcquireObject()
		self.control.data = self.control.data or ZO_Object:New(self)
		self.control.texture = self.control:GetNamedChild("Color")
		updateControlFunctions[LHASFork.ST_COLOR](self, lastControl)
	...
	[LHASFork.ST_ICONPICKER] = function(self, lastControl)
		self.control, self.controlKey = LHASFork.iconpickerPool:AcquireObject()
		self.control.data = self.control.data or ZO_Object:New(self)
		updateControlFunctions[LHASFork.ST_ICONPICKER](self, lastControl)
	end
}

local cleanControlFunctions = {
	[LHASFork.ST_CHECKBOX] = function(self)
		ZO_CheckButton_SetToggleFunction(self.control:GetNamedChild("Checkbox"), nil)
		LHASFork.checkboxPool:ReleaseObject(self.controlKey)
	end,
	[LHASFork.ST_SLIDER] = function(self)
		self.control.slider:SetHandler("OnValueChanged", nil)
		LHASFork.sliderPool:ReleaseObject(self.controlKey)
	end,
	[LHASFork.ST_BUTTON] = function(self)
		self.control:GetNamedChild("Button"):SetHandler("OnClicked", nil)
		LHASFork.buttonPool:ReleaseObject(self.controlKey)
	end,
	[LHASFork.ST_EDIT] = function(self)
		local editControl = self.control:GetNamedChild("EditBackdrop"):GetNamedChild("Edit")
		...
		LHASFork.editPool:ReleaseObject(self.controlKey)
	end,
	[LHASFork.ST_DROPDOWN] = function(self)
		local combobox = self.control:GetDropDown()
		combobox:ClearItems()
		LHASFork.dropdownPool:ReleaseObject(self.controlKey)
	end,
	[LHASFork.ST_LABEL] = function(self)
		self.control:GetNamedChild("Name"):SetText(nil)
		LHASFork.labelPool:ReleaseObject(self.controlKey)
	end,
	[LHASFork.ST_SECTION] = function(self)
		self.control:GetNamedChild("Label"):SetText(nil)
		LHASFork.sectionPool:ReleaseObject(self.controlKey)
	end,
	[LHASFork.ST_COLOR] = function(self)
		self.control:GetNamedChild("Name"):SetText(nil)
		self.control:GetNamedChild("ColorSection"):SetHandler("OnMouseUp", nil)
		LHASFork.colorPool:ReleaseObject(self.controlKey)
	end,
	[LHASFork.ST_ICONPICKER] = function(self)
		self.control.iconControl:SetTexture(nil)
		self.control.checkBox:SetHandler("OnClicked", nil)
		self.control.items = nil
		LHASFork.iconpickerPool:ReleaseObject(self.controlKey)
	end
}

local setupControlFunctions = {
	[LHASFork.ST_CHECKBOX] = function(self, params)
	...
	[LHASFork.ST_SLIDER] = function(self, params)
		self.min = params.min
		self.max = params.max
	...
	[LHASFork.ST_BUTTON] = function(self, params)
	...
	[LHASFork.ST_EDIT] = function(self, params)
	...
	[LHASFork.ST_DROPDOWN] = function(self, params)
	...
	[LHASFork.ST_LABEL] = function(self, params)
		self.labelText = params.label
		self.tooltipText = params.tooltip
	end,
	[LHASFork.ST_SECTION] = function(self, params)
		self.labelText = params.label
		self.tooltipText = params.tooltip
	end,
	[LHASFork.ST_COLOR] = function(self, params)
		self.labelText = params.label
		self.tooltipText = params.tooltip
		self.setFunction = params.setFunction
		self.getFunction = params.getFunction
	...
	[LHASFork.ST_ICONPICKER] = function(self, params)
		self.items = params.items
		self.labelText = params.label
		self.tooltipText = params.tooltip
		self.setFunction = params.setFunction
...
-----
function LHASFork.AddonSettingsControl:SetupControl(params)
	if setupControlFunctions[self.type] then
		setupControlFunctions[self.type](self, params)
	end
end

function LHASFork.AddonSettingsControl:SetupTooltip(control)
	control:SetHandler(
		"OnMouseEnter",
		function(sender, ...)
...
function LHASFork.AddonSettingsControl:SetEnabled(state)
	if self.control and changeControlStateFunctions[self.type] then
		changeControlStateFunctions[self.type](self.control, state)
	end
end

function LHASFork.AddonSettingsControl:UpdateControl(lastControl)
...
function LHASFork.AddonSettingsControl:CleanUp()
...
function LHASFork.AddonSettingsControl:CreateControl(lastControl)
...
-----
function LHASFork.AddonSettings:AddToOptionsPanel(panelID)
	self.control.data = {
		panel = panelID,
...
function LHASFork.AddonSettings:InitHandlers()
	local label = self.control:GetNamedChild("Label")
...
function LHASFork.AddonSettings:CreateControls()
	local last = LHASFork.container
...
function LHASFork.AddonSettings:UpdateControls()
	local last = LHASFork.container
	for i = 1, #self.settings do
...
function LHASFork:SelectFirstAddon()
	currentSettings = LHASFork.addons[1]
...
local function PoolCreateControlBase(name, pool)
	...
	local control = WINDOW_MANAGER:CreateControl(controlNamespace, LHASFork.container, CT_CONTROL)
...
local function OptionsWindowFragmentStateChangeRefresh(oldState, newState)
	...
	elseif newState == SCENE_FRAGMENT_SHOWING then
		if needUpdate and currentSettings ~= nil then
			LHASFork:RefreshAddonSettings()
		elseif #LHASFork.addons == 1 then
			LHASFork:SelectFirstAddon()
		end
	end
end

function LHASFork:CreateAddonSettingsPanel()
	...
	self.container = WINDOW_MANAGER:CreateControl("LHASForkContainer", GuiRoot, CT_SCROLL)
	...
	CALLBACK_MANAGER:RegisterCallback("LHASFork_AddonSelected", function(_, addonSettings)
...
function LHASFork:CreateControlPools()
...
function LHASFork:CreateAddonList()
	table.sort(LHASFork.addons, function(el1, el2)
			return el1.name < el2.name
		end)

	local control, template
	local prev = nil
	for i = 1, #LHASFork.addons do
		template = i > 1 and "ZO_Options_SectionTitle_WithDivider" or "ZO_Options_SectionTitle"
		control = WINDOW_MANAGER:CreateControlFromVirtual("LHASForkAddon" .. LHASFork.addons[i].name .. "Name", ZO_OptionsWindowSettingsScrollChild, template)

		LHASFork.addons[i].control = control
		control.addonSettings = LHASFork.addons[i]
		LHASFork.addons[i]:InitHandlers()
		LHASFork.addons[i]:SetAnchor(prev)

		prev = LHASFork.addons[i]
		LHASFork.addons[i]:AddToOptionsPanel(LHASFork.panelID)
	end
	if prev ~= nil then
