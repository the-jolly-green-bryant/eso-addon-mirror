...
local type, version = LGS.MESSAGE_TYPE_RESOURCES, 4
...
local function NumCallbacks()
    ...
    callbacks = registry and #registry or 0
end
...
function handler:Send()
    if(not saveData.enabled or not IsUnitGrouped("player") or callbacks == 0) then return end
...
local function Load()
	...
	EVENT_MANAGER:AddFilterForEvent("LibGroupSocketResourceHandler",EVENT_UNIT_CREATED, REGISTER_FILTER_UNIT_TAG_PREFIX, "group")
    EVENT_MANAGER:RegisterForEvent("LibGroupSocketResourceHandler", EVENT_UNIT_DESTROYED, OnUnitDestroyed)
	EVENT_MANAGER:AddFilterForEvent("LibGroupSocketResourceHandler",EVENT_UNIT_DESTROYED, REGISTER_FILTER_UNIT_TAG_PREFIX, "group")
