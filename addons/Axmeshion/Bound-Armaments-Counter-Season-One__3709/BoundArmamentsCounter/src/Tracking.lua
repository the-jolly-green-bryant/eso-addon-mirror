...
function BAC:RegisterStacksForId(abilityId)
    ...
    if not stackId then
        BAC:Trace(1, "Ignoring non-Bound Armaments abilityId: <<1>>", abilityId)
        return
    end
...
function BAC:CheckSkillSlotted()
    ...
    -- Only treat Bound Armaments Morph as active.
    -- Hide UI if another Morph is slotted.
    if abilityId ~= nil and self.skills[abilityId] ~= nil then
    ...
    if abilityId ~= nil then
        BAC:Trace(1, "Skill <<1>> (<<2>>) is not Bound Armaments, hiding UI", GetAbilityName(abilityId), abilityId)
    else
