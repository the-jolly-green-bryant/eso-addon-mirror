...
if not LAM:RegisterWidget("fontable_dropdown", widgetVersion) then return end
...
local LUIE_COMBO_ENTRY_ITEMFONT_KEY = "luiEItemFont"
...
local function PostHook_ComboBoxDropdown_SetupEntryLabel_ItemFont(self, labelControl, data)
    ...
    local itemFontFn = item and item[LUIE_COMBO_ENTRY_ITEMFONT_KEY]
...
local function PostHook_ComboBox_Base_ItemSelectedClickHelper_ItemFont(self, item, ignoreCallback)
    ...
    local itemFontFn = item[LUIE_COMBO_ENTRY_ITEMFONT_KEY]
    if type(itemFontFn) ~= "function" then
        return
    end
    local font = itemFontFn(GetComboItemCallbackValue(item), item.name)
...
local function SetupFontableDropdownHooks()
...
local LUIE_FONTABLE_DROPDOWN_MIN_CONTAINER_WIDTH = 175

local function ApplyFontableDropdownContainerWidth(control, combobox, dropdown, minWidth)
    minWidth = minWidth or LUIE_FONTABLE_DROPDOWN_MIN_CONTAINER_WIDTH
...
function LAMCreateControl.fontable_dropdown(parent, fontableDropdownData, controlName)
    assert(type(fontableDropdownData.itemFont) == "function", "[LUIE] fontable_dropdown requires itemFont")
    local control = LAM.util.CreateLabelAndContainerControl(parent, fontableDropdownData, controlName)
    control.choices = {}
    ...
    ApplyFontableDropdownContainerWidth(control, combobox, dropdown, fontableDropdownData.dropdownMinWidth)

    local isMultiSelectionEnabled = GetDefaultValue(fontableDropdownData.multiSelect)
    ...
    if isMultiSelectionEnabled == true then
        ...
        combobox:SetHandler("OnMouseUp", function (...) OnMultiSelectComboBoxMouseUp(control, ...) end, "LUIEFontableDropdownWidgetOnMouseUp")

        local multiSelectionTextFormatter = GetDefaultValue(fontableDropdownData.multiSelectTextFormatter) or GetString(SI_COMBO_BOX_DEFAULT_MULTISELECTION_TEXT_FORMATTER)
        local multiSelectionNoSelectionText = GetDefaultValue(fontableDropdownData.multiSelectNoSelectionText) or GetString(SI_COMBO_BOX_DEFAULT_NO_SELECTION_TEXT)
        dropdown:EnableMultiSelect(multiSelectionTextFormatter, multiSelectionNoSelectionText)

        local maxSelections = GetDefaultValue(fontableDropdownData.multiSelectMaxSelections)
    ...
    if fontableDropdownData.sort then
        local sortInfo = GrabSortingInfo(fontableDropdownData.sort)
        control.m_sortType, control.m_sortOrder = SORT_TYPES[sortInfo[1]], SORT_ORDERS[sortInfo[2]]
    elseif fontableDropdownData.choicesValues then
        control.m_sortType, control.m_sortOrder = ZO_SORT_ORDER_UP, SORT_BY_VALUE
    end

    if fontableDropdownData.warning ~= nil or fontableDropdownData.requiresReload then
    ...
    control:UpdateChoices(fontableDropdownData.choices, fontableDropdownData.choicesValues)
    control.UpdateValue = UpdateValue
    control:UpdateValue()
    if fontableDropdownData.disabled ~= nil then
