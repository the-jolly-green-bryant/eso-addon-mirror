local Localization={
	en={
		...
		PANEL_INFO_FONT="$(CHAT_FONT)|18|soft-shadow-thin",
	},
	it={
	...
	ru={
		...
		PANEL_INFO_FONT="$(BUI_MEDIUM_FONT)|$(KB_18)|soft-shadow-thin"
	},
	es={
		VERSION="Versión: <<X:1>>",
	...
	zh={
		VERSION="版本: <<X:1>>",
		WEBSITE="访问网站",
		PANEL_INFO_FONT="EsoZh/fonts/univers57.otf|18|soft-shadow-thin",
	},
	pl={
		VERSION="Wersja: <<X:1>>",
...
local icon_m_size=30
local font_bold="$(BOLD_FONT)|$(KB_22)|soft-shadow-thick"
--Menu
...
local function CreateMenuList(name, parent)
	...
	ZO_ScrollList_AddDataType(list, 1, "ZO_SelectableLabel", 36, listRow_Setup)
...
local function SettingsWindow_Init()
	...
	ui.logo	=BUI.UI.Texture("$(parent)_logo", ui, {100,100}, {TOPRIGHT,TOPRIGHT,-20,60}, "/SatuveXboxUI/textures/Satuve_logo.dds", false, DL_BACKGROUND)
...
--			ShowReloadDialogIfNeeded()
	...
	--Settings menu entry
	-- Xbox-PC compatibility: KEYBOARD_OPTIONS may not exist during addon startup.
	local keyboardOptions = rawget(_G, "KEYBOARD_OPTIONS")
	ui.id = (keyboardOptions and tonumber(keyboardOptions.currentPanelId)) or 9000
	local data={
		id=ui.id,
		name=BUI.ShortName,
...
--			KEYBOARD_OPTIONS:ChangePanels(ui.id)
	...
	if keyboardOptions then
		keyboardOptions.currentPanelId = ui.id + 1
...
--	SCENE_MANAGER:SetInUIMode(false)
	local function SettingsMenu()
		...
		local gameMenu = inGameMenu and inGameMenu.gameMenu
		local headers = gameMenu and gameMenu.headerControls
		local settingsName = type(GetString) == "function" and SI_GAME_MENU_SETTINGS and GetString(SI_GAME_MENU_SETTINGS) or nil
		local settingsMenu = headers and settingsName and headers[settingsName]
		if settingsMenu then
			local gameMenu = settingsMenu
	...
	local gameMenuScene = sceneManager and sceneManager.GetScene and sceneManager:GetScene("gameMenuInGame")
	if not gameMenuScene then
		if BUI.SettingsPanelData and BUI.SettingsPanelData.callback then
			BUI.SettingsPanelData.callback()
		end
		return
	end
	if gameMenuScene:GetState()==SCENE_SHOWN then
		SettingsMenu()
	else
		if sceneManager.CallWhen then sceneManager:CallWhen("gameMenuInGame", SCENE_SHOWN, SettingsMenu) end
		if sceneManager.Show then sceneManager:Show("gameMenuInGame") end
	end
end
...
local function CreateOptions(parent,options,panel,submenu)
	...
	for i,data in pairs(options) do
		...
		elseif data.type=="dropdown" then
			...
			frame.label=BUI.UI.Label("$(parent)_Label", frame, {w/3*2,h}, {TOPLEFT,TOPLEFT,0,0}, font_bold, {.8,.8,.6,1}, {0,1}, name)
			frame.control=BUI.UI.ComboBox(data.reference or "$(parent)_DropBox", frame.label, {w/3,34}, {TOPLEFT,TOPRIGHT,0,0}, data.choices, data.getFunc(),func,false,data.scrollable)
			anchor={TOPLEFT,BOTTOMLEFT,0,space,frame}
		...
		if frame then
			if frame.label and data.type~="header" and data.type~="submenu" then
				...
				frame:SetHandler("OnMouseEnter", function(self)
					...
					if tooltip then
						...
						InformationTooltip:AddLine(tooltip, "$(BOLD_FONT)|20|soft-shadow-thick", 1, 1, 1, LEFT, MODIFY_TEXT_TYPE_NONE, TEXT_ALIGN_LEFT, true)
					end
				end)
				frame:SetHandler("OnMouseExit", function() highlight:SetHidden(true) ClearTooltip(InformationTooltip) end)
			end
			if data.warning then
				...
				warn:SetHandler("OnMouseEnter", function(self)
					local warningText=type(data.warning)=="string" and (BUI.Localization[BUI.language] and BUI.Localization[BUI.language][data.warning] or BUI.Localization.en[data.warning]) or BUI.Loc("ReloadUiWarn1")
					InitializeTooltip(InformationTooltip, self, TOP, 0, -8, BOTTOM)
					InformationTooltip:ClearLines()
					InformationTooltip:AddLine(warningText, "$(BOLD_FONT)|20|soft-shadow-thick", 1, 0.85, 0.25, LEFT, MODIFY_TEXT_TYPE_NONE, TEXT_ALIGN_LEFT, true)
				end)
				warn:SetHandler("OnMouseExit", function() ClearTooltip(InformationTooltip) end)
			end
