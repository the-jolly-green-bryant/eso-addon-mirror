...
local number=1
local base=BUI.name=="SatuveXboxUI"


-- Satuve Xbox UI: fixed font rules for every addon-created text control.
-- No control should depend on ESO's variable ZoFont aliases anymore.
BUI.UI.FontSizes = {
    body = 20,
    small = 18,
    button = 20,
...
local function ExplicitFont(font, fallbackSize)
    local size = fallbackSize or BUI.UI.FontSizes.body
    if font == nil or font == "" then
        return "$(MEDIUM_FONT)|"..size.."|soft-shadow-thick"
    end
    if type(font) ~= "string" then return font end
    if font == "ZoFontGame" or font == "ZoFontGameSmall" or font == "ZoFontWinH4" or font == "ZoFontHeader3" then
        local mapped = (font == "ZoFontWinH4" or font == "ZoFontHeader3") and BUI.UI.FontSizes.heading or size
        return "$(MEDIUM_FONT)|"..mapped.."|soft-shadow-thick"
    end
    return font
end

local function ShowFixedTooltip(control, anchor, text)
    if not text or text == "" then return end
    InitializeTooltip(InformationTooltip, control, anchor or BOTTOM, 0, 8)
    InformationTooltip:ClearLines()
    InformationTooltip:AddLine(text, "$(BOLD_FONT)|"..BUI.UI.FontSizes.tooltip.."|soft-shadow-thick", 1, 1, 1, LEFT, MODIFY_TEXT_TYPE_NONE, TEXT_ALIGN_LEFT, true)
end
...
--	if (name==nil or name=="") then return end
	parent=(parent==nil) and GuiRoot or parent
	if (#anchor~=4 and #anchor~=5) then return end
	font	=ExplicitFont(font, BUI.UI.FontSizes.body)
...
function BUI.UI.Button(name, parent, dims, anchor, state, font, align, normal, pressed, mouseover, hidden)
	...
	font=ExplicitFont(font, BUI.UI.FontSizes.button)
...
--	if name==nil or name=="" then return end
	...
	elseif type(tex)=="number" then
		...
		tex="/SatuveXboxUI/textures/buttons.dds"
	end
	...
	button:SetHandler("OnMouseEnter", function(self)
		...
		if tooltip then ShowFixedTooltip(self, BOTTOM, (type(tooltip)=="string" and tooltip or tooltip())) end
	end)
	button:SetHandler("OnMouseExit", function(self)
		self.over=nil
		local color=self.disabled and {.1,.1,0,1} or {.6,.57,.46,1}
		self:SetColor(unpack(color))
		if tooltip then ClearTooltip(InformationTooltip) end
	end)
...
--	if name==nil or name=="" then return end
	...
	control:SetTexture(tex or "/SatuveXboxUI/textures/buttons.dds")
	control:SetHidden(hidden)
...
--	slider:SetHandler("OnMouseExit", function() ZO_Options_OnMouseExit(control) end)
	...
	control.minText:SetFont(BUI.UI.Font("standard",18,true))
	...
	control.maxText:SetFont(BUI.UI.Font("standard",18,true))
...
function BUI.UI.ComboBox(name, parent, dims, anchor, array, val, fun, hidden, scroll)
	...
	local fs=math.max(BUI.UI.FontSizes.combo,math.min(22,dims[2]-8))
...
--	if (name==nil or name=="") then return end
	...
	control.eb:SetFont(BUI.UI.Font("standard",BUI.UI.FontSizes.edit,true))
...
function BUI.UI.Font(font, size, shadow, outline)
	local size=size or BUI.UI.FontSizes.body
	local font=fonts[font] or ExplicitFont(font, size)
