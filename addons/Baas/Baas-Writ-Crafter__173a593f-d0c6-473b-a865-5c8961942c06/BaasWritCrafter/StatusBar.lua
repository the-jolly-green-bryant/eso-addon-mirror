...
local StatusBar = BaasLazyWritStatus
local containerMaximum = 0
local statusBackdrop = BaasLazyWritStatusBackdrop
...
local function toggleStatusWindow(override)
	if containerMaximum > 0 and not WritCreater:GetSettings().showStatusBar then
		...
		BaasLazyWritStatusContainer:SetHidden(false)
	else
...
local function updateQuestStatus(event)
	...
	BaasLazyWritStatusBackdrop:SetCenterColor(255, 255, 255, WritCreater:GetSettings().transparentStatusBar and 0 or 255)
	...
	BaasLazyWritStatusBackdropOutput:SetText(workingString)
	local width = BaasLazyWritStatusBackdropOutput:GetTextWidth()+30
...
local function wipeContainer()
	containerMaximum = 0
	opened = 0
	-- BaasLazyWritStatusContainer:SetHidden(true)
...
function WritCreater.updateContainerCooldown()
	...
	animation = animation or ZO_AlphaAnimation:New(BaasLazyWritStatusContainer)
	if not WritCreater:GetSettings().lootContainerOnReceipt then
		return
	end
	BaasLazyWritStatusContainer:SetHidden(false)
	local container = BaasLazyWritStatusContainer
	local cooldown = BaasLazyWritStatusContainerCooldown
	local label = BaasLazyWritStatusContainerRemaining
	...
	if numContainers == 0 then
		...
		opened = 0
		-- BaasLazyWritStatusContainer:SetHidden(true)
		BaasLazyWritStatusContainer:SetAlpha(0.7)
	...
	BaasLazyWritStatusContainer:SetWidth(width)
...
-- BaasLazyWritStatus:SetHidden(false)
-- BaasLazyWritStatusContainer:SetHidden(false)
