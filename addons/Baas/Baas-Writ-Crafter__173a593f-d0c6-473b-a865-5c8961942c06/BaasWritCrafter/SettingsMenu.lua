	...
	EVENT_MANAGER:RegisterForEvent("WritCrafterLocalizationError", EVENT_PLAYER_ACTIVATED, function()
		if not WritCreater.writCompleteStrings then 
			...
			if validLanguages[language] == nil then
				d("Baas Writ Crafter: Your language is not fully supported for this addon. Some features may not work as expected. If you are looking to translate the addon, check the lang/en.lua file for more instructions.")
			elseif validLanguages[language] == false then
				d("Baas Writ Crafter: The Localization file could not be loaded.")
				d("Troubleshooting:")
				...
				d("2. If the error persists, contact @Baas in-game")
			end
		end
...
if isCheeseOn() and WritCreater.cheeseyLocalizations and WritCreater.cheeseyLocalizations.superAmazingCraftSounds then
    ...
    local activityCache
    if false then -- dev-only block removed
...
function WritCreater.initializeSettingsMenu()
	...
	if LAM and not ZO_IsConsoleOrGameCoreUI() then
		LAM:RegisterAddonPanel("BaasWritCrafter", WritCreater.settings["panel"])
		WritCreater.settings["options"] = WritCreater.Options()
		LAM:RegisterOptionControls("BaasWritCrafter", WritCreater.settings["options"])
	end
end