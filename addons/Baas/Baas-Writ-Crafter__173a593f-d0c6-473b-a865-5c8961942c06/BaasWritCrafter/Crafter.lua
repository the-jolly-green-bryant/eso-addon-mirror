...
local function getOut()
	return BaasWritsBackdropOutput:GetText()
end

local function appendOut(str)
	local currentText = getOut()
	BaasWritsBackdropOutput:SetText(currentText..str)
end
...
local function showCraftButton(craftingWrits)
	if not IsInGamepadPreferredMode() then
		BaasWritsBackdropCraft:SetHidden(craftingWrits) 
	else
...
SCENE_MANAGER:RegisterCallback("SceneStateChanged", function(scene, newState)
	...
	if craftingRootScenes[sceneName] and newState == SCENE_SHOWING and not BaasWrits:IsHidden() then
...
local function setupConditionsTable(quest, indexTableToUse)
	...
	for condition = 1, GetJournalQuestNumConditions(quest,1) do
		...
		BaasWritsBackdropQuestOutput:AddText("\n"..conditionsTable["text"][condition])
...
local function writCompleteUIHandle()
	...
	BaasWritsBackdropQuestOutput:SetText("")
	...
	if WritCreater:GetSettings().hideWhenDone then BaasWrits:SetHidden(true) end
	closeOnce = false
	BaasWritsBackdropCraft:SetHidden(true)
	...
	elseif ZO_CraftingUtils_IsPerformingCraftProcess() then
		...
		if WritCreater:GetSettings()[station] and writs[station] then
			if station ~= CRAFTING_TYPE_PROVISIONING and station ~= CRAFTING_TYPE_ENCHANTING and station ~= CRAFTING_TYPE_ALCHEMY and station ~=0 then
				BaasWrits:SetHidden(not WritCreater:GetSettings().showWindow)
				smithingCrafting(writs[station],craftingWrits)
			end
		end
...
local function createMatRequirementText(matsRequired)
	...
	if  haveMats then
		...
		BaasWritsBackdropCraft:SetText(WritCreater.strings.craft)
	else
		text=text..WritCreater.strings.smithingMissing
		BaasWritsBackdropCraft:SetText(WritCreater.strings.craftAnyway)
	end
	out(text)
...
function smithingCrafting(quest, craftItems)
	...
	out("If you see this, something is wrong.\nPlease update the addon\n If the issue persists, copy the quest conditions, and send to @Baas in-game")
...
function BaasWritsBackdropQuestOutput.AddText(self,text)
...
local function enchantCrafting(quest,add)
	...
	for i = 1, numConditions do
		...
		elseif conditions["cur"][i] == conditions["max"][i]  then
			...
			local needed = conditions["max"][i] - conditions["cur"][i]
			incomplete = true
			-- BaasWritsBackdropCraft:SetHidden(false)
			BaasWritsBackdropCraft:SetText(WritCreater.strings.craft)
			local ta={}
			local essence={}
			local potency={}
...
local function closeWindow(event, station)
	matSaver = 0
	BaasWritsFeedback:SetHidden(true)
	BaasWrits:SetHidden(true)
	craftingWrits = false
	WritCreater:GetSettings().OffsetX = BaasWrits:GetRight()
	WritCreater:GetSettings().OffsetY = BaasWrits:GetTop()
	queue = {}
	BaasWritsBackdropCraft:SetHidden(true)
	closeOnce = false
