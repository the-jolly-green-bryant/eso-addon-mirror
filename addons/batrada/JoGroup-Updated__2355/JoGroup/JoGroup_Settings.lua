...
function Jo.CreateSettingsWindow()
	local panelData = {
		...
		name = Jo.name,
		displayName = Jo.name,
		author = "Immortalized007 & JoyceKimberly",
	...
	local optionsData = {
		...
		{
			type = "checkbox",
			name = GetString(SI_JOGROUP_SHOWBOTH),
			default = Jo.defaults.lgsShowBoth,
			disabled = function() return not Jo.lgsIsEnabled end,
			getFunc = function() return Jo.savedVars.lgsShowBoth end,
			setFunc = function(newValue)
				Jo.savedVars.lgsShowBoth = newValue
				Jo.UpdateUnits()
			end,
		},
	}
