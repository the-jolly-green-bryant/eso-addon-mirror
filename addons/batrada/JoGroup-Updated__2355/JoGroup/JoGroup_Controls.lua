...
--]]
	...
	for i = 1, GROUP_SIZE_MAX do
		...
		-- LGS
		frame.lgs = wim:CreateControl(nil, frame.statusBar, CT_CONTROL)
		frame.lgs:SetDimensions(frame.statusBar:GetWidth()-(4*sf), 0)
		frame.lgs:SetAnchor(TOP, frame.bar, BOTTOM, 0, sf)

			frame.lgsMagBar = wim:CreateControl(nil, frame.lgs, CT_STATUSBAR)
			frame.lgsMagBar:SetDimensions(70, 0)
			frame.lgsMagBar:SetAnchor(BOTTOMLEFT, frame.lgs, BOTTOMLEFT)
			frame.lgsMagBar:SetDrawLayer(1)
			frame.lgsMagBar:SetDrawLevel(3)
			ZO_StatusBar_SetGradientColor(frame.lgsMagBar, ZO_POWER_BAR_GRADIENT_COLORS[POWERTYPE_MAGICKA])
			frame.lgsMagBar:SetBarAlignment(1)
			frame.lgsMagBar:SetMinMax(0, 1000)
			frame.lgsMagBar:SetValue(1000)

				frame.lgsMagBarGloss = wim:CreateControl(nil, frame.lgsMagBar, CT_STATUSBAR)
				frame.lgsMagBarGloss:SetAnchorFill()
				frame.lgsMagBarGloss:SetDrawLayer(1)
				frame.lgsMagBarGloss:SetDrawLevel(4)
				frame.lgsMagBarGloss:SetTexture("EsoUI/Art/Miscellaneous/timerBar_genericFill_gloss.dds")
				frame.lgsMagBarGloss:SetTextureCoords(0, 1, 0, 0.8125)
				frame.lgsMagBarGloss:SetBarAlignment(1)
				frame.lgsMagBarGloss:SetMinMax(0, 1000)
				frame.lgsMagBarGloss:SetValue(1000)

			frame.lgsStaBar = wim:CreateControl(nil, frame.lgs, CT_STATUSBAR)
			frame.lgsStaBar:SetDimensions(70, 0)
			frame.lgsStaBar:SetAnchor(BOTTOMLEFT, frame.lgsMagBar, BOTTOMRIGHT, sf, 0)
			frame.lgsStaBar:SetDrawLayer(1)
			frame.lgsStaBar:SetDrawLevel(3)
			ZO_StatusBar_SetGradientColor(frame.lgsStaBar, ZO_POWER_BAR_GRADIENT_COLORS[POWERTYPE_STAMINA])
			frame.lgsStaBar:SetBarAlignment(0)
			frame.lgsStaBar:SetMinMax(0, 1000)
			frame.lgsStaBar:SetValue(1000)

				frame.lgsStaBarGloss = wim:CreateControl(nil, frame.lgsStaBar, CT_STATUSBAR)
				frame.lgsStaBarGloss:SetAnchorFill()
				frame.lgsStaBarGloss:SetDrawLayer(1)
				frame.lgsStaBarGloss:SetDrawLevel(4)
				frame.lgsStaBarGloss:SetTexture("EsoUI/Art/Miscellaneous/timerBar_genericFill_gloss.dds")
				frame.lgsStaBarGloss:SetTextureCoords(0, 1, 0, 0.8125)
				frame.lgsStaBarGloss:SetBarAlignment(0)
				frame.lgsStaBarGloss:SetMinMax(0, 1000)
				frame.lgsStaBarGloss:SetValue(1000)
