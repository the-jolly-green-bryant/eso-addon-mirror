...
function Jo.UpdateUnit(event, unitTag)
	...
	if not IsUnitOnline(unitTag) then
		...
		frame.shield:SetWidth(0)
		frame.lgs:SetHeight(0)
		frame.lgsMagBar:SetHeight(0)
		frame.lgsStaBar:SetHeight(0)
	...
	-- LGS
	if (GetFrameTimeSeconds()-unit.lgsUpdate) > 30 and unit.lgsUpdate > 0 then
		frame.lgs:SetHeight(0)
		frame.lgsMagBar:SetHeight(0)
		frame.lgsStaBar:SetHeight(0)
...
function Jo.UpdateBuffs(unitTag)
	...
	local unit = Jo.unit[unitTag]
	local frame = Jo.frame[unitTag]
	local numBuffs = GetNumBuffs(unitTag) and GetNumBuffs(unitTag) or 0
		...
		for i = 1, numBuffs do
			...
			if Jo.savedVars.show.warHorn then
				if Jo.isHorny[abilityId] then
					frame.bgColor:SetCenterColor(GetInterfaceColor(INTERFACE_COLOR_TYPE_PROGRESSION, PROGRESSION_COLOR_CP_HEALTH_END))
					frame.bgColor:SetAlpha(0.6)
				end
			end
		end
	end
...
function Jo.CustomizeUnits()
	...
	for i = 1, GROUP_SIZE_MAX do
		...
		if compactMode then
			...
			if Jo.lgsIsEnabled then Jo.statusBarHeight = fontSize+(12*sf) end
		...
		Jo.lgsBarHeight = Jo.statusBarHeight/(6*sf)
		...
		frame.lgsMagBarGloss:SetAlpha(glossAlpha)
		frame.lgsStaBarGloss:SetAlpha(glossAlpha)
