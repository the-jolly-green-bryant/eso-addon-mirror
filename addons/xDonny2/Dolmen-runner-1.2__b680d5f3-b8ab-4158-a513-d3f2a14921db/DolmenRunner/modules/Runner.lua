...
function Runner:GetDirectionLabel()
    return Runner.data.direction == Runner.directions.cw and "CW" or "CCW"
end
...
function Runner:Start()
    ...
    Runner.started = true
    DR:Log(zo_strformat(GetString(DR_RUNNER_STATUS_RUNNING), self:GetDirectionLabel()))
    DR:RefreshSettings()
    self:GetActiveBuff()
end
...
function Runner:Stop()
    ...
    DR:Log(GetString(DR_RUNNER_STOPPED))
    DR:RefreshSettings()
end
...
function Runner:CW()
    ...
    DR:Log(zo_strformat(GetString(DR_RUNNER_DIRECTION_SET), "CW"))
    DR:RefreshSettings()
end

function Runner:CCW()
    Runner.data.direction = Runner.directions.ccw
    DR:Log(zo_strformat(GetString(DR_RUNNER_DIRECTION_SET), "CCW"))
    DR:RefreshSettings()
end
...
function Runner:OnFastTravelInteraction(currentWayshrine)
    if not Runner.started or not DR.settings.runner.autoTravel or Runner.wayshrines[currentWayshrine] == nil then
        return
    end
    ...
    if not discovered then
        DR:Log(zo_strformat(GetString(DR_RUNNER_UNKNOWN_WAYSHRINE), name))
        return
    end
...
function Runner:OnCombatState(inCombat)
    ...
    if not inCombat and DolmenRunner.settings.runner.reapplyBuff then
        ...
        if Runner.activeBuff > 0 and not self:IsBuffActive(Runner.activeBuff) then
            DR:Log(zo_strformat(GetString(DR_RUNNER_REAPPLYING), GetCollectibleInfo(Runner.activeBuff)))
            UseCollectible(Runner.activeBuff)
        end
    end

    if inCombat and DolmenRunner.settings.runner.autoDismiss then
        for _, id in pairs(assistants) do
            if IsCollectibleActive(id) then
                DR:Log(zo_strformat(GetString(DR_RUNNER_DISMISSING), GetCollectibleInfo(id)))
                UseCollectible(id)
            end
        end
    end
