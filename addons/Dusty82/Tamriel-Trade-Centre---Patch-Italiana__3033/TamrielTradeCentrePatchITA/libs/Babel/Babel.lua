...
function Babel.Menu()
	...
	table.insert(optionsTable, {
 	...
 	table.insert(optionsTable, {
		type = "description",
		text = "Aggiorna il database in caso di problemi o di un'aggiornamento della versione del gioco.",
		width = "half",
	})
	...
	table.insert(optionsTable, {
		type = "button",
		name = "Aggiorna",
		tooltip = "Aggiorna il database in caso di problemi o di un'aggiornamento della versione del gioco.",
        func = function()
		...
		warning = "Attenzione:\nQuesta opzione ricaricherà automaticamente l'interfaccia utente!",
		width = "half",
    })
...
local function OnActivated(eventCode)
	...
	if Babel.savedVars.APIVersion ~= GetAPIVersion() then
		d("È necessario aggiornare il database di \"Babel\"!\nApri impostazioni, vai nel menù degli addon nella sezione Babel e premi il pulsante \"Aggiorna\".")
	end
end
