...
--========================================================================	
	...
	local function Create(pool, objectKey)
		...
		-- Vel --
		local price		= CreateControl(nil, control, CT_LABEL)
		--
		...
		-- Vel --
		 price:SetFont(PP.f.u57 .. "|16|shadow")
		 price:SetAnchor(CENTER, control, CENTER, 45, 0)
		 price:SetMaxLineCount(1)
		 price:SetWrapMode(TEXT_WRAP_MODE_ELLIPSIS)
		 price:SetHorizontalAlignment(TEXT_ALIGN_CENTER)
		--
	...
	function PROVISIONER:AddIngredientRowsTooltip(tooltip, numIngredients, recipeListIndex, recipeIndex, listId)
		...
		for ingredientIndex = 1, numIngredients do
			...
			-- Vel --
			item_Price										= GetRecipeIngredientItemLink(recipeListIndex,recipeIndex,ingredientIndex)
			--
			...
			-- Vel --
			local ItemPriceData = TamrielTradeCentrePrice:GetPriceInfo(item_Price)
			if (ItemPriceData == nil) then
				ItemPriceData = "Price not found"
			else
				sell_Price = tonumber(ItemPriceData["Avg"])
				sell_Price = sell_Price * requiredQuantity			-- Умножаем цену одного предмета на количество
				Total_price = Total_price + sell_Price				-- Общая цена всех ингредиентов чертежа
				sell_Price = TamrielTradeCentre:FormatNumber(sell_Price, 0)
			end
			--
			...
			-- Vel --
			row.price:SetText(zo_strformat(SI_TOOLTIP_ITEM_NAME, sell_Price))
			--
			...
			Total_price = TamrielTradeCentre:FormatNumber(Total_price, 0)
			tooltip:AddLine(string.format("Total crafting price:   " .. tostring(Total_price)))
			--
	end
