...
local buttonProperties = {
    [Drawing.ButtonProperties.Large] = GetString(COMPANIONFRAME_BUTTONPROPERTIESLARGE),
    [Drawing.ButtonProperties.Medium] = GetString(COMPANIONFRAME_BUTTONPROPERTIESMEDIUM),
    [Drawing.ButtonProperties.Small] = GetString(COMPANIONFRAME_BUTTONPROPERTIESSMALL),
}
local buttonPropertyKeys = {}
for key, _ in pairs(CF.Drawing.Fonts) do
    table.insert(fonts, key)
end

for k,v in pairs(buttonProperties) do
    buttonPropertyKeys[v]=k
end
...
local options = {
    ...
    [19] = {
        type = "dropdown",
        name = GetString(COMPANIONFRAME_COMPANIONBUTTONSIZE),
        choices = buttonProperties,
        getFunc = function()
            return buttonProperties[CF.Vars.CompanionButtonSize]
        end,
        setFunc = function(value)
            CF.Vars.CompanionButtonSize = buttonPropertyKeys[value]
            CF.CreateCompanionButtonFrame()
        end,
        width = "full",
        requiresReload = false,
        default = CF.Defaults.CompanionButtonSize
    },
    [20] = {
        type = "dropdown",
        name = GetString(COMPANIONFRAME_COMPANIONBUTTONSPACING),
        choices = buttonProperties,
        getFunc = function()
            return buttonProperties[CF.Vars.CompanionButtonSpacing]
        end,
        setFunc = function(value)
            CF.Vars.CompanionButtonSpacing = buttonPropertyKeys[value]
            CF.CreateCompanionButtonFrame()
        end,
        width = "full",
        requiresReload = false,
        default = CF.Defaults.CompanionButtonSpacing
    },
    [21] = {
        type = "header",
