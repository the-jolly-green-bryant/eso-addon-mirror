...
function BS.GetCollectibleId(bagId, slotIndex)
    local collectibleId = GetItemLinkContainerCollectibleId(GetItemLink(bagId, slotIndex, LINK_STYLE_DEFAULT))
    local quantity = 1
    ...
    if (collectibleId == 0) then
        ...
        for runeboxItemId, fragmentData in pairs(RUNEBOX_FRAGMENTS) do
            if (ZO_IsElementInNumericallyIndexedTable(fragmentData.fragments, itemId)) then
                collectibleId = (fragmentData.cid --[[@as integer]]) or
                    GetItemLinkContainerCollectibleId(BS.LC.MakeItemLink(runeboxItemId))
