...
---    choicesValues = {95, 96, 119, 116},
    ...
    {  type = "description",
       ...
       text = "Allows you to enable or disable the Lion, Gryphon & Wamasu markers inside the crystal rooms. Typing 'g' 'w' or 'l' followed by the correct numeric crystal sequence (from Sanity's Edge Assist Addons numbers) into guild chat will display A to E markers on the crystals in the order they need to be destroyed for anyone running this addon, so W53241 for example will add the A to E markers to the Wamasu run. You can clear existing markers before going to the rooms again by typing 'vcse' into group chat.",
       width = "full",
    },
    {  type    = "checkbox",
...
local pattern = "^[LlWwGg][12345][12345][12345][12345][12345]$"
  if string.len(messageText) == 6 and string.match(messageText, pattern) then
   local firstChar = string.sub(messageText, 1, 1)
    if firstChar == "L" or firstChar == "l" then -- IN LION ROOM
     local LionString = string.sub(messageText, 2) -- extract the numbers after the letter
...
if firstChar == "G" or firstChar == "g" then -- IN GRYPHON ROOM
     local GryphonString = string.sub(messageText, 2) -- extract the numbers after the letter
...
if firstChar == "W" or firstChar == "w" then -- in Wamasu Room
     local WamasuString = string.sub(messageText, 2) -- extract the numbers after the letter
