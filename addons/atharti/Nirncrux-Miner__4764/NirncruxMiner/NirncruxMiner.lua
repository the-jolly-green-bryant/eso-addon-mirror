...
function NM.ProcessNextItem()
	...
	if currentIndex > #pendingItems then
		...
		ZO_ClearNumericallyIndexedTable(pendingItems)
		currentIndex = 0
...
function NM.OnAddonLoaded(event, addonName)
	...
	EM:UnregisterForEvent(NM.name, EVENT_ADD_ON_LOADED)

	NM.RegisterDialog()
	LibCustomMenu:RegisterContextMenu(NM.AddNirncruxOption, LibCustomMenu.CATEGORY_LATE)

	EM:RegisterForEvent(NM.name, EVENT_RETRAIT_STATION_INTERACT_START, function()
		atRetraitStation = true
	end)

	EM:RegisterForEvent(NM.name, EVENT_INTERACTION_ENDED, function(_, interactType)
		if interactType == INTERACTION_RETRAIT then
			atRetraitStation = false
		end
	end)

	EM:RegisterForEvent(NM.name, EVENT_RETRAIT_RESPONSE, NM.OnRetraitResponse)

	EM:RegisterForEvent(NM.name, EVENT_CRAFTING_STATION_INTERACT, function(event, craftingType)
...
EM:RegisterForEvent(NM.name, EVENT_ADD_ON_LOADED, NM.OnAddonLoaded)