...
function ALT.RefreshResultsList()
	...
	for addonName, data in pairs(ALT.SV.results) do
	...
	if durationLabel then
		local durationSec = ALT.SV.testDuration or 0
	...
	if baselineLabel then
		local bTimeMs = ALT.SV.baselineTime or 0
		local bTimeSec = bTimeMs / 1000
		local bMemKB = ALT.SV.baselineMemory or 0
