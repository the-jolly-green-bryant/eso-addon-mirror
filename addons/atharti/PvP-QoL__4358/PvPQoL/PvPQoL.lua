...
function PQ.GetAccountFromCache(targetName)
    local info = PQ.targetCache[targetName]
    if info and info.account and info.account ~= "" then
        return info.account
    end
    return nil
end
...
local function CreateUI()
	...
	-- In CreateUI function, replace the anchor setup section:
	local anchor = PQ.SV.anchor or "LEFT"
	local x = tonumber(PQ.SV.windowOffsetX) or 20
	local y = tonumber(PQ.SV.windowOffsetY) or 20
	
	local scale = PQ.SV.uiScale or 1.0
...
function PQ.GetStatsTooltipLines()
	...
	local kills = PQ.SV.lifetimeKills or 0
    local deaths = PQ.SV.lifetimeDeaths or 1
    
    table.insert(lines, string.format(" |t22:22:%s|t %s", ICON_EXECUTE, FormatNumber(PQ.SV.lifetimeKills or 0)))
    table.insert(lines, string.format(" |t20:20:%s|t %s", ICON_DEATH, FormatNumber(PQ.SV.lifetimeDeaths or 0)))
    ...
    table.insert(lines, string.format(" |t20:20:%s|t %s", ICON_TV, FormatNumber(PQ.SV.lifetimeTelvar or 0)))
    table.insert(lines, string.format(" |t20:20:%s|t %s |t22:22:%s|t", ICON_TV, FormatNumber(PQ.SV.lifetimeTelvarFromKills or 0), ICON_EXECUTE))
    table.insert(lines, string.format(" |t20:20:%s|t %s", ICON_AP, FormatNumber(PQ.SV.lifetimeAP or 0)))
	 ...
	 if PQ.SV.trackItems then
		...
		for _, entry in ipairs(sortedItems) do
			local looted = PQ.SV[entry.data.svFlag] or false
...
function PQ.ApplyUIScale()
    ...
    local newScale = clamp(PQ.SV.uiScale or 1.0, 1.0, 2.0)
    ...
    if PQ.SV.anchor == "RIGHT" then
        window:SetAnchor(TOPRIGHT, GuiRoot, TOPRIGHT,
            -(right * newScale),
            top * newScale)
    else
...
function PQ.UpdateUI()
    ...
    if PQ.isBgActive and not PQ.SV.useUnifiedStats then
        kills = PQ.bgKills or 0
        deaths = PQ.bgDeaths or 0
    else
        kills = PQ.SV.killsToday or 0
        deaths = PQ.SV.deathsToday or 0
    end
    ...
    -- =========================
    -- Detect edge stickiness
    -- =========================
    local rightDist = (screenW - window:GetRight()) / scale
    local isNearRightEdge = rightDist <= 2  -- tolerance

    -- =========================
    -- Resize WITHOUT breaking anchor
    -- =========================
    window:SetWidth(newWidth)

    -- =========================
    -- Handle anchor switching ONLY when needed
    -- =========================
    if isNearRightEdge and PQ.SV.anchor ~= "RIGHT" then
        ...
        window:SetAnchor(TOPRIGHT, GuiRoot, TOPRIGHT,
            -(rightDist * scale),
            PQ.SV.windowOffsetY * scale)

    elseif not isNearRightEdge and PQ.SV.anchor ~= "LEFT" then
        -- Go back to left anchored mode
...
function PQ.OnAlliancePointUpdate(eventCode, alliancePoints, playSound, difference, reason, reasonSupplementaryInfo)
    ...
    local minAmount = PQ.SV.minAPMessage or 100
    local isSmall = difference < minAmount
    
    PQ.SV.lifetimeAP = (PQ.SV.lifetimeAP or 0) + difference
...
function PQ.OnTelvarUpdate(eventCode, newTelvar, oldTelvar, reason, reasonSupplementaryInfo)
    ...
    local minAmount = PQ.SV.minTelvarMessage or 100
    local isSmall = amount < minAmount
	
	PQ.SV.lifetimeTelvar = (PQ.SV.lifetimeTelvar or 0) + amount
...
function PQ.ApplyStandInsSetting()
    ...
    if PQ.isInAvA or PQ.isBgActive then
        ...
        SetCVar("PlayerStandInsMaxPerFrame", tostring(PQ.SV.standInsPerFrame or 8))
    else
...
function PQ.OnKillingBlow(_, result, _, abilityName, _, _, sourceName, sourceType, targetName, targetType, _, _, _, _, _, _, _, _)		
    ...
    -- Only for Cyrodiil/IC now (BG kills handled separately)
    PQ.SV.killsToday = (PQ.SV.killsToday or 0) + 1
    PQ.SV.lifetimeKills = (PQ.SV.lifetimeKills or 0) + 1

    PQ.UpdateUI()
    PQ.TriggerKillAlert()

    local cachedInfo = PQ.GetCachedTargetInfo(target)
    local cachedAccount = cachedInfo and cachedInfo.account or nil
    ...
    -- Process pending currency messages (AP/Telvar - only relevant in Cyrodiil/IC)
    ...
    for _, entry in ipairs(tvMessages) do
        ...
        if entry.amount then
            entry.fromKill = true
            PQ.SV.lifetimeTelvarFromKills = (PQ.SV.lifetimeTelvarFromKills or 0) + entry.amount
        end
    end
...
function PQ.OnBattlegroundKill(eventCode, killedPlayerCharacterName, killedPlayerDisplayName, killedPlayerBattlegroundTeam,
    ...
    if battlegroundKillType == BATTLEGROUND_KILL_TYPE_KILLED_BY_ENEMY_TEAM then
        if killedPlayerCharacterName == PQ.playerName then
            ...
            if not PQ.SV.useUnifiedStats then
                PQ.bgDeaths = (PQ.bgDeaths or 0) + 1
            else
                PQ.SV.deathsToday = (PQ.SV.deathsToday or 0) + 1
            end
            
            PQ.SV.lifetimeDeaths = (PQ.SV.lifetimeDeaths or 0) + 1
...
function PQ.FlushBGCountersIfLeft()
    if not PQ.isBgActive and ((PQ.bgKills or 0) > 0 or (PQ.bgDeaths or 0) > 0) then
        PQ.SV.killsToday = (PQ.SV.killsToday or 0) + (PQ.bgKills or 0)
        PQ.SV.deathsToday = (PQ.SV.deathsToday or 0) + (PQ.bgDeaths or 0)
