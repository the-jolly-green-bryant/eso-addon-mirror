...
local devAccount = CURRENT_ACCOUNT == "ohmygoron" or CURRENT_ACCOUNT == "goron_spice"
local friendAccounts = (GoronDev and GoronDev.__friendAccounts) or {
    autumnmcbottom = true,
    designnerd_ = true,
    treenerd__ = true,
    shibnibblies = true,
...
local friendEnabled = GoronDev and GoronDev.__friendAccessEnabled
local friendAccount = friendAccounts[CURRENT_ACCOUNT] == true

local function canLoadDevBuild()
    if GoronDev then
        local decision = GoronDev.__allowRezBotDev
        if decision ~= nil then
            if decision then
                return true
            else
                if friendAccount then
                    return false
                end
                return false, "|cFFFF66GoronDev|r: GoronDev restricted to authorized accounts."
            end
        end
    end

    if devAccount or (friendEnabled and friendAccount) then
        return true
    end
...
RezBot                = RezBot or {}
RezBot.__source       = "GoronDev"

local SAVE_ROOT       = "GoronDevSV"
local SAVE_NAMESPACE  = "RezBotDev"

RezBot.name           = "RezBot"
RezBot.version        = "1.9.5"
...
local function ensureSavedVars()
    local saved = ZO_SavedVars:NewAccountWide(SAVE_ROOT, 1, SAVE_NAMESPACE, RezBot.defaults)
    for key, defaultValue in pairs(RezBot.defaults) do
        if saved[key] == nil then
            saved[key] = copyDefaults(defaultValue)
        end
    end
    if GoronDev then
        GoronDev.savedRezbot = saved
    end
    return saved
end
...
local warnedMissingSelectedRole = false
local warnedMissingAssignedRole = false
local CLEANUP_INTERVAL_MS = 5000

local function isAccountActive(accountName, entry)
    if not accountName or accountName == "" then return false end

    if DoesUnitExist and DoesUnitExist("player") and GetUnitDisplayName then
        local playerName = normalizeName(tostring(GetUnitDisplayName("player")))
        if playerName == accountName then
            return true
        end
    end

    if GetGroupSize and GetGroupSize() > 0 then
        for i = 1, GROUP_SIZE_MAX do
            local tag = "group" .. i
            if DoesUnitExist and DoesUnitExist(tag) and GetUnitDisplayName then
                local display = normalizeName(tostring(GetUnitDisplayName(tag)))
                if display == accountName then
                    return true
                end
            end
        end
    ...
    if entry and entry.charName and DoesUnitExist and GetUnitName then
        local cleanChar = cleanse(entry.charName)
        for i = 1, GROUP_SIZE_MAX do
            local tag = "group" .. i
            if DoesUnitExist(tag) then
                local groupChar = cleanse(GetUnitName(tag))
                if groupChar == cleanChar then
                    return true
                end
            end
        end
...
local function cleanupStaleEntries()
    if not resStates or next(resStates) == nil then return end

    local removed = false
    for name, entry in pairs(resStates) do
        if not isAccountActive(name, entry) then
            resStates[name] = nil
            rowColorCache[name] = nil
            removed = true
            if RezBot.dbg then
                RezBot.dbg("Removed stale entry: " .. tostring(name))
            end
        end
    end

    if removed then
        for k in pairs(rowColorCache) do
            if not resStates[k] then
                rowColorCache[k] = nil
            end
        end
        uiDirty = true
        ...
        if DoesUnitExist(tag) then
            ...
            if unitDisplayName == accountName then
                if GetGroupMemberSelectedRole then
                    warnedMissingSelectedRole = false
                    ...
                    if not warnedMissingSelectedRole then
                        RezBot.dbg("    GetGroupMemberSelectedRole is nil")
                        warnedMissingSelectedRole = true
                    end
                end
            end
        end
    ...
    if DoesUnitExist("player") then
        ...
        if unitDisplayName == accountName then
            if GetGroupMemberAssignedRole then
                warnedMissingAssignedRole = false
                ...
                if not warnedMissingAssignedRole then
                    RezBot.dbg("    GetGroupMemberAssignedRole is nil")
                    warnedMissingAssignedRole = true
                end
            end
        end
    end
...
local function onUpdate()
    ...
    for name, entry in pairs(resStates) do
        ...
        if entry.status == RezBot.STATUS_REZZING and entry.tag and not IsUnitBeingResurrected(entry.tag) then
    ...
    if now - lastCleanupMs >= CLEANUP_INTERVAL_MS then
        cleanupStaleEntries()
        lastCleanupMs = now
    end
...
local function AddSettings()
    ...
    panel:AddSetting({
        ...
        default = false,
        getFunction = function() return not RezBot.saved.uiEnabled end,
        setFunction = function(val)
            RezBot.saved.uiEnabled = not val
            RefreshUI()
        end,
    })
...
local function onLoaded(_, addon)
    local expectedAddonName = GoronDev and GoronDev.name or RezBot.name
    if addon ~= expectedAddonName and addon ~= RezBot.name then return end

    local allowed, errMsg = canLoadDevBuild()
    if not allowed then
        if errMsg then d(errMsg) end
        return
    end

    RezBot.saved = ensureSavedVars()

    -- Failsafe: fill in any missing keys
    for k, v in pairs(RezBot.defaults) do
        if RezBot.saved[k] == nil then
            RezBot.saved[k] = copyDefaults(v)
        end
    end
