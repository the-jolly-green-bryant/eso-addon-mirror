...
do
    ...
    local g_increasedPowerControlCounter = 0
    local g_increasedPowerControlNameToControl = {}
    ...
    function ZO_UnitVisualizer_ArmorDamage:GetOrCreateIncreasePowerPool()
        if not g_increasedPowerPools[self.layoutData] then
            ...
            local function CreateIncreasedPower(pool)
                local baseName = INCREASED_POWER_TEXTURE..self.layoutData.type
                local increasedPowerTexture
                local uniqueId

                repeat
                    g_increasedPowerControlCounter = g_increasedPowerControlCounter + 1
                    uniqueId = g_increasedPowerControlCounter
                    local controlName = baseName..uniqueId
                    local existingControl = GetControl(controlName)
                    if not existingControl then
                        increasedPowerTexture = CreateControlFromVirtual(controlName, GuiRoot, INCREASED_POWER_TEXTURE)
                        if increasedPowerTexture then
                            g_increasedPowerControlNameToControl[controlName] = increasedPowerTexture
                            break
                        end
                    end
                until false

                local increasedPowerGlow = ZO_ObjectPool_CreateControl(self.layoutData.increasedPowerGlowTemplate, pool, GuiRoot)

                if not increasedPowerTexture or not increasedPowerGlow then
                    return {}
                end
...
function ZO_UnitVisualizer_ArmorDamage:PlayPowerIncreaseAnimation(bar, info, instant)
    ...
    if not increasedPowerTexture or not increasedPowerGlow or not loopingAnimation or not fadeAnimation then
        return nil
    end
