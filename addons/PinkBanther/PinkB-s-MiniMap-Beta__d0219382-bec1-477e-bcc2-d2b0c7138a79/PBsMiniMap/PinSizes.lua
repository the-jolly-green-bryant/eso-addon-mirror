...
function addon:InitPinSizes()
	...
	local settings = LibHarvensAddonSettings:AddAddon(addon.baseTitle .. " Pin Sizes")
	...
	local function AddPin(pinType, caption, stringId)
		...
		local task = async:Create("PBsMiniMapPinSize" .. caption)
	...
	AddPins(ZO_MapPin.FAST_TRAVEL_KEEP_PIN_TYPES, "Keep Fast Travel", SI_PBSMINIMAP_PINSIZE_KEEP_FAST_TRAVEL)

	AddPins(ZO_MapPin.OBJECTIVE_PIN_TYPES, "AvA Objectives", SI_MAPFILTER2)
	AddPins(ZO_MapPin.KEEP_PIN_TYPES, "Keeps", SI_PBSMINIMAP_PINSIZE_KEEPS)
	-- AddPins(ZO_MapPin.IMPERIAL_CITY_GATE_TYPES, "Imperial City Gates", SI_MAPFILTER12);
	AddPins(ZO_MapPin.DISTRICT_PIN_TYPES, "Districts", SI_PBSMINIMAP_PINSIZE_DISTRICTS)
	...
	local task = async:Create("PBsMiniMapPinSizeOthers")
...
local function PlayerActivated()
	em:UnregisterForEvent("PBSMINIMAP_PINSIZES", EVENT_PLAYER_ACTIVATED)
	-- Part of the InitMiniMap layer for bisection purposes (see initLevel in Main.lua).
	if (addon.initLevel or 0) >= 3 then
		addon:InitPinSizes()
	end
end
em:RegisterForEvent("PBSMINIMAP_PINSIZES", EVENT_PLAYER_ACTIVATED, PlayerActivated)
