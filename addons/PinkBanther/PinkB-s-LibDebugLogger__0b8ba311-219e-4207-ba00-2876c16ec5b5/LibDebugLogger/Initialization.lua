...
-- we only remember which add-on manager index a name belongs to and build the message when it is
-- actually logged. Holding a formatted string for every installed add-on was pure overhead.
local addOnIndex = {}
local skippedAddOnIndex = {}
...
for i = 1, numAddons do
    ...
    if(enabled) then
        if(state == ADDON_STATE_ENABLED) then
            ...
            skippedAddOnIndex[name] = i
        end
        numEnabledAddons = numEnabledAddons + 1
    end
end

local function FormatAddOnInfo(i)
    local name = AddOnManager:GetAddOnInfo(i)
    return strformat("Addon loaded: %s, AddOnVersion: %d, directory: '%s'",
        name, AddOnManager:GetAddOnVersion(i), AddOnManager:GetAddOnRootDirectoryPath(i))
end

local function FormatSkippedAddOnInfo(i)
    ...
    return strformat("Did not load addon: %s, AddOnVersion: %d, directory: '%s', state: %s",
        name, AddOnManager:GetAddOnVersion(i), AddOnManager:GetAddOnRootDirectoryPath(i), stateString)
end
...
EVENT_MANAGER:RegisterForEvent(lib.id, EVENT_LUA_ERROR, function(eventCode, errorString, errorCode)
    -- the time sync hack that used to raise an error here on purpose was removed, see the note in
    -- LogHandler.lua, so there is nothing left to filter out
    if(errorString) then
...
EVENT_MANAGER:RegisterForEvent(lib.id, EVENT_PLAYER_ACTIVATED, function(event, initial)
    ...
    if(regularLoadingScreen) then
        ...
        for name in pairs(addOnIndex) do
        ...
        ZO_ClearTable(addOnIndex)
        for _, i in pairs(skippedAddOnIndex) do
            internal.Log(internal.LOG_LEVEL_WARNING, LDL_LOGGER_CONFIG, FormatSkippedAddOnInfo(i))
        end
        -- these are only reported once, so we let go of the table afterwards
        ZO_ClearTable(skippedAddOnIndex)
    end
...
EVENT_MANAGER:RegisterForEvent(lib.id, EVENT_ADD_ON_LOADED, function(event, name)
    local index = addOnIndex[name]
    internal.Log(internal.LOG_LEVEL_INFO, LDL_LOGGER_CONFIG,
        index and FormatAddOnInfo(index) or strformat("UI module loaded: %s", name))
    addOnIndex[name] = nil
