...
function M:Accept()
    ...
    self.phase='accepting'; self:Reliable(K.ACCEPT); self:Notify('開始を待っています'); return true
end
function M:Close(reason,notifyPeer)
    ...
    self:Notify(reason or '対局を終了しました（無効対局）')
end
function M:Decline()
    self:Send(K.DECLINE); self:Close('対戦申請を取り消しました')
end
function M:Playable()
...
function M:Commit(packed)
    if self.seq>=N.MAX_LOG then self:Close('行動数の上限を超えました',true); return false end
    local command,actor=W.Unpack(packed)
    if not command then return false,'不正な行動です' end
    ...
    self:Notify('相手の確認を待っています'); return true,reason
end
function M:Submit(command)
    local surrender=command.type=='resign'
    if self.phase~='active' or self.pending or (not surrender and self.state.player~=self.seat) then return false,'通信確認中、または相手の手番です' end
    local packed=W.Pack(command,self.seat)
    if not packed then return false,'不正な行動です' end
    local valid=W.Unpack(packed); if not valid then return false,'不正な行動です' end
    local ok,reason
...
function M:StartSync()
    ...
    if self.seat==2 then
        self:Reliable(K.SYNC,self.seq); self:Notify('盤面を再同期しています'); return
    end
    ...
    for _,entry in ipairs(self.log) do
        ...
        if not command or not W.Apply(rebuilt,actor,command) or E.Checksum(rebuilt)~=entry.hash then
            self:Close('行動記録を復元できません。無効対局にします',true); return
        end
    end
    self.state,self.seq=rebuilt,#self.log; self.replaying=0; self.syncEpoch=self.syncEpoch+1
    self:ReplayPacket(0); self:Notify('行動記録から盤面を再同期しています')
end
function M:Receive(peer,p)
    ...
    local kind=p.kind
    if kind==K.DECLINE and (self.phase=='inviting' or self.phase=='accepting' or self.phase=='starting') then self:Close('対戦申請が拒否／取消されました'); return end
    if kind==K.CLOSE then self:Close('相手が終了しました（無効対局）'); return end
    if self.seat==1 and kind==K.ACCEPT then
        if self.phase=='inviting' then self.phase='starting'; self:Reliable(K.START,0,0,E.Checksum(self.state)); self:Notify('対戦開始の確認中')
    ...
    if self.seat==2 and kind==K.START then
        if self.phase=='accepting' then
            if p.hash~=E.Checksum(self.state) then self:Close('初期盤面が一致しません',true); return end
            self.phase,self.pending='active',nil; self:Notify('対戦開始：あなたはP2です')
        end
    ...
    if self.seat==1 and kind==K.READY and self.phase=='starting' then
        if p.hash~=E.Checksum(self.state) then self:Close('初期盤面が一致しません',true); return end
        self.phase,self.pending='active',nil; self:Notify('対戦開始：あなたはP1です'); return
    end
    ...
    if self.seat==1 and kind==K.REPLAY_ACK and self.phase=='syncing' then
        ...
        if index==#self.log+1 then
            self.pending,self.replaying=nil,nil; self:CommittedPhase(); self:Notify('再同期が完了しました')
    ...
    if self.seat==2 and kind==K.REPLAY then
        ...
        if index==0 then
            if p.cmd>N.MAX_LOG or p.hash~=E.Checksum(self:InitialState()) then self:Close('再同期データが不正です',true); return end
            ...
            self:Send(K.REPLAY_ACK,p.seq,0,p.hash); self:Notify('行動記録から再同期しています'); return
        end
        ...
        if index==self.syncCount+1 then
            if p.cmd~=0 or p.hash~=E.Checksum(self.stage) then self:Close('再同期結果が一致しません',true); return end
            self.state,self.seq,self.stage,self.pending=self.stage,self.syncCount,nil,nil
            self.completedSyncEpoch=epoch
            self:CommittedPhase(); self:Send(K.REPLAY_ACK,p.seq,0,p.hash); self:Notify('再同期が完了しました'); return
        end
        local command,actor=W.Unpack(p.cmd)
        if not command or not W.Apply(self.stage,actor,command) or E.Checksum(self.stage)~=p.hash then self:Close('再同期行動が不正です',true); return end
    ...
    if self.seat==1 and kind==K.REQUEST then
        ...
        local command,actor=W.Unpack(p.cmd)
        if not command or actor~=2 then self:Close('不正な行動要求を受信しました',true); return end
        local ok=self:Commit(p.cmd)
        if not ok then self:Close('相手の行動を適用できません（無効対局）',true) end
        return
    end
    if self.seat==2 and kind==K.COMMIT then
        ...
        local command,actor=W.Unpack(p.cmd)
        if not command then self:Close('不正な行動を受信しました',true); return end
        ...
        if actor==2 and not matching then
            self:Close('未申請の自軍行動を受信しました',true); return
        end
        local copy=E.Clone(self.state)
        ...
        self:CommittedPhase(); self:Send(K.ACK,self.seq,0,p.hash); self:Notify('盤面を同期しました'); return
    end
    if self.seat==1 and kind==K.ACK and self.pending and self.pending.kind==K.COMMIT and p.seq==self.seq then
        ...
        self.pending=nil; self:Notify('盤面を同期しました')
    end
end
function M:Tick()
    ...
    if (self.phase=='invited' or self.phase=='inviting') and now-self.started>N.INVITE_TIMEOUT then self:Close('対戦申請の期限が切れました',true); return end
    if self.phase=='syncing' and self.syncAt and now-self.syncAt>N.SYNC_TIMEOUT then self:Close('再同期がタイムアウトしました（無効対局）',true); return end
    if self.pending then
        if now>self.pending.deadline then self:Close('通信確認がタイムアウトしました（無効対局）',true); return end
    ...
    if self.phase=='active' or self.phase=='syncing' then
        if now-self.lastHeard>N.TIMEOUT then self:Close('相手との接続が途切れました（無効対局）',true); return end
