...
function E.InBounds(x, y)
    return integer(x) and integer(y) and x >= 1 and x <= C.SIZE and y >= 1 and y <= C.SIZE
end
function E.At(s, x, y)
...
function E.Flag(s, x, y)
    for i, f in ipairs(C.FLAGS) do if f.x == x and f.y == y then return s.flags[i], i end end
end
-- setup=true is used by live games; default is a neutral rules/test fixture.
function E.New(setup)
    local s = { version = 4, factions = {0,0}, scrollUsed={false,false}, scrollOwner=0, scrollReader=0, scrollTurn=0, turn = 1, player = 1, actions = C.ACTIONS_PER_TURN,
    ...
    for i = 1, #C.FLAGS do s.flags[i] = 0 end
    for player = 1, 2 do
        for _, start in ipairs(C.START) do
            local id = #s.pieces + 1
            s.pieces[id] = { id = id, owner = player, kind = start.kind,
                x = player == 1 and start.x or C.SIZE + 1 - start.x, y = start.y, alive = true }
        end
...
function E.CanMove(s, p, x, y, distance)
    if not E.InBounds(x, y) then return false, "outside" end
...
function E.Attack(s,p) return PBWT.Factions.Attack(s,p) end
function E.Defense(s, p, siege)
    ...
    return C.PIECES[p.kind].defense + (siege and 0 or bonus) + PBWT.Factions.Defense(s,p)
end
function E.CanAttack(s, attacker, target, siege)
    ...
    if siege and (target.kind ~= "guardian" or E.Flag(s, target.x, target.y) == nil) then return false, "siege_target" end
    return true
end
function E.ResolveAttack(s, attacker, target, siege)
    s.actions = s.actions - 1
    if E.Attack(s,attacker) >= E.Defense(s, target, siege) then
...
function E.Tiebreak(s)
    ...
    if s.score[1] ~= s.score[2] then return s.score[1] > s.score[2] and 1 or 2 end
    if f1 ~= f2 then return f1 > f2 and 1 or 2 end
    if p1 ~= p2 then return p1 > p2 and 1 or 2 end
    return 0
end
function E.IsHome(player, x, y)
    for _, p in ipairs(C.START) do
        if (player == 1 and p.x or C.SIZE + 1 - p.x) == x and p.y == y then return true end
    end
    return false
end
function E.EndTurn(s)
    ...
    if s.score[player] >= C.WIN_SCORE then
    ...
    elseif s.turn >= C.MAX_TURNS then
        s.status, s.winner, s.reason = "finished", E.Tiebreak(s), "turn_limit"
    else
        s.turn, s.player, s.actions = s.turn + 1, 3 - player, C.ACTIONS_PER_TURN
...
function E.Serialize(s)
    local values = { s.version, s.factions[1], s.factions[2], s.turn, s.player, s.actions, s.status, s.winner or -1, s.reason or "-", s.score[1], s.score[2] }
