...
function R.Record(state,mode,difficulty,seat,key)
    ...
    R.seen[state]=true
    local category=mode=='solo' and difficulty or 'online'
...
function R.Text(index)
    ...
    for _,item in ipairs({{'beginner','初級'},{'intermediate','中級'},{'advanced','上級'},{'online','対人'}}) do
        local t=data.totals[item[1]] or {wins=0,losses=0,draws=0}
        lines[#lines+1]=string.format('%s：%d勝 %d敗 %d分',item[2],t.wins,t.losses,t.draws)
    end
