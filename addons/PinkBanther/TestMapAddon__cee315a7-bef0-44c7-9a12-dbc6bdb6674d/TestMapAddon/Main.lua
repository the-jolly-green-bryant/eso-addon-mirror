if TESTMAPADDON then
	return
end

-- Private test build: only load for the account(s) listed below.
-- Add/remove @DisplayNames as needed (comparison is case-insensitive).
-- IMPORTANT: fill these in with your own @DisplayName(s) before uploading to Bethesda.net.
local ALLOWED_DISPLAY_NAMES = {
	["@pinkbanther"] = true,
	["@yukiblack1123"] = true,
}
if not ALLOWED_DISPLAY_NAMES[GetDisplayName():lower()] then
	return
end
...
MAP_MODE_TESTMAPADDON = 41

local MAP_MODE_TESTMAPADDON = MAP_MODE_TESTMAPADDON
...
function addon:InitTweaks()
	...
	do
		local task = async:Create("TESTMAPADDON_RefreshAllPOIs")
		...
		function ZO_WorldMap_RefreshAllPOIs()
			if not WORLD_MAP_MANAGER:IsInMode(MAP_MODE_TESTMAPADDON) then
	...
	do
		local task = async:Create("TESTMAPADDON_RefreshWayshrines")
		-- Vanilla implementation, used as-is whenever the standard (non-minimap) World Map
		-- is showing, so this add-on has zero involvement (and zero add-on memory cost) in
		-- that case.
	...
	if ZO_MapLocationPins_Manager then
		local task = async:Create("TESTMAPADDON_RefreshLocations")
		-- Vanilla implementation, used as-is whenever the standard (non-minimap) World Map
		-- is showing, so this add-on has zero involvement (and zero add-on memory cost) in
		-- that case.
	...
	local function DeferRefresh(methodName, identifier, delay)
		local task = async:Create("TESTMAPADDON_" .. identifier)
	...
	do
		local task = async:Create("TESTMAPADDON_MAP_CUSTOM_PIN_UPDATE")
		-- Vanilla implementation, used as-is whenever the standard (non-minimap) World Map
		-- is showing, so this add-on has zero involvement (and zero add-on memory cost) in
	...
	do
		local task = async:Create("TESTMAPADDON_MAP_UPDATE_MAP_SIZE_CHANGE")
...
function addon:InitCameraAngle()
	...
	local function setupCameraAngle(control)
		control:SetTexture("TestMapAddon/ViewLimit.dds")
...
function addon:InitMiniMap()
	do
		...
		orgZO_WorldMap_GetMapDimensions =
			...
			function()
				if WORLD_MAP_MANAGER:IsInMode(MAP_MODE_TESTMAPADDON) then
	...
	local myMode = vars[MAP_MODE_TESTMAPADDON]
	if not myMode then
		myMode = ZO_DeepTableCopy(vars[MAP_MODE_SMALL_CUSTOM])
		vars[MAP_MODE_TESTMAPADDON] = myMode
	...
	local control = wm:CreateControl("TestMapAddonBg", ZO_WorldMap, CT_BACKDROP)
	...
	orgCanMapZoom =
		...
		"CanMapZoom",
		function(...)
			return orgCanMapZoom(...) or WORLD_MAP_MANAGER:IsInMode(MAP_MODE_TESTMAPADDON)
		end
	)

	local asyncCallbacks = async:Create("TESTMAPADDON_MAP_DO_CALLBACKS")
	...
	local function UpdateMap(force)
		...
		if not force then
			if HUD_UI_SCENE:IsShowing() or not WORLD_MAP_MANAGER:IsInMode(MAP_MODE_TESTMAPADDON) then
				return
			end
		end
	...
	do
		...
		local updateTask = async:Create("TestMapAddonUpdateMap")
		...
		addon:SetWorldMapUpdateHandler(WORLD_MAP_MANAGER:IsInMode(MAP_MODE_TESTMAPADDON))
	end
	...
	do
		...
		local function RefreshVisibility()
			isDirty = false
			if GetScene():IsShowing() or not WORLD_MAP_MANAGER:IsInMode(MAP_MODE_TESTMAPADDON) then
				return true
			end
	...
	orgZO_WorldMap_GetMapTitle =
		...
		function(...)
			if WORLD_MAP_MANAGER:IsInMode(MAP_MODE_TESTMAPADDON) and DoesCurrentMapMatchMapForPlayerLocation() then
	...
	do
		...
		function ZO_WorldMapManager.TryShowSpectacleMapHeader(manager)
			if manager:GetMode() == MAP_MODE_TESTMAPADDON then
				return
			else
	...
	do
		local function DoIt(orgZO_WorldMap_UpdateMap, skipWorldMapUpdate)
			...
			WORLD_MAP_MANAGER:SetToMode(MAP_MODE_TESTMAPADDON)
		...
		function addon:GoMiniMapMode(skipWorldMapUpdate)
			...
			if mode ~= MAP_MODE_TESTMAPADDON then
				if WORLD_MAP_MANAGER.inSpecialMode then
	...
	local function updateCachedMapMode()
		cachedMapMode = WORLD_MAP_MANAGER:IsInMode(MAP_MODE_TESTMAPADDON)
	end
	...
	ZO_PreHook(
		...
		function(self, mode)
			if mode == MAP_MODE_TESTMAPADDON then
				cachedMapMode = true
			elseif mode ~= MAP_MODE_TESTMAPADDON and cachedMapMode then
				cachedMapMode = false
			end
		end
	)
	...
	ZO_PreHook(
		...
		function(self, ...)
			--TestMapAddon uses a dedicated mapmode to show the minimap is used. If we ask for that being active we can detect it's the minimap addon and not the normal worldmap. Other addons like MapPins and Fyrakin hopefully got such dedicated MapModes themselves  !!!
		...
		local function WorldFragmentStateChanged(oldState, newState)
			if newState == SCENE_FRAGMENT_SHOWING then
				if addon.account.showClock then
					EVENT_MANAGER:RegisterForUpdate("TESTMAPADDON_MAP_CLOCK", 5000, addon.ShowClock)
					addon.ShowClock()
				end
				self:SetMapHeader()
			elseif newState == SCENE_FRAGMENT_HIDING then
				EVENT_MANAGER:UnregisterForUpdate("TESTMAPADDON_MAP_CLOCK")
			end
	...
	do
		...
		function ZO_WorldMapManager.RefreshMapFrameAnchor(manager, ...)
			if addon.account and manager:IsInMode(MAP_MODE_TESTMAPADDON) then
	...
	orgGetNormalizedPositionFocusZoomAndOffset =
		...
		function(panZoom, normalizedX, normalizedY, useCurrentZoom)
			if WORLD_MAP_MANAGER:GetMode() ~= MAP_MODE_TESTMAPADDON then
...
function addon:Initialize()
	...
	self.account = ZO_SavedVars:NewAccountWide("TestMapAddon_Data", 1, nil, accountDefaults)
	...
	self.player = ZO_SavedVars:NewCharacterIdSettings("TestMapAddon_Data", 1, nil, defaults)
...
do
	...
	function addon:ToggleShowMap()
		...
		CENTER_SCREEN_ANNOUNCE:AddMessage(EVENT_BROADCAST, CSA_CATEGORY_SMALL_TEXT, nil, string.format("%s: %s", GetString(SI_TESTMAPADDON_SHOW_MAP), GetString(self.player.showMap and SI_CHECK_BUTTON_ON or SI_CHECK_BUTTON_OFF)))
		self:UpdateVisibility()
		UpdateControls()
	end
	...
	function addon:StepZoom(add)
		if WORLD_MAP_MANAGER:GetMode() ~= MAP_MODE_TESTMAPADDON or WORLD_MAP_FRAGMENT:IsHidden() or addon.isSpecialZoom or not self.account[self.zoomMode] then
...
--
-- Chat output is useless for the reported failure: the client dies (and clears the chat log)
-- in the same frame the limit is hit, so nothing survives to read. Instead the readings are
-- written into SavedVariables as they happen and dumped on the next login, which is why the
-- trail persists across the crash.
...
local function InitMemoryWatchdog()
	...
	if type(previous) == "table" and #previous > 0 then
		df("[TestMapAddon] --- add-on memory trail from previous session ---")
		for i = 1, #previous do
			df("[TestMapAddon] %s", previous[i])
		end
		df("[TestMapAddon] --- end of trail (%d entries) ---", #previous)
	end
	...
	local function Record(used, label)
		-- Keep only the most recent entries: the interesting part is the tail, right before
		-- the limit was hit.
		...
		log[#log + 1] =
			string.format(
			"%.1fMB mapType=%s zoom=%.2f mode=%s map=%s%s",
			used,
			tostring(GetMapType()),
			addon.panZoom and addon.panZoom:GetCurrentNormalizedZoom() or -1,
	...
	local function Check()
		local used = GetTotalUserAddOnMemoryPoolUsageMB()
		if not used then
			return
		end
		if used >= lastRecorded + 1 or used <= lastRecorded - 1 then
			Record(used)
		end
	end
	EVENT_MANAGER:RegisterForUpdate(addon.name .. "MemoryWatch", 0, Check)
