...
function addon:InitPinSizes()
	...
	local settings = LibHarvensAddonSettings:AddAddon("TestMapAddon Pin Sizes")
	...
	local function AddPin(pinType, caption, stringId)
		...
		local task = async:Create("TestMapAddonPinSize" .. caption)
	...
	AddPins(ZO_MapPin.FAST_TRAVEL_KEEP_PIN_TYPES, "Keep Fast Travel", SI_TESTMAPADDON_PINSIZE_KEEP_FAST_TRAVEL)

	AddPins(ZO_MapPin.OBJECTIVE_PIN_TYPES, "AvA Objectives", SI_MAPFILTER2)
	AddPins(ZO_MapPin.KEEP_PIN_TYPES, "Keeps", SI_TESTMAPADDON_PINSIZE_KEEPS)
	-- AddPins(ZO_MapPin.IMPERIAL_CITY_GATE_TYPES, "Imperial City Gates", SI_MAPFILTER12);
	AddPins(ZO_MapPin.DISTRICT_PIN_TYPES, "Districts", SI_TESTMAPADDON_PINSIZE_DISTRICTS)
	...
	local task = async:Create("TestMapAddonPinSizeOthers")
...
local function PlayerActivated()
	em:UnregisterForEvent("TESTMAPADDON_MINI_MAP_PINSIZES", EVENT_PLAYER_ACTIVATED)
	if addon.account.enableMap then
		addon:InitPinSizes()
	end
end
em:RegisterForEvent("TESTMAPADDON_MINI_MAP_PINSIZES", EVENT_PLAYER_ACTIVATED, PlayerActivated)
