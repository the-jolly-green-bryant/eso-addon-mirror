PBWT = PBWT or {}
PBWT.Config = {
    -- Words come from Locale; these keys are what the screens look up.
    TITLE_KEY = "title", SUBTITLE_KEY = "subtitle",
    ...
    NETWORK = { DEVELOPMENT_ID = 508, VERSION = 1, BUILD = 1700,
        RETRY = 6, TIMEOUT = 60, INVITE_TIMEOUT = 45, HEARTBEAT = 15, MAX_LOG = 390, SYNC_TIMEOUT = 300 },
    PIECES = {
        soldier = { key = "soldier", attack = 2, defense = 2, move = 1 },
        scout = { key = "scout", attack = 1, defense = 1, move = 2 },
        guardian = { key = "guardian", attack = 2, defense = 3, move = 1 },
    },
    ...
    VARIANTS = {
        ...
        standard = { id = 2, key = "standard",
            ...
            FLAGS = {
                { x = 4, y = 3, points = 1, key = "aleswell" },
                { x = 6, y = 3, points = 1, key = "chalman" },
                { x = 3, y = 5, points = 1, key = "ash" },
                { x = 7, y = 5, points = 1, key = "bluroad" },
                { x = 4, y = 7, points = 1, key = "roebeck" },
                { x = 6, y = 7, points = 1, key = "alessia" },
            },
            START = {
    ...
    AI = {
        ...
        DIFFICULTIES = {
            beginner = { key = "beginner", width = 1, nodes = 256, perTick = 8 },
            intermediate = { key = "intermediate", width = 4, nodes = 800, perTick = 16 },
            advanced = { key = "advanced", width = 5, nodes = 1600, perTick = 24, combinations = true },
        },
    },
}
...
-- Words for the data above. Kept beside the data so no screen has to know the key layout.
function PBWT.Config.Title() return PBWT.L("title") end
function PBWT.Config.Subtitle() return PBWT.L("subtitle") end
function PBWT.Config.PieceName(kind,short)
    return PBWT.L("piece_"..kind..(short and "_short" or ""))
end
function PBWT.Config.VariantName(variant,short)
    return PBWT.L("variant_"..variant.key..(short and "_short" or ""))
end
function PBWT.Config.KeepName(flag) return flag.key and PBWT.L("keep_"..flag.key) or nil end
function PBWT.Config.LevelName(difficulty)
    local profile=PBWT.Config.AI.DIFFICULTIES[difficulty]
    return profile and PBWT.L("level_"..profile.key) or difficulty
end
