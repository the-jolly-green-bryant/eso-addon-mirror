...
function P.New(ui)
    ...
    self.board=texture(ui.boardFrame,ui.state.variant=='standard' and 'board_material' or 'board_light',0,0,612,612)
...
function P:Poll()
    ...
    if self.ui.factionUI then self.ui.factionUI:DiceSurface() end
end
function P:Refresh()
    local ui=self.ui; local state=ui.state; local full=PBWT.Records.Data().presentation=='full'
    local boardKind=state.variant=='standard' and 'board_material' or 'board_light'
    if self.board.pbwtArt~=boardKind then
        self.board.pbwtArt=boardKind
        if full then PBWT.Assets.Apply(self.board,boardKind,true) end
    end
    ...
    if finished then
        ...
        local title=state.winner==0 and PBWT.L("ui_draw") or ui.mode=='local' and ('P'..state.winner..PBWT.L("ui_victory_suffix")) or state.winner==seat and PBWT.L("res_win") or PBWT.L("res_loss")
        self.resultText:SetText(title..(state.reason=='elder_scroll' and PBWT.L("res_by_scroll") or '')..'\n'..state.score[1]..'  ―  '..state.score[2]..'   /   '..state.turn..PBWT.L("res_turns"))
