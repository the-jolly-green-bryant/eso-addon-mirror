...
function YeOldePromoEvents:ShowTooltip(control)
	...
	local headerText = campaignData and campaignData:GetDisplayName()
		or GetString(SI_YEOLDEINFOS_PROMO_EVENTS_HEADER)
	...
	if not campaignData then
		...
		handler:AddLine(
			GetString(SI_YEOLDEINFOS_PROMO_EVENTS_NONE),
			"ZoFontGame",
...
function YeOldePromoEvents:InitializeSettingsMenu(controls)
	controls[#controls + 1] = {
		type = "checkbox",
		name = GetString(SI_YEOLDEINFOS_PROMO_EVENTS_HIDE_WHEN_NO_EVENT),
		tooltip = GetString(SI_YEOLDEINFOS_TT_PROMO_HIDE),
		getFunc = function()
