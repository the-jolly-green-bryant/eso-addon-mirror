...
function YeOldeCraftingInventory:GetTooltipLines(craftingType)
	...
	if mats == nil or #mats == 0 then
		result[1] = "|c888888" .. GetString(SI_YEOLDEINFOS_UNTRACKED_MATS) .. "|r"
		return result
	end
	...
	for _, matId in ipairs(mats) do
		...
		if info then
			...
			local invStr = zo_strformat(GetString(SI_YEOLDEINFOS_INV), ZO_CommaDelimitNumber(inv))
			local bankStr = zo_strformat(GetString(SI_YEOLDEINFOS_BANK), ZO_CommaDelimitNumber(bank))
