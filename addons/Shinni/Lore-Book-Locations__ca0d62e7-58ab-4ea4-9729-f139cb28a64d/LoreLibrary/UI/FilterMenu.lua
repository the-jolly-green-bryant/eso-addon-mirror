...
local FilterMenu = {}
LoreLibrary:RegisterModule("filterMenu", FilterMenu)
...
turn either kind of pin on/off. LoreLibrary.settings holds the actual on/off
state; these checkboxes just read/write it.
]]--

local PIN_TYPE_LABELS = LoreLibrary.pinTypeLabels
...
-- loaded (i.e. on console); IsConsoleUI() is true there.
function FilterMenu:InitializeKeyboard()
	...
	for _, pinTypeId in ipairs(LoreLibrary.PINTYPES) do
		...
		ZO_CheckButton_SetCheckState(checkBox, LoreLibrary.settings:IsPinTypeEnabled(pinTypeId))
		ZO_CheckButton_SetToggleFunction(checkBox, function(_, checked)
			LoreLibrary.settings:SetPinTypeEnabled(pinTypeId, checked)
		end)
...
function FilterMenu:RefreshKeyboardCheckBoxes()
	...
	for _, control in panel.checkBoxPool:ActiveObjectIterator() do
		...
		if pinTypeId then
			ZO_CheckButton_SetCheckState(control, LoreLibrary.settings:IsPinTypeEnabled(pinTypeId))
		end
	end
end
...
function FilterMenu:AddGamepadCheckBox(panel, pinTypeId)
	local function ToggleFunction(data)
		...
		LoreLibrary.settings:SetPinTypeEnabled(pinTypeId, data.currentValue)
		panel:BuildControls()
	...
	checkBox.currentValue = LoreLibrary.settings:IsPinTypeEnabled(pinTypeId)
