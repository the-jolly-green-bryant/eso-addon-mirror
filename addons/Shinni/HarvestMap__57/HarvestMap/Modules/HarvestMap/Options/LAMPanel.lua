...
function Settings:InitializeLAM()
	...
	optionsTable:insert({
		type = "submenu",
		name = Harvest.GetLocalization("styleoptions"),
		controls = submenuTable,
	})

	submenuTable:insert({
		type = "dropdown",
		name = Harvest.GetLocalization("restoredefaultlayout"),
		choices = Harvest.settings.presetNames,
		choicesValues = Harvest.settings.presets,
		getFunc = function()
			return Harvest.settings.presets[1]
		end,
		setFunc = function(preset)
			if preset.invalid then return end
			
			for _, pinTypeId in ipairs( Harvest.PINTYPES ) do
				if not Harvest.HIDDEN_PINTYPES[pinTypeId] then
					if preset[pinTypeId] then 
						Harvest.SetPinTypeTexture(pinTypeId, preset[pinTypeId].texture)
						Harvest.SetPinColor(pinTypeId, preset[pinTypeId].tint:UnpackRGBA())
					end
				end
			end
			if preset.worldBase then
				Harvest.SetWorldBaseTexture(preset.worldBase.texture)
			end
		end,
	})
	...
	submenuTable:insert({
		type = "iconpicker",
		name = Harvest.GetLocalization("worldBaseTexture"),
		getFunc = function()
			return Harvest.GetWorldBaseTexture()
		end,
		setFunc = function(newTexture)
			Harvest.SetWorldBaseTexture(newTexture)
		end,
		choices = Harvest.settings.availableWorldBaseTextures,
		default = Harvest.settings.defaultSettings.pinLayouts.worldBase.texture,
	})
