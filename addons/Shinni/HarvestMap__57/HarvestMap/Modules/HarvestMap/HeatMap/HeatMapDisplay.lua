...
function HarvestHeat.RefreshTiles()
	...
	if HarvestHeat.needsRefresh then
		if HarvestHeat.mapCache then
			HarvestHeat.mapCache:UnregisterAccess(HarvestHeat)
		end
			
		local mapMetaData = Harvest.mapTools:GetViewedMapMetaData()
		HarvestHeat.mapCache = Harvest.Data:GetMapCache(mapMetaData)
		HarvestHeat.mapCache:RegisterAccess(HarvestHeat)
...
function HarvestHeat.Initialize()
	...
	local function OnNodeChanged(event, mapCache, nodeId)
		...
		if HarvestHeat.mapCache ~= mapCache then return end
