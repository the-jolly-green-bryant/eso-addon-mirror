...
function MapCache:ForAllNodesInDivisionRange(worldX, worldY, distance, pinTypeId, callback, ...)
	...
	local range = zo_ceil(distance / self.DivisionWidthInMeters)
				...
				for j = startJ, endJ do
					...
					if division then
						for _, nodeId in pairs(division) do
							callback(self, pinTypeId, nodeId, ...)
						end
					end
				end
			end
...
function MapCache:GetVisibleNodes(x, y, pinTypeId, visibleDistanceInMeters, queue)
	...
	if visibleDistanceInMeters > 0 then
		...
		local maxDistance = visibleDistanceInMeters + 0.71 * self.DivisionWidthInMeters
