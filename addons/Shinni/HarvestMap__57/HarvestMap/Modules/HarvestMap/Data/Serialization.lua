...
function Serialization:LoadDownloadedData()
	...
	assert(#downloadedData % 8 == 0, "Downloaded data is corrupted. Try redownloading.")
			...
			if valid then
				...
				if nodeId then
					if (self.timestamp[nodeId] or 0) < timestamp then
						...
						-- current node is outdated, there is already a newer discovered node
						if self.nodeIndex[nodeId] then
							valid = false
						end
					end
