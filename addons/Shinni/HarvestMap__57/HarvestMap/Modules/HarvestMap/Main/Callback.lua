...
AddEvent("NODE_DELETION_REQUEST")
AddEvent("FIRST_OF_PINTYPE")
...
function CallbackManager:RegisterCallback(event, callback)
	assert(callback)
	for _, entry in pairs(self.callbacks[event]) do
		assert(entry ~= callback, "Callback was registered twice!")
	end
