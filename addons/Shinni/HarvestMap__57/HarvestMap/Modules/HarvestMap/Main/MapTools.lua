...
function MapTools:GetMapFromTexture(textureName)
	...
	textureName = string.gsub(textureName, "/u%d+_", "/uXX_")
...
function MapTools:GetViewedMapMetaData(forcePlayerZone)
	...
	local mapMetaData = self:GetMapMetaDataForZoneIndexAndMap(zoneIndex, map, mapId)
	
	return mapMetaData
end

function MapTools:GetMapMetaDataForZoneIndexAndMap(zoneIndex, map, mapId)
	...
	if not mapMetaData then
		...
		mapMetaData = {map = map, mapId = mapId, zoneIndex = zoneIndex, zoneId = zoneId, isBlacklisted = self:IsMapBlacklisted(map)}
		self.mapMetaData[zoneIndex][map] = mapMetaData	
	end
	mapMetaData.mapId = mapMetaData.mapId or mapId
	return mapMetaData
end
