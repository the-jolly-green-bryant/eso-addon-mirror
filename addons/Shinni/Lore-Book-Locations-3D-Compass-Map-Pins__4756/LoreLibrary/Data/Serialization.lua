...
--[[
LoreLibraryData[zoneId] is a string encoding every book in that zone.
...
local Serialization = {}
LoreLibrary:RegisterModule("serialization", Serialization)

function Serialization:Initialize()
	--[[
	self.childZoneIds = {}
	local parentZoneId, list
	for zoneId, data in pairs(LoreLibraryData) do
...
function Serialization:LoadZoneToCache(zoneCache)
	local data = LoreLibraryData[zoneCache.zoneId]
	if not data then
		return 0
	end

	assert(#data % 10 == 0, "corrupt LoreLibraryData entry: length is not a multiple of 10")
