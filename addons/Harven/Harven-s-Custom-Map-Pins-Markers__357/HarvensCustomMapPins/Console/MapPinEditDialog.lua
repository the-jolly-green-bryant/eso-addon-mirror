...
local Dialog = LibConsoleDialogs:Create("Enter Pin Description")
...
function HarvensCustomMapPinsEditDialog_Initialize(control, sv)
	...
	Dialog:AddSetting(
		{
			...
			label = GetString(SI_HARVEN_CMP_PIN_DESCRIPTION),
	...
	Dialog:AddSetting {
		...
		label = GetString(SI_HARVEN_CMP_PIN_ICON),
		setFunction = function(iconpicker, index)
			d("Selected icon: " .. index)
			self.icon = index
		end,
		getFunction = function()
			return self.icon
		...
		default = 1,
		items = HarvensCustomMapPinsIconList
	}
	Dialog:AddSetting {
		type = LibHarvensAddonSettings.ST_COLOR,
		label = GetString(SI_HARVEN_CMP_PIN_COLOR),
	...
	Dialog:AddSetting {
		...
		label = GetString(SI_HARVEN_CMP_PREDEFINED_PIN),
		...
		items = function()
			local items = {}
			for k, v in pairs(HarvensCustomMapPins.sv.predefined) do
	...
	HarvensCustomMapPins.dialog = Dialog
end
