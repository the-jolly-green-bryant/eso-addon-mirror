...
local function RefreshSkillInfoOrg(self, forceInit)
...
function HarvensImprovedSkillsWindow:Initialize()
	...
	local function createCheckbox(name, anchor, label, setting)
		...
		ZO_CheckButton_SetToggleFunction(
			...
			function()
				...
				local skillLineData = SKILLS_WINDOW:GetSelectedSkillLineData()
				if skillLineData and not skillLineData.isSubclassingNode then
	...
	function SKILLS_WINDOW:RefreshSkillLineInfo(...)
		local skillLineData = self:GetSelectedSkillLineData()
		...
		local lineRank = skillLineData:GetCurrentRank(skillType, skillIndex)
	...
	ZO_SkillsSkillInfoRank:SetDimensions(0, 0)
	ZO_SkillsSkillInfoRank:SetDimensionConstraints(60, 0, 200, 0)
