...
function R.Create()
    ...
    root:SetDimensions(385,34); root:SetAnchor(BOTTOMLEFT,GuiRoot,BOTTOMLEFT,sv.x,sv.y); root:SetScale(sv.scale or 1); root:SetMovable(true); root:SetMouseEnabled(true); root:SetClampedToScreen(true)
    ...
    root:SetHandler("OnMouseUp",function(self,mouseButton) if mouseButton==MOUSE_BUTTON_INDEX_LEFT and defaults().unlocked then self:StopMovingOrResizing(); sv.x=self:GetLeft(); sv.y=self:GetTop() end end)
    root:SetHandler("OnMoveStop",function(self) sv.x=self:GetLeft(); sv.y=self:GetTop() end)
    R.menu=button(root,"RAID CONTROLS",0,0,145,function(btn) showRaidControlsMenu(btn) end)
    R.readyButton=button(root,"READY CHECK",155,0,110,function() R.ReadyCheck() end)
    R.pullButton=button(root,"PULL "..tostring(sv.pullSeconds or 5).."s",275,0,110,function() R.StartPull() end)
...
function R.SetScale(scale)
    ...
    if R.root then
        ...
        R.root:ClearAnchors()
        R.root:SetAnchor(BOTTOMLEFT,GuiRoot,BOTTOMLEFT,sv.x or 248,sv.y or -58)
    end
end
