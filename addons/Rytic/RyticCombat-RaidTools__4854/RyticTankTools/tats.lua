...
function S.CloseWindow()
    local wasVisible = S.window and not S.window:IsHidden()
    if S.window then S.window:SetHidden(true) end

    -- Only release camera/UI mode when TankStats actually acquired it.
    -- Module OFF can be toggled from another UI (such as Settings); forcing
    -- SetGameCameraUIMode(false) there steals the cursor/input from that UI.
    if wasVisible and S.ownsCameraUIMode then
        if type(SetGameCameraUIMode)=="function" then SetGameCameraUIMode(false) end
        S.ownsCameraUIMode=false
    end
end
...
local function unregisterRuntime()
    ...
    EM:UnregisterForEvent("RyticTankTSEffects",EVENT_EFFECT_CHANGED)
    EM:UnregisterForUpdate("RyticTankTSTick")
end
...
function S.SetEnabled(enabled)
    ...
    if RyticTank.saved then
        RyticTank.saved.stats=RyticTank.saved.stats or {}
        RyticTank.saved.stats.enabled=on
    end
