...
function R.Create()
    ...
    root:SetDimensions(145,34); root:SetAnchor(BOTTOMLEFT,GuiRoot,BOTTOMLEFT,sv.x,sv.y); root:SetScale(sv.scale or 1); root:SetMovable(true); root:SetMouseEnabled(true); root:SetClampedToScreen(true)
    ...
    -- Root is anchored BOTTOMLEFT, so save offsets in that same coordinate system.
    -- Saving GetTop() as a BOTTOMLEFT Y offset clamps vertical movement and makes
    -- the control appear to move only side-to-side.
    local function saveRootPosition(self)
        local current=defaults()
        current.x=self:GetLeft() or current.x
        local rootBottom=self:GetBottom()
        local guiBottom=GuiRoot:GetBottom()
        if rootBottom and guiBottom then current.y=rootBottom-guiBottom end
    end
