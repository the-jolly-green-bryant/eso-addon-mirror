...
local function refreshProgressBar(control, data)
    local show = ArcanumGuildHall.db.showTomeProgressBar and not data.isFullyCompleted
...
local function setupTrackAndActionHandlers(control)
    control.trackButton:SetHandler("OnMouseEnter", function(button)
        ...
        control.trackButtonIcon:SetAlpha(1)
        if data.isTracked then
            showTooltip(button, ArcanumGuildHall.GetDefaultLocaleString("TOME_WINDOW_BUTTON_UNTRACK"))
        else
            showTooltip(button, ArcanumGuildHall.GetDefaultLocaleString("TOME_WINDOW_BUTTON_TRACK"))
        end
    end)
    ...
    control.trackButton:SetHandler("OnClicked", function()
        ...
        if not data.canTrack then
            if data.isTracked then
                ...
                ArcanumGuildHall:RefreshTomeWindow()
            end
            return
        end
    ...
    control.actionButton:SetHandler("OnMouseEnter", function(button)
        ...
        if data.canReroll then
            ...
            if data.usesGoldReroll and data.goldRerollCost then
                ...
                showTooltip(button, string.format("%s (%d |t16:16:%s|t)", ArcanumGuildHall.GetDefaultLocaleString("TOME_WINDOW_BUTTON_REROLL"), data.goldRerollCost, goldIcon))
            else
                showTooltip(button, string.format("%s (%d)", ArcanumGuildHall.GetDefaultLocaleString("TOME_WINDOW_BUTTON_REROLL"), data.rerollsLeft))
            end
        elseif data.showRerollUnavailable then
            showTooltip(button, ArcanumGuildHall.GetDefaultLocaleString("TOME_WINDOW_BUTTON_REROLL_UNAVAILABLE"))
        end
    end)
...
function ArcanumGuildHall:RestoreTomeWindowPosition()
...
function ArcanumGuildHall:SaveTomeWindowPosition()
    local control = self.tomeWindow and self.tomeWindow.control
    if not control then
        return
    end
...
function ArcanumGuildHall:RestoreTomeIconPosition()
...
function ArcanumGuildHall:GetTomePointsTotal()
    return GetCurrencyAmount(TOME_POINTS_CURRENCY_TYPE, CURRENCY_LOCATION_ACCOUNT)
end

function ArcanumGuildHall:CanClaimAllTomeRewards()
...
function ArcanumGuildHall:RefreshTomeViewModeButtonVisual(state)
...
function ArcanumGuildHall:RefreshTomeClaimAllButtonVisual()
...
function ArcanumGuildHall:RefreshTomeClaimAllButton()
    if not self.tomeWindow or not self.tomeWindow.claimAllButton then
        return
    end
...
function ArcanumGuildHall:IsTomeWindowCompactMode()
    return self.db.tomeWindowCompactMode == true
end

function ArcanumGuildHall:ApplyTomeWindowLayout()
...
function ArcanumGuildHall:UpdateTomeWindowPoints()
    if not self.tomeWindow then
        return
    end
...
function ArcanumGuildHall:GetTomeTypeTimeRemainingSeconds(activityType)
    local currentTime = GetTimeStamp()
...
function ArcanumGuildHall:GetTomeTypeTimeRemainingText(activityType)
...
function ArcanumGuildHall:UpdateTomeWindowTimeRemaining()
    if not self.tomeWindow then
        return
    end
...
function ArcanumGuildHall:StartTomeWindowTimeRemainingUpdate()
...
function ArcanumGuildHall:StopTomeWindowTimeRemainingUpdate()
    EVENT_MANAGER:UnregisterForUpdate(self.name .. "TomeWindowTimeRemaining")
end

function ArcanumGuildHall:ShowTomeWindow()
    if not self.tomeWindow then
        return
    end
...
function ArcanumGuildHall:HideTomeWindow()
    if not self.tomeWindow then
        return
    end
...
function ArcanumGuildHall:SetTomeIconVisible(enable)
...
function ArcanumGuildHall:InitializeTomeIcon()
    local control = ArcanumGuildHallTomeIcon
    ...
    moveHandle:SetHandler("OnMoveStop", function(self)
        ...
        WINDOW_MANAGER:SetMouseCursor(0)
        self:SetAlpha(0)
        ArcanumGuildHall:SaveTomeIconPosition()
    end)

    button:SetHandler("OnMouseEnter", function()
        icon:SetAlpha(1)
        showTooltip(button, ArcanumGuildHall.GetDefaultLocaleString("TOME_ICON_UI_OPEN"))
    end)
...
function ArcanumGuildHall:InitializeTomeWindow()
    local control = ArcanumGuildHallTomeWindow
    local listBg = control:GetNamedChild("ListBG")
    ...
    local function requestRefresh()
        ArcanumGuildHall.Debounce("ArcanumTomeWindowRefresh", 100, function()
    ...
    ZO_ScrollList_AddDataType(window.list, ROW_TYPE_TOME, "ArcanumGuildHallTomeRow", TOME_ROW_HEIGHT, setupTomeRow)
    ZO_ScrollList_AddDataType(window.compactList, ROW_TYPE_TOME_COMPACT, "ArcanumGuildHallTomeCompactEntry", TOME_COMPACT_ROW_HEIGHT, setupTomeCompactEntry)
...
function ArcanumGuildHall:RefreshTomeWindowIfVisible()
...
function ArcanumGuildHall:SetTomeWindowFilter(filterKey)
...
function ArcanumGuildHall:UpdateTomeTabState()
    if not self.tomeWindow then
        return
    end
...
function ArcanumGuildHall:BuildTomeWindowEntries()
    local entries = {}
...
function ArcanumGuildHall:RefreshTomeWindowNormal(entries)
    local window = self.tomeWindow
...
function ArcanumGuildHall:RefreshTomeWindowCompact(entries)
    local window = self.tomeWindow
...
function ArcanumGuildHall:RefreshTomeWindow()
    if not self.tomeWindow then
        return
    end
...
function ArcanumGuildHall:ToggleTomeWindow()
    if not self.tomeWindow then
        return
    end
...
ZO_CreateStringId("SI_BINDING_NAME_AA_TOME_WINDOW", ArcanumGuildHall.GetDefaultLocaleString("BINDING_OPEN_TOMES_WINDOW"))