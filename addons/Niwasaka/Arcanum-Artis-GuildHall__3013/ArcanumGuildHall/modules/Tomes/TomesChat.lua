...
local function printTomeProgress(index, currentProgress)
    ...
    if activityType == TIMED_ACTIVITY_TYPE_SEASONAL then
        color = ArcanumGuildHall.db.tomeColorSeasonal
    else
        color = ArcanumGuildHall.db.tomeColorWeekly
    end

    if currentProgress >= maxProgress then
        if ArcanumGuildHall.db.showTomeReward == false then
            if reward ~= "" then
                messageComplete = " " .. reward
            end
        else
            if reward ~= "" then
                messageComplete = " " .. res.Ccolor10 .. "[" .. reward .. "]"
            end
        end
    end

    if ArcanumGuildHall.db.showTomeIcon == false then
        message = color .. name .. " [" .. progressText .. runText .. "]" .. messageComplete .. "|r"
    else
        message = res.IconAA .. color .. " " .. name .. " [" .. progressText .. runText .. "]" .. messageComplete .. "|r"
    end

    if ArcanumGuildHall.db.locationTomeAlert == 0 then
...
local function tryClaimAvailableTomeRewards()
    if not IsTimedActivitySystemAvailable() or not ArcanumGuildHall.db.enableTomeAutoClaim then
        return
    end
...
function ArcanumGuildHall.TomeProgressUpdate(_, index, previousProgress, currentProgress)
    local activityType = GetTimedActivityType(index)
    if activityType ~= TIMED_ACTIVITY_TYPE_WEEKLY and activityType ~= TIMED_ACTIVITY_TYPE_SEASONAL then
        return
    end
    ...
    if activityType == TIMED_ACTIVITY_TYPE_SEASONAL and not ArcanumGuildHall.db.showSeasonalTomeProgress then
        if currentProgress < maxProgress then
            return
        end
    end
    ...
    if currentProgress >= maxProgress then
        if ArcanumGuildHall.db.enableTomeAutoClaim then
            ArcanumGuildHall.Debounce("ArcanumTomeAutoClaim", 300, function()
                tryClaimAvailableTomeRewards()
            end)
        elseif ArcanumGuildHall.db.enableTomeAlert then
            printTomeProgress(index, currentProgress)
        end
        return
    end

    if not ArcanumGuildHall.db.enableTomeAlert then
        return
    end
    ...
    ArcanumGuildHall.Debounce("ArcanumTomeProgressUpdate" .. index, 500, function()
        printTomeProgress(index, currentProgress)
    end)
end

function ArcanumGuildHall.TomeCurrencyUpdate(_, currencyType, currencyLocation)
    ...
    for index, claimData in pairs(pendingClaims) do
        if now - claimData.startedAt > 4000 then
            ...
            if numTimesClaimed > claimData.numTimesClaimed then
                if ArcanumGuildHall.db.enableTomeAlert then
                    local maxProgress = GetTimedActivityMaxProgress(index)
...
function ArcanumGuildHall.TomePlayerActivated()
    if not ArcanumGuildHall.db.enableTomeAutoClaim then
        return
    end

    ArcanumGuildHall.Debounce("ArcanumTomePlayerActivated", 1500, function()
        tryClaimAvailableTomeRewards()
    end)
end

function ArcanumGuildHall:InitializeTomeAlerts()
