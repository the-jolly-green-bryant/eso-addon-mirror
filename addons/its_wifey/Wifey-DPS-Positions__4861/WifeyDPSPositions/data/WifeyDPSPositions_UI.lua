...
local function showPlayerContextMenu(playerName, currentZoneId, mechKey, positionIndex, anchorControl, positionLabel)
    ...
    AddMenuItem("-- Whisper assignment --", function()
        StartChatInput(string.format("/w %s Your assignment: %s",
            playerName, positionLabel))
    end)
...
function UI:Create()
    ...
    local bg = wm:CreateControl("WifeyDPSPositionsPanel_BG", self.window, CT_BACKDROP)
    bg:SetAnchorFill(self.window)
    bg:SetCenterColor(0, 0, 0, 0.6)
    bg:SetEdgeColor(1, 1, 1, 0.3)
    self.bg = bg

    self.title = wm:CreateControl("WifeyDPSPositionsPanel_Title", self.window, CT_LABEL)
    self.title:SetFont("ZoFontWinH5")
    self.title:SetAnchor(TOP, self.window, TOP, 0, 10)
    self.title:SetText("Wifey DPS Positions")
    self.title:SetHorizontalAlignment(TEXT_ALIGN_CENTER)
    self.title:SetColor(0.6, 0.6, 0.6, 0.5)

    local titleButton = wm:CreateControl("WifeyDPSPositionsPanel_TitleButton", self.window, CT_BUTTON)
    ...
    titleButton:SetHandler("OnClicked", function()
        StartChatInput("DPS Positions: Mechanics will be automatically assigned. Type '+' if you're comfortable handling any mechanic.", CHAT_CHANNEL_PARTY)
    end)

    self.container = wm:CreateControl("WifeyDPSPositionsPanel_Container", self.window, CT_CONTROL)
            ...
            for i, entry in ipairs(savedZone[mechName]) do
                ...
                playerLabel:SetHandler("OnMouseUp", function(control, button, upInside)
                    ...
                    elseif button == MOUSE_BUTTON_INDEX_LEFT and playerName ~= myName then
                        local unitTag = DDP.GetUnitTagByName(playerName)
                        if unitTag then
                            StartChatInput(string.format("/w %s Your assignment: %s", playerName, pos))
                        else
...
function UI:Toggle()
    ...
    if self.hudFragment then
        ...
        -- Fallback only if HUD scenes were unavailable during initialization.
        self.window:SetHidden(false)
    end
end

SLASH_COMMANDS["/wdp"] = function()
    if _G.WifeyDPSPanel then
        _G.WifeyDPSPanel:Toggle()
    end
end
