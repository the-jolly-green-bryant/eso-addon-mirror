...
GSY.name = 'GammaScry'
GSY.displayname = 'GammaScry'
GSY.version = 'v1.0.0'
GSY.author = 'Latetide'
GSY.init = false
GSY.accountVariableVersion = 1
GSY.characterVariableVersion = 1
...
GSY.grouping = GSY.groupingEnum.Original -- from the enum above, used to pick the default selection in the combo
...
GSY.sorting = GSY.sortingEnum.Original -- should pick from the enum above, used to pick default from the combo, only used after loading
...
function GSY.ToggleDebug() 
    ...
    if GSY.isDebug then
        d("GammaScry: debug messages ON")
    else
        d("GammaScry: debug messages OFF")
    end
...
function GSY.InitUI()
    ...
    GSY.topLevelWindow = WINDOW_MANAGER:CreateControlFromVirtual("GammSrcy", cParent, "GSYHangingWindow") 
    GSY.topLevelWindow:ClearAnchors()
    GSY.topLevelWindow:SetAnchor(BOTTOMLEFT, cParent, TOPLEFT, 0, -100)
...
function GSY.Initialize()
    ...
    local function OnAntiquityLeadAcquired(event, antiquityId)
        ...
        d("|cFFAA33GammaScry - new lead:|r ["..name.."]") -- TODO: make this clickable?
    end
    
	SLASH_COMMANDS["/gsydbg"] = GSY.ToggleDebug
	SLASH_COMMANDS["/gsyins"] = GSY.InspectAntiquity
 
    EM:RegisterForEvent("GammaScry", EVENT_ANTIQUITY_LEAD_ACQUIRED, OnAntiquityLeadAcquired)
