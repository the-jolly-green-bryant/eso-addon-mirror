...
local function GetQuestTooltipText(questIds)
	...
	for _, questId in ipairs(questIds) do
		...
		table.insert(quests, FormatQuestName(questName, earned))
	end
...
local function GetPDTooltipText(pdung)
	return GetAchievementLink(pdung.achievement, LINK_TYPE_ACHIEVEMENT)
end
...
local function SPT_UpdateGUITable()
	local pd = SPT.ptsData
	SPT.GUI = {
		GSP = {
			{ 1, GS(SPT_GUI_CHAR_LEVEL),  GetSV(pd.Level),    SPT.ptsTots.Level,    GS(SPT_QUEST_NA)                    },
			{ 2, GS(SPT_GUI_MAIN_QUEST),  GetSV(pd.MainQ),    SPT.ptsTots.MainQ,    GetQuestTooltipText(SPT.data.MQ)    },
			...
			{ 7, zf("<<t:1>>", GZNBId(SPT.data.ZId.ZN.EA)), GetSV(pd.EndlArch), SPT.ptsTots.EndlArch, GetQuestTooltipText(SPT.data.EA) },
		},
...
function SPT:SetupSqsItem(control, data, selected)
	...
	local font = data.header and "ZoFontGamepad34" or "ZoFontGamepad27"
	...
	zone:SetFont(font);   zone:SetText(data.zone)
	ss:SetFont(font);     ss:SetText(data.skyshards)
	quests:SetFont(font); quests:SetText(data.quests)
...
function SPT:SetupGdqItem(control, data, selected)
	...
	zone:SetFont(font);     zone:SetText(data.zone)
	dungeon:SetFont(font);  dungeon:SetText(data.dungeon)
	progress:SetFont(font); progress:SetText(data.progress)
	SPT_SetSelectedVisual(control, selected)
end
...
function SPT:SetupGeneralItem(control, data, selected)
	...
	local progress = control:GetNamedChild("_Progress")
	source:SetFont(font);   source:SetText(data.source)
	progress:SetFont(font); progress:SetText(data.progress)
	SPT_SetSelectedVisual(control, selected)
end
...
function SPT:SetupValues()
	...
	SPT.keybindDescriptors = {
		...
		},
		{
			...
			name     = GetString(SI_GAMEPAD_BACK),
...
function SPT:HelpSlash()
	d(GS(SPT_MSG_CMD_TITLE))
	d(strF(GS(SPT_MSG_CMD_OPTION), (SPT.active and GS(SPT_MSG_DEACTVATE) or GS(SPT_MSG_ACTIVATE))))
end

function SPT:BadSlash()
	d(GS(SPT_MSG_BAD_SLASH))
	d(GS(SPT_MSG_CMD_TITLE))
	d(strF(GS(SPT_MSG_CMD_OPTION), (SPT.active and GS(SPT_MSG_DEACTVATE) or GS(SPT_MSG_ACTIVATE))))
end
...
local function SPT_Initialized(eventCode, addonName)
	...
	d(addonName .. " loaded, initializing SPT...")

	SPT_InitSetup()

	SLASH_COMMANDS["/whatsmissing"] = function(keyWord, argument)
...
local displayName = GetDisplayName() or ""
local accountName = string.lower(displayName):gsub("^%s*@?", ""):gsub("%s*$", "")
if accountName == "xbutch" then
	d(strF("Running SPT WIP build for %s.", displayName))
	EVENT_MANAGER:RegisterForEvent(SPT.AddonName, EVENT_ADD_ON_LOADED, SPT_Initialized)
else
	d(strF("Test addon locked for display name: %s", displayName ~= "" and displayName or "<empty>"))
end
