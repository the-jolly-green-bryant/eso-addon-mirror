---------------------------------------------------------------------
-- DM2_ParseFightStats.lua — v3.7.7 (rotation colors + history redesign; overlay default)
...
R._latestAnnouncementVersion = "3.7.7"
...
local function getAbilityIcon(abilityId)
  ...
  if ABILITY_ICON_CACHE[abilityId] then return ABILITY_ICON_CACHE[abilityId] end
  local icon
  if type(GetAbilityIcon) == "function" then
    local ok, tex = pcall(GetAbilityIcon, abilityId)
    if ok and tex and tex ~= "" then icon = tex end
  end
  if icon then ABILITY_ICON_CACHE[abilityId] = icon end
  return icon
end
