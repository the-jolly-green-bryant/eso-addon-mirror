...
local function encodeSignupTime(playerName)
    local currentSecs = GetTimeStamp()
    local absoluteMinutes = math.floor(currentSecs / 60) 
    local totalMinutes = (absoluteMinutes - 5760) % 10080
    local nameHash = HashString(tostring(playerName)) % 97
    local cryptoNum = (totalMinutes * 100) + nameHash + 10000
    return string.format("%04X", cryptoNum)
end
...
local function GuildEventsEventSignupInitialize(control)
    ...
    --local content   = GetControl("Content")
    --local acceptBtn = GetControl( "Accept")
    --local cancelBtn = GetControl( "Cancel")
    --local descLabel = GetControl( "Text")
...
--signUpForEvent: Signs the member up for the event with a hidden crypto timestamp
function GuildEventsUI:signUpForEvent()
    ...
    local timeToken = encodeSignupTime(GetDisplayName())
    local signupPayload = eventId.."/"..GuildEvents.roleSelected.."/"..timeToken
    ...
    if memberEvents == nil then
        ...
        if memberEvents == "" then
            newNote = newNote.."#"..signupPayload.."#"
        else
            newNote = newNote.."#"..memberEvents..";"..signupPayload.."#"
        end
    end
...
--getAttendingText: Returns a string of attendees with chronological queue numbers
function GuildEventsUI:getAttendingText(eventId, excludeMember)
    ...
    for i = 1, totalMembers do
        ...
        if name ~= excludeMember then
            ...
            if signUpInfo ~= nil then
                ...
                local rawInfo = tostring(signUpInfo)
                local minutesToken = nil
                local roleStr = rawInfo

                if string.find(rawInfo, "/") then
                    roleStr = string.sub(rawInfo, 1, string.find(rawInfo, "/") - 1)
                    minutesToken = string.sub(rawInfo, string.find(rawInfo, "/") + 1)
                else
                    roleStr = rawInfo
                    minutesToken = nil
                end

                local roleId = tonumber(roleStr)
                if roleId and GuildEvents.roles[roleId] and GuildEvents.roles[roleId].icon ~= nil then
                ...
                local currentMinutes = 10080

                if minutesToken ~= nil then
                    minutesToken = string.match(tostring(minutesToken), "([0-9A-Fa-f]+)")
                end
                
                if minutesToken ~= nil and type(minutesToken) == "string" then
                    local tokenLen = string.len(minutesToken)
                    if tokenLen == 4 or tokenLen == 5 then
                        local cryptoNum = tonumber(minutesToken, 16)
                        if cryptoNum then
                            local basis = cryptoNum - 10000
                            
                            local expectedHash = HashString(tostring(name)) % 97
                            
                            local storedHash = basis % 100
                            local mins = math.floor(basis / 100)
                            
                            if storedHash == expectedHash and mins >= 0 and mins <= 10080 then
                                currentMinutes = mins
                            end
                        end
                    end
                ...
                if currentMinutes < 10080 then
                    for k = 1, totalMembers do
                        local otherName, otherNote = GetGuildMemberInfo(GuildEventsUI.selectedGuildId, k)
                        local otherInfo = GuildEventsUI:getSignUpInfoForEventId(otherNote, eventId)
                        if otherInfo ~= nil then
                            local otherStr = tostring(otherInfo)
                            local otherToken = nil
                            
                            if string.find(otherStr, "/") then
                                otherToken = string.sub(otherStr, string.find(otherStr, "/") + 1)
                            end
                            
                            if otherToken ~= nil then
                               otherToken = string.match(tostring(otherToken), "([0-9A-Fa-f]+)")
                            end

                            
                            if otherToken ~= nil and type(otherToken) == "string" then
                                local oLen = string.len(otherToken)
                                if oLen == 4 or oLen == 5 then
                                    local oCrypto = tonumber(otherToken, 16)
                                    if oCrypto then
                                        local oBasis = oCrypto - 10000
                                        
                                        local expectedOtherHash = HashString(tostring(otherName)) % 97
                                        
                                        local oHash = oBasis % 100
                                        local oMins = math.floor(oBasis / 100)
                                        
                                        if oHash == expectedOtherHash and oMins < currentMinutes then
    ...
    local currentServerMinutes = (math.floor(GetTimeStamp() / 60) + 4320) % 10080

    table.sort(attendeesList, function(a, b)
        local posA = a.pos
        local posB = b.pos

        if posA and posB and posA < 9999 and posB < 9999 then
            if posA <= currentServerMinutes then posA = posA + 10080 end
            if posB <= currentServerMinutes then posB = posB + 10080 end
        end
    ...
    for idx, player in ipairs(attendeesList) do
        local finalName = player.text
        
        if player.pos < 9999 then
            finalName = "|cE6B800[" .. idx .. "]|r " .. finalName
        else
            finalName = "|c636563[X]|r " .. finalName
        end
...
function GuildEvents:inviteAttendees(eventId)
    ...
    for i = 1, #members do
        if members[i] ~= name then
            ...
            --Check if player is in group
            local isMemberInGroup = IsPlayerInGroup(members[i])
            local isMemberOffline = false

            -- Offline-Status ermitteln
            for z=1, GetNumGuildMembers(GuildEventsUI.selectedGuildId) do
                local memberName, _, _, memberPlayerStatus = GetGuildMemberInfo(GuildEventsUI.selectedGuildId, z)
...
function GuildEventsUI:getSignUpInfoForEventId(note, eventId)
    if note == nil or eventId == nil then return nil end
    
    local cleanNote = string.match(note, "#([^#]+)#")
    if not cleanNote then return nil end

    for entry in string.gmatch(cleanNote, "([^;]+)") do
        local prefix = eventId .. "/"
        if string.sub(entry, 1, string.len(prefix)) == prefix then
            return string.sub(entry, string.len(prefix) + 1)
        end
    end
...
function GuildEventsUI:ClearAllSignUps()
    local notesForUpdate = {}

    for i = 1, GetNumGuildMembers(GuildEventsUI.selectedGuildId) do
        local name, note = GetGuildMemberInfo(GuildEventsUI.selectedGuildId, i)
        
        if note ~= nil and string.find(note, "#") ~= nil then
            local newNote = string.gsub(note, "#[%d/;A-Fa-f]+#", "")
            if note ~= newNote then
                table.insert(notesForUpdate, {index = i, text = newNote})
            end
        end
    end
    ...
    for k, v in pairs(notesForUpdate) do
        zo_callLater(function() 
            GuildEvents:updateGuildMemberNote(GuildEventsUI.selectedGuildId, v.index, v.text, true)
        end, delay)
        
        sendCount = sendCount + 1
        
        if sendCount % 4 == 0 then
            delay = delay + 20000 
        else
            delay = delay + 5000 
        end
    ...
    if showProgressDialog and #notesForUpdate > 0 then
        showProgressDialog(#notesForUpdate)
        zo_callLater(function() if eventRemoveEnd then eventRemoveEnd() end end, delay)
    end
    d("[GE Reset] " .. string.format(L.chatResetStarted, #notesForUpdate))
end
