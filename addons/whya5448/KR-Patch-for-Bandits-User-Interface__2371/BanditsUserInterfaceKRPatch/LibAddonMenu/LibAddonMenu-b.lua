--Original LibAddonMenu-2.0 & its files © Ryan Lakanen (Seerah)

--Register LAM with LibStub
local lam=LibStub:NewLibrary("LibAddonMenu-b", 26) if not lam then return end --the same or newer version of this lib is already loaded into memory
local lam2=LibStub("LibAddonMenu-2.0",true) if not lam2 then lam2=LibStub:NewLibrary("LibAddonMenu-2.0", 1) end
local eventHandle="LibAddonMenu-b"
local lang=GetCVar("Language.2")
local LangString
local messages={}
local MESSAGE_PREFIX="[LAMb] "
local function PrintLater(msg)
	if CHAT_SYSTEM.primaryContainer then d(MESSAGE_PREFIX .. msg) else messages[#messages+1]=msg end
end

local function FlushMessages()
	for i=1, #messages do d(MESSAGE_PREFIX .. messages[i]) end
	messages={}
end
...
local optionsState={}
lam2.widgets=lam2.widgets or {}
local widgets=lam2.widgets
lam2.util=lam2.util or {}
local util=lam2.util
...
local function GetStringFromValue(value)
	if type(value)=="function" then return value()
	elseif type(value)=="number" then return GetString(value) end
	return value
end
...
local function CreateLabelAndContainerControl(parent, controlData, controlName)
	...
	container:SetDimensions(width/(control.isHalfWidth and 6 or 3), MIN_HEIGHT)
	...
	control:SetDimensions(width/(control.isHalfWidth and 2 or 1), MIN_HEIGHT)
...
local function GetTopPanel(panel)
	while panel.panel and panel.panel~=panel do panel=panel.panel end
	return panel
end

local function IsSame(objA, objB)
	if #objA~=#objB then return false end
	for i=1, #objA do if objA[i]~=objB[i] then return false end end
	return true
end
...
local function RequestRefreshIfNeeded(control)
	...
	if panel.data.registerForRefresh then cm:FireCallbacks("LAM-RefreshPanel", control) end
...
local function RegisterForRefreshIfNeeded(control)
	...
	if panelData.registerForRefresh or panelData.registerForDefaults then
		table.insert(panel.controlsToRefresh or {}, control) --prevent errors on custom panels
	end
end

local function RegisterForReloadIfNeeded(control)
	if control.data.requiresReload then
		table.insert(controlsForReload, control)
...
local function GetReloadDialog()
	if(not ESO_Dialogs[LAM_RELOAD_DIALOG]) then
		ESO_Dialogs[LAM_RELOAD_DIALOG]={
			canQueue=true,
			title={
				text=LangString["RELOAD_DIALOG_TITLE"],
			},
			mainText={
				text=LangString["RELOAD_DIALOG_TEXT"],
			},
			buttons={
				[1]={
					text=LangString["RELOAD_DIALOG_RELOAD_BUTTON"],
					callback=function() ReloadUI() end,
				},
				[2]={
					text=LangString["RELOAD_DIALOG_DISCARD_BUTTON"],
	...
	return ESO_Dialogs[LAM_RELOAD_DIALOG]
end
...
local function UpdateWarning(control)
	...
	if control.data.requiresReload then
		if not warning then
			warning=string.format("|cff0000%s", LangString["RELOAD_UI_WARNING"])
		else
			warning=string.format("%s\n\n|cff0000%s", warning, LangString["RELOAD_UI_WARNING"])
		end
	end
...
local localization={
	en={
		PANEL_NAME="|c4B8BFEBandits|r UI",
		PANEL_LONG_NAME="|c4B8BFEBandits|r User Interface",
	...
	ru={
		PANEL_NAME="|c4B8BFEБандитский|r Мод",
		VERSION="Версия: <<X:1>>",
	...
	br={ -- provided by mlsevero
		...
		RELOAD_UI_WARNING="Mudancas nessa configuracao requer a releitura da UI para ter efeito.",
		RELOAD_DIALOG_TITLE="Releitura da UI requerida",
		RELOAD_DIALOG_TEXT="Algumas mudancas requerem a releitura da UI para ter efeito. Voce deseja reler agora ou descartar as mudancas?",
		RELOAD_DIALOG_RELOAD_BUTTON="Reler",
		RELOAD_DIALOG_DISCARD_BUTTON="Descartar",
	},
}

LangString=ZO_ShallowTableCopy(localization[lang], localization["en"])
if not util.L then util.L = ZO_ShallowTableCopy(localization[lang], localization["en"]) end
...
--INTERNAL FUNCTION
--scrolls ZO_ScrollList `list` to move the row corresponding to `data` into view (does nothing if there is no such row in the list)
--unlike ZO_ScrollList_ScrollDataIntoView, this function accounts for fading near the list's edges-it avoids the fading area by scrolling
...
--otherwise returns a filter function, which takes a `data` table argument and returns true iff `data.filterText` matches the pattern
...
function lam2:RegisterWidget(widgetType, widgetVersion)
...
--	local srchEdit=LAMAddonSettingsWindow_b:GetNamedChild("SearchFilterEdit") srchEdit:Clear()
...
function lam:RegisterAddonPanel(addonID, panelData)
	...
	local panel=LAMCreateControl.panel(container, panelData, addonID) --addonID==global name of panel
	...
	local addonData={
		...
		filterText=stripMarkup(table.concat(filterParts, "\t")):lower(),
	}

	table.insert(addonsForList, addonData)
...
local function CreateAddonSettingsMenuEntry()
	local panelData={
		...
		name=LangString["PANEL_NAME"],
		longname=LangString["PANEL_LONG_NAME"],
	}
	...
	function panelData.callback()
		...
		local title=LAMAddonSettingsWindow_b:GetNamedChild("Title")
		title:SetText(panelData.longname)

		if not addonListSorted and #addonsForList>0 then
			local searchEdit=LAMAddonSettingsWindow_b:GetNamedChild("SearchFilterEdit")
...
local function CreateAddonSettingsWindow()
	local tlw=wm:CreateTopLevelWindow("LAMAddonSettingsWindow_b")
	tlw:SetHidden(true)
	...
	local logo=wm:CreateControl("$(parent)BUI_Logo", tlw, CT_TEXTURE)
	logo:SetTexture("/BanditsUserInterface/textures/Bandits_logo.dds")
	logo:SetDimensions(100, 100)
	logo:SetAnchor(TOPRIGHT, nil, TOPRIGHT, -20, 60)
	logo:SetDrawLayer(DL_BACKGROUND)
	...
	--Crate website info
	local website=wm:CreateControl("$(parent)Website", tlw, CT_BUTTON)
	website:SetClickSound("Click")
	website:SetFont(LangString["PANEL_INFO_FONT"])
	website:SetNormalFontColor(ZO_ColorDef:New("5959D5"):UnpackRGBA())
	website:SetMouseOverFontColor(ZO_ColorDef:New("B8B8D3"):UnpackRGBA())
	website:SetAnchor(TOPRIGHT, nil, TOPRIGHT, -125, 80)
	website:SetText(LangString["WEBSITE"])
	website:SetDimensions(website:GetLabelControl():GetTextDimensions())
	website.url=LangString["URL"]
	website:SetHandler("OnClicked",function(self)RequestOpenUnsafeURL(self.url)end)
	--Crate author info
	local info=wm:CreateControl("$(parent)Info", tlw, CT_LABEL)
	local version=tostring(BUI.Version) local l=string.len(version) while l<5 do version=version.."0" l=string.len(version) end
	info:SetFont(LangString["PANEL_INFO_FONT"])
	info:SetAnchor(TOPRIGHT,website,TOPLEFT,-10,0)
	info:SetText(zo_strformat(LangString["AUTHOR"], LangString["CREATOR"]).."  "..zo_strformat(LangString["VERSION"], version))
	--Divider
...
function lam:GetAddonSettingsFragment()
	...
	if not LAMAddonSettingsFragment_b then
		local window=CreateAddonSettingsWindow()
		LAMAddonSettingsFragment_b=ZO_FadeSceneFragment:New(window, true, 100)
		LAMAddonSettingsFragment_b:RegisterCallback("StateChange", function(oldState, newState)
	...
	return LAMAddonSettingsFragment_b
end
