...
function ShowMount.PopulateMounts()
    ...
    local mountsCategoryIndex, mountsCategoryId
    for i = 1, #ZO_COLLECTIBLE_DATA_MANAGER.collectibleCategoryIdToDataMap do
        if ZO_COLLECTIBLE_DATA_MANAGER.collectibleCategoryIdToDataMap[i]:GetName() == "Mounts" then
            mountsCategoryIndex = ZO_COLLECTIBLE_DATA_MANAGER.collectibleCategoryIdToDataMap[i].categoryIndex
            mountsCategoryId = i
            break
        end
    end

    local mountsCategory = ZO_COLLECTIBLE_DATA_MANAGER.collectibleCategoryIdToDataMap[mountsCategoryId]

    ShowMount.mountsCategoryIndex = mountsCategoryIndex
    ShowMount.mountsCategory = mountsCategory

    -- Each subcategory.
    for i = 1, mountsCategory:GetNumSubcategories() do
        -- Each collectible.
        for j = 1, mountsCategory.orderedSubcategories[i]:GetNumCollectibles() do
            if maxMatches == 0 then break end

            local mountId = GetCollectibleId(mountsCategoryIndex, i, j)
            local mount = mountsCategory.orderedSubcategories[i].orderedCollectibles[j]

             -- Add if unlocked.
            if GetCollectibleUnlockStateById(mountId) ~= 0 then
                local index = #ShowMount.mountsIds+1
                ShowMount.mountsIds[index]          = mountId
                ShowMount.mountsNames[index]        = mount:GetFormattedName()
                ShowMount.mountsLinks[index]        = GetCollectibleLink(mountId)
                ShowMount.mountsTextures[index]     = mount:GetIcon()

                -- For menu lists without affecting indexes.
                ShowMount.mountsNamesMenu[#ShowMount.mountsNamesMenu+1] = mount:GetFormattedName()
