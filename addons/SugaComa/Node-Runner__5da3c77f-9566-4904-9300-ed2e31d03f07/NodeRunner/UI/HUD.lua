...
local function SetHarvestDangerColor(label, remainingMs)
    if not label or type(label.SetColor) ~= "function" then return end

    local dangerMs = (tonumber(Project.Config.dangerHarvestSeconds) or 60) * 1000
    if (tonumber(remainingMs) or 0) > 0 and (tonumber(remainingMs) or 0) <= dangerMs then
        label:SetColor(1, 0.18, 0.18, 1)
    else
...
function HUD:Initialize()
    ...
    local root = wm:CreateTopLevelWindow("NodeRunnerHUD")
...
function HUD:Refresh(nowMs)
    ...
    if state:IsCountdown() then
        ...
        self.controls.title:SetText("NODE RUNNER")
        self.controls.harvest:SetText("STARTING IN " .. tostring(number))
        SetHarvestDangerColor(self.controls.harvest, nil)
    ...
    if state:IsRunning() then
        local current = tonumber(nowMs) or GetNowMs()
        local harvestRemaining = Project.Game:GetRemainingMs()
        local gatherRemaining = Project.Game:GetGatherRemainingMs(current)
        local survived = Project.Game:GetElapsedMs(current)

        self.controls.title:SetText("NODE RUNNER")
        self.controls.harvest:SetText("HARVEST  " .. FormatCountdownMs(harvestRemaining))
        SetHarvestDangerColor(self.controls.harvest, harvestRemaining)
        self.controls.clocks:SetText(string.format(
            "GATHER %s   •   SURVIVED %s",
    ...
    if state.phase == "finished" and state.lastResult then
        ...
        self.controls.status:SetText(result.newBest and "NEW PERSONAL BEST" or "NODE RUNNER COMPLETE")
...
function HUD:ShowFinishAlert(result)
    ...
    local text = string.format(
        "Node Runner complete — survived %s — %d nodes",
