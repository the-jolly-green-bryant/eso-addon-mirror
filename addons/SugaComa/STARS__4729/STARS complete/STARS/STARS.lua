...
-- 0.6.11: EVENT_KEEP_* is shared by every AvA objective represented as a
-- "keep" internally, including resources and Imperial City districts. STARS
-- records literal Cyrodiil keeps only. Fail closed when the client cannot
-- identify the objective so a new or unknown objective can never inflate the
-- keep totals.
function STARS:IsTrackableCyrodiilKeep(keepId)
    if not self:IsInCyrodiil() then return false end
    if IsInImperialCity and IsInImperialCity() == true then return false end
    if IsActiveWorldBattleground and IsActiveWorldBattleground() == true then return false end
    if not GetKeepType or KEEPTYPE_KEEP == nil then return false end

    local ok, keepType = pcall(GetKeepType, keepId)
    if not ok or keepType ~= KEEPTYPE_KEEP then
        Debug(string.format("Ignored non-keep objective: id=%s type=%s",
            tostring(keepId), tostring(ok and keepType or "unknown")))
        return false
    end
    return true
end
...
function STARS:QueueKeepContribution(kind, keepId)
    if not self:IsTrackableCyrodiilKeep(keepId) then return end
...
function STARS:OnKeepAllianceOwnerChanged(_, keepId, battlegroundContext, owningAlliance, oldOwningAlliance)
    if not self:IsEnabled() or not self:IsTrackableCyrodiilKeep(keepId) then return end
...
function STARS:InitSettingsMenu()
    ...
    settings:AddSetting({
        type=HAS.ST_LABEL,
        label="Artwork review only Ã¢ÂÂ removed before release",
    ...
    settings:AddSetting({
        type = HAS.ST_SECTION,
        subMenu = false,
        label = "|cFFD700Built on tea, toast and ADHD Ã¢ÂÂ tested live on PS5.|r\n" ..
