STARS_JOURNAL_KEYBOARD = STARS_JOURNAL_KEYBOARD or {}
local KeyboardJournal = STARS_JOURNAL_KEYBOARD
...
STARS_Journal_Keyboard = STARS_Journal_Gamepad:Subclass()

function STARS_Journal_Keyboard:New(control)
    local object = ZO_Object.New(self)
    object:Initialize(control)
    return object
end

function STARS_Journal_Keyboard:Initialize(control)
    ...
    STARS_JOURNAL_KEYBOARD_FRAGMENT = ZO_FadeSceneFragment:New(control)
    STARS_JOURNAL_KEYBOARD_FRAGMENT:RegisterCallback("StateChange", function(_, newState)
    ...
    STARS_JOURNAL_KEYBOARD_SCENE = ZO_Scene:New(SCENE_NAME, SCENE_MANAGER)
    if FRAGMENT_GROUP and FRAGMENT_GROUP.MOUSE_DRIVEN_UI_WINDOW then
        STARS_JOURNAL_KEYBOARD_SCENE:AddFragmentGroup(FRAGMENT_GROUP.MOUSE_DRIVEN_UI_WINDOW)
    end
    if UI_SHORTCUTS_ACTION_LAYER_FRAGMENT then
        STARS_JOURNAL_KEYBOARD_SCENE:AddFragment(UI_SHORTCUTS_ACTION_LAYER_FRAGMENT)
    end
    STARS_JOURNAL_KEYBOARD_SCENE:AddFragment(STARS_JOURNAL_KEYBOARD_FRAGMENT)
end

function STARS_Journal_Keyboard:ApplyContentFrameAnchors()
...
function STARS_Journal_Keyboard:SelectPage(pageId)
...
function STARS_Journal_Keyboard:RefreshChronicleButtons()
...
function STARS_Journal_Keyboard:ChangeKeyboardMemory(delta)
    self:ChangeChronicleMemory(delta)
    self:RefreshChronicleButtons()
end

function STARS_Journal_Keyboard:InitializeMouseControls()
    if not self.navigation then return end
    local pages = {
...
function KeyboardJournal:Initialize()
    ...
    STARS_JOURNAL_KEYBOARD_SCREEN = STARS_Journal_Keyboard:New(STARSJournalKeyboard)
