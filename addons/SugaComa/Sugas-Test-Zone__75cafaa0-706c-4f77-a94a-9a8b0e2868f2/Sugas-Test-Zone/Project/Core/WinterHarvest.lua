...
Project.WinterHarvest = Project.WinterHarvest or {}
local Game = Project.WinterHarvest

local UPDATE_NAME = Project.Config.hostAddonName .. "_WinterHarvestTick"
...
function Game:Initialize()
    ...
    if EVENT_LOOT_RECEIVED then
        Project.Events:Register("WinterHarvestLootReceived", EVENT_LOOT_RECEIVED, function(...)
            Game:OnLootReceived(...)
        end)
    end
    if EVENT_INVENTORY_SINGLE_SLOT_UPDATE then
        Project.Events:Register("WinterHarvestInventory", EVENT_INVENTORY_SINGLE_SLOT_UPDATE, function(...)
            Game:OnInventorySlotUpdated(...)
        end)
    end
    if EVENT_PLAYER_DEACTIVATED then
        Project.Events:Register("WinterHarvestPlayerDeactivated", EVENT_PLAYER_DEACTIVATED, function(...)
            Game:OnPlayerDeactivated(...)
        end)
    end

    self.initialized = true
    Project.Diagnostics:Log("Winter's Harvest foundation gameplay initialized", true)
end
...
Project.Controller:RegisterModule("WinterHarvest", Game)
