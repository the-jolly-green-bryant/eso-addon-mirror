...
local module = {
    ...
    description = "A gathering survival game built around three clocks: protect Harvest, keep gathering, and survive as long as you can.",
    defaultActive = true,
    entryIndex = 1,
}

function Presentations:GetRecordPresentation()
    local data = Project.WinterHarvest:EnsureRecords()
    ...
    if last then
        ...
        lines[#lines + 1] = ""
        lines[#lines + 1] = "Complete your first harvest to begin the record."
    end

    return {
        title = "WINTER'S HARVEST",
...
function Presentations:GetControlPresentation()
    ...
    elseif state:IsRunning() then
        ...
        lines = {
            "Harvest: " .. FormatCountdownMs(Project.WinterHarvest:GetRemainingMs()),
            "Gather: " .. FormatCountdownMs(Project.WinterHarvest:GetGatherRemainingMs(current)),
            "Survived: " .. FormatElapsedMs(Project.WinterHarvest:GetElapsedMs(current)),
...
function module:GetActions()
    ...
    return {
        primary = {
            name = state.phase == "finished" and "New Harvest" or "Start Harvest",
            visible = function()
            ...
            callback = function()
                ...
                return Project.WinterHarvest:StartRun()
            end,
        },
        secondary = {
            ...
            callback = function()
                module.entryIndex = 2
                Project.State.entryIndex = 2
                return Project.WinterHarvest:RestartRun()
            end,
        },
        tertiary = {
            name = "Cancel Harvest",
            ...
            callback = function()
                return Project.WinterHarvest:CancelRun("Harvest cancelled.")
            end,
        },
    }
end
...
function Presentations:Initialize()
    if not LibSTARSConnect or type(LibSTARSConnect.RegisterModule) ~= "function" then
        Project.Diagnostics:Warn("LibSTARSConnect unavailable; Winter's Harvest not registered")
        return
    end

    local ok, err = LibSTARSConnect:RegisterModule(module)
    if not ok then
        Project.Diagnostics:Warn("Winter's Harvest registration failed: " .. tostring(err))
        return
    end

    Project.Diagnostics:Log("Winter's Harvest registered with STARS", true)
end
