...
local PAGE_ORDER = { 6, 7, 2, 3, 4, 5 }

local function IsKeyboardMode()
    return not (IsInGamepadPreferredMode and IsInGamepadPreferredMode())
end

local function IsJournalShowing()
    return SCENE_MANAGER
        and type(SCENE_MANAGER.IsShowing) == "function"
        and SCENE_MANAGER:IsShowing(SCENE_NAME)
end
...
-- Keyboard bindings call these globals directly. They deliberately do
-- nothing unless the STARS scene is active, preserving the scene-only input
-- lifecycle used by the console release.
function STARS_PreviousPage()
    if IsJournalShowing() and STARS_JOURNAL_GAMEPAD then
        STARS_JOURNAL_GAMEPAD:NavigatePc(-1)
    end
end

function STARS_NextPage()
    if IsJournalShowing() and STARS_JOURNAL_GAMEPAD then
        STARS_JOURNAL_GAMEPAD:NavigatePc(1)
    end
end

function STARS_CloseJournal()
    if IsJournalShowing() and SCENE_MANAGER.HideCurrentScene then
        SCENE_MANAGER:HideCurrentScene()
    end
end
...
if ZO_CreateStringId then
    ...
    ZO_CreateStringId("SI_BINDING_NAME_STARS_PREVIOUS_PAGE", "STARS: Previous Page")
    ZO_CreateStringId("SI_BINDING_NAME_STARS_NEXT_PAGE", "STARS: Next Page")
    ZO_CreateStringId("SI_BINDING_NAME_STARS_CLOSE_JOURNAL", "STARS: Close Journal")
end
...
function STARS_Journal_Gamepad:Initialize(control)
    ...
    self.pcControls = control:GetNamedChild("PcControls")
    self:InitializePcControls()
...
function STARS_Journal_Gamepad:InitializePcControls()
    if not self.pcControls then return end

    local previousButton = self.pcControls:GetNamedChild("Previous")
    local nextButton = self.pcControls:GetNamedChild("Next")
    local previousMemoryButton = self.pcControls:GetNamedChild("PreviousMemory")
    local nextMemoryButton = self.pcControls:GetNamedChild("NextMemory")
    local closeButton = self.pcControls:GetNamedChild("Close")

    if previousButton then
        previousButton:SetHandler("OnClicked", function() self:NavigateSection(-1) end)
    end
    if nextButton then
        nextButton:SetHandler("OnClicked", function() self:NavigateSection(1) end)
    end
    if previousMemoryButton then
        previousMemoryButton:SetHandler("OnClicked", function() self:ChangeChronicleMemory(-1) end)
    end
    if nextMemoryButton then
        nextMemoryButton:SetHandler("OnClicked", function() self:ChangeChronicleMemory(1) end)
    end
    if closeButton then
        closeButton:SetHandler("OnClicked", STARS_CloseJournal)
    end
    ...
    if self.contentFrame then
        ...
        self.contentFrame:SetHandler("OnMouseWheel", function(_, delta)
            if IsKeyboardMode() then
                self:NavigatePc(delta > 0 and -1 or 1)
            end
        end)
    end
end

function STARS_Journal_Gamepad:NavigatePc(delta)
    delta = tonumber(delta) or 0
    if delta == 0 then return end

    -- Inside the Chronicle the same controls turn its pages first, matching
    -- L1/R1 on console. Every other view cycles through Journal sections.
    if self.currentPage == PAGE_CHRONICLE and self:GetChronicleMemoryCount() > 1 then
        self:ChangeChronicleMemory(delta)
        return
    end

    self:NavigateSection(delta)
end

function STARS_Journal_Gamepad:NavigateSection(delta)
    delta = tonumber(delta) or 0
    if delta == 0 then return end
    local currentIndex = 1
    for index, pageId in ipairs(PAGE_ORDER) do
        if pageId == self.currentPage then
            currentIndex = index
            break
        end
    end
    currentIndex = ((currentIndex - 1 + delta) % #PAGE_ORDER) + 1
    self:ShowPage(PAGE_ORDER[currentIndex])
end

function STARS_Journal_Gamepad:RefreshPcControls()
    if not self.pcControls then return end
    self.pcControls:SetHidden(not IsKeyboardMode())

    local previousButton = self.pcControls:GetNamedChild("Previous")
    local nextButton = self.pcControls:GetNamedChild("Next")
    local previousMemoryButton = self.pcControls:GetNamedChild("PreviousMemory")
    local nextMemoryButton = self.pcControls:GetNamedChild("NextMemory")
    local inChronicle = self.currentPage == PAGE_CHRONICLE and self:GetChronicleMemoryCount() > 1
    if previousButton then previousButton:SetText("Previous section") end
    if nextButton then nextButton:SetText("Next section") end
    if previousMemoryButton then previousMemoryButton:SetHidden(not inChronicle) end
    if nextMemoryButton then nextMemoryButton:SetHidden(not inChronicle) end
end
...
function STARS_Journal_Gamepad:ShowPage(page)
    ...
    if page == PAGE_CHRONICLE and self.chroniclePage then
        ...
        if (tonumber(chronicle.firstYear) or 0) > 0 and (tonumber(chronicle.lastYear) or 0) > 0 then
            yearsText = FormatYear(chronicle.firstYear) .. " â " .. FormatYear(chronicle.lastYear)
        end
    ...
    elseif page == PAGE_CAMPAIGN and self.campaignPage then
        ...
        if veterancy.active then
            ...
            SetLabel(self.campaignPage, "Subtitle",
                ...
                .. "  â¢  " .. FormatTimeRemaining(veterancy.timeRemainingS) .. " REMAINING")
    ...
    elseif page == PAGE_PRESTIGE and self.prestigePage then
        ...
        local recordSpan = "No dated achievement record"
        if (tonumber(chronicle.firstYear) or 0) > 0 and (tonumber(chronicle.lastYear) or 0) > 0 then
            recordSpan = FormatYear(chronicle.firstYear) .. " â " .. FormatYear(chronicle.lastYear)
        end
        SetLabel(self.prestigePage, "CoverRecord", string.format(
            "ACHIEVEMENT RECORD  %s   â¢   %s ACHIEVEMENTS   â¢   %s POINTS",
            recordSpan,
            FormatNumber(chronicle.completed),
            FormatNumber(chronicle.earnedPoints)))

        local busiestText = "No dated achievement history is available yet"
        if chronicle.busiest then
            busiestText = string.format("%s   â¢   %s achievements",
                ChronicleDateText(chronicle.busiest.year, chronicle.busiest.month, chronicle.busiest.day),
                FormatNumber(chronicle.busiest.count))
        end
        SetLabel(self.prestigePage, "CoverHighlights", "BUSIEST DAY\n" .. busiestText)

        local trackingStarted = STARS and STARS.sv and STARS.sv.stats
            and (STARS.sv.stats.trackingStarted
                or (STARS.sv.stats.underworld and STARS.sv.stats.underworld.trackingStarted)) or 0
        SetLabel(self.prestigePage, "CoverTracking",
            "STARS LEGACY RECORDED SINCE " .. FormatTrackingDate(trackingStarted))
        SetLabel(self.prestigePage, "Footer", "A MEMORY BOOK OF LIFE IN TAMRIEL")
