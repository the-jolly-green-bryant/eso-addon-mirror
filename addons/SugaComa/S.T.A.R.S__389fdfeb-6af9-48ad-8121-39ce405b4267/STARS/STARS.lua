...
local DEFAULTS = {
    ...
    stats = {
        ...
        underworld = {
            ...
            trackingStarted = 0,
            bladeOfWoeAbilityIds = {[146301]=true,[76325]=true,[77101]=true,[77102]=true},
        },
...
function STARS:RefreshJournal()
    local gamepad = self.JOURNAL and self.JOURNAL.GAMEPAD
    if gamepad and gamepad.ShowPage then
        gamepad:ShowPage(gamepad.currentPage or 6)
    end
end

function STARS:ResetUnderworldStats()
...
function STARS:ResetPvpStats()
    ...
    self.sv.stats.campaigns = { current = nil, history = {} }
...
function STARS:OnVeterancyUpdated()
    ...
    if STARS.JOURNAL.GAMEPAD and STARS.JOURNAL.GAMEPAD.ShowPage and STARS.JOURNAL.GAMEPAD.currentPage then
        STARS.JOURNAL.GAMEPAD:ShowPage(STARS.JOURNAL.GAMEPAD.currentPage)
    end
end
...
function STARS:IsBladeOfWoeAbility(abilityId, abilityName)
    ...
    if self.UNDERWORLD_ABILITY_IDS.BLADE_OF_WOE[numericId] == true then
        return true
    end
...
function STARS:GetUnderworldExecution(abilityId, abilityName)
    local numericId = tonumber(abilityId) or 0
    if self.UNDERWORLD_ABILITY_IDS.VAMPIRE_FEED[numericId] == true then
        return "vampireFeeds", "Vampire Feed", "lastVampireFeedMs"
    elseif self.UNDERWORLD_ABILITY_IDS.WEREWOLF_SLAUGHTER[numericId] == true then
...
-- Count the same keep-tick notification that supplies ESO's loot-history toast.
function STARS:OnKeepTickNotification(keepId, reason)
    if self.keepTrackingSuspended or not self:IsEnabled()
        or not self:IsInCyrodiil() or not IsInCyrodiil() then return end
    local key
    if reason == CURRENCY_CHANGE_REASON_OFFENSIVE_KEEP_REWARD then key = "keepsTaken"
    elseif reason == CURRENCY_CHANGE_REASON_DEFENSIVE_KEEP_REWARD then key = "keepsDefended"
    else return end
    if type(keepId) ~= "number" or keepId <= 0
        or GetKeepType(keepId) ~= KEEPTYPE_KEEP
        or GetKeepResourceType(keepId) ~= RESOURCETYPE_NONE then
        Debug("Keep tick ignored: objective=" .. tostring(keepId) .. " is not a keep")
        return
    end
    self:IncrementPvpLifetime(key, 1)
    self:IncrementCampaign(key, 1)
    Debug("Keep tick credited: " .. tostring(keepId) .. " " .. GetKeepName(keepId) .. " " .. key)
    self:RefreshJournal()
end

function STARS:InstallKeepTickHook()
    if self.keepTickHookInstalled then return end
    SecurePostHook(ZO_LootHistory_Shared, "OnKeepTickAwarded", function(history, keepId, reason)
        if history:CanShowItemsInHistory() then
            self:OnKeepTickNotification(keepId, reason)
        end
    end)
    self.keepTickHookInstalled = true
end
...
function STARS:ResetAllStats()
    ...
    self:EnsureVeterancySeason()
    self:TouchSV()
    if STARS.JOURNAL.GAMEPAD and STARS.JOURNAL.GAMEPAD.ShowPage then
        -- Page 6 is the memory-book cover. Profile formerly occupied page 1
        -- and no longer exists after the Veterancy merge.
        STARS.JOURNAL.GAMEPAD:ShowPage(STARS.JOURNAL.GAMEPAD.currentPage or 6)
    end
end
...
function STARS:UpgradeSavedVars()
    ...
    for abilityId in pairs(self.UNDERWORLD_ABILITY_IDS.BLADE_OF_WOE) do
        underworld.bladeOfWoeAbilityIds[abilityId] = true
    end
...
function STARS:RegisterResetDialogs()
    ...
    local function RegisterResetDialog(dialogName, title, message, actionText, callback)
        ZO_Dialogs_RegisterCustomDialog(dialogName, {
            gamepadInfo = { dialogType = GAMEPAD_DIALOGS and GAMEPAD_DIALOGS.BASIC },
            title = { text = title },
            mainText = { text = message .. "\n\nX - " .. actionText .. "\nO - Back" },
            buttons = {
                { text = actionText, keybind = "DIALOG_PRIMARY", callback = callback },
    ...
    RegisterResetDialog(
        "STARS_RESET_PVP",
        "Reset PvP?",
        "This clears PvP totals, Battleground totals and campaign history. Underworld and Prestige are unchanged.",
        "Reset PvP",
        function() STARS:ResetPvpStats() end)

    RegisterResetDialog(
        "STARS_RESET_PRESTIGE",
        "Reset Prestige?",
        "This resets only the STARS Prestige baseline. Champion Points and achievement memories remain intact.",
        "Reset Prestige",
        function() STARS:ResetPrestigeBaseline(); STARS:RefreshJournal() end)
end

function STARS:ShowResetDialog(dialogName)
...
function STARS:InitSettingsMenu()
    ...
    local function RefreshCurrentJournalPage()
        if STARS.JOURNAL.GAMEPAD and STARS.JOURNAL.GAMEPAD.ShowPage
            and STARS.JOURNAL.GAMEPAD.currentPage then
            STARS.JOURNAL.GAMEPAD:ShowPage(STARS.JOURNAL.GAMEPAD.currentPage)
        end
    end
    ...
    settings:AddSetting({type=HAS.ST_CHECKBOX,label="Enable STARS",getFunction=function() return self.sv.options.enabled end,setFunction=function(v)
        self.sv.options.enabled = v == true
        self:TouchSV()
    ...
    AddCategoryHeader("NOTIFICATIONS")
    ...
    settings:AddSetting({
        ...
        tooltip="Choose the data group first. Every reset uses X to confirm and O to go Back.",
    })
    settings:AddSetting({
        type=HAS.ST_BUTTON,
        label="Underworld Reset",
        buttonText="Reset",
        clickHandler=function() self:ShowResetDialog("STARS_RESET_UNDERWORLD") end,
    })
    settings:AddSetting({
        type=HAS.ST_BUTTON,
        label="PvP Reset",
        buttonText="Reset",
        clickHandler=function() self:ShowResetDialog("STARS_RESET_PVP") end,
    })
    settings:AddSetting({
        type=HAS.ST_BUTTON,
        label="Prestige Reset",
        tooltip="Returns STARS Prestige to its baseline only. It does not alter Champion Points or achievement memories.",
        buttonText="Reset",
        clickHandler=function() self:ShowResetDialog("STARS_RESET_PRESTIGE") end,
    })
...
local TRACKING_EVENTS = {
    ...
    { "STARS_CameraActivated", EVENT_GAME_CAMERA_ACTIVATED },
    { "STARS_CameraDeactivated", EVENT_GAME_CAMERA_DEACTIVATED },
    { "STARS_Resurrect", EVENT_RESURRECT_RESULT },
    { "STARS_Currency", EVENT_CURRENCY_UPDATE },
    { "STARS_RewardProgress", EVENT_REWARD_TRACK_PROGRESS_GAINED },
    { "STARS_RewardUpdate", EVENT_REWARD_TRACK_UPDATE_RECEIVED },
    { "STARS_RewardSettings", EVENT_REWARD_TRACK_SETTINGS_UPDATE_RECEIVED },
    { "STARS_Holidays", EVENT_HOLIDAYS_CHANGED },
    { "STARS_Battleground", EVENT_BATTLEGROUND_STATE_CHANGED },
}

function STARS:UnregisterTrackingEvents()
...
function STARS:RegisterTrackingEvents()
    if self.trackingEventsRegistered or not self:IsEnabled() then return end

    EVENT_MANAGER:RegisterForEvent("STARS_Experience", EVENT_EXPERIENCE_GAIN, function(_, reason, level, previousExperience, currentExperience, championPoints) self:OnExperienceGain(reason, level, previousExperience, currentExperience, championPoints) end)
    EVENT_MANAGER:RegisterForEvent("STARS_PendingExperience", EVENT_PENDING_EXPERIENCE_REWARD_CACHED, function(_, reason, amount) self:OnPendingExperienceReward(reason, amount) end)

    EVENT_MANAGER:RegisterForEvent("STARS_CombatSource", EVENT_COMBAT_EVENT, function(...) self:OnCombatEvent(...) end)
    ...
    EVENT_MANAGER:RegisterForEvent("STARS_Pickpocket", EVENT_JUSTICE_GOLD_PICKPOCKETED, function(...) self:OnJusticeGoldPickpocketed(...) end)
    EVENT_MANAGER:RegisterForEvent("STARS_CameraActivated", EVENT_GAME_CAMERA_ACTIVATED, function() self:OnGameCameraEvent("activated") end)
    EVENT_MANAGER:RegisterForEvent("STARS_CameraDeactivated", EVENT_GAME_CAMERA_DEACTIVATED, function() self:OnGameCameraEvent("deactivated") end)
    EVENT_MANAGER:RegisterForEvent("STARS_Resurrect", EVENT_RESURRECT_RESULT, function(...) self:OnResurrectResult(...) end)
    EVENT_MANAGER:RegisterForEvent("STARS_Currency", EVENT_CURRENCY_UPDATE, function(...) self:OnCurrencyUpdate(...) end)
    EVENT_MANAGER:RegisterForEvent("STARS_RewardProgress", EVENT_REWARD_TRACK_PROGRESS_GAINED, function(...) self:OnRewardTrackProgressGained(...) end)
    EVENT_MANAGER:RegisterForEvent("STARS_RewardUpdate", EVENT_REWARD_TRACK_UPDATE_RECEIVED, function() self:OnVeterancyUpdated() end)
    ...
    self:InstallKeepTickHook()
    EVENT_MANAGER:RegisterForEvent("STARS_KeepDeactivated", EVENT_PLAYER_DEACTIVATED, function() self.keepTrackingSuspended = true end)
    self.trackingEventsRegistered = true
end

function STARS:RefreshEventRegistrations()
...
function STARS:Initialize()
    ...
    self:EnsureCampaign()
    self:EnsureVeterancySeason()
    if STARS.JOURNAL and STARS.JOURNAL.Initialize then STARS.JOURNAL:Initialize() end
    if STARS.JOURNAL.Keyboard and STARS.JOURNAL.Keyboard.Initialize then
        STARS.JOURNAL.Keyboard:Initialize()
    end
