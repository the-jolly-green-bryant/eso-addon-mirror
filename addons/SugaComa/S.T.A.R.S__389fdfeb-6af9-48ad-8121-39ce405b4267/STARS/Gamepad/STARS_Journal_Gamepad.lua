...
local GamepadJournalScreen = ZO_Gamepad_ParametricList_Screen:Subclass()
Journal.GamepadClass = GamepadJournalScreen
...
function GamepadJournalScreen:New(control)
    local object = ZO_Object.New(self)
    object:Initialize(control)
    return object
end

function GamepadJournalScreen:ApplyContentFrameAnchors()
...
function GamepadJournalScreen:ApplyThreeColumnLayout(page, topY, bodyHeight, bodyGap)
...
function GamepadJournalScreen:ApplyTwoColumnLayout(page, topY, bodyHeight, bodyGap)
    if not page or not self.contentFrame then return end
    local width = self.contentFrame:GetWidth() or 0
    if width <= 0 then return end
...
function GamepadJournalScreen:ApplyVeterancyLayout()
...
function GamepadJournalScreen:ApplyMemoryBookCoverLayout()
...
function GamepadJournalScreen:ApplyChronicleLayout()
...
function GamepadJournalScreen:ApplyResponsiveLayout()
...
function GamepadJournalScreen:Initialize(control)
    ...
    Journal.gamepadFragment = ZO_FadeSceneFragment:New(control)
    Journal.gamepadFragment:RegisterCallback("StateChange", function(_, newState)
    ...
    Journal.gamepadScene = ZO_Scene:New(SCENE_NAME, SCENE_MANAGER)
    Journal.gamepadScene:AddFragmentGroup(FRAGMENT_GROUP.GAMEPAD_DRIVEN_UI_WINDOW)
    Journal.gamepadScene:AddFragmentGroup(FRAGMENT_GROUP.FRAME_TARGET_GAMEPAD)
    Journal.gamepadScene:AddFragment(GAMEPAD_NAV_QUADRANT_1_BACKGROUND_FRAGMENT)
    ...
    if GAMEPAD_NAV_QUADRANT_2_3_4_BACKGROUND_FRAGMENT then
        Journal.gamepadScene:AddFragment(GAMEPAD_NAV_QUADRANT_2_3_4_BACKGROUND_FRAGMENT)
    elseif GAMEPAD_NAV_QUADRANT_2_3_BACKGROUND_FRAGMENT then
        Journal.gamepadScene:AddFragment(GAMEPAD_NAV_QUADRANT_2_3_BACKGROUND_FRAGMENT)
    end
    Journal.gamepadScene:AddFragment(GAMEPAD_GENERIC_FOOTER_FRAGMENT)
    Journal.gamepadScene:AddFragment(GAMEPAD_MENU_SOUND_FRAGMENT)
    if FRAME_EMOTE_FRAGMENT_SOCIAL then
        Journal.gamepadScene:AddFragment(FRAME_EMOTE_FRAGMENT_SOCIAL)
    end
    Journal.gamepadScene:AddFragment(Journal.gamepadFragment)
    ...
    ZO_Gamepad_ParametricList_Screen.Initialize(
        ...
        ACTIVATE_ON_SHOW,
        Journal.gamepadScene
    )
    self:SetListsUseTriggerKeybinds(true)
end

function GamepadJournalScreen:OnDeferredInitialize()
    self.headerData = {
        titleText = "STARS",
...
function GamepadJournalScreen:GetChronicleMemoryCount()
...
function GamepadJournalScreen:ChangeChronicleMemory(delta)
...
function GamepadJournalScreen:InitializeKeybindStripDescriptors()
...
function GamepadJournalScreen:OnShowing()
...
function GamepadJournalScreen:OnHiding()
    ZO_Gamepad_ParametricList_Screen.OnHiding(self)
end

function GamepadJournalScreen:PerformUpdate()
    self.dirty = false
end

function GamepadJournalScreen:RefreshList()
...
local function RefreshPrestigeHeraldry(parent, progression, fallbackIcon)
    ...
    if enabled and view and type(STARS.BADGES) == "table"
        and type(STARS.BADGES.BuildModel) == "function"
        and type(STARS.BADGES.Render) == "function" then
        --[[
        ...
        if preview and preview.enabled == true then
            if preview.mode == "prestige"
                and type(STARS.BADGES.BuildPrestigePreviewModel) == "function" then
                modelOk, model = pcall(
                    STARS.BADGES.BuildPrestigePreviewModel,
                    STARS.BADGES,
                    preview.prestigeTier,
                    preview.prestigeRankKey)
            elseif type(STARS.BADGES.BuildLegacyPreviewModel) == "function" then
                modelOk, model = pcall(
                    STARS.BADGES.BuildLegacyPreviewModel,
                    STARS.BADGES,
                    preview.rankKey,
                    preview.stage)
            end
        else
            modelOk, model = pcall(STARS.BADGES.BuildModel, STARS.BADGES, progression)
        end
        ]]

        local modelOk, model = pcall(STARS.BADGES.BuildModel, STARS.BADGES, progression)
        if modelOk and model then
            local renderOk, rendered = pcall(STARS.BADGES.Render, STARS.BADGES, view, model)
    ...
    if view then
        if type(STARS.BADGES) == "table" and type(STARS.BADGES.Release) == "function" then
            pcall(STARS.BADGES.Release, STARS.BADGES, view)
        else
            view:SetHidden(true)
        end
    end
...
function GamepadJournalScreen:ShowPage(page)
...
function Journal:Initialize()
    ...
    Journal.GAMEPAD = GamepadJournalScreen:New(control)
