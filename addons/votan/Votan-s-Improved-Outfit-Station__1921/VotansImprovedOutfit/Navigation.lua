...
function addon:InitStyleList()
    ...
    local function filterBaseStyle(data)
        ...
        local style = icon:match("^/esoui/art/icons/gear_([^_]+)_") or ""
        return baseStyles[style] ~= 1
    end
            ...
            function ShowMenu(...)
                ...
                if not self.selectedFashion and self.selectedStyle ~= style and #style > 0 then
                    AddCustomMenuItem(
                        ...
                        function()
                            if ZO_RESTYLE_STATION_KEYBOARD.contentSearchEditBox then
                                ZO_RESTYLE_STATION_KEYBOARD.contentSearchEditBox:SetText("")
                            end
                            if ITEM_SET_COLLECTIONS_BOOK_KEYBOARD.contentSearchEditBox then
                                ITEM_SET_COLLECTIONS_BOOK_KEYBOARD.contentSearchEditBox:SetText("")
                            end
