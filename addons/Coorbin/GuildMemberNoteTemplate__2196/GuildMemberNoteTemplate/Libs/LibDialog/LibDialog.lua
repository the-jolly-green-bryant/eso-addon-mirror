...
local MAJOR, MINOR = "LibDialog", 1.21
...
function lib:RegisterDialog(uniqueAddonName, uniqueDialogName, title, body, callbackYes, callbackNo, callbackSetup, forceUpdate)
    ...
    assert (titleStr ~= nil, string.format("[libDialog]Error: Missing title for dialog with the unique identifier \'%s\', addon \'%s\'!", tostring(uniqueDialogName), tostring(uniqueAddonName)))
    assert (bodyStr ~= nil, string.format("[libDialog]Error: Missing body text for dialog with the unique identifier \'%s\', addon \'%s\'!", tostring(uniqueDialogName), tostring(uniqueAddonName)))
    ...
    if not forceUpdate then
        assert(dialogs[uniqueDialogName] == nil, string.format("[libDialog]Error: Dialog with the unique identifier \'%s\' is already registered for the addon \'%s\'!", tostring(uniqueDialogName), tostring(uniqueAddonName)))
    end
...
function lib:ShowDialog(uniqueAddonName, uniqueDialogName, data)
    --Show the dialog now
    local dialogName = uniqueAddonName .. "_" .. uniqueDialogName
    assert(ESO_Dialogs[dialogName] ~= nil, string.format("[libDialog]Error: Dialog with the unique identifier \'%s\' does not exist in ESO_Dialogs, addon \'%s\'!", tostring(uniqueDialogName), tostring(uniqueAddonName)))
    --Is there already a dialog for this addon and does the uniqueDialogName already exist?
    local dialogs = existingDialogs[uniqueAddonName]
    assert(dialogs ~= nil and dialogs[uniqueDialogName] ~= nil, string.format("[libDialog]Error: Dialog with the unique identifier \'%s\' does not exist for the addon \'%s\'!", tostring(uniqueDialogName), tostring(uniqueAddonName)))
    --Show the dialog now
