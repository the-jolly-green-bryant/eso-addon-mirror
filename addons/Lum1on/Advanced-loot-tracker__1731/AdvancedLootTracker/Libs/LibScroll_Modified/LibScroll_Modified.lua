...
local MAJOR, MINOR = "LibScroll_Modified", 1
local libScroll_Modified, oldminor = LibStub:NewLibrary(MAJOR, MINOR)
if not libScroll_Modified then return end	--the same or newer version of this lib is already loaded into memory 
...
local function UpdateScrollList(scrollList, dataTable)
	...
	for k, dataItem in ipairs(dataTableCopy) do
		...
		-- @Lum1on:
		table.insert(dataList, 1.0, entry)
...
local function CreateScrollList(scrollData)
	...
	local template = scrollData.rowTemplate or "ZO_SelectableLabel" -- ZO_WindowTitle
	...
	local selectTemplate = scrollData.selectTemplate -- @Lum1on: or "ZO_ThinListHighlight"
...
function libScroll_Modified:CreateScrollList(scrollData)
