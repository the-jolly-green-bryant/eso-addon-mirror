...
function LibPrice.CanATTPrice()
  return ArkadiusTradeTools and ArkadiusTradeTools.Modules and ArkadiusTradeTools.Modules.Sales and
    ArkadiusTradeTools.Modules.Sales.addMenuItems and
    true
end
...
function LibPrice.ATTPrice(item_link)
  ...
  if
  not (ArkadiusTradeTools and ArkadiusTradeTools.Modules and ArkadiusTradeTools.Modules.Sales and
    ArkadiusTradeTools.Modules.Sales.addMenuItems)
  then
    return nil
  end
  ...
  for _, day_ct in ipairs({ self.day_ct_short, self.day_ct_long }) do
    att = ArkadiusTradeTools.Modules.Sales:GetAveragePricePerItem(
      item_link,
      GetTimeStamp() - (ZO_ONE_DAY_IN_SECONDS * day_ct)
    )
...
function LibPrice.FurCPrice(item_link)
  ...
  local item_id = FurC.Utils.GetItemId(item_link)
  local recipe_array = FurC.Find(item_link)
  if not recipe_array then
    return nil
  end
  ...
  if func then
    currency_type, currency_ct, currency_notes, ingredient_list = func(item_link, recipe_array)
  end
...
function LibPrice.From_FurC_Crafting(item_link, recipe_array)
  local self = LibPrice
  if not recipe_array.blueprint then
    return nil, nil
  end
  local total_ingr_gold = 0
  local ingr_list = {}
  local notes = "Crafting cost"
  local blueprint_link = FurC.Utils.GetItemLink(recipe_array.blueprint)
...
function LibPrice.From_FurC_Rolis(item_link, recipe_array)
  ...
  local version_data, found_seller_index = nil, nil

  for sellerIndex, seller in ipairs(seller_list) do
    version_data = seller[recipe_array.version]
    if version_data and version_data[item_id] then
      found_seller_index = sellerIndex
      break
    end
  end

  if not (version_data and version_data[item_id]) then
    return nil
  end
  local ct = version_data[item_id]
  return self.CURRENCY_TYPE_WRIT_VOUCHERS, ct, seller_names[found_seller_index]
end
...
function LibPrice.From_FurC_PVP(item_link, recipe_array, currency_type)
  ...
  local entry, found_vendor, found_location = nil, nil, nil
  ...
  local notes = (found_vendor or "?") .. " in " .. (found_location or "?")
...
function LibPrice.TTCPriceNormalize(ttc)
  ...
  if ttc.Avg ~= nil then
    table.insert(
      prices,
      { type = LibPrice.PRICE_ASK, [LibPrice.CURRENCY_TYPE_GOLD] = ttc.Avg, count = ttc.EntryCount }
    )
  end
  if ttc.SuggestedPrice ~= nil then
    table.insert(
      prices,
      {
        type = LibPrice.PRICE_AVG,
        [LibPrice.CURRENCY_TYPE_GOLD] = ttc.SuggestedPrice / 0.8,
        count = math.ceil(ttc.EntryCount * 0.3)
      }
    )
  end
  if ttc.SaleAvg ~= nil then
    table.insert(
      prices,
      { type = LibPrice.PRICE_SALE, [LibPrice.CURRENCY_TYPE_GOLD] = ttc.SaleAvg, saleCount = ttc.SaleEntryCount }
    )
  end
  return prices
end
