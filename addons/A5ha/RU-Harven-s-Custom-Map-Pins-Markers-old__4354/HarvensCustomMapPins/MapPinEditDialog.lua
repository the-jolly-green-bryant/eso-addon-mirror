...
function Dialog:SetupDialog(dialog, data)
	...
	if data.color then
		...
		if data.edit and data.pin then
			dialog.info.title.text = "Метка (Замена)"
		elseif data.edit then
			dialog.info.title.text = "Метка (Редактирование)"
		else
			dialog.info.title.text = "Метка (Создание)"
		end
...
function Dialog:Initialize(control, sv)
	...
	ZO_CheckButton_SetLabelText(advanced, "Расширенные настройки")
