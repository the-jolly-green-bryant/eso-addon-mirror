...
function HarvensCustomMapPins:InjectKeystrip()
	...
	KEYBIND_STRIP.AddKeybindButtonGroup = function(keybindStripManager, strip, ...)
		...
		if found then
			table.insert(strip, {name = "Поставить метку", keybind = "UI_SHORTCUT_NEGATIVE", callback = HarvensCustomMapPins_WorldMapPlacePin})
...
function HarvensCustomMapPins:SetupOptions()
	...
	local settings = LibHarvensAddonSettings:AddAddon("Harven's Custom Map Pins (RU)")
	...
	local pinLevel = {
		type = LibHarvensAddonSettings.ST_SLIDER,
		label = "Уровень слоя метки",
		min = 2,
		max = 200,
		step = 10,
		format = "%d",
		tooltip = "Чем выше число, тем выше уровень слоя метки.",
		getFunction = function()
	...
	settings:AddSetting {
		...
		label = "Распределенный рендеринг",
		tooltip = "Распределение рендеринга с течением времени для уменьшения нагрузки на CPU",
		getFunction = function()
	...
	settings:AddSetting {
		type = LibHarvensAddonSettings.ST_CHECKBOX,
		label = "Разрешить подфильтры меток" .. zo_iconFormat("esoui/art/miscellaneous/eso_icon_warning.dds", 28, 28),
		tooltip = "Разрешить отображение/скрытие предварительных определений (вкладка фильтров карты). UI Будет перезапущен!",
		getFunction = function()
