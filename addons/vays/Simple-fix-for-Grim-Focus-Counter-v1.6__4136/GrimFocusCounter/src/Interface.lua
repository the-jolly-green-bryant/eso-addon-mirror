...
function GFC:UpdateUI()
    ...
    elseif stacks >= GFC.procStacks - 1 and stacks < GFC.procStacks then
        self:SetSkillColorOverlay('four')
    elseif stacks >= GFC.procStacks * 2 and GFC.preferences.overlay.overproc then
        self:SetSkillColorOverlay('overproc')
    elseif stacks >= GFC.procStacks then
...
function GFC:UpdateStacks(stackCount)
    ...
    if stackCount > 0 then
        ...
        if stackFrame > 5 then stackFrame = 5 end
    else
