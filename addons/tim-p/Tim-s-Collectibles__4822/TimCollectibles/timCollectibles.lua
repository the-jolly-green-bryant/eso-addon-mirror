...
local function OnAddOnLoaded( eventCode, addonName )
  if (addonName ~= timCollectibles.name) then return end

  ZO_CreateStringId("SI_BINDING_NAME_tmC_Mount_Player", "Mount Group Member")
  ZO_CreateStringId("SI_BINDING_NAME_tmC_Mount_RandomFav", "Set Mount to Random Favorite")
  ZO_CreateStringId("SI_BINDING_NAME_tmC_Mount_Random", "Set Mount to Random")

  EVENT_MANAGER:UnregisterForEvent(timCollectibles.name, EVENT_ADD_ON_LOADED)

  for id, code in pairs(timCollectibles.collectibles) do
...
function timCollectibles.PrintCooldownOrUseCollectible(collectibleID)
...
function timCollectibles.SetRdmMount(mountID, friendlyName)
  SetRandomMountType(mountID)
  df("|cff0055Equipping " .. friendlyName .. " Mount...|r")
end

function timCollectibles.HandleAssistant(id)
	local duration = GetFrameTimeMilliseconds() - (PressTime or 0)			
	local currentId = GetActiveCollectibleByType(COLLECTIBLE_CATEGORY_TYPE_ASSISTANT)

	if duration > 500 and currentId == id then
		-- HOLD = dismiss (only if he's active and you have held the button)
		if GetActiveCollectibleByType(COLLECTIBLE_CATEGORY_TYPE_ASSISTANT) == id then
			UseCollectible(id)
		end
	else
		-- TAP = summon-only
...
function timCollectibles.DistanceToUnit(unitID)
...
-- Function shamelessly stolen from RidinDirty
function timCollectibles.MountPlayer()
  ...
  for iD = 1, GetGroupSize() do
    ...
    if playerDisplayName ~= GetUnitDisplayName("player") and IsUnitOnline(playerID) and IsUnitInGroupSupportRange(playerID) and isMountable and timCollectibles.DistanceToUnit(playerID) < 5.0 then
...
EVENT_MANAGER:RegisterForEvent(timCollectibles.name, EVENT_ADD_ON_LOADED, OnAddOnLoaded)
