...
local SPAULDER_AURA_OF_PRIDE_ABILITY_IDS = {
    [SPAULDER_AURA_OF_PRIDE_ABILITY_ID] = true,
}
...
function Greed:IsPlayerCrouchedForSpaulder(stealthState)
    -- EVENT_STEALTH_STATE_CHANGED supplies the state that caused the event.
    -- Trust that value before querying the live APIs, which can still report
    -- the previous state while the callback is running.
    if type(stealthState) == "number" then
    ...
    if type(IsUnitSneaking) == "function" then
        ...
        if ok and type(sneaking) == "boolean" then return sneaking end
    end
    ...
    if type(GetUnitStealthState) == "function" then
        ...
        if ok and type(state) == "number" then
            local noneState = type(STEALTH_STATE_NONE) == "number" and STEALTH_STATE_NONE or 0
            return state ~= noneState and state ~= 0
        end
    end
...
function Greed:IsPlayerDeadForSpaulder()
    if self.spaulderPlayerDead == true then return true end

    if type(IsUnitDeadOrReincarnating) == "function" then
        local ok, dead = pcall(function()
            return IsUnitDeadOrReincarnating("player")
        end)
        if ok and dead == true then
            self.spaulderPlayerDead = true
            return true
        end
    end
...
local function IsSpaulderPromptLocationEligible()
    if type(GetCurrentZoneHouseId) == "function" then
        local houseId = GetCurrentZoneHouseId()
        if type(houseId) == "number" and houseId ~= 0 then
            return false
        end
    end

    if type(IsActiveWorldBattleground) == "function" and IsActiveWorldBattleground() then return false end
    if type(IsInImperialCity) == "function" and IsInImperialCity() then return false end
    if type(IsInCyrodiil) == "function" and IsInCyrodiil() then return false end
    if type(IsPlayerInAvAWorld) == "function" and IsPlayerInAvAWorld() then return false end

    if type(IsDelveOrPublicDungeon) == "function" and IsDelveOrPublicDungeon() then return true end
    if type(IsInstanceEndlessDungeon) == "function" and IsInstanceEndlessDungeon() then return true end
    ...
    if type(GetCurrentZoneDungeonDifficulty) == "function" then
        ...
        if type(difficulty) == "number" and difficulty > (DUNGEON_DIFFICULTY_NONE or 0) then
            return true
        end
    end
...
function Greed:ClearSpaulderTransitionGuard()
    self.spaulderTransitionToken = (self.spaulderTransitionToken or 0) + 1
    self.spaulderTransitionActive = false
    -- Establish the current crouch state as the post-transition baseline.
    -- Resetting this to false caused a duplicate crouched callback after a
    -- door or reload to be misread as a fresh Spaulder toggle.
    self.spaulderPromptWasCrouched = self:IsPlayerCrouchedForSpaulder()
end
...
function Greed:CancelSpaulderPendingReactivationChecks()
    ...
    self.spaulderAuraRevalidationExpectedActive = nil
    self.spaulderAuraRevalidationStateGeneration = nil
    self.spaulderTransitionToken = (self.spaulderTransitionToken or 0) + 1
    self.spaulderTransitionActive = false
    self:CancelSpaulderFadeToggleConfirmation()
end

function Greed:IsSpaulderAuraCurrentlyActive()
    local prompts = self.savedVars and self.savedVars.textPrompts
    return self.spaulderAuraEventActive == true
        or self.spaulderAuraAssumedActive == true
        or (type(prompts) == "table" and prompts.spaulderAuraActive == true)
end

function Greed:ToggleSpaulderAuraFromCrouch()
    self:CancelSpaulderPendingReactivationChecks()
    local keepCrouchedBaseline = self.spaulderPromptWasCrouched == true

    if self:IsSpaulderAuraCurrentlyActive() then
        self:ResetSpaulderAuraState()
        self.spaulderPromptWasCrouched = keepCrouchedBaseline
        self.spaulderExpectedToggleActive = false
...
function Greed:OnSpaulderStealthStateChanged(_, unitTag, stealthState)
    ...
    if crouched == wasCrouched then
        return
    end

    self.spaulderPromptWasCrouched = crouched

    -- Stealth state can be rebuilt while loading. Record the new baseline but
    -- never interpret it as a user Spaulder toggle during the transition.
    ...
    if self:IsPlayerDeadForSpaulder() == true then
        self:RefreshSpaulderTextPrompt()
        return
    end

    local spaulderEquipped = self:IsSpaulderEquipped()
    if self:IsPromptEnabled("spaulder") ~= true or spaulderEquipped ~= true then
        self:RefreshSpaulderTextPrompt()
        return
    end

    self:ToggleSpaulderAuraFromCrouch()
end
...
function Greed:ResetSpaulderAuraState()
    ...
    self.spaulderPromptWasCrouched = self:IsPlayerCrouchedForSpaulder() == true
    self:ClearSpaulderUserToggleExpectation()
    self:CancelSpaulderFadeToggleConfirmation()
...
function Greed:MarkSpaulderAuraActive(active)
    ...
    local prompts = self.savedVars and self.savedVars.textPrompts
    local persistedChanged = type(prompts) == "table" and prompts.spaulderAuraActive ~= nextActive
    local stateChanged = not wasKnown or wasActive ~= nextActive
    ...
    self.spaulderAuraRestoredFromReload = false
    if stateChanged or persistedChanged then
        self:PersistSpaulderAuraState(nextActive, self:GetCurrentSpaulderZoneId())
    end
    if nextActive then
        if stateChanged or type(self.spaulderAuraActivatedAt) ~= "number" then
            self.spaulderAuraActivatedAt = GetFrameTimeMilliseconds and GetFrameTimeMilliseconds() or nil
        end
    else
        if stateChanged or self.spaulderAuraActivatedAt ~= nil then
            self.spaulderAuraActivatedAt = nil
            self.spaulderAuraExpirationToken = (self.spaulderAuraExpirationToken or 0) + 1
        end
    end
...
function Greed:UpdateSpaulderAuraState(spaulderEquipped, auraDetected)
    if spaulderEquipped == true and auraDetected == true then
        self:MarkSpaulderAuraActive(true)
    end
end
...
function Greed:IsSpaulderAuraAssumedActive(auraDetected)
    local prompts = self.savedVars and self.savedVars.textPrompts
    return auraDetected == true
        or self.spaulderAuraEventActive == true
        or self.spaulderAuraAssumedActive == true
        or (type(prompts) == "table" and prompts.spaulderAuraActive == true)
end

local function IsSpaulderAuraOfPrideAbilityId(abilityId)
    abilityId = tonumber(abilityId)
    return abilityId ~= nil and SPAULDER_AURA_OF_PRIDE_ABILITY_IDS[abilityId] == true
end

function Greed:PlayerHasAuraOfPride()
    return self.spaulderAuraEventActive == true or self.spaulderAuraAssumedActive == true
end

function Greed:OnSpaulderEffectChanged(_, changeType, effectSlot, effectName, unitTag, beginTime, endTime, stackCount, iconName, buffType, effectType, abilityType, statusEffectType, unitName, unitId, abilityId, sourceType)
    if not IsSpaulderAuraOfPrideAbilityId(abilityId) then return end
    if self:IsPlayerDeadForSpaulder() == true then return end

    -- Aura of Pride may be reported as a player effect, a ground-area effect,
    -- or with an empty unit tag. The exact ability-id event filter is the
    -- authoritative identity; reject only clearly unrelated unit tags.
    if unitTag ~= nil and unitTag ~= "" and unitTag ~= "player" and unitTag ~= "groundaoe" then
        return
    end

    local gained = EFFECT_RESULT_GAINED ~= nil and changeType == EFFECT_RESULT_GAINED
    local updated = EFFECT_RESULT_UPDATED ~= nil and changeType == EFFECT_RESULT_UPDATED
    local fullRefresh = EFFECT_RESULT_FULL_REFRESH ~= nil and changeType == EFFECT_RESULT_FULL_REFRESH
    local transferred = EFFECT_RESULT_TRANSFER ~= nil and changeType == EFFECT_RESULT_TRANSFER

    -- Exact Aura of Pride active events are optional positive evidence. A late
    -- event after a crouch-off edge cannot overwrite that newer edge.
    if gained or updated or fullRefresh or transferred then
    ...
    if EFFECT_RESULT_FADED ~= nil and changeType == EFFECT_RESULT_FADED then
        if self.spaulderTransitionActive == true then
            -- Internal doors can emit a temporary fade. Preserve the last
            -- active state until a real player crouch edge or a gained event.
            return
        end

        -- Crouch activation toggles Spaulder state. Standing and unrelated fade
        -- events are not authoritative deactivation signals.
    end
end

function Greed:ScheduleSpaulderAlivePromptRefresh()
    self.spaulderAuraRevalidationToken = (self.spaulderAuraRevalidationToken or 0) + 1
    local token = self.spaulderAuraRevalidationToken

    local function refreshAfterAlive()
        if self.spaulderAuraRevalidationToken ~= token then return end
        self.spaulderPlayerDead = false
        self:RefreshSpaulderTextPrompt()
    end
    ...
    zo_callLater(refreshAfterAlive, SPAULDER_AURA_REVALIDATE_DELAY_MS)
end

function Greed:OnSpaulderPlayerActivated(_, initial)
    self:ClearSpaulderTransitionGuard()
    if self:IsPlayerDeadForSpaulder() == true then
        self:RefreshSpaulderTextPrompt()
        return
    end
    ...
    if self:IsSpaulderEquipped() ~= true then
        self:ResetSpaulderAuraState()
    elseif self.spaulderAuraEventActive ~= true and self.spaulderAuraAssumedActive ~= true then
        ...
        -- saved state is therefore the reliable reload continuation signal.
        local restored = self:RestorePersistedSpaulderAuraState()
...
function Greed:OnSpaulderPlayerDead()
    self.spaulderPlayerDead = true
    self:CancelSpaulderPendingReactivationChecks()
    self:ResetSpaulderAuraState()
    self:RefreshSpaulderTextPrompt()
...
function Greed:OnSpaulderPlayerAlive()
    local wasDead = self.spaulderPlayerDead == true
    self.spaulderPlayerDead = false

    if wasDead then
        self:CancelSpaulderPendingReactivationChecks()
        self:ResetSpaulderAuraState()
        self:ScheduleSpaulderAlivePromptRefresh()
        return
    end
...
function Greed:RegisterSpaulderCombatWatcher()
    ...
    local effectEventName = self.name .. "SpaulderAuraEffectChanged"
    if EVENT_EFFECT_CHANGED ~= nil then
        EVENT_MANAGER:RegisterForEvent(effectEventName, EVENT_EFFECT_CHANGED, function(...)
            self:OnSpaulderEffectChanged(...)
        end)
        if type(EVENT_MANAGER.AddFilterForEvent) == "function"
            and REGISTER_FILTER_ABILITY_ID ~= nil then
            pcall(function()
                EVENT_MANAGER:AddFilterForEvent(effectEventName, EVENT_EFFECT_CHANGED, REGISTER_FILTER_ABILITY_ID, SPAULDER_AURA_OF_PRIDE_ABILITY_ID)
            end)
        end
    end
    ...
    if EVENT_INVENTORY_SINGLE_SLOT_UPDATE ~= nil then
        EVENT_MANAGER:RegisterForEvent(inventoryEventName, EVENT_INVENTORY_SINGLE_SLOT_UPDATE, function(_, bagId, slotIndex)
            local shoulderUpdate = bagId == BAG_WORN and slotIndex == EQUIP_SLOT_SHOULDERS
            local spaulderEquipped = true
            if shoulderUpdate then
                spaulderEquipped = self:IsSpaulderEquipped()
            end
            if shoulderUpdate and spaulderEquipped ~= true then
                self:ResetSpaulderAuraState()
            end
            self:RefreshSpaulderTextPrompt()
        end)
...
function Greed:RefreshSpaulderTextPrompt()
    ...
    local controls = self.spaulderPromptControls
    if not controls or not controls.window then
        return
    end
    ...
    local unlocked = settings.locked ~= true
    local playerDead = self:IsPlayerDeadForSpaulder() == true

    if not enabled or playerDead then
    ...
    self:UpdateSpaulderAuraState(spaulderEquipped, auraDetected)

    local auraAssumedActive = self:IsSpaulderAuraAssumedActive(auraDetected)
    local shouldShow = spaulderEquipped
        and self.spaulderTransitionActive ~= true
        and not auraAssumedActive
        and IsSpaulderPromptLocationEligible()

    local showPreview = unlocked and self:IsDropOptionsWindowVisible()
    local showLivePrompt = shouldShow and not showPreview
    local setHidden = not showPreview and not showLivePrompt

    controls.window:SetHidden(setHidden)
    if controls.previewBackdrop then
...
function Greed:StartTextPromptWatcher()
    ...
    -- Restore the last crouch-authoritative state because ESO does not replay
    -- an already-active Spaulder effect event to the new Lua environment.
