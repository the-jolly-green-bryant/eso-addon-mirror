...
function Greed:IsOwnedItemIndicatorRefreshReady()
    return self.ownedPlayerActivated == true
        and self.savedVars ~= nil
        and self.controls ~= nil
        and self.controls.window ~= nil
        and self.displayFavorites ~= nil
        and self.visibleColumns ~= nil
end

function Greed:RequestInitialOwnedItemIndicatorRefresh()
    self.ownedInitialRefreshPending = true
    self.ownedItemIndexDirty = true
end

function Greed:RefreshCollectionStatus()
    if not self:IsOwnedItemIndicatorRefreshReady() then
        self:RequestInitialOwnedItemIndicatorRefresh()
        return
    end
...
function Greed:RefreshOwnedItemIndicators()
    ...
    self.ownedInitialRefreshPending = false
    local mainVisible = self.controls and self.controls.window and not self.controls.window:IsHidden()
    local pickerVisible = type(self.HasVisiblePiecePickerWindow) == "function" and self:HasVisiblePiecePickerWindow()
    if not mainVisible and not pickerVisible then
        self.ownedItemIndexDirty = true
...
function Greed:OnOwnedPlayerActivated()
    self.ownedPlayerActivated = true
    if self.ownedInitialRefreshPending == true or self.ownedItemIndexDirty == true then
        self:QueueOwnedItemIndicatorsRefresh()
    end
end
...
function Greed:RegisterOwnedInventoryRefreshEvents()
    ...
    if EVENT_PLAYER_ACTIVATED ~= nil then
        EVENT_MANAGER:RegisterForEvent(self.name .. "OwnedPlayerActivated", EVENT_PLAYER_ACTIVATED, function()
            self:OnOwnedPlayerActivated()
        end)
    else
        self.ownedPlayerActivated = true
        self:QueueOwnedItemIndicatorsRefresh()
...
function Greed:GetPieceOwnershipTargetCount(column, piece)
    if not column then return 1 end

    if column.key == "ring" then
        return math.max(1, math.floor(tonumber((piece and piece.total) or column.total or 2) or 2))
    end

    if column.kind == "weapon" then
        local weaponKey = (piece and piece.type) or column.key
        if TWO_COPY_WEAPON_KEYS[weaponKey] == true then
            return 2
        end
    end
...
function Greed:ShouldDisplayOwnershipQuantity(column, piece, neededCount)
    return (tonumber(neededCount) or self:GetPieceOwnershipTargetCount(column, piece)) > 1
end

function Greed:GetOwnershipQuantityLabel(collectedCount, neededCount)
    neededCount = math.max(1, math.floor(tonumber(neededCount) or 1))
    collectedCount = math.max(0, math.floor(tonumber(collectedCount) or 0))
    collectedCount = math.min(collectedCount, neededCount)
    return string.format("%d/%d", collectedCount, neededCount)
end

function Greed:GetPieceCounts(column, piece, setData, ownedItemIndex)
    local needed = self:GetPieceOwnershipTargetCount(column, piece)
