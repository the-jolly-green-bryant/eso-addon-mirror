...
local GREED_TRADE_CHAT_FALLBACK_MAX_CHARS = 1000
local GREED_TRADE_CHAT_SAFETY_MARGIN_CHARS = 12
...
local GREED_TRADE_DEBUG_MAX_LINES = 20
...
local function GetTradeLootItemIdKey(itemId)
    itemId = tonumber(itemId)
    if not itemId or itemId <= 0 then return nil end
    return tostring(math.floor(itemId))
end
...
function Greed:GetTradeLootSession()
    if type(self.tradeLootSession) ~= "table" then
        self.tradeLootSession = {
            ...
            itemsByUniqueId = {},
            orderedUniqueIds = {},
            lootedItemCountsByItemId = {},
            pendingLoot = {},
            recentSlots = {},
            validationDirty = true,
            preparedBatches = nil,
            preparedBatchMetadata = nil,
            preparedBatchIndex = 0,
            preparedSnapshotHasNewerLoot = false,
        }
    end

    self.tradeLootSession.itemsByUniqueId = self.tradeLootSession.itemsByUniqueId or {}
    self.tradeLootSession.orderedUniqueIds = self.tradeLootSession.orderedUniqueIds or {}
    self.tradeLootSession.lootedItemCountsByItemId = self.tradeLootSession.lootedItemCountsByItemId or {}
    self.tradeLootSession.pendingLoot = self.tradeLootSession.pendingLoot or {}
    self.tradeLootSession.recentSlots = self.tradeLootSession.recentSlots or {}
...
function Greed:ClearTradeLootPreparedBatches()
    ...
    if type(session.preparedBatches) == "table" and #session.preparedBatches > 0 then
        session.preparedSnapshotHasNewerLoot = true
        return
    end
    session.preparedBatches = nil
    session.preparedBatchIndex = 0
    session.validationDirty = true
    session.preparedBatchMetadata = nil
    session.preparedSnapshotHasNewerLoot = false
end
...
function Greed:GetCurrentTradeActivityIdentity()
    ...
    local difficulty = noneDifficulty
    if type(GetCurrentZoneDungeonDifficulty) == "function" then
        local okDifficulty, resolvedDifficulty = pcall(GetCurrentZoneDungeonDifficulty)
        if okDifficulty and type(resolvedDifficulty) == "number" then
            difficulty = math.max(noneDifficulty, math.floor(resolvedDifficulty))
        end
    end
    ...
    local parentZoneId = zoneId or 0
    ...
    local kind = "group"
    if reviveCounterRaid or raidId > 0 then
        kind = "trial"
    elseif difficulty > noneDifficulty then
        kind = "dungeon"
    end
...
function Greed:StartTradeLootRun(activity)
    ...
    session.lootedItemCountsByItemId = {}
    session.pendingLoot = {}
    session.recentSlots = {}
    session.validationDirty = true
    session.preparedBatches = nil
...
function Greed:RefreshTradeLootActivityState()
    ...
    if activity then
        if session.activityKey == activity.key and (session.state == "active" or type(session.preparedBatches) == "table") then
            session.state = "active"
            session.activity = activity
            return
        end
...
function Greed:GetTradeLootSlotTradeabilityStatus(bagId, slotIndex)
    if not self:HasTradeLootValidationApis() then return "missing-api" end

    local itemLink = self:GetSafeBagItemLink(bagId, slotIndex)
    if not itemLink or itemLink == "" then return "missing" end

    local okTradeable, tradeable = pcall(IsItemBoPAndTradeable, bagId, slotIndex)
    if not okTradeable or tradeable ~= true then return "bound" end
...
function Greed:IsTradeLootSlotTradeable(bagId, slotIndex)
    return self:GetTradeLootSlotTradeabilityStatus(bagId, slotIndex) == "eligible"
end
...
function Greed:RecordTradeLootedItemId(session, itemId, quantity)
    local itemIdKey = GetTradeLootItemIdKey(itemId)
    if not session or not itemIdKey then return end

    quantity = tonumber(quantity) or 1
    quantity = math.max(1, math.floor(quantity))
    session.lootedItemCountsByItemId = session.lootedItemCountsByItemId or {}
    session.lootedItemCountsByItemId[itemIdKey] = (session.lootedItemCountsByItemId[itemIdKey] or 0) + quantity
end
...
function Greed:RecordTradeLootEvent(itemLink, itemId, quantity)
    ...
    self:RecordTradeLootedItemId(session, itemId, quantity)
    self:ClearTradeLootPreparedBatches()
...
function Greed:ValidateTradeLootSessionItems(diagnostics)
    ...
    if type(diagnostics) == "table" then
        local lootedRunItems = 0
        for _, lootedCount in pairs(session.lootedItemCountsByItemId or {}) do
            lootedRunItems = lootedRunItems + (tonumber(lootedCount) or 0)
        end
        local trackedRunItems = math.max(#(session.orderedUniqueIds or {}), lootedRunItems)
        diagnostics.trackedRunItems = trackedRunItems
        diagnostics.stillPresent = 0
        diagnostics.eligibleForTrade = 0
        diagnostics.rejectedBound = 0
        diagnostics.rejectedExpired = 0
        diagnostics.rejectedMissing = 0
        diagnostics.trackedPresentByUniqueId = {}
        diagnostics.presentCountsByItemId = {}
    end
    ...
    local foundCountsByItemId = {}
    local scanCandidates = {}
    local lootedCounts = session.lootedItemCountsByItemId or {}
    for _, bagId in ipairs(self:GetOwnedBagIds()) do
        ...
        if bagSize > 0 then
            for slotIndex = 0, bagSize - 1 do
                local snapshot = self:GetTradeLootSlotSnapshot(bagId, slotIndex)
                if snapshot then
                    local remembered = wanted[snapshot.uniqueId]
                    local itemIdKey = GetTradeLootItemIdKey(snapshot.itemId)
                    local wasLootedItemId = itemIdKey and (lootedCounts[itemIdKey] or 0) > 0
                    local status = self:GetTradeLootSlotTradeabilityStatus(bagId, slotIndex)
                    if type(diagnostics) == "table" and (remembered or wasLootedItemId) then
                        diagnostics.stillPresent = diagnostics.stillPresent + 1
                        if remembered then
                            diagnostics.trackedPresentByUniqueId[snapshot.uniqueId] = true
                        end
                        if itemIdKey then
                            diagnostics.presentCountsByItemId[itemIdKey] = (diagnostics.presentCountsByItemId[itemIdKey] or 0) + 1
                        end
                        if status == "bound" then
                            diagnostics.rejectedBound = diagnostics.rejectedBound + 1
                        elseif status == "expired" then
                            diagnostics.rejectedExpired = diagnostics.rejectedExpired + 1
                        end
                    end

                    if status == "eligible" then
                        if remembered then
                            local itemId = remembered.itemId or snapshot.itemId
                            foundByUniqueId[snapshot.uniqueId] = {
                                uniqueId = snapshot.uniqueId,
                                itemId = itemId,
                                itemLink = snapshot.itemLink,
                                order = remembered.order or 0,
                            }
                            itemIdKey = GetTradeLootItemIdKey(itemId)
                            if itemIdKey then
                                foundCountsByItemId[itemIdKey] = (foundCountsByItemId[itemIdKey] or 0) + 1
                            end
                        elseif itemIdKey and (lootedCounts[itemIdKey] or 0) > 0 then
                            table.insert(scanCandidates, snapshot)
                        end
                    end
                end
            end
    ...
    for _, snapshot in ipairs(scanCandidates) do
        local itemIdKey = GetTradeLootItemIdKey(snapshot.itemId)
        local remainingForItemId = (lootedCounts[itemIdKey] or 0) - (foundCountsByItemId[itemIdKey] or 0)
        if remainingForItemId > 0 and not wanted[snapshot.uniqueId] and not foundByUniqueId[snapshot.uniqueId] then
            session.itemOrder = (session.itemOrder or 0) + 1
            wanted[snapshot.uniqueId] = {
                uniqueId = snapshot.uniqueId,
                itemId = snapshot.itemId,
                order = session.itemOrder,
                acquiredAt = snapshot.capturedAt or GetGreedTradeNowMs(),
    ...
    if type(diagnostics) == "table" then
        for itemIdKey, lootedCount in pairs(lootedCounts) do
            local missingCount = (tonumber(lootedCount) or 0) - (diagnostics.presentCountsByItemId[itemIdKey] or 0)
            if missingCount > 0 then
                diagnostics.rejectedMissing = diagnostics.rejectedMissing + missingCount
            end
        end
        diagnostics.trackedPresentByUniqueId = nil
        diagnostics.presentCountsByItemId = nil
    end
    ...
    if type(diagnostics) == "table" then
        diagnostics.eligibleForTrade = #validItems
    end
    return validItems
end

local function GetTradeLootControlMaxInputChars(control)
    if not control or type(control.GetMaxInputChars) ~= "function" then return nil end

    local ok, maxChars = pcall(function()
        return control:GetMaxInputChars()
    end)
    if ok and type(maxChars) == "number" and maxChars > 0 then
...
local function GetTradeLootChatSystem()
...
local function GetTradeLootChatEditControl(chatSystem)
    local textEntry = chatSystem and chatSystem.textEntry or nil
    local textEdit = textEntry and textEntry.editControl or nil

    if textEdit and type(textEdit.GetText) == "function" then return textEdit end
    if textEntry and type(textEntry.GetText) == "function" then return textEntry end

    local editBox = _G and _G.ZO_ChatWindowTextEntryEditBox or nil
    if editBox and type(editBox.GetText) == "function" then return editBox end

    return nil
end

local function GetTradeLootChatEditText(editControl)
    if editControl and type(editControl.GetText) == "function" then
...
local function SetTradeLootChatEditText(editControl, text)
    if not editControl then return false end

    if type(editControl.TakeFocus) == "function" then
        pcall(function()
            editControl:TakeFocus()
        end)
    end

    if type(editControl.SetText) == "function" then
        local okSet = pcall(function()
            editControl:SetText(text)
        end)
        if okSet and GetTradeLootChatEditText(editControl) == text then return true end
    end

    if type(editControl.InsertText) == "function" and GetTradeLootChatEditText(editControl) == "" then
        local okInsert = pcall(function()
            editControl:InsertText(text)
        end)
        if okInsert and GetTradeLootChatEditText(editControl) == text then return true end
    end

    return false
end

local function PopulateTradeLootGroupChatDraft(batch, openChatInput)
    local okOpen = pcall(openChatInput)
    if not okOpen then return false end

    local chatSystem = GetTradeLootChatSystem()
    local editControl = GetTradeLootChatEditControl(chatSystem)
    if GetTradeLootChatEditText(editControl) == batch then return true end
    return SetTradeLootChatEditText(editControl, batch)
end

function Greed:GetTradeLootChatInputCapacity()
    local editBox = _G and _G.ZO_ChatWindowTextEntryEditBox or nil
    local maxChars = GetTradeLootControlMaxInputChars(editBox)
    if maxChars then return maxChars, "ZO_ChatWindowTextEntryEditBox:GetMaxInputChars" end

    local chatSystem = GetTradeLootChatSystem()
    local textEntry = chatSystem and chatSystem.textEntry or nil
    maxChars = GetTradeLootControlMaxInputChars(textEntry and textEntry.editControl or nil)
        or GetTradeLootControlMaxInputChars(textEntry)
    if maxChars then return maxChars, "CHAT_SYSTEM.textEntry:GetMaxInputChars" end

    if type(MAX_TEXT_CHAT_INPUT_CHARACTERS) == "number" and MAX_TEXT_CHAT_INPUT_CHARACTERS > 0 then
        return math.floor(MAX_TEXT_CHAT_INPUT_CHARACTERS), "MAX_TEXT_CHAT_INPUT_CHARACTERS"
    end

    return GREED_TRADE_CHAT_FALLBACK_MAX_CHARS, "Greed fallback"
end

function Greed:GetTradeLootChatInputMaxChars()
    local maxChars = self:GetTradeLootChatInputCapacity()
    return maxChars
end

function Greed:GetTradeLootChatSafeMaxChars()
    local maxChars = self:GetTradeLootChatInputMaxChars()
    local safeMax = maxChars - GREED_TRADE_CHAT_SAFETY_MARGIN_CHARS
    if safeMax < 1 then return maxChars end
    return safeMax
end

function Greed:GetTradeLootMessageToken(itemLink, quantity)
    if type(itemLink) ~= "string" or itemLink == "" then return nil end

    quantity = tonumber(quantity) or 1
    quantity = math.floor(quantity)
    if quantity > 1 then
        return itemLink .. "x" .. tostring(quantity)
    end

    return itemLink
end

function Greed:GetTradeLootTextEffectiveLength(text)
    text = tostring(text or "")
    if text == "" then return 0 end

    local effectiveText = text:gsub("|H.-|h(.-)|h", "%1")
    return #effectiveText
end

function Greed:GetTradeLootMessageTokens(items)
    local tokens = {}
    local linksByItemLink = {}

    for _, item in ipairs(items or {}) do
        local itemLink = item and item.itemLink or nil
        if type(itemLink) == "string" and itemLink ~= "" then
            local entry = linksByItemLink[itemLink]
            if not entry then
                entry = {
                    itemLink = itemLink,
                    quantity = 0,
                }
                linksByItemLink[itemLink] = entry
                table.insert(tokens, entry)
            end
    ...
    for index, entry in ipairs(tokens) do
        tokens[index] = self:GetTradeLootMessageToken(entry.itemLink, entry.quantity)
    end
...
function Greed:BuildTradeLootBatches(items)
    local batches = {}
    local tokens = self:GetTradeLootMessageTokens(items)
    local chatMaxChars, chatLimitSource = self:GetTradeLootChatInputCapacity()
    local safeMaxChars = chatMaxChars - GREED_TRADE_CHAT_SAFETY_MARGIN_CHARS
    if safeMaxChars < 1 then safeMaxChars = chatMaxChars end
    local current = nil
    local currentEffectiveLength = 0
    local currentRawLength = 0
    local currentTokenCount = 0
    local batchTokenCounts = {}
    local batchLengths = {}
    local batchRawLengths = {}
    local oversizedToken = nil

    for _, token in ipairs(tokens) do
        if type(token) == "string" and token ~= "" then
            local tokenEffectiveLength = self:GetTradeLootTextEffectiveLength(token)
            local tokenRawLength = #token
            if tokenEffectiveLength > safeMaxChars or tokenRawLength > safeMaxChars then
                oversizedToken = {
                    token = token,
                    effectiveLength = tokenEffectiveLength,
                    rawLength = tokenRawLength,
                }
                current = nil
            ...
            elseif not current then
                current = token
                currentEffectiveLength = tokenEffectiveLength
                currentRawLength = tokenRawLength
                currentTokenCount = 1
            else
                local candidateEffectiveLength = currentEffectiveLength + 1 + tokenEffectiveLength
                local candidateRawLength = currentRawLength + 1 + tokenRawLength
                if candidateEffectiveLength > safeMaxChars or candidateRawLength > safeMaxChars then
                    table.insert(batches, current)
                    table.insert(batchTokenCounts, currentTokenCount)
                    table.insert(batchLengths, currentEffectiveLength)
                    table.insert(batchRawLengths, currentRawLength)
                    current = token
                    currentEffectiveLength = tokenEffectiveLength
                    currentRawLength = tokenRawLength
                    currentTokenCount = 1
                    ...
                    currentEffectiveLength = candidateEffectiveLength
                    currentRawLength = candidateRawLength
    ...
    return batches, {
        builder = "Greed:BuildTradeLootBatches",
        prefixAdded = false,
        chatMaxChars = chatMaxChars,
        chatLimitSource = chatLimitSource,
        safeMaxChars = safeMaxChars,
        safetyMarginChars = GREED_TRADE_CHAT_SAFETY_MARGIN_CHARS,
        lengthMode = "display-and-raw",
        consolidatedTokenCount = #tokens,
        batchTokenCounts = batchTokenCounts,
        batchLengths = batchLengths,
        batchRawLengths = batchRawLengths,
        oversizedToken = oversizedToken,
    }
end

function Greed:GetCurrentChatDraftText()
    return GetTradeLootChatEditText(GetTradeLootChatEditControl(GetTradeLootChatSystem())) or ""
end
...
function Greed:PrepareTradeLootBatchInGroupChat(batch, batchIndex, batchCount)
    ...
    if chatSystem and type(chatSystem.StartTextEntry) == "function" then
        okDraft = PopulateTradeLootGroupChatDraft(batch, function()
    ...
    if not okDraft then
        SafeAnnounce(T("Greed: Could not open the chat input for this trade batch. The trade batch was kept for retry."))
        return false
    end

    if batchIndex < batchCount then
        SafeAnnounce(T("Greed: Prepared trade batch %d/%d in Group chat. Press Enter to send, then use /greedtradenext for batch %d.", batchIndex, batchCount, batchIndex + 1))
    else
        SafeAnnounce(T("Greed: Prepared trade batch %d/%d in Group chat. Press Enter to send.", batchIndex, batchCount))
    end
    return true
end
...
function Greed:PrepareGreedTrade()
    ...
    local batches, metadata = self:BuildTradeLootBatches(items)
    if metadata and metadata.oversizedToken then
        session.preparedBatches = nil
        session.preparedBatchMetadata = metadata
        session.preparedBatchIndex = 0
        session.preparedSnapshotHasNewerLoot = false
        SafeAnnounce(T("Greed: One trade item link is too long for the chat input limit, so no trade batch was prepared."))
        return
    end
    if #batches == 0 then
        session.preparedBatches = nil
...
function Greed:PrepareNextGreedTrade()
    local session = self:GetTradeLootSession()
    if type(session.preparedBatches) ~= "table" or #session.preparedBatches == 0 then
        SafeAnnounce(T("Greed: Run /greedtrade before using /greedtradenext."))
        return
    end

    local nextIndex = (session.preparedBatchIndex or 0) + 1
    if nextIndex > #session.preparedBatches then
        SafeAnnounce(T("Greed: No additional prepared trade batches remain."))
        return
    end
...
local function AddGreedTradeDebugLine(lines, text)
    if #lines >= GREED_TRADE_DEBUG_MAX_LINES then return end
    table.insert(lines, text)
end

function Greed:GetGreedTradeDebugLines()
    -- Temporary diagnostic for 0.9.8-beta manual QA. Remove before release packaging.
    local lines = {}
    self:RefreshTradeLootActivityState()
    local session = self:GetTradeLootSession()
    local diagnostics = {}
    local items, reason = self:ValidateTradeLootSessionItems(diagnostics)
    items = items or {}
    local batches, metadata = self:BuildTradeLootBatches(items)
    local preparedBatches = type(session.preparedBatches) == "table" and session.preparedBatches or batches
    local preparedMetadata = type(session.preparedBatchMetadata) == "table" and session.preparedBatchMetadata or metadata
    local tokenCounts = preparedMetadata.batchTokenCounts or {}
    local batchLengths = preparedMetadata.batchLengths or {}
    local preparedCount = type(preparedBatches) == "table" and #preparedBatches or 0

    AddGreedTradeDebugLine(lines, "Greed trade debug (temporary; remove before release)")
    AddGreedTradeDebugLine(lines, string.format("builder=%s prefixAdded=%s chatMax=%s safeMax=%s margin=%s",
        tostring(preparedMetadata.builder or "unknown"),
        tostring(preparedMetadata.prefixAdded == true),
        tostring(preparedMetadata.chatMaxChars or "?"),
        tostring(preparedMetadata.safeMaxChars or "?"),
        tostring(preparedMetadata.safetyMarginChars or GREED_TRADE_CHAT_SAFETY_MARGIN_CHARS)))
    AddGreedTradeDebugLine(lines, string.format("limitSource=%s lengthMode=%s",
        tostring(preparedMetadata.chatLimitSource or "unknown"),
        tostring(preparedMetadata.lengthMode or "raw")))
    AddGreedTradeDebugLine(lines, string.format("state=%s tracked=%d present=%d eligible=%d consolidated=%d",
        tostring(session.state or "none"),
        diagnostics.trackedRunItems or 0,
        diagnostics.stillPresent or 0,
        diagnostics.eligibleForTrade or #items,
        preparedMetadata.consolidatedTokenCount or 0))
    AddGreedTradeDebugLine(lines, string.format("rejected bound=%d expired=%d missing=%d reason=%s",
        diagnostics.rejectedBound or 0,
        diagnostics.rejectedExpired or 0,
        diagnostics.rejectedMissing or 0,
        tostring(reason or "ok")))
    AddGreedTradeDebugLine(lines, string.format("batches=%d cursor=%d newerLootAfterPrepare=%s",
        preparedCount,
        session.preparedBatchIndex or 0,
        tostring(session.preparedSnapshotHasNewerLoot == true)))

    local batchRawLengths = preparedMetadata.batchRawLengths or {}
    local maxBatchLines = math.max(0, GREED_TRADE_DEBUG_MAX_LINES - #lines - 1)
    for index = 1, math.min(preparedCount, maxBatchLines) do
        AddGreedTradeDebugLine(lines, string.format("batch %d: tokens=%d chars=%d raw=%d",
            index,
            tokenCounts[index] or 0,
            batchLengths[index] or (preparedBatches[index] and self:GetTradeLootTextEffectiveLength(preparedBatches[index]) or 0),
            batchRawLengths[index] or (preparedBatches[index] and #preparedBatches[index] or 0)))
    end
    if preparedCount > maxBatchLines and #lines < GREED_TRADE_DEBUG_MAX_LINES then
        AddGreedTradeDebugLine(lines, string.format("batch list truncated at %d/%d", maxBatchLines, preparedCount))
    end

    return lines
end

function Greed:AnnounceGreedTradeDebug()
    for _, line in ipairs(self:GetGreedTradeDebugLines()) do
        SafeAnnounce(line)
    end
end
...
function Greed:HandleGreedTradeCommand(args)
    ...
    SafeAnnounce(T("Greed: Use /greedtrade or /greedtradenext."))
end

local function GetAntiquityLeadNameForDropLog(greed, antiquityId)
    if type(GetAntiquityName) ~= "function" then
        return T("Unknown Antiquity")
    end

    local name = GetAntiquityName(antiquityId)
    name = greed:CleanEsoDisplayText(name)
...
local function GetAntiquityLeadZoneForDropLog(greed, antiquityId)
    local zoneId
    if type(GetAntiquityZoneId) == "function" then
        zoneId = GetAntiquityZoneId(antiquityId)
    end

    local zoneName
    if type(zoneId) == "number" and zoneId > 0 and type(GetZoneNameById) == "function" then
        zoneName = GetZoneNameById(zoneId)
    end

    zoneName = greed:CleanEsoDisplayText(zoneName)
    if type(zoneName) == "string" and zoneName ~= "" then
        return zoneName, zoneId
    end

    return T("Unknown Zone"), zoneId
end

local function GetAntiquityLeadIconForDropLog(antiquityId)
    if type(GetAntiquityIcon) ~= "function" then return nil end

    local iconPath = GetAntiquityIcon(antiquityId)
...
local function GetAntiquityLeadQualityForDropLog(antiquityId)
    if type(GetAntiquityQuality) ~= "function" then return nil end

    local ok, quality = pcall(function()
        return GetAntiquityQuality(antiquityId)
    end)
...
local function AddAntiquityLeadDropLogEntry(greed, antiquityId)
    greed:InitializeDropLogSettings()
    if not greed.savedVars or not greed.savedVars.dropLog or greed.savedVars.dropLog.enabled ~= true then
        return false
    end

    antiquityId = tonumber(antiquityId)
    if not antiquityId or antiquityId <= 0 then
        return false
    end

    local leadName = GetAntiquityLeadNameForDropLog(greed, antiquityId)
    local zoneName, zoneId = GetAntiquityLeadZoneForDropLog(greed, antiquityId)
    local playerName = greed:NormalizeLootRecipient(nil, true)
    local displayName = greed:GetLootRecipientDisplayName(nil, playerName, true)
    local entry = {
        entryType = "antiquityLead",
        receivedBy = playerName,
        rawReceivedBy = "",
        looterCharacterName = playerName,
        looterDisplayName = displayName,
        antiquityId = antiquityId,
        antiquityName = leadName,
        antiquityQuality = GetAntiquityLeadQualityForDropLog(antiquityId),
        iconPath = GetAntiquityLeadIconForDropLog(antiquityId),
        itemName = leadName,
        zoneId = zoneId,
        zoneName = zoneName,
        traitName = T("Antiquity Lead"),
        slotLabel = T("Antiquity Lead"),
        quantity = 1,
        selfLoot = true,
        ...
        setName = "",
        source = "antiquityLead",
        timestamp = GetTimeStamp and GetTimeStamp() or 0,
        timestampText = GetTimeString and GetTimeString() or "",
    }

    greed:AddOrUpdateDropLogEntry(entry)
    greed:ShowDropListWindow(false)
    greed:DropLogDebug("antiquity lead logged: " .. tostring(leadName))
    return true
end

local function RegisterDropLogAntiquityLeadEvents(greed)
    if greed.dropLogAntiquityLeadEventsRegistered == true then return end
    if not EVENT_MANAGER or EVENT_ANTIQUITY_LEAD_ACQUIRED == nil then return end

    EVENT_MANAGER:RegisterForEvent(greed.name .. "DropLogAntiquityLeadAcquired", EVENT_ANTIQUITY_LEAD_ACQUIRED, function(_, antiquityId)
        AddAntiquityLeadDropLogEntry(greed, antiquityId)
    end)
    greed.dropLogAntiquityLeadEventsRegistered = true
end

function Greed:RegisterTradeLootRuntimeEvents()
    RegisterDropLogAntiquityLeadEvents(self)
...
function Greed:GetFirstActiveWantedDropLogTestData()
    local context = self:BuildDropLogActiveWishlistMatchContext()
    local anyRows = false

    for _, page in ipairs(context.pages or {}) do
        if page.rows and #page.rows > 0 then
            anyRows = true
            break
        end
    end
...
function Greed:AddDropLogTestEntry()
    ...
    if dropListEnabled then
        ...
        self:TrimDropLogEntriesToHistoryLimit(self.savedVars.dropLog.entries)
...
function Greed:GetArmorTraitTypeFromItemLink(itemLink)
    if not itemLink or type(GetItemLinkTraitInfo) ~= "function" then return nil end

    local ok, traitType = pcall(function()
        return GetItemLinkTraitInfo(itemLink)
    end)

    if ok and type(traitType) == "number" then
        return traitType
    end
...
function Greed:DoesLootMatchRow(row, lootInfo, slotKind, slotKey)
    ...
    if slotKind == "armor" then
        ...
        local isArmorPiece = self:IsArmorSlotKey(slotKey)
        if self:IsMonsterSetData(row) then
            if not MONSTER_ARMOR_SLOT_KEYS[slotKey] then return false end
        end
        if isArmorPiece then
            local armorType = self:GetArmorTypeFromItemLink(lootInfo.itemLink)
            if not self:ArmorWeightAllowsArmorType(row.armorWeights or row.armorWeight or row.weightPreference, armorType, row) then
                return false
            end

            local traitType = self:GetArmorTraitTypeFromItemLink(lootInfo.itemLink)
            if not self:ArmorTraitAllowsTraitType(row.armorTraits or row.armorTraitPreference, traitType) then
                return false
            end
        end
        return true
...
function Greed:FindDropLogWishlistMatchInContext(context, lootInfo, slotKind, slotKey)
    if not context or not lootInfo or not slotKind or not slotKey then return nil end
    local result
    local matchedPages = {}
    local matchedPageList = {}

    for _, page in ipairs(context.pages or {}) do
        local pageName = page and page.pageName or nil
        local rows = page and page.rows or nil
...
function Greed:BuildDropLogWishlistMatchContext()
    local pages = {}
    for _, pageName in ipairs(self:GetPageNames()) do
        table.insert(pages, {
            pageName = pageName,
            rows = self:GetDisplayFavoritesForPage(pageName),
        })
    end
...
function Greed:FindDropLogWishlistMatch(itemLink)
    ...
    return self:FindDropLogWishlistMatchInContext(self:BuildDropLogWishlistMatchContext(), lootInfo, slotKind, slotKey)
end

function Greed:BuildDropLogActiveWishlistMatchContext()
    return self:BuildDropLogWishlistMatchContext()
end
...
function Greed:GetActiveDropLogWishlistMatch(entry, context)
    ...
    return self:FindDropLogWishlistMatchInContext(context, lootInfo, slotKind, slotKey)
end
...
function Greed:GetDropLogDuplicateParts(entry)
    ...
    local itemKey
    if entry and entry.entryType == "antiquityLead" and tonumber(entry.antiquityId) then
        itemKey = "antiquity:" .. tostring(tonumber(entry.antiquityId))
    elseif itemId then
...
function Greed:AddOrUpdateDropLogEntry(entry)
    ...
    for index, existing in ipairs(entries) do
        ...
        if samePlayer and sameItem and sameTrait and sameSlot and closeTime then
            ...
            self.savedVars.dropLog.scrollOffset = 0
            self:TrimDropLogEntriesToHistoryLimit(entries)
            return
        end
    end
...
function Greed:ShouldConsiderLootItemBeforeWishlist(itemLink, rawItemText)
    ...
    if self:IsDropLogAntiquityLeadItem(itemLink, rawItemText) then
        return false, nil, "antiquity lead handled by acquisition event"
    end
...
function Greed:OnLootReceivedForDropLog(receivedBy, itemName, quantity, lootType, lootedBySelf, itemId)
    ...
    local isAntiquityLeadItem = self:IsDropLogAntiquityLeadItem(itemLink, itemName)
    if isSelfLoot and not isAntiquityLeadItem then
