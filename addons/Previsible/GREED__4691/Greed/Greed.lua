...
local ROW_HEIGHT = 48
local ROW_GAP = 6
local MAIN_WEIGHT_COLUMN_X = 216
local MAIN_WEIGHT_COLUMN_WIDTH = 86
...
local DROP_LOG_HISTORY_LIMIT_CHOICES = { "50", "100", "250", "500" }
local DROP_LOG_DEFAULT_HISTORY_LIMIT = tonumber(DROP_LOG_HISTORY_LIMIT_CHOICES[1]) or 50
local SAVED_VAR_DEFAULTS = {
    ...
    dropLog = {
        ...
        traitFilters = {},
        historyLimit = DROP_LOG_DEFAULT_HISTORY_LIMIT,
    },
    textPrompts = {
        locked = false,
...
local SLASH_COMMAND_DOCS = {
    ...
    { command = "/greedtradenext", description = T("Prepare the next stored trade batch in Group chat.") },
...
local function GetLocalizedArmorTypeLabel(armorType, fallbackKey)
    if armorType and GetString then
        local ok, armorTypeName = pcall(function()
            return GetString("SI_ARMORTYPE", armorType)
        end)
        if ok and type(armorTypeName) == "string" and armorTypeName ~= "" then
...
local ARMOR_WEIGHT_OPTIONS = {
    { key = "light", label = GetLocalizedArmorTypeLabel(ARMORTYPE_LIGHT, "Light"), armorType = ARMORTYPE_LIGHT },
    { key = "medium", label = GetLocalizedArmorTypeLabel(ARMORTYPE_MEDIUM, "Medium"), armorType = ARMORTYPE_MEDIUM },
    { key = "heavy", label = GetLocalizedArmorTypeLabel(ARMORTYPE_HEAVY, "Heavy"), armorType = ARMORTYPE_HEAVY },
}

local ARMOR_WEIGHT_BY_KEY = {}
for _, option in ipairs(ARMOR_WEIGHT_OPTIONS) do
    ARMOR_WEIGHT_BY_KEY[option.key] = option
end

local MONSTER_ARMOR_WEIGHT_OPTIONS = ARMOR_WEIGHT_OPTIONS

local ARMOR_TRAIT_OPTION_DEFS = {
    { key = "sturdy", globalName = "ITEM_TRAIT_TYPE_ARMOR_STURDY", fallbackLabelKey = "Sturdy" },
    { key = "impenetrable", globalName = "ITEM_TRAIT_TYPE_ARMOR_IMPENETRABLE", fallbackLabelKey = "Impenetrable" },
    { key = "reinforced", globalName = "ITEM_TRAIT_TYPE_ARMOR_REINFORCED", fallbackLabelKey = "Reinforced" },
    { key = "wellFitted", globalName = "ITEM_TRAIT_TYPE_ARMOR_WELL_FITTED", fallbackLabelKey = "Well-fitted" },
    { key = "training", globalName = "ITEM_TRAIT_TYPE_ARMOR_TRAINING", fallbackLabelKey = "Training" },
    { key = "infused", globalName = "ITEM_TRAIT_TYPE_ARMOR_INFUSED", fallbackLabelKey = "Infused" },
    { key = "invigorating", globalNames = { "ITEM_TRAIT_TYPE_ARMOR_INVIGORATING", "ITEM_TRAIT_TYPE_ARMOR_PROSPEROUS" }, fallbackLabelKey = "Invigorating" },
    { key = "divines", globalName = "ITEM_TRAIT_TYPE_ARMOR_DIVINES", fallbackLabelKey = "Divines" },
    { key = "ornate", globalName = "ITEM_TRAIT_TYPE_ARMOR_ORNATE", fallbackLabelKey = "Ornate" },
    { key = "intricate", globalName = "ITEM_TRAIT_TYPE_ARMOR_INTRICATE", fallbackLabelKey = "Intricate" },
    { key = "nirnhoned", globalName = "ITEM_TRAIT_TYPE_ARMOR_NIRNHONED", fallbackLabelKey = "Nirnhoned" },
    { key = "aggressive", globalName = "ITEM_TRAIT_TYPE_ARMOR_AGGRESSIVE", fallbackLabelKey = "Aggressive" },
    { key = "augmented", globalName = "ITEM_TRAIT_TYPE_ARMOR_AUGMENTED", fallbackLabelKey = "Augmented" },
    { key = "bolstered", globalName = "ITEM_TRAIT_TYPE_ARMOR_BOLSTERED", fallbackLabelKey = "Bolstered" },
    { key = "focused", globalName = "ITEM_TRAIT_TYPE_ARMOR_FOCUSED", fallbackLabelKey = "Focused" },
    { key = "prolific", globalName = "ITEM_TRAIT_TYPE_ARMOR_PROLIFIC", fallbackLabelKey = "Prolific" },
    { key = "quickened", globalName = "ITEM_TRAIT_TYPE_ARMOR_QUICKENED", fallbackLabelKey = "Quickened" },
    { key = "shattering", globalName = "ITEM_TRAIT_TYPE_ARMOR_SHATTERING", fallbackLabelKey = "Shattering" },
    { key = "soothing", globalName = "ITEM_TRAIT_TYPE_ARMOR_SOOTHING", fallbackLabelKey = "Soothing" },
    { key = "vigorous", globalName = "ITEM_TRAIT_TYPE_ARMOR_VIGOROUS", fallbackLabelKey = "Vigorous" },
}
...
Internal.MAIN_WEIGHT_COLUMN_X = MAIN_WEIGHT_COLUMN_X
Internal.MAIN_WEIGHT_COLUMN_WIDTH = MAIN_WEIGHT_COLUMN_WIDTH
...
Internal.ARMOR_SLOT_KEYS = ARMOR_SLOT_KEYS
Internal.ARMOR_WEIGHT_OPTIONS = ARMOR_WEIGHT_OPTIONS
Internal.ARMOR_WEIGHT_BY_KEY = ARMOR_WEIGHT_BY_KEY
Internal.ARMOR_TRAIT_OPTION_DEFS = ARMOR_TRAIT_OPTION_DEFS
...
Internal.DROP_LOG_HISTORY_LIMIT_CHOICES = DROP_LOG_HISTORY_LIMIT_CHOICES
Internal.DROP_LOG_DEFAULT_HISTORY_LIMIT = DROP_LOG_DEFAULT_HISTORY_LIMIT
...
function Greed:ClampDropLogHistoryLimit(value)
    local limit = tonumber(value)
    if not limit then
        return DROP_LOG_DEFAULT_HISTORY_LIMIT
    end

    if limit < 75 then
        return tonumber(DROP_LOG_HISTORY_LIMIT_CHOICES[1]) or DROP_LOG_DEFAULT_HISTORY_LIMIT
    elseif limit < 175 then
        return tonumber(DROP_LOG_HISTORY_LIMIT_CHOICES[2]) or DROP_LOG_DEFAULT_HISTORY_LIMIT
    elseif limit < 375 then
        return tonumber(DROP_LOG_HISTORY_LIMIT_CHOICES[3]) or DROP_LOG_DEFAULT_HISTORY_LIMIT
    end

    return tonumber(DROP_LOG_HISTORY_LIMIT_CHOICES[4]) or DROP_LOG_DEFAULT_HISTORY_LIMIT
end

function Greed:GetDropLogHistoryLimit()
    if not self.savedVars or type(self.savedVars.dropLog) ~= "table" then
        return DROP_LOG_DEFAULT_HISTORY_LIMIT
    end

    return self:ClampDropLogHistoryLimit(self.savedVars.dropLog.historyLimit)
end

function Greed:TrimDropLogEntriesToHistoryLimit(entries)
    if not self.savedVars or type(self.savedVars.dropLog) ~= "table" then return 0 end

    entries = entries or self.savedVars.dropLog.entries
    if type(entries) ~= "table" then return 0 end

    local limit = self:GetDropLogHistoryLimit()
    local originalCount = #entries
    for index = originalCount, limit + 1, -1 do
        entries[index] = nil
    end

    local maxScrollOffset = math.max(0, #entries - DROP_LOG_MAX_VISIBLE_ROWS)
    ...
    return originalCount - #entries
end

function Greed:SetDropLogHistoryLimit(value)
    self.savedVars = self.savedVars or {}
    self.savedVars.dropLog = self.savedVars.dropLog or {}
    self.savedVars.dropLog.historyLimit = self:ClampDropLogHistoryLimit(value)
...
function Greed:Initialize()
    ...
    SLASH_COMMANDS["/greedtradenext"] = function()
...
function Greed:ApplySavedTrackingOverride(row, sourceSetData)
    ...
    if override.armorTraits or override.armorTraitPreference then
        row.armorTraits = self:NormalizeArmorTraits(override.armorTraits or override.armorTraitPreference)
    end
...
function Greed:CreateDisplaySetRow(setData, perfectedOnly)
    local row = {
        ...
        armorTraits = self:NormalizeArmorTraits(setData.armorTraits or setData.armorTraitPreference),
    }
...
function Greed:CreateDisplaySetRowFromSaved(rowKey, savedRow)
    ...
    local row = {
        ...
        armorTraits = self:NormalizeArmorTraits(savedRow.armorTraits or savedRow.armorTraitPreference),
    }
...
function Greed:CreateFavoriteRow(index, setData, ownedItemIndex)
    ...
    row:SetHandler("OnMouseUp", function(control, button)
        if button == MOUSE_BUTTON_INDEX_RIGHT then
            self:ShowItemMenu(control, setData)
        end
    end)
    ...
    name:SetDimensions(MAIN_WEIGHT_COLUMN_X - 24, 24)
    ...
    source:SetDimensions(MAIN_WEIGHT_COLUMN_X - 24, 18)
    ...
    local weight = WINDOW_MANAGER:CreateControl(rowBaseName .. "Weight", row, CT_LABEL)
    weight:SetDimensions(MAIN_WEIGHT_COLUMN_WIDTH, 18)
    weight:SetAnchor(TOPLEFT, row, TOPLEFT, MAIN_WEIGHT_COLUMN_X, 29)
    weight:SetFont("ZoFontGameSmall")
    weight:SetHorizontalAlignment(TEXT_ALIGN_CENTER)
    weight:SetColor(sourceColor[1], sourceColor[2], sourceColor[3], sourceColor[4])
    local armorWeightText = self:GetArmorWeightDisplayText(setData)
    weight:SetText(armorWeightText)
    weight:SetMouseEnabled(true)
    SetSimpleTooltip(weight, T("Armor Weight: %s", armorWeightText == "-" and T("Not Applicable") or armorWeightText))
    weight:SetHandler("OnMouseWheel", function(_, delta)
        self:ScrollRows(delta)
    end)
    weight:SetHandler("OnMouseUp", function(control, button)
        if button == MOUSE_BUTTON_INDEX_RIGHT then
            self:ShowItemMenu(control, setData)
        end
    end)
    ...
    for slotIndex, column in ipairs(self.visibleColumns) do
        ...
        if piece then
            ...
            self:CreateEmptySlotBox(row, index, slotIndex, setData, column)
        end
    end
end
...
function Greed:CreateEmptySlotBox(row, rowIndex, slotIndex, setData, column)
    ...
    box:SetHandler("OnMouseUp", function(control, button)
        if button == MOUSE_BUTTON_INDEX_RIGHT then
            self:ShowItemMenu(control, setData, column)
        end
    end)
...
function Greed:CreateSlotBox(row, rowIndex, slotIndex, setData, column, piece, ownedItemIndex)
    ...
    -- Some tracked pieces need two physical copies, but still use one checkbox.
    ...
    local showOwnershipQuantity = self:ShouldDisplayOwnershipQuantity(column, piece, neededCount)
    ...
    if column.kind == "weapon" and not showOwnershipQuantity then
        self:CreateWeaponBadge(box, name, piece)
    elseif showOwnershipQuantity then
        ...
        countLabel:SetText(self:GetOwnershipQuantityLabel(collectedCount, neededCount))
    end
...
function Greed:ShowItemMenu(control, setData, column, piece)
    ...
    self:AddContextMenuItem(T("Copy Item Link"), function()
        if piece and piece.itemLink then
...
function Greed:PostItemLinkToChat(setData, column, piece)
    local itemName = (piece and column) and self:GetDisplayItemName(setData, column, piece) or (setData and (setData.name or setData.baseName)) or T("Unknown Set")
    local text = (piece and piece.itemLink) or string.format("[Greed] %s", itemName)
