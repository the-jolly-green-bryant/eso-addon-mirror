...
local ARMOR_SLOT_KEYS = Internal.ARMOR_SLOT_KEYS
local ARMOR_WEIGHT_OPTIONS = Internal.ARMOR_WEIGHT_OPTIONS
local MONSTER_ARMOR_SLOT_KEYS = Internal.MONSTER_ARMOR_SLOT_KEYS
local MONSTER_ARMOR_WEIGHT_OPTIONS = Internal.MONSTER_ARMOR_WEIGHT_OPTIONS or ARMOR_WEIGHT_OPTIONS
...
local PIECE_PICKER_CHECKBOX_MOUSEOVER_COLOR = { 1, 0.95, 0.72, 1 }

local ARMOR_TRAIT_OPTIONS = {
    { key = "sturdy", traitType = ITEM_TRAIT_TYPE_ARMOR_STURDY, fallbackLabelKey = "Sturdy" },
    { key = "impenetrable", traitType = ITEM_TRAIT_TYPE_ARMOR_IMPENETRABLE, fallbackLabelKey = "Impenetrable" },
    { key = "reinforced", traitType = ITEM_TRAIT_TYPE_ARMOR_REINFORCED, fallbackLabelKey = "Reinforced" },
    { key = "wellFitted", traitType = ITEM_TRAIT_TYPE_ARMOR_WELL_FITTED, fallbackLabelKey = "Well-fitted" },
    { key = "training", traitType = ITEM_TRAIT_TYPE_ARMOR_TRAINING, fallbackLabelKey = "Training" },
    { key = "infused", traitType = ITEM_TRAIT_TYPE_ARMOR_INFUSED, fallbackLabelKey = "Infused" },
    { key = "invigorating", traitType = ITEM_TRAIT_TYPE_ARMOR_INVIGORATING or ITEM_TRAIT_TYPE_ARMOR_PROSPEROUS, fallbackLabelKey = "Invigorating" },
    { key = "divines", traitType = ITEM_TRAIT_TYPE_ARMOR_DIVINES, fallbackLabelKey = "Divines" },
    { key = "nirnhoned", traitType = ITEM_TRAIT_TYPE_ARMOR_NIRNHONED, fallbackLabelKey = "Nirnhoned" },
}
...
local function CallPickerControlColorMethod(control, methodName, color)
    if control and color and type(control[methodName]) == "function" then
        control[methodName](control, color[1], color[2], color[3], color[4])
    end
end

local function CreatePickerColorDef(color)
    if not color or not ZO_ColorDef or type(ZO_ColorDef.New) ~= "function" then return nil end
    return ZO_ColorDef:New(color[1], color[2], color[3], color[4] or 1)
end

local function SetPickerCheckboxLabelDefaultColors(label, normalColor, mouseOverColor)
    if not label or type(ZO_CheckButtonLabel_SetDefaultColors) ~= "function" then return end

    local normalColorDef = CreatePickerColorDef(normalColor)
    local mouseOverColorDef = CreatePickerColorDef(mouseOverColor or normalColor)
    if normalColorDef and mouseOverColorDef then
        ZO_CheckButtonLabel_SetDefaultColors(label, normalColorDef, mouseOverColorDef)
    end
end

local function SetPickerCheckboxLabelTextColor(checkbox, color)
    if checkbox and color and type(ZO_CheckButtonLabel_SetTextColor) == "function" then
        ZO_CheckButtonLabel_SetTextColor(checkbox, color[1], color[2], color[3])
    end
end
...
local function SetPickerCheckboxColor(checkbox, color, mouseOverColor)
    if not checkbox then return end

    CallPickerControlColorMethod(checkbox, "SetNormalColor", color)
    CallPickerControlColorMethod(checkbox, "SetMouseOverColor", mouseOverColor or color)
    ...
    local label = GetPickerCheckboxLabel(checkbox)
    CallPickerControlColorMethod(label, "SetNormalColor", color)
    CallPickerControlColorMethod(label, "SetMouseOverColor", mouseOverColor or color)
    SetPickerCheckboxLabelDefaultColors(label, color, mouseOverColor or color)
    SetPickerCheckboxLabelTextColor(checkbox, color)
...
function Greed:IsArmorSlotKey(slotKey)
    return ARMOR_SLOT_KEYS and ARMOR_SLOT_KEYS[slotKey] == true
end

function Greed:NormalizeArmorWeights(weights)
    local normalized = {
        light = false,
        medium = false,
        heavy = false,
    ...
    if type(weights) == "table" then
        for _, option in ipairs(ARMOR_WEIGHT_OPTIONS or {}) do
...
function Greed:NormalizeMonsterArmorWeights(weights)
    return self:NormalizeArmorWeights(weights)
end

function Greed:GetSelectedArmorWeightCount(weights)
    local normalized = self:NormalizeArmorWeights(weights)
    local count = 0
    for _, option in ipairs(ARMOR_WEIGHT_OPTIONS or {}) do
...
function Greed:GetSelectedMonsterArmorWeightCount(weights)
    return self:GetSelectedArmorWeightCount(weights)
end

function Greed:GetArmorWeightKeyForArmorType(armorType)
    if armorType == nil then return nil end
    for _, option in ipairs(ARMOR_WEIGHT_OPTIONS or {}) do
...
function Greed:GetArmorWeightAvailabilityCacheKey(setData)
    local _, normalSetId, perfectedSetId = self:ResolveTrackedRowIdentity(setData or {})
    local selectedSetId = tonumber(setData and setData.setId)
    return table.concat({
        tostring(selectedSetId or ""),
        tostring(normalSetId or ""),
        tostring(perfectedSetId or ""),
    }, "|")
end

function Greed:GetArmorWeightAvailability(setData)
    local availability = {
        light = false,
        medium = false,
        heavy = false,
    ...
    local cacheKey = self:GetArmorWeightAvailabilityCacheKey(setData)
    self.armorWeightAvailabilityCache = self.armorWeightAvailabilityCache or {}
    local cached = self.armorWeightAvailabilityCache[cacheKey]
    if cached then
        return cached.availability, cached.hasAny, cached.count
    end
    ...
    local selectedSetId, normalSetId, perfectedSetId = self:ResolveTrackedRowIdentity(setData)
    addSetId(selectedSetId)
    addSetId(setData.setId)
    addSetId(normalSetId)
    addSetId(perfectedSetId)

    local function markArmorType(armorType)
        local key = self:GetArmorWeightKeyForArmorType(tonumber(armorType) or armorType)
        if key then
            availability[key] = true
        end
    end

    local function addSetArmorTypes(setId)
        if not libSets or type(libSets.GetSetArmorTypes) ~= "function" then return end

        local armorTypes = libSets.GetSetArmorTypes(setId)
        if type(armorTypes) ~= "table" then return end

        for armorType, available in pairs(armorTypes) do
            if available == true or available == 1 then
                markArmorType(armorType)
            end
        end
    end

    local function addSetItemArmorTypes(setId)
        if not libSets or type(libSets.GetSetItemIds) ~= "function" or type(libSets.GetItemsArmorType) ~= "function" then return end

        local itemIds = libSets.GetSetItemIds(setId)
        if type(itemIds) ~= "table" then return end

        for itemId, value in pairs(itemIds) do
            ...
            if resolvedItemId then
                markArmorType(libSets.GetItemsArmorType(resolvedItemId))
            end
        end
    end

    local function addArmorTypeFallback(setId)
        if not libSets or type(libSets.IsArmorTypeSet) ~= "function" then return end

        for _, option in ipairs(ARMOR_WEIGHT_OPTIONS or {}) do
            if option.armorType ~= nil and libSets.IsArmorTypeSet(setId, option.armorType) == true then
    ...
    for _, setId in ipairs(setIds) do
        addSetArmorTypes(setId)
        addSetItemArmorTypes(setId)
        addArmorTypeFallback(setId)
    end

    local count = 0
    for _, option in ipairs(ARMOR_WEIGHT_OPTIONS or {}) do
        if availability[option.key] == true then
            count = count + 1
        end
    end
    ...
    self.armorWeightAvailabilityCache[cacheKey] = cached

    return availability, cached.hasAny, count
end

function Greed:SetHasArmorWeightApplicability(setData)
    local _, hasAny = self:GetArmorWeightAvailability(setData)
    return hasAny == true
end

function Greed:GetArmorWeightLabelListForAvailability(availability)
    local labels = {}
    for _, option in ipairs(ARMOR_WEIGHT_OPTIONS or {}) do
        if availability and availability[option.key] == true then
...
function Greed:GetEffectiveArmorWeights(setData, weights)
    local availability, hasAny, availableCount = self:GetArmorWeightAvailability(setData)
    local effective = self:NormalizeArmorWeights(nil)
    if not hasAny then
        return effective, availability, false, 0, 0
    end

    local normalized = self:NormalizeArmorWeights(weights or (setData and (setData.armorWeights or setData.armorWeight or setData.weightPreference)))
    local selectedAvailableCount = 0
    for _, option in ipairs(ARMOR_WEIGHT_OPTIONS or {}) do
        if normalized[option.key] == true and availability[option.key] == true then
            selectedAvailableCount = selectedAvailableCount + 1
            effective[option.key] = true
        end
    end

    if selectedAvailableCount == 0 then
        for _, option in ipairs(ARMOR_WEIGHT_OPTIONS or {}) do
            if availability[option.key] == true then
                effective[option.key] = true
            end
    ...
    return effective, availability, true, availableCount, selectedAvailableCount
end

function Greed:GetArmorWeightDisplayText(setData)
    local weights, availability, hasAny, availableCount, selectedAvailableCount = self:GetEffectiveArmorWeights(setData)
    if not hasAny then
        return "-"
    end
    ...
    if availableCount == 1 then
        return labels[1] or table.concat(self:GetArmorWeightLabelListForAvailability(availability), ", ")
    end

    if selectedAvailableCount == 0 or selectedAvailableCount == availableCount then
        return T("All")
    end
...
function Greed:GetMonsterWeightPreferenceText(setData)
    local weights, _, hasAny, availableCount, selectedAvailableCount = self:GetEffectiveArmorWeights(setData)
    local selected = {}
    ...
    if not hasAny or selectedAvailableCount == 0 or selectedAvailableCount == availableCount then
        return T("Any")
    end
...
function Greed:MonsterWeightAllowsArmorType(weights, armorType, setData)
    return self:ArmorWeightAllowsArmorType(weights, armorType, setData)
end

function Greed:ArmorWeightAllowsArmorType(weights, armorType, setData)
    if armorType == nil then return true end

    local armorTypeKey = self:GetArmorWeightKeyForArmorType(armorType)
    if not armorTypeKey then
        -- If this client does not expose the armor-type constants, do not block the row.
        return true
    end

    if setData then
        local effective, availability, hasAny = self:GetEffectiveArmorWeights(setData, weights)
        if hasAny then
            return availability[armorTypeKey] == true and effective[armorTypeKey] == true
        end
    end

    local normalized = self:NormalizeArmorWeights(weights)
    local selectedCount = self:GetSelectedArmorWeightCount(normalized)
    if selectedCount == 0 or selectedCount == #(ARMOR_WEIGHT_OPTIONS or {}) then
        return true
    end

    return normalized[armorTypeKey] == true
end
...
function Greed:GetArmorTraitLabel(traitType, fallbackLabelKey)
    if traitType and GetString then
    ...
    return T(fallbackLabelKey or "Unknown")
end

function Greed:GetArmorTraitOptions()
    if self.armorTraitOptions then
        return self.armorTraitOptions
    end

    local options = {}
    local seenTraitTypes = {}

    local function addOption(key, traitType, fallbackLabelKey)
        if type(key) ~= "string" or key == "" or type(traitType) ~= "number" then return end
        if ITEM_TRAIT_TYPE_NONE and traitType == ITEM_TRAIT_TYPE_NONE then return end
        if seenTraitTypes[traitType] then return end

        seenTraitTypes[traitType] = true
        table.insert(options, {
            key = key,
            traitType = traitType,
            label = self:GetArmorTraitLabel(traitType, fallbackLabelKey),
            fallbackLabelKey = fallbackLabelKey,
        })
    end

    for _, option in ipairs(ARMOR_TRAIT_OPTIONS) do
        addOption(option.key, option.traitType, option.fallbackLabelKey)
    end

    self.armorTraitOptions = options
    return options
end

function Greed:NormalizeArmorTraits(traits)
    local normalized = {}
    local validKeys = {}
    local validByTraitType = {}

    for _, option in ipairs(self:GetArmorTraitOptions() or {}) do
        validKeys[option.key] = true
        if option.traitType ~= nil then
            validByTraitType[tostring(option.traitType)] = option.key
        end
        normalized[option.key] = false
    end

    local function enableTrait(value)
        if value == nil then return end
        if type(value) == "number" then
            local key = validByTraitType[tostring(value)]
            if key then normalized[key] = true end
            return
        end

        local text = tostring(value)
        if text == "" or string.lower(text) == "any" or string.lower(text) == "all" then return end
        if validKeys[text] then
            normalized[text] = true
            return
        end

        local key = validByTraitType[text]
        if key then
            normalized[key] = true
        end
    end
    ...
    if type(traits) == "table" then
        for key, enabled in pairs(traits) do
            if enabled == true then
                enableTrait(key)
            elseif type(enabled) == "string" or type(enabled) == "number" then
                enableTrait(enabled)
            end
        end
    else
...
function Greed:GetSelectedArmorTraitCount(traits)
    local normalized = self:NormalizeArmorTraits(traits)
    local count = 0
    for _, option in ipairs(self:GetArmorTraitOptions() or {}) do
        if normalized[option.key] == true then
            count = count + 1
...
function Greed:ArmorTraitAllowsTraitType(traits, traitType)
    if traitType == nil then return true end

    local normalized = self:NormalizeArmorTraits(traits)
    local selectedCount = self:GetSelectedArmorTraitCount(normalized)
    local options = self:GetArmorTraitOptions()
    if selectedCount == 0 or selectedCount == #(options or {}) then
        return true
    end

    local canCompare = false
    for _, option in ipairs(options or {}) do
        if normalized[option.key] == true and option.traitType ~= nil then
            canCompare = true
            if option.traitType == traitType then
                return true
            end
        end
    end
...
local function SetPickerCheckboxLabelWidth(checkbox, width)
    if not checkbox then return end

    local label = GetPickerCheckboxLabel(checkbox)
    if label then
        label:SetDimensions(width, 18)
    end
end

function Greed:CreateArmorWeightPickerControls(prefix, parent, anchorControl, offsetX, offsetY)
    local container = WINDOW_MANAGER:CreateControl(prefix .. "ArmorWeights", parent, CT_CONTROL)
    container:SetDimensions(180, 112)
    container:SetAnchor(TOPLEFT, anchorControl, TOPLEFT, offsetX or 0, offsetY or 0)
    container.greedAnchorControl = anchorControl
    container.greedAnchorOffsetX = offsetX or 0
    container.greedAnchorOffsetY = offsetY or 0
    container:SetHidden(true)
    container:SetMouseEnabled(false)

    local header = WINDOW_MANAGER:CreateControl(prefix .. "ArmorWeightsHeader", container, CT_LABEL)
    header:SetDimensions(176, 18)
    ...
    header:SetText(T("Armor Weight"))

    local help = WINDOW_MANAGER:CreateControl(prefix .. "ArmorWeightsHelp", container, CT_LABEL)
    help:SetDimensions(176, 16)
    help:SetAnchor(TOPLEFT, header, BOTTOMLEFT, 0, 0)
    help:SetFont("ZoFontGameSmall")
    help:SetColor(COLORS.mutedText[1], COLORS.mutedText[2], COLORS.mutedText[3], COLORS.mutedText[4])
    help:SetText(T("No selection = All"))

    local checks = {}
    for index, option in ipairs(ARMOR_WEIGHT_OPTIONS or {}) do
        local checkbox = WINDOW_MANAGER:CreateControlFromVirtual(prefix .. "ArmorWeight" .. option.key, container, "ZO_CheckButton")
        checkbox:SetAnchor(TOPLEFT, help, BOTTOMLEFT, 0, 3 + (index - 1) * 24)
        if ZO_CheckButton_SetLabelText then
            ZO_CheckButton_SetLabelText(checkbox, option.label)
        end
        checkbox.greedWeightHelp = help
        SetPickerCheckboxLabelWidth(checkbox, 142)
        if ZO_CheckButton_SetToggleFunction then
            ZO_CheckButton_SetToggleFunction(checkbox, function(control, checked)
                control.greedChecked = checked == true
            end)
...
function Greed:CreateMonsterWeightPickerControls(prefix, parent, anchorControl, offsetX, offsetY)
    return self:CreateArmorWeightPickerControls(prefix, parent, anchorControl, offsetX, offsetY)
end

function Greed:CreateArmorTraitPickerControls(prefix, parent, anchorControl, offsetX, offsetY)
    local options = self:GetArmorTraitOptions()
    local container = WINDOW_MANAGER:CreateControl(prefix .. "ArmorTraits", parent, CT_CONTROL)
    container:SetDimensions(180, 270)
    container:SetAnchor(TOPLEFT, anchorControl, TOPLEFT, offsetX or 0, offsetY or 0)
    container.greedAnchorControl = anchorControl
    container.greedAnchorOffsetX = offsetX or 0
    container.greedAnchorOffsetY = offsetY or 0
    ...
    local header = WINDOW_MANAGER:CreateControl(prefix .. "ArmorTraitsHeader", container, CT_LABEL)
    header:SetDimensions(176, 18)
    header:SetAnchor(TOPLEFT, container, TOPLEFT, 0, 0)
    header:SetFont("ZoFontGameSmall")
    header:SetColor(COLORS.text[1], COLORS.text[2], COLORS.text[3], COLORS.text[4])
    header:SetText(T("Preferred Armor Trait"))

    local help = WINDOW_MANAGER:CreateControl(prefix .. "ArmorTraitsHelp", container, CT_LABEL)
    help:SetDimensions(176, 16)
    help:SetAnchor(TOPLEFT, header, BOTTOMLEFT, 0, 0)
    help:SetFont("ZoFontGameSmall")
    help:SetColor(COLORS.mutedText[1], COLORS.mutedText[2], COLORS.mutedText[3], COLORS.mutedText[4])
    help:SetText(T("No selection = All Traits"))

    local checks = {}
    for index, option in ipairs(options or {}) do
        local checkbox = WINDOW_MANAGER:CreateControlFromVirtual(prefix .. "ArmorTrait" .. option.key, container, "ZO_CheckButton")
        checkbox:SetAnchor(TOPLEFT, help, BOTTOMLEFT, 0, 3 + (index - 1) * 24)
        if ZO_CheckButton_SetLabelText then
            ZO_CheckButton_SetLabelText(checkbox, option.label)
        end
...
function Greed:SetArmorWeightCheckboxes(weightChecks, weights)
    local normalized = self:NormalizeArmorWeights(weights)
    for _, option in ipairs(ARMOR_WEIGHT_OPTIONS or {}) do
...
function Greed:SetMonsterWeightCheckboxes(weightChecks, weights)
    self:SetArmorWeightCheckboxes(weightChecks, weights)
end

function Greed:ReadArmorWeightCheckboxes(weightChecks)
    local weights = self:NormalizeArmorWeights(nil)
    for _, option in ipairs(ARMOR_WEIGHT_OPTIONS or {}) do
...
function Greed:ReadMonsterWeightCheckboxes(weightChecks)
    return self:ReadArmorWeightCheckboxes(weightChecks)
end

function Greed:SetArmorTraitCheckboxes(traitChecks, traits)
    local normalized = self:NormalizeArmorTraits(traits)
    for _, option in ipairs(self:GetArmorTraitOptions() or {}) do
        self:SetEditCheckboxState(traitChecks and traitChecks[option.key], normalized[option.key] == true)
    end
end

function Greed:ReadArmorTraitCheckboxes(traitChecks)
    local traits = self:NormalizeArmorTraits(nil)
    for _, option in ipairs(self:GetArmorTraitOptions() or {}) do
        traits[option.key] = self:IsEditCheckboxChecked(traitChecks and traitChecks[option.key])
    end
    return traits
end

function Greed:SetArmorWeightCheckboxAvailability(weightChecks, setData)
    local availability, _, availableCount = self:GetArmorWeightAvailability(setData)
    local visibleIndex = 0
    for _, option in ipairs(ARMOR_WEIGHT_OPTIONS or {}) do
        local checkbox = weightChecks and weightChecks[option.key]
        local available = availability and availability[option.key] == true
        if checkbox then
            checkbox:SetHidden(not available)
            checkbox:SetMouseEnabled(available)
            if available then
                visibleIndex = visibleIndex + 1
                if checkbox.greedWeightHelp then
                    checkbox:ClearAnchors()
                    checkbox:SetAnchor(TOPLEFT, checkbox.greedWeightHelp, BOTTOMLEFT, 0, 3 + (visibleIndex - 1) * 24)
                end
            end
...
function Greed:LayoutArmorPreferenceControls(controls, showWeightSelector)
    local weightContainer = controls and (controls.armorWeightContainer or controls.monsterWeightContainer)
    local traitContainer = controls and controls.armorTraitContainer
    if not traitContainer then return end

    local traitAnchor = traitContainer.greedAnchorControl
    local traitX = traitContainer.greedAnchorOffsetX or 0
    local traitY = traitContainer.greedAnchorOffsetY or 0

    if weightContainer then
        if showWeightSelector then
            traitY = (weightContainer.greedAnchorOffsetY or 0) + 118
        else
            traitY = weightContainer.greedAnchorOffsetY or traitY
        end
        traitAnchor = weightContainer.greedAnchorControl or traitAnchor
        traitX = weightContainer.greedAnchorOffsetX or traitX
    end

    if traitAnchor then
        traitContainer:ClearAnchors()
        traitContainer:SetAnchor(TOPLEFT, traitAnchor, TOPLEFT, traitX, traitY)
    end
end

function Greed:ApplyMonsterControlsMode(controls, isMonsterSet, setData)
    ...
    local _, hasArmorWeights, availableWeightCount = self:GetArmorWeightAvailability(setData)
    self:SetArmorWeightCheckboxAvailability(controls.armorWeightChecks or controls.monsterWeightChecks, setData)
    local showWeightSelector = hasArmorWeights and availableWeightCount > 1
    self:SetControlTreeHiddenAndMouse(controls.armorWeightContainer or controls.monsterWeightContainer, not showWeightSelector)
    self:SetControlTreeHiddenAndMouse(controls.armorTraitContainer, not hasArmorWeights)
    self:LayoutArmorPreferenceControls(controls, showWeightSelector)
end
...
function Greed:CreateEditWindow()
    ...
    local armorWeightContainer, armorWeightChecks = self:CreateArmorWeightPickerControls("GreedEditTrackedPieces", window, armorHeader, 380, 0)
    local armorTraitContainer, armorTraitChecks = self:CreateArmorTraitPickerControls("GreedEditTrackedPieces", window, armorHeader, 380, 118)
    ...
    self.editControls = {
        ...
        armorWeightContainer = armorWeightContainer,
        armorWeightChecks = armorWeightChecks,
        armorTraitContainer = armorTraitContainer,
        armorTraitChecks = armorTraitChecks,
        monsterWeightContainer = armorWeightContainer,
        monsterWeightChecks = armorWeightChecks,
    }
...
function Greed:ShowEditTrackedPiecesWindow(setData)
    ...
    self:SetArmorWeightCheckboxes(controls.armorWeightChecks or controls.monsterWeightChecks, setData.armorWeights or setData.armorWeight or setData.weightPreference)
    self:SetArmorTraitCheckboxes(controls.armorTraitChecks, setData.armorTraits or setData.armorTraitPreference)
    self:ApplyMonsterControlsMode(controls, self:IsMonsterSetData(setData), setData)
...
function Greed:SaveEditTrackedPieces()
    ...
    if self:SetHasArmorWeightApplicability(setData) then
        override.armorWeights = self:ReadArmorWeightCheckboxes(controls.armorWeightChecks or controls.monsterWeightChecks)
        override.armorTraits = self:ReadArmorTraitCheckboxes(controls.armorTraitChecks)
    end
...
function Greed:CreateAddSetWindow()
    ...
    local armorWeightContainer, armorWeightChecks = self:CreateArmorWeightPickerControls("GreedAddSet", picker, armorHeader, 250, 0)
    local armorTraitContainer, armorTraitChecks = self:CreateArmorTraitPickerControls("GreedAddSet", picker, armorHeader, 250, 118)
...
function Greed:ResetAddPicker()
    ...
    self:SetArmorWeightCheckboxes(controls.armorWeightChecks or controls.monsterWeightChecks, nil)
    self:SetArmorTraitCheckboxes(controls.armorTraitChecks, nil)
    self:ApplyMonsterControlsMode(controls, false, nil)
...
function Greed:ApplyAddSetTrackingToPicker(existingTracking)
    ...
    self:SetArmorWeightCheckboxes(controls.armorWeightChecks or controls.monsterWeightChecks, existingTracking and (existingTracking.armorWeights or existingTracking.armorWeight or existingTracking.weightPreference))
    self:SetArmorTraitCheckboxes(controls.armorTraitChecks, existingTracking and (existingTracking.armorTraits or existingTracking.armorTraitPreference))
end
...
function Greed:RefreshAddSetPickerForCurrentSelection()
    ...
    self:ApplyMonsterControlsMode(controls, result.isMonsterSet == true, result)
...
function Greed:BuildPiecePickerSetData(source, isPerfected, selectedSetId)
    ...
    return {
        ...
        armorTraits = source.armorTraits or source.armorTraitPreference,
...
function Greed:RefreshPiecePickerCheckboxVisual(checkbox)
    ...
    local color = checkbox.greedOwnershipColor or COLORS.text
    local mouseOverColor = checkbox.greedPreserveOwnershipColorOnHover and color or PIECE_PICKER_CHECKBOX_MOUSEOVER_COLOR

    SetPickerCheckboxText(checkbox, checkbox.greedOwnershipLabel or checkbox.greedBaseLabel)
    SetPickerCheckboxColor(checkbox, color, mouseOverColor)
end
...
function Greed:ApplyPiecePickerOwnershipStyle(checkbox, setData, column, piece, ownedItemIndex)
    ...
    if (collectedCount or 0) > 0 then
        ...
        if self:ShouldDisplayOwnershipQuantity(column, piece, neededCount) then
            labelText = string.format("%s %s", baseLabel, self:GetOwnershipQuantityLabel(collectedCount, neededCount))
        end
    ...
    checkbox.greedPreserveOwnershipColorOnHover = color == COLORS.collected or color == COLORS.stickerBook
    self:RefreshPiecePickerCheckboxVisual(checkbox)
end
...
function Greed:ResetPiecePickerOwnershipStyles(controls)
    ...
    for _, slot in ipairs(GreedData.armorSlots or {}) do
        ...
        if checkbox then
            ...
            checkbox.greedPreserveOwnershipColorOnHover = nil
            self:RefreshPiecePickerCheckboxVisual(checkbox)
        end
    end
...
function Greed:SaveAddSet()
    ...
    if self:SetHasArmorWeightApplicability({
        setId = selectedSetId,
        normalSetId = normalSetId,
        perfectedSetId = perfectedSetId,
    }) then
        armorWeights = self:ReadArmorWeightCheckboxes(controls.armorWeightChecks or controls.monsterWeightChecks)
        armorTraits = self:ReadArmorTraitCheckboxes(controls.armorTraitChecks)
    end
...
function Greed:CreateSavedOverrideFromDisplayRow(setData, destinationPageName)
    ...
    return {
        ...
        perfected = setData.isPerfectedRow == true,
        isMonsterSet = self:IsMonsterSetData(setData),
        armorWeights = self:SetHasArmorWeightApplicability(setData) and self:NormalizeArmorWeights(setData.armorWeights or setData.armorWeight or setData.weightPreference) or nil,
        armorTraits = self:SetHasArmorWeightApplicability(setData) and self:NormalizeArmorTraits(setData.armorTraits or setData.armorTraitPreference) or nil,
        pieces = pieces,
        weapons = weapons,
    }
end
