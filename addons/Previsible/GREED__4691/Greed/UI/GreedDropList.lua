...
local DROP_LOG_HISTORY_LIMIT_CHOICES = Internal.DROP_LOG_HISTORY_LIMIT_CHOICES
local DROP_LOG_DEFAULT_HISTORY_LIMIT = Internal.DROP_LOG_DEFAULT_HISTORY_LIMIT
...
local function ColorizeAntiquityLeadName(entry, leadName)
    if type(leadName) ~= "string" or leadName == "" then
        return leadName
    end

    local quality = tonumber(entry and entry.antiquityQuality)
    if not quality or type(GetAntiquityQualityColor) ~= "function" then
        return leadName
    end

    local ok, color = pcall(function()
        return GetAntiquityQualityColor(quality)
    end)
    if not ok or type(color) ~= "table" or type(color.Colorize) ~= "function" then
        return leadName
    end

    local colorizedOk, colorizedName = pcall(function()
        return color:Colorize(leadName)
    end)
    if colorizedOk and type(colorizedName) == "string" and colorizedName ~= "" then
...
function Greed:InitializeDropLogSettings()
    ...
    self.savedVars.dropLog.historyLimit = self:ClampDropLogHistoryLimit(self.savedVars.dropLog.historyLimit)
...
function Greed:DropLogEntryPassesTraitFilter(entry)
    ...
    if entry and entry.entryType == "antiquityLead" then return true end
...
function Greed:CreateDropListWindow()
    ...
    subtitle:SetText(T("Showing the %d most recent log entries.", DROP_LOG_DEFAULT_HISTORY_LIMIT))
...
function Greed:BuildDropListEntryText(entry, highlightWanted)
    ...
    if entry.entryType == "antiquityLead" then
        local leadName = entry.antiquityName or entry.itemName or T("Unknown Antiquity")
        leadName = ColorizeAntiquityLeadName(entry, leadName)
        local zoneName = entry.zoneName or T("Unknown Zone")
        return T("%s acquired %s: %s (%s)", timeText .. playerText, T("Antiquity Lead"), leadName, zoneName)
    end
...
function Greed:RefreshDropListWindow(ownedItemIndex)
    ...
    if controls.subtitle then
        local historyLimit = self:GetDropLogHistoryLimit()
        if entryCount < historyLimit then
            controls.subtitle:SetText(T("Showing %d log entries.", entryCount))
        else
            controls.subtitle:SetText(T("Showing the %d most recent log entries.", historyLimit))
        end
    end
...
function Greed:CreateDropOptionsWindow()
    ...
    makeLabel("GreedDropOptionsHistoryLimitLabel", T("Drop Log History Limit"), rightX, y, 170)
    local historyLimitDropdown = makeDropdown("GreedDropOptionsHistoryLimitDropdown", rightX + 190, y - 3, 170)
    ...
    self.dropOptionsControls = {
        ...
        historyLimitDropdown = historyLimitDropdown,
...
function Greed:RefreshDropOptionsState()
    ...
    if controls.historyLimitDropdown then
        local historyLimitLabel = tostring(self:GetDropLogHistoryLimit())
        self:ConfigureChoiceDropdown(controls.historyLimitDropdown, DROP_LOG_HISTORY_LIMIT_CHOICES, historyLimitLabel, function(label)
            controls.historyLimitDropdown.greedPendingHistoryLimit = self:ClampDropLogHistoryLimit(label)
        end)
        controls.historyLimitDropdown.greedPendingHistoryLimit = self:ClampDropLogHistoryLimit(historyLimitLabel)
    end
...
function Greed:SaveDropOptions()
    ...
    if controls.historyLimitDropdown then
        self:SetDropLogHistoryLimit(controls.historyLimitDropdown.greedPendingHistoryLimit or self:GetDropLogHistoryLimit())
    end
    if controls.lockPromptsToggle then
...
function Greed:ApplyDropListDefaultSettings(clearEntries)
    ...
    self.savedVars.dropLog.pageFilterName = DROP_LOG_PAGE_FILTER_ALL_DROPS
    self.savedVars.dropLog.traitFilters = {}
    self.savedVars.dropLog.historyLimit = DROP_LOG_DEFAULT_HISTORY_LIMIT
    self.savedVars.dropLog.scrollOffset = 0
end
