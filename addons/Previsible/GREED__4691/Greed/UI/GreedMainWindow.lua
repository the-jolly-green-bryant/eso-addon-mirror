...
local MAIN_WEIGHT_COLUMN_X = Internal.MAIN_WEIGHT_COLUMN_X
local MAIN_WEIGHT_COLUMN_WIDTH = Internal.MAIN_WEIGHT_COLUMN_WIDTH
...
function Greed:UpdateMainWindowLayout()
    ...
    if self.controls.weightHeader then
        self.controls.weightHeader:ClearAnchors()
        self.controls.weightHeader:SetAnchor(TOPLEFT, self.controls.window, TOPLEFT, MAIN_ROWS_LEFT_INSET + MAIN_WEIGHT_COLUMN_X, MAIN_WINDOW_SLOT_HEADER_TOP)
        self.controls.weightHeader:SetDimensions(MAIN_WEIGHT_COLUMN_WIDTH, 18)
    end
...
function Greed:BuildSlotHeader()
    ...
    if self.controls.weightHeader then
        self.controls.weightHeader:SetHidden(true)
    end

    self.slotHeaderLabels = {}
    self.slotHeaderBuildId = (self.slotHeaderBuildId or 0) + 1

    local weightHeader = WINDOW_MANAGER:CreateControl("GreedWeightHeader" .. self.slotHeaderBuildId, self.controls.window, CT_LABEL)
    self.controls.weightHeader = weightHeader
    weightHeader:SetDimensions(MAIN_WEIGHT_COLUMN_WIDTH, 18)
    weightHeader:SetAnchor(TOPLEFT, self.controls.window, TOPLEFT, MAIN_ROWS_LEFT_INSET + MAIN_WEIGHT_COLUMN_X, MAIN_WINDOW_SLOT_HEADER_TOP)
    weightHeader:SetFont("ZoFontGameSmall")
    weightHeader:SetHorizontalAlignment(TEXT_ALIGN_CENTER)
    weightHeader:SetColor(COLORS.mutedText[1], COLORS.mutedText[2], COLORS.mutedText[3], COLORS.mutedText[4])
    weightHeader:SetText(T("Weight"))
