...
--[[  LootWall Ultimate  (Vicodine Lootwall continued)

http://www.esoui.com/downloads/fileinfo.php?id=1988
...
v 1.0.6
      + minor correction
v 1.0.5
      + modified chat entry when an item is pending
...
local LW_ITEM_QUANTITY = 0

local LW_CONF_CURRENCY = "currency"
local LW_CONF_LIMITSTOCK = "limitstock"

local LW_CONF_CHAT_SETTING = "logtochat"
...
--*0.7
-- 0.8
local LW_CONF_KEEPRES_JEWELMAT = "keepJewelryMaterials"
local LW_CONF_KEEPRES_JEWEL = "keepJewelryResearch"
--
...
local LW_CONF_DEFAULTS = {
  ...
  [LW_CONF_CHAT_SETTING] = true,
  [LW_CONF_CURRENCY] = 10,
  [LW_CONF_LIMITSTOCK] = 5,
  [LW_CONF_KEEPRES_JEWEL] = LW_PRESET_ACTION_PROMPT,
  [LW_CONF_KEEPRES_JEWELMAT] = LW_PRESET_ACTION_PROMPT
}
-------------------------------------------------------------------------------
function LootWall:Initialize()
  ...
  if svVersion < self.savedVarsVersion then
    ...
    if svVersion < 2 then
      ...
      self.savedVariables.conf[LW_CONF_CHAT_SETTING] = true
      self.savedVariables.conf[LW_CONF_LIMITSTOCK] = 5
      self.savedVariables.conf[LW_CONF_CURRENCY] = 10

      self.savedVariables.conf[LW_CONF_KEEPRES_JEWEL] = self:ConvertToV2(self.savedVariables.conf[LW_CONF_KEEPRES_JEWEL])
      self.savedVariables.conf[LW_CONF_KEEPRES_JEWELMAT] = self:ConvertToV2(self.savedVariables.conf[LW_CONF_KEEPRES_JEWELMAT])
    ...
    if svVersion < 3 then
      ...
      self.savedVariables.conf[LW_CONF_KEEPRES_JEWEL] = LW_CONF_DEFAULTS[LW_CONF_KEEPRES_JEWEL]
      self.savedVariables.conf[LW_CONF_KEEPRES_JEWELMAT] = LW_CONF_DEFAULTS[LW_CONF_KEEPRES_JEWELMAT]
    end
  ...
  local panelData = {
    type = "panel",
    name = "LootWall Ultimate",
    author = "Vicodine v0.7 / Redgabber v1.0+",
    slashCommand = "/lootwall",
    version = "1.0.8 SummerSet",
  ...
  local optionsControls = {
    ...
    [3]={
      type = "checkbox",
      name = GetString(LWS_CHAT_SETTING),
      getFunc = function() return self:GetConfValueForKey(LW_CONF_CHAT_SETTING) end,
      setFunc = function(value) self:SetConfValueForKey(LW_CONF_CHAT_SETTING,value) end
    },
    [4] = {
			type = "slider",
			name = GetString(LWS_LIMITSTOCK),
			-- tooltip = "",
			min = 5.0,
			max = 50.0,
			step = 5.0,
			default = 5.0,
			width = "full",
      getFunc = function() return self:GetConfValueForKey(LW_CONF_LIMITSTOCK) end,
      setFunc = function(value) self:SetConfValueForKey(LW_CONF_LIMITSTOCK,value) end
      },
      
    [5] = {
			type = "slider",
			name = GetString(LWS_CURRENCY),
			-- tooltip = "",
			min = 0.0,
			max = 100.0,
			step = 5.0,
			default = 10.0,
			width = "full",
      getFunc = function() return self:GetConfValueForKey(LW_CONF_CURRENCY) end,
      setFunc = function(value) self:SetConfValueForKey(LW_CONF_CURRENCY,value) end
      },
 ...
 [6]={
    ...
    [22]={
      type = "dropdown",
      name = GetString(LWS_PRESET_JEWELRYMATS),
      tooltip = GetString(LWS_PRESET_JEWELRYMATS_TOOLTIP),
      choices = {LW_PRESET_ACTION_STRING[LW_PRESET_ACTION_PROMPT],LW_PRESET_ACTION_STRING[LW_PRESET_ACTION_DESTROY],LW_PRESET_ACTION_STRING[LW_PRESET_ACTION_JUNK],LW_PRESET_ACTION_STRING[LW_PRESET_ACTION_KEEP]},
      getFunc = function() return self:GetConfValueForKey(LW_CONF_KEEPRES_JEWELMAT) end,
      setFunc = function(value) self:SetConfValueForKey(LW_CONF_KEEPRES_JEWELMAT,value) end
    },
    [23]={
      type = "header",
    ...
    [27]={
      type = "dropdown",
      name = GetString(LWS_PRESET_RESEARCH_JEWELRY),
      tooltip = GetString(LWS_PRESET_RESEARCH_JEWELRY_TOOLTIP),
      choices = {LW_PRESET_ACTION_STRING[LW_PRESET_ACTION_PROMPT],LW_PRESET_ACTION_STRING[LW_PRESET_ACTION_DESTROY],LW_PRESET_ACTION_STRING[LW_PRESET_ACTION_JUNK],LW_PRESET_ACTION_STRING[LW_PRESET_ACTION_KEEP]},
      getFunc = function() return self:GetConfValueForKey(LW_CONF_KEEPRES_JEWEL) end,
      setFunc = function(value) self:SetConfValueForKey(LW_CONF_KEEPRES_JEWEL,value) end
    },
  ...
  --EVENT_MANAGER:RegisterForEvent(self.name, EVENT_LOOT_RECEIVED, function(...)self:OnItemLooted(...) end)
...
function LootWall:DefaultActionForLink(itemLink)
  ...
  local icon, itemCurrency = GetItemLinkInfo ( itemLink )
  
  local researchNeeded,craftskillType = self.NfR:DoesPlayerNeedTrait(GetUnitName("player"),itemLink,nil)
  
  local temp_Icon = zo_strformat ( "|t24:24:<<1>>|t",icon )
    
 -- ***********************************************
 
  if (itemType == ITEMTYPE_SOUL_GEM) or 
        (itemtype == REWARD_ITEM_TYPE_ITEM) or 
          (itemtype == ITEMTYPE_CROWN_ITEM) or
            (itemtype == ITEMTYPE_DYE_STAMP) or
              (itemtype == ITEMTYPE_LOCKPICK) or
                (itemtype == ITEMTYPE_MASTER_WRIT) or
                  (itemtype == ITEMTYPE_MOUNT) or
                    (itemtype == ITEMTYPE_SPELLCRAFTING_TABLET) or
  ...
  if tonumber(itemQuality) >= tonumber(self.savedVariables.conf[LW_CONF_QUALITY_THRESHOLD]) then    -- <<<<< is legendary check
    return LW_ACTION_KEEP_ALWAYS
  end
  ...
  -- note: if trash or normal quality, refers to CONF rule 
   if (itemType == ITEMTYPE_LURE) then
    if(self.savedVariables.conf[LW_CONF_KEEPBAIT] > LW_PRESET_ACTION_PROMPT) then
 ...
 --d("2")
  if(self:IsItemOfType(itemLink,{ITEM_TRAIT_TYPE_ARMOR_NIRNHONED})) or (self:IsItemOfType(itemLink,{ITEM_TRAIT_TYPE_WEAPON_NIRNHONED})) or (self:IsItemOfType(itemLink,{ITEM_TRAIT_TYPE_JEWELRY_ORNATE})) then
...
if (researchNeeded == true) then 
    ...
    if(self.savedVariables.conf[LW_CONF_KEEPRES_BS] > LW_PRESET_ACTION_PROMPT) then 
      if (craftskillType == CRAFTING_TYPE_BLACKSMITHING) then
        return self:PresetToAction(self.savedVariables.conf[LW_CONF_KEEPRES_BS])
      end
    end
          
   -- d("4") 
    if (self.savedVariables.conf[LW_CONF_KEEPRES_CLOTH] > LW_PRESET_ACTION_PROMPT) then
      if(craftskillType == CRAFTING_TYPE_CLOTHIER) then
        return self:PresetToAction(self.savedVariables.conf[LW_CONF_KEEPRES_CLOTH])
      end
    end

   -- d("5")
    if (self.savedVariables.conf[LW_CONF_KEEPRES_WW] > LW_PRESET_ACTION_PROMPT) then
      if(craftskillType == CRAFTING_TYPE_WOODWORKING) then
        return self:PresetToAction(self.savedVariables.conf[LW_CONF_KEEPRES_WW])
      end
    end

    -- SUMMERSET JEWELRY
    if (self.savedVariables.conf[LW_CONF_KEEPRES_JEWEL] > LW_PRESET_ACTION_PROMPT) then
      if(craftskillType == CRAFTING_TYPE_JEWELRYCRAFTING) then
        return self:PresetToAction(self.savedVariables.conf[LW_CONF_KEEPRES_JEWEL])
      end
    end
...
--d("6")
if (self:IsItemOfType(itemLink,{ITEMTYPE_STYLE_MATERIAL})) then
  
  if(self.savedVariables.conf[LW_CONF_KEEPSTYLEMATS] > LW_PRESET_ACTION_PROMPT) then
...
--d("7")
if (self:IsItemOfType(itemLink,{ITEMTYPE_REAGENT, ITEMTYPE_ALCHEMY_BASE})) then
  
  if(self.savedVariables.conf[LW_CONF_KEEPALCHMAT] > LW_PRESET_ACTION_PROMPT) then
 ...
 --d("8")
if (self:IsItemOfType(itemLink,{ITEMTYPE_WEAPON_TRAIT,ITEMTYPE_ARMOR_TRAIT,ITEMTYPE_BLACKSMITHING_MATERIAL, ITEMTYPE_BLACKSMITHING_RAW_MATERIAL, ITEMTYPE_BLACKSMITHING_BOOSTER})) then
  
    if(self.savedVariables.conf[LW_CONF_KEEPBSMAT] > LW_PRESET_ACTION_PROMPT) then
...
--d("9")
if (self:IsItemOfType(itemLink,{ITEMTYPE_CLOTHIER_MATERIAL,ITEMTYPE_ARMOR_TRAIT, ITEMTYPE_CLOTHIER_RAW_MATERIAL,ITEMTYPE_CLOTHIER_BOOSTER})) then
  
  if(self.savedVariables.conf[LW_CONF_KEEPCLOTHMAT] > LW_PRESET_ACTION_PROMPT) then
...
--d("10")
if (self:IsItemOfType(itemLink,{ITEMTYPE_ENCHANTING_RUNE_ASPECT, ITEMTYPE_ENCHANTING_RUNE_ESSENCE, ITEMTYPE_ENCHANTING_RUNE_POTENCY})) then
  
  if(self.savedVariables.conf[LW_CONF_KEEPENCHMAT] > LW_PRESET_ACTION_PROMPT)then
...
-- d("11")
if (self:IsItemOfType(itemLink,{ITEMTYPE_WEAPON_TRAIT,ITEMTYPE_WOODWORKING_MATERIAL, ITEMTYPE_WOODWORKING_RAW_MATERIAL, ITEMTYPE_WOODWORKING_BOOSTER})) then
    if(self.savedVariables.conf[LW_CONF_KEEPWWMAT] > LW_PRESET_ACTION_PROMPT) then
    ...
    if (self:IsItemOfType(itemLink,{
      ...
      then
          if(self.savedVariables.conf[LW_CONF_KEEPRES_JEWELMAT] > LW_PRESET_ACTION_PROMPT) then
            return self:PresetToAction(self.savedVariables.conf[LW_CONF_KEEPRES_JEWELMAT])
          else
              return LW_ACTION_UNDEFINED
          end
    end
 ...
 --d("12")
if (self:IsItemOfType(itemLink,{ITEMTYPE_INGREDIENT})) then
  
  if(self.savedVariables.conf[LW_CONF_KEEPPROVMAT] > LW_PRESET_ACTION_PROMPT)then
  ...
  ------- no specific behavior found ? do following action by default
  if (itemQuality == ITEM_QUALITY_TRASH) or (itemQuality == ITEM_QUALITY_NORMAL)  and
          (self.savedVariables.conf[LW_CONF_KEEPTRASH] > LW_PRESET_ACTION_PROMPT) 
    then 
...
function LootWall:GetConfValueForKey(key)
  ...
  if key == LW_CONF_CURRENCY then return self.savedVariables.conf[key] end
    
  if key == LW_CONF_LIMITSTOCK then return self.savedVariables.conf[key] end    
  
  if key == LW_CONF_CHAT_SETTING then return self.savedVariables.conf[key] end
...
function LootWall:SetConfValueForKey(key,value)
  ...
  if key == LW_CONF_CHAT_SETTING then
     self.savedVariables.conf[key] = value 
  end
  
  if key == LW_CONF_CURRENCY then  self.savedVariables.conf[key] = value  end
  if key == LW_CONF_LIMITSTOCK then  self.savedVariables.conf[key] = value  end 
...
function LootWall:ApplyRuleToItem(rule,itemBagId,itemSlotId,itemLink,stackCountChange)
    
  local str = GetItemLinkName(itemLink) --GetItemLinkName(itemLink)
  local icon, sellPrice, meetsUsageRequirement, equipType, itemStyle = GetItemLinkInfo ( itemLink )
  local temp_Icon = ""
  
  local limitStock = self.savedVariables.conf[LW_CONF_LIMITSTOCK]
  local currencyConf = self.savedVariables.conf[LW_CONF_CURRENCY]  
      
  local stock=GetItemTotalCount(itemBagId, itemSlotId)
  temp_Icon = zo_strformat ( "|t24:24:<<1>>|t",icon )
  
  local strQty
  if stackCountChange == 0 then -- prevent prompt
    strQty = ""
  else
    strQty = "x" .. stackCountChange
  end
  
  -- JUNK
  if (rule == LW_ACTION_TRASH_ALWAYS)or(rule == LW_ACTION_TRASH_NOW)then
  
    if (self.savedVariables.conf[LW_CONF_CHAT_SETTING]) then
        d("[Loot] Junk " .. strQty .. " " .. temp_Icon .. " " .. itemLink .. " [" .. stock .. "]")
    end
    SetItemIsJunk(itemBagId,itemSlotId,true)
  end
  
  -- DESTROY
  if(rule == LW_ACTION_DESTROY_ALWAYS)or(rule == LW_ACTION_DESTROY_NOW)then
    
    -- 1st check item currency value
    if sellPrice >= currencyConf then
        if (self.savedVariables.conf[LW_CONF_CHAT_SETTING]) then 
          d("[Loot] > " .. temp_Icon .. " " .. itemLink .. " Sellprice: " .. sellPrice) 
          d("[Loot] > Destroy aborted... junked.")
        end
        SetItemIsJunk(itemBagId,itemSlotId,true)
    elseif stock <= limitStock then
        -- Now, check limitstock before destroying
            if (self.savedVariables.conf[LW_CONF_CHAT_SETTING]) then 
              d("[Loot] Destroy " .. temp_Icon .. " " .. itemLink ) 
            end
            DestroyItem(itemBagId,itemSlotId)
        else
          if (self.savedVariables.conf[LW_CONF_CHAT_SETTING]) then
            d("[Loot] > " .. temp_Icon .. " " .. itemLink .. " stock limit [" .. stock .. "]")
            d("[Loot] > Destroy aborted.")
         end
--        SetItemIsJunk(itemBagId,itemSlotId,true) v 1.0.8 keep
        end
  end
  
  -- KEEP
  if(rule == LW_ACTION_KEEP_ALWAYS)or(rule == LW_ACTION_KEEP_NOW) then
    if (self.savedVariables.conf[LW_CONF_CHAT_SETTING]) then
        d("[Loot] Keep ".. strQty .. " " .. temp_Icon .. " " .. itemLink .. " [" .. stock .. "]")
    end
      SetItemIsJunk(itemBagId,itemSlotId,false)
  end
...
function LootWall:DestroyNowAction()
  ...
  self:ApplyRuleToItem(self.savedVariables.itemList[self:MakeItemStringId(displayedItem.itemLink)],displayedItem.bagId,displayedItem.slotId,displayedItem.itemLink,0)
  self:HideWindow()
end
-------------------------------------------------------------------------------
...
function LootWall:DestroyAlwaysAction()
  ...
  if (self.savedVariables.conf[LW_CONF_CHAT_SETTING]) then
    d("[Loot] Create rule for " .. displayedItem.itemLink .. ".") 
  end
...
function LootWall:OnInventoryUpdate(eventCode,bagId,slotId,isNewItem,itemSoundCategory,updateReason, stackCountChange)
  ...
  if updateReason == INVENTORY_UPDATE_REASON_DYE_CHANGE then return end
...
--  **********************TEST RULES ****************
  if self:DefaultActionForLink(itemLink) ~= LW_ACTION_UNDEFINED then
    ...
    self:ApplyRuleToItem(self:DefaultActionForLink(itemLink),bagId,slotId,itemLink,stackCountChange)   
    
  elseif self:ItemHasRule(itemLink) then
    
    self:ApplyRuleToItem(self.savedVariables.itemList[self:MakeItemStringId(itemLink)],bagId,slotId,itemLink,stackCountChange) 
    
  else
    if (self.savedVariables.conf[LW_CONF_CHAT_SETTING]) and (IsUnitInCombat("player") == true)
        then d("[Loot] Item is pending... wait out of combat") 
    end
