...
ShouldIUlt.turningTideCooldownTimer = nil
ShouldIUlt.turningTideCooldownEndTime = 0
ShouldIUlt.turningTideCooldownDuration = 15000
ShouldIUlt.turningTideIcon = nil
...
function ShouldIUlt:GetDisplayBuffs()
    if ShouldIUlt.savedVars.simulationMode then
        ...
        if self:IsTurningTideCooldownActive() and ShouldIUlt.savedVars.trackTurningTide and not self.buffData[167350] then
            allBuffs[999998] = {
                name = "Turning Tide",
                iconName = self.turningTideIcon or self:GetBuffIcon("Turning Tide"),
                endTime = self.turningTideCooldownEndTime,
                startTime = self.turningTideCooldownEndTime - self.turningTideCooldownDuration,
                stackCount = 1,
                duration = self.turningTideCooldownDuration,
                slot = 998,
                isTurningTideCooldown = true,
                isPermanent = false,
            }
        end
...
local function Initialize()
    ...
    -- Initialize Turning Tide Tracker
end
...
function ShouldIUlt:OnEffectChanged(changeType, effectSlot, effectName, unitTag, beginTime, endTime, stackCount, iconName,
    ...
    if changeType == EFFECT_RESULT_GAINED or changeType == EFFECT_RESULT_UPDATED then
        ...
        local durationMs = endTimeMs - startTimeMs
        if self:IsTurningTideBuff(abilityId) then
            self.turningTideIcon = iconName
    ...
    elseif changeType == EFFECT_RESULT_FADED then
        local fadingTurningTide = self:IsTurningTideBuff(abilityId)
        if fadingTurningTide and self.buffData[abilityId] and self.buffData[abilityId].iconName then
            self.turningTideIcon = self.buffData[abilityId].iconName
        end
        ...
        if fadingTurningTide and ShouldIUlt.savedVars.trackTurningTide then
...
function ShouldIUlt:IsTurningTideBuff(abilityId)
    return abilityId == 167350
end

function ShouldIUlt:StartTurningTideCooldown()
    local currentTime = GetGameTimeMilliseconds()

    -- Exactly 10 seconds from the moment Turning Tide disappears
    self.turningTideCooldownEndTime = currentTime + 15000

    -- Clean up any previous update
    if self.turningTideCooldownTimer then
        EVENT_MANAGER:UnregisterForUpdate(self.turningTideCooldownTimer)
    end

    local timerName = ADDON_NAME .. "TurningTideCooldown"
    self.turningTideCooldownTimer = timerName

    EVENT_MANAGER:RegisterForUpdate(timerName, 100, function()
        if GetGameTimeMilliseconds() >= self.turningTideCooldownEndTime then
            -- 10 seconds finished: remove the red icon
            self:ClearTurningTideCooldown()
            self:UpdateUI()
        else
    ...
    -- Immediately show the red icon at 10.0
    self:UpdateUI()
end

function ShouldIUlt:ClearTurningTideCooldown()
    if self.turningTideCooldownTimer then
        EVENT_MANAGER:UnregisterForUpdate(self.turningTideCooldownTimer)
        self.turningTideCooldownTimer = nil
    end
    self.turningTideCooldownEndTime = 0
end

function ShouldIUlt:IsTurningTideCooldownActive()
    return self.turningTideCooldownEndTime > 0 and GetGameTimeMilliseconds() < self.turningTideCooldownEndTime
end
    ...
    for abilityId, buffInfo in pairs(displayBuffs) do
        ...
        if buffIcon then
            ...
            if iconControl and buffInfo.iconName then
                ...
                if buffInfo.isTurningTideCooldown then
                    iconControl:SetColor(1, 0.02, 0, 1) -- red cooldown
                elseif buffInfo.isOffBalanceImmunity or buffInfo.isImmunityTimer then
                ...
                -- Timer display
                if timerLabel and (ShouldIUlt.savedVars.showTimer or buffInfo.isTurningTideCooldown) then
