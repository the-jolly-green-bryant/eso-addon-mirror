...
local versionName, version = "v1.1.9", 26071801500
...
ZO_PreHook( "ProcessMapClick", function ( xN, yN )
   ...
   if theme then
      ...
      if map then
         ...
         if not map:UseDefaultZones() and selectedZone then
            lmt_NavigateToMap( selectedZone:GetZoneId() )
            return true
         end
      end
...
local function CompilePoi( theme, map, poiId, poi )
   ...
   local poiRename = poi.name
   compiled.GetRename = function ( ) return poiRename end
...
local function CompileZoneHitbox( theme, zone, blob )
   ...
   compiled:SetHandler( "OnMouseEnter", function ( )
      ...
      selectedZone = zone
      local zoneName = selectedZone:GetZoneName()
      WORLD_MAP_MANAGER.mouseoverCurrentLocation = zoneName
      ZO_WorldMapMouseoverName:SetText( zo_strformat(SI_WINDOW_TITLE_WORLD_MAP, zoneName) )
...
local function CompileZone( theme, map, zoneId, zone )
   ...
   compiled.GetZoneDescription = function ( ) return theme:GetMapDescription( zone.name ) or theme:GetMapDescription( GetMapNameById( zoneId ) ) or theme:GetMapDescription( zoneId ) end
   ...
   compiled.GetZoneName = function ( )
      ...
      if theme:IsRenamesEnabled() then
         mapName = theme:GetRename( mapName ) or theme:GetRename( zoneId ) or mapName
      else
         -- Fix map clipping
...
local function CompileMap( theme, mapId, map )
   ...
   compiled.GetMapName = function ( )
      ...
      if theme:IsRenamesEnabled() then
         mapName = theme:GetRename( mapName ) or theme:GetRename( mapId )
      end
      return mapName
   end

   compiled.GetMapDescription = function ( ) return theme:GetMapDescription( GetMapNameById( mapId ) ) or theme:GetMapDescription( mapId ) end
...
local function lmt_ResolvePath( functionName )
   local container = _G
   local fieldName = functionName
   if functionName:find( "%." ) then
      local path = { }
      for segment in functionName:gmatch( "[^%.]+" ) do
         path[ #path + 1 ] = segment
      end
      fieldName = path[ #path ]
      container = _G
...
local function lmt_PostHookFunction( functionName )
   if not lmt_originalFunctions[ functionName ] then
      local container, fieldName = lmt_ResolvePath( functionName )
      if not container or type( container[ fieldName ] ) ~= "function" then
         return
      end

      lmt_originalFunctions[ functionName ] = container[ fieldName ]
      local _GFunction = lmt_originalFunctions[ functionName ]

      container[ fieldName ] = function ( ... )
...
local function CompileTheme( theme )
   ...
   compiled.GetRename = function ( self, name )
      return themeRenames[ name ] or nil
   end
