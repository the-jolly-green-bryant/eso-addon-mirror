...
local function countSurveys()
        ...
        for i = 1, GetBagSize(bankNum) do
            ...
            if special ==SPECIALIZED_ITEMTYPE_TROPHY_SURVEY_REPORT then
                ...
                if surveyType == 8 then
                	d("Не удалось определить тип исследования для "..GetItemLink(bankNum, i))
                end
    ...
    if storageIncluded then
    	...
    	d(zo_strformat(WritCreater.strings.countSurveys,total.overall).." (инвентарь: "..total["inventory"]..", банк: "..total["bank"]..", хранилище: "..total["storage"]..")")
	else
		d(zo_strformat(WritCreater.strings.countSurveys,total.overall).." (инвентарь: "..total["inventory"]..", банк: "..total["bank"]..")")
    end
    
    for i = 1, 7 do
    	if detailedCount[i] >0 then
    		if storageIncluded then
    			d(names[i].." : "..detailedCount[i] .. " (инвентарь: "..bagCounts["inventory"][i]..", банк: "..bagCounts["bank"][i]..", хранилище: "..bagCounts["storage"][i]..")")
    		else
    			d(names[i].." : "..detailedCount[i] .. " (инвентарь: "..bagCounts["inventory"][i]..", банк: "..bagCounts["bank"][i]..")")
    		end
    	end
    end
end
...
local function countVouchers()
    ...
    for k, v in pairs(craftTotals) do 
    	d(names[k].." : "..v.." (запечатанных заказов: "..sealedTotals[k]..")")
    end
    d(zo_strformat(WritCreater.strings.countVouchers,total).." (запечатанных заказов: "..totalSealed..")")
...
local function outputStats(showChances)
	...
	d("Общее количество выполненных заказов: "..WritCreater.savedVarsAccountWide.total.." за последние "..tostring(daysSinceReset).." дней")
end
...
local function activateDebug(str)
	if string.lower(str) == "bank" then
		...
		d("Отладка банка "..tostring( WritCreater.savedVarsAccountWide.bankDebug) )
	elseif string.lower(str) == "delay" then
		...
		d("Отладка задержки "..tostring( WritCreater.savedVarsAccountWide.bankDebug) )
	else
		
		WritCreater:GetSettings().debug = not WritCreater:GetSettings().debug 
		d("Отладка ремесла ".. tostring(WritCreater:GetSettings().debug))
	end
end
...
local function abandonWrits()
	local a = WritCreater.writSearch() d("Покинуть корабль!!!") for i, v in pairs(a) do AbandonQuest(a[i]) end
end
