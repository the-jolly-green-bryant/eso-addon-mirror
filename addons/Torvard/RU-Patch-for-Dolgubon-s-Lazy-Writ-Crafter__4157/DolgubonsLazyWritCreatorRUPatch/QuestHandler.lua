...
local function OnQuestAdded(eventId, questIndex)
	...
	if rejectedMat then
		d("Writ Crafter отказался от "..GetJournalQuestName(questIndex)..", потому что для этого требуется "..rejectedMat..", использование которого было запрещено в настройках")
...
local function checkIfCanAcceptQuest()
	...
	if 25 - getBuffer() + numWrits < GetNumJournalQuests() + 1 then
		...
		ZO_Alert(UI_ALERT_CATEGORY_ERROR, SOUNDS.GENERAL_ALERT_ERROR ,"Буфер заданий Lazy Writ Crafter™ не позволяет вам принять это задание")
		return true
	end
	return false
end
...
function WritCreater.InitializeQuestHandling()
	...
	AcceptOfferedQuest = function()
		...
		elseif string.find(GetOfferedQuestInfo(), completionStrings["Rolis Hlaalu"]) and not lastSlotCraftable then
			ZO_Alert(UI_ALERT_CATEGORY_ERROR, SOUNDS.GENERAL_ALERT_ERROR ,"Lazy Writ Crafter™ не позволил вам принять этот заказ, потому что вы не можете его создать")
		else
			-- d("Accept")
