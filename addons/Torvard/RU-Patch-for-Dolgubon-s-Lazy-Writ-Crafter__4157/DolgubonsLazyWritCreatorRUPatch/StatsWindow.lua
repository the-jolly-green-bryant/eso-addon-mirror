...
{
	["master"] = {"Запечатанные заказы",5000},
	["intricate"] = {"Сложное снаряжение",200},
	["material"] = {"Поставки материалов",50},
	["survey"] = {"Исследования",5000},
	["soulGem"] = {"Пустые камни Душ",30},
	["glyph"] = {"Глифы",100},
	["repair"] = {"Ремонтные наборы",200},
	["ornate"] = {"Ценное снаряжение",200},
	["voucher"] = {"Ваучеры",500}
}
...
function RewardsScroll:BuildMasterList()
	...
	for craft, craftRewards in pairs(self.data) do
		...
		local traitMatTable =  {["craft"] = craft ,["item"]="Камни Стиля", ["amount"]=0, estimateKey="traitMat"}
		local pulverizedTraitMatTable =  {["craft"] = craft ,["item"]="Фрагменты Стиля", ["amount"]=0, estimateKey="pulverizedMat"}
		...
		if not (craft == CRAFTING_TYPE_ENCHANTING or craft == CRAFTING_TYPE_ALCHEMY) then
			...
			if craft == CRAFTING_TYPE_PROVISIONING then
				self.masterList[i][craft].item = "Белые материалы"
			end
			i = i+1
		...
		for reward, amount in pairs(craftRewards) do
			...
			if reward=="num" then
				craftHeader.amount=amount.." заказов выполнено"
				self.craftTotals[craft] = amount
			elseif reward=="fragment" then
				local name
				if craft == CRAFTING_TYPE_PROVISIONING then
					name = "Фрагменты Псиджиков"
				else
					name = "Фрагменты Стекла"
				end
				if amount > 0 then
					self.masterList[i] = self.masterList[i] or {}
					...
					for qualifier, qualifierAmount in pairs(amount) do
						if qualifierAmount>0 then
							...
							elseif qualifier == "purple" then
								name = "Фиолетовые Рецепты"
							elseif qualifier == "green" then
								name = "Зеленые Рецепты"
							end
		...
		headerName:SetText(zo_strformat("<<1>>",GetCraftingSkillName(craftHeader.craft)).. " Награды")
		...
		questGoldControl:SetText("Золото за квесты: "..formatNum(questGold).."")
		local itemValue = GetControl(DolgubonsLazyWritStatsWindowBackdrop, "ItemGold")
		itemValue:SetText("Общая стоимость товара: "..formatNum(totalValue).."")
		local itemValue = GetControl(DolgubonsLazyWritStatsWindowBackdrop, "TotalGold")
		itemValue:SetText("Общая сумма наград: "..formatNum(totalValue+questGold).."")
	end
...
local function updateNonScrollElements()
	...
	DolgubonsLazyWritStatsWindowBackdropWritCounter:SetText("Заказов выполнено: ".. WritCreater.savedVarsAccountWide.total.." за последние "..daysSinceReset.." дней")
