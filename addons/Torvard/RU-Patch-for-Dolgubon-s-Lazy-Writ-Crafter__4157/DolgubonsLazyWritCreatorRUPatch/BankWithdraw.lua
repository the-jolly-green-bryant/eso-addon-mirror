...
local function runProcessDeposits()
	...
	if WritCreater.savedVars.goldToDeposit > 0 and WritCreater:GetSettings().rewardHandling["currency"].all == 2 then
		...
		numItems = numItems + 1
		d("Writ Crafter положил в банк "..WritCreater.savedVars.goldToDeposit.." золота")
	...
	for k, v in pairs(WritCreater.savedVars.depositList) do
		...
		if not doesItemExistInSlot then
			d("Writ Crafter не смог найти "..v[1]..", чтобы положить в банк. Возможно, предмет был уничтожен или перемещен")
		...
		elseif DoesBagHaveSpaceFor(bankingBag, bag, index) or (canAlsoBePlacedInSubscriberBank and DoesBagHaveSpaceFor(BAG_SUBSCRIBER_BANK, bag, index)) then
			...
			d("Writ Crafter положил в банк "..tostring(v[1]))
			WritCreater.savedVars.depositList[k] = nil
			depositedItem = true
			numItems = numItems - 1
			return zo_callLater( runProcessDeposits, GetLatency()+20)
