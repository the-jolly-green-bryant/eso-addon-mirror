...
{
	["en"]=true,["de"] = true,["fr"] = true,["jp"] = true, ["ru"] = true, ["zh"] = false, ["pl"] = false,["es"] = true
}
if true then
	EVENT_MANAGER:RegisterForEvent("WritCrafterLocalizationError", EVENT_PLAYER_ACTIVATED, function()
		if not WritCreater.writCompleteStrings then 
			...
			if validLanguages[language] == nil then
				d("Dolgubon's Lazy Writ Crafter: Ваш язык не полностью поддерживается этим дополнением. Некоторые функции могут работать не так, как ожидалось. Если вы хотите перевести дополнение, ознакомьтесь с файлом lang/en.lua для получения дополнительных инструкций.")
			elseif validLanguages[language] == false then
				d("Dolgubon's Lazy Writ Crafter: Не удалось загрузить файл локализации.")
				d("Диагностика:")
				d("1. Ваш язык поддерживается патчем для Writ Crafter. Пожалуйста, убедитесь, что вы скачали соответствующий патч.")
				d("2. Удалите, а затем переустановите Writ Crafter и патч.")
				d("3. Если у вас все еще есть проблемы, свяжитесь с автором патча.")
			else
				d("Dolgubon's Lazy Writ Crafter: Не удалось загрузить файл локализации.")
				d("Диагностика:")
				d("1. Попробуйте удалить, а затем переустановить дополнение.")
				d("2. Если ошибка повторится, свяжитесь с @Dolgubon в игре или по адресу tinyurl.com/WritCrafter.")
			end
		end
...
local function styleCompiler()
	...
	submenuTable = {
		...
		{
			type = "description",
			text = "Напоминание: Вы можете купить базовые камни расового стиля у любого NPC-торговца по цене 15 золотых за каждый",
		},
		{
			type = "divider",
			height = 15,
		...
		} ,
		{
			...
			WritCreater.LLCInteractionMaster:cancelItem()
				if value  then
					...
					d("Очередь на создание мастерских заказов очищена")
				end
			end,
		},
		{
            ...
            tooltip = "Помещает в очередь все запечатанные заказы в вашем инвентаре. Не ставит в очередь запечатанные заказы по алхимии. Пропускает заказы, отмеченные как хлам",
			func = function(value) 
