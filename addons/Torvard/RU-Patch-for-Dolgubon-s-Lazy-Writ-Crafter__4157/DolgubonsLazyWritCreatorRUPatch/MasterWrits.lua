...
local function EnchantingMasterWrit(itemId, levelId, quality, reference, name)
	...
	elseif levelId == 225 then
		...
		d("Не удалось сопоставить идентификатор уровня "..levelId)
	end
	...
	d(zo_strformat("<<t:1>>: Создать <<t:2>>", name, resultLink))
end
...
local function ProvisioningMasterWrit(resultItemId, quantity, reference, name)
	...
	if not known then
		d(zo_strformat("<<t:1>>: Не удалось поставить в очередь, так как вы не знаете рецепта для <<t:2>>", name, resultLink))
		return
	end
	...
	d(zo_strformat("<<t:1>>: Создать <<t:3>>x <<t:2>>", name, resultLink, amountToCreate))
end
...
local function provisioningJournal(journalIndex, name)
	...
	if quantity > 0 then
		...
		d(name..": Не удалось поставить заказ в очередь. Возможно, вы не знаете необходимого рецепта")
	end
end
...
local function provisioningRightClick(link, station, uniqueId)
	...
	if quantity == 0 then
		d("Не удалось определить, сколько предметов изготовить. Попробуйте принять заказ")
		return
	end
...
function WritCreater.InventorySlot_ShowContextMenu(rowControl,debugslot)
    ...
    zo_callLater(function ()
        AddCustomMenuItem("Создать запечатанный заказ", function ()
...
function WritCreater.queueAllSealedWrits(bag)
	...
	if outputCounter >= 10 then
		CHAT_ROUTER:AddSystemMessage("Writ Crafter поставил в очередь запечатанных заказов: "..outputCounter..".")
	end
	outputCounter = nil
end
