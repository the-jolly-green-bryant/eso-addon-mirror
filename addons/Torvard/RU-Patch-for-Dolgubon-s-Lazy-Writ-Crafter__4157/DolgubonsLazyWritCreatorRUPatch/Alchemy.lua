...
local function getShortlist(...)
	local params = {...}
	if #params > 3 then d("Поиск слишком большого количества параметров") return end
	local searchEffects = {}
	for i = 1, #params do
...
function WritCreater.alchemyWrit(solvent, reagents, requiredItemId, quantity, craftingWrits, journalIndex)
	...
	if solvent and not minCombo then
		out("Не удалось найти достаточно дешевую комбинацию известных реагентов. Попробуйте приобрести другие виды алхимических предметов")
		return
	end
	...
	if solvent and solvent.itemId and minCombo then
		...
		out(zo_strformat("Для создания потребуется <<t:4>> <<t:1>>, <<t:4>> <<t:2>> и <<t:4>> <<t:3>>", getItemLinkFromItemId(solvent.itemId), getItemLinkFromItemId(minCombo[1]), getItemLinkFromItemId(minCombo[2]), quantity))
...
Returns: string link, ProspectiveAlchemyResult prospectiveAlchemyResult
	...
	if not isMasterWrit then
		...
		if ZO_IsTableEmpty(reagents) or not solvent then
			out("Не удалось найти комбинацию известных реагентов. Попробуйте изучить или приобрести больше алхимических предметов")
			return
		end
...
local function queueAlchemyMasterWrit(solvent, r1, r2, r3, quantity, reference, name)
	...
	d(zo_strformat("<<t:1>>: Создание <<t:2>> с использованием <<t:3>>, <<t:4>> и <<t:5>>", name, type, getItemLinkFromItemId(r1), getItemLinkFromItemId(r2), getItemLinkFromItemId(r3)))
